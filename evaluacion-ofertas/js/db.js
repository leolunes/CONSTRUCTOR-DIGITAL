...
export function actualizarProceso(cambios = {}) {
  DB.proceso = {
    ...
    puntajeSostenibilidad: Number(cambios.puntajeSostenibilidad ?? DB.proceso.puntajeSostenibilidad) || 0,
...
export function crearOfertaBase(empresa = "", valor = 0) {
  return {
    ...
    puntaje: 0,
    ranking: 0,
    puntajesImportados: false,

    // NUEVOS FACTORES
    cumpleCalidad: false,
    cumpleSostenibilidad: false,
    cumpleIndustria: false,
    cumpleDiscapacidad: false,
    cumpleMujer: false,
    cumpleMipyme: false,
...
function crearSnapshotVacioProyecto() {
  return {
    proceso: {
      nombre: "",
      objeto: "",
      ...
      // NUEVOS FACTORES DE EVALUACIÓN
      puntajeCalidad: 0,
      puntajeSostenibilidad: 0,
      puntajeIndustriaNacional: 0,
      puntajeDiscapacidad: 0,
      puntajeMujer: 0,
      puntajeMipyme: 0,
      numeroPropuestasHabilitadas: 0,
...
export function crearNuevoProyecto(nombre = "Proyecto sin nombre") {
  ...
  const proyecto = {
    id,
    nombre,
    data: crearSnapshotVacioProyecto()
  };

  PROYECTOS.lista.push(proyecto);
  PROYECTOS.activoId = id;

  importarDB(crearSnapshotVacioProyecto());
...
export function guardarProyectoActual(nombre = null) {
  ...
  if (!PROYECTOS.activoId) {
    const id = Date.now();

    const proyecto = {
      id,
      nombre: nombre || DB.proceso.nombre || "Proyecto sin nombre",
      data: snapshot
    };
  ...
  const proyecto = PROYECTOS.lista.find(p => p.id === PROYECTOS.activoId);
  ...
  if (!proyecto) {
    ...
    PROYECTOS.lista.push(nuevoProyecto);
    return nuevoProyecto;
  }

  proyecto.data = snapshot;
  proyecto.nombre = nombre || proyecto.nombre || DB.proceso.nombre || "Proyecto sin nombre";
...
export function cargarProyecto(id) {
  ...
  PROYECTOS.activoId = proyecto.id;
...
// ===============================
// FUNCIONES AVANZADAS DE PROYECTOS
// ===============================

// 🔴 ELIMINAR PROYECTO
export function eliminarProyecto(id) {
  PROYECTOS.lista = PROYECTOS.lista.filter(p => Number(p.id) !== Number(id));

  // si eliminas el activo, limpiar DB
  if (Number(PROYECTOS.activoId) === Number(id)) {
    PROYECTOS.activoId = null;
    resetDB();
  }
}
...
export function duplicarProyecto(id) {
  ...
  const copia = {
    ...
    data: JSON.parse(JSON.stringify(proyecto.data))
  };

  PROYECTOS.lista.push(copia);
  return copia;
}

// 🔍 BUSCAR PROYECTOS
export function buscarProyectos(texto = "") {
  const filtro = String(texto).toLowerCase();

  return PROYECTOS.lista.filter(p =>
...
// ===============================
export function actualizarUrlSecop(url = "") {
  DB.proceso.urlSecop = String(url || "").trim();
}

export function agregarDocumentoAnexo(doc = "") {
  if (!doc) return;
  if (!Array.isArray(DB.proceso.documentosAnexos)) {
    DB.proceso.documentosAnexos = [];
  }
  DB.proceso.documentosAnexos.push(String(doc));
}

export function eliminarDocumentoAnexo(index) {
  if (!Array.isArray(DB.proceso.documentosAnexos)) return;
  DB.proceso.documentosAnexos.splice(index, 1);
}

export function obtenerDocumentosAnexos() {
  return Array.isArray(DB.proceso.documentosAnexos)
    ? DB.proceso.documentosAnexos
    : [];
}

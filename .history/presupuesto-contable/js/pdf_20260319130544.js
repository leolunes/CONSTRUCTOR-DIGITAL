(function(){
  /* =========================================================
     ...
     ✅ AJUSTE NUEVO (2026-03 presupuesto dual):
     - Todos los PDFs deben poder trabajar con el presupuesto activo:
         * oficial
         * contratista
     - Si no se envía budgetKey, el PDF toma:
         1) opts.budgetKey
         2) project.activeBudgetKey
         3) "contratista"
     - Los cálculos, items, APUs, desagregados, IVA y resúmenes se hacen
       sobre el presupuesto seleccionado.
     ========================================================= */

  // =========================
  // CONSTANTES PRESUPUESTO
  // =========================
  ...
  function resolveBudgetKey(project, opts){
    return normalizeBudgetKey(
      opts?.budgetKey ||
      project?.activeBudgetKey ||
      BUDGET_CONTRATISTA
    );
  }

  function budgetLabel(key){
    return normalizeBudgetKey(key) === BUDGET_OFICIAL
      ? "PRESUPUESTO OFICIAL"
  ...
  function getBudgetBucket(project, budgetKey){
    const key = normalizeBudgetKey(budgetKey);
    const budgets = project?.budgets;
    if(budgets && typeof budgets === "object" && budgets[key] && typeof budgets[key] === "object"){
      return budgets[key];
    }
    if(key === BUDGET_CONTRATISTA){
      return {
        items: Array.isArray(project?.items) ? project.items : [],
        chapters: Array.isArray(project?.chapters) ? project.chapters : [],
        apuOverrides: (project?.apuOverrides && typeof project.apuOverrides === "object") ? project.apuOverrides : {},
        facturacion: Array.isArray(project?.facturacion) ? project.facturacion : [],
        customAPUs: {}
      };
    }
    return {
  ...
  function projectForBudget(project, budgetKey){
    const key = normalizeBudgetKey(budgetKey);
    const bucket = getBudgetBucket(project, key);

    const p = clonePlain(project || {});
    p.activeBudgetKey = key;
    p.items = Array.isArray(bucket.items) ? bucket.items : [];
    p.chapters = Array.isArray(bucket.chapters) && bucket.chapters.length
      ? bucket.chapters
      : (Array.isArray(project?.chapters) ? project.chapters : []);
    p.apuOverrides = (bucket.apuOverrides && typeof bucket.apuOverrides === "object")
      ? bucket.apuOverrides
      : {};
    p.facturacion = Array.isArray(bucket.facturacion)
      ? bucket.facturacion
      : (Array.isArray(project?.facturacion) ? project.facturacion : []);
    p.__budgetCustomAPUs = (bucket.customAPUs && typeof bucket.customAPUs === "object")
      ? bucket.customAPUs
      : {};
    return p;
  }

  function getItemsForBudget(project, budgetKey){
    return projectForBudget(project, budgetKey).items || [];
  }

  function getChaptersForBudget(project, budgetKey){
    return projectForBudget(project, budgetKey).chapters || [];
  }

  function getApuOverridesForBudget(project, budgetKey){
    return projectForBudget(project, budgetKey).apuOverrides || {};
  }

  function getBudgetCustomAPU(project, code, budgetKey){
    const p = projectForBudget(project, budgetKey);
    const c = String(code || "").trim();
    if(!c) return null;

    if(p.__budgetCustomAPUs && p.__budgetCustomAPUs[c]){
      return p.__budgetCustomAPUs[c];
    }

    try{
      if(window.StorageAPI?.getBudgetCustomAPU){
        const rec = StorageAPI.getBudgetCustomAPU(project?.id, c, normalizeBudgetKey(budgetKey));
        if(rec) return rec;
      }
    }catch(_){}
    ...
    try{
      if(window.StorageAPI?.getCustomAPU){
        const globalRec = StorageAPI.getCustomAPU(c);
  ...
  function buildProjectForCalc(project, budgetKey){
    return projectForBudget(project, budgetKey);
  }

  function groupByChaptersCompat(project, budgetKey){
    const p = buildProjectForCalc(project, budgetKey);
    if(window.Calc && typeof Calc.groupByChapters === "function"){
      return Calc.groupByChapters(p);
    }
    ...
    for(const it of items){
      const ch = String(it.chapterCode || "").trim() || (String(it.code||"").split(".")[0] || "SIN");
      if(!map.has(ch)){
        map.set(ch, {
          chapterCode: ch,
      ...
      const g = map.get(ch);
      g.itemsCount += 1;
      g.subtotal += Number(it.parcial || 0);
      g.items.push(it);
      if(!g.chapterName && it.chapterName) g.chapterName = String(it.chapterName);
    }
  ...
  function buildFilename(project, suffix="", budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Presupuesto_${name}_${tag}${suffix ? "_"+suffix : ""}_${Date.now()}.pdf`;
  }
  function buildFilenameSpecs(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Especificaciones_Tecnicas_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameDesagregado(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Presupuesto_Obra_Desagregado_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenDesagregado(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Resumen_Presupuesto_Obra_Desagregado_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameDistribucionPctDirectos(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Distribucion_Porcentual_Costos_Directos_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameRendimientos(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Rendimiento_Equipo_Mano_Obra_Por_Actividad_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenMaterialesActividad(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Resumen_Materiales_Por_Actividad_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameCantRecursosInsumos(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `Cantidad_Recursos_Insumos_Presupuesto_${name}_${tag}_${Date.now()}.pdf`;
  }
  function buildFilenameIvaProyecto(project, budgetKey){
    const name = sanitizeName(project?.name || "proyecto");
    const bk = normalizeBudgetKey(budgetKey);
    const tag = bk === BUDGET_OFICIAL ? "OFICIAL" : "CONTRATISTA";
    return `IVA_Proyecto_${name}_${tag}_${Date.now()}.pdf`;
  }
  ...
  function stampHeaderFooter(doc, { docType, projectName, logoDataUrl, budgetKey }){
    ...
    const budgetTxt = budgetLabel(budgetKey);
    ...
    for(let p=1; p<=pageCount; p++){
      ...
      doc.setFont("helvetica","bold");
      doc.setFontSize(9.5);
      doc.text(`${safe(docType)} · ${budgetTxt}`, textStartX, 28);
  ...
  function drawInstitutionalCover(doc, L, project, docTitle, budgetKey){
    try{
      doc.setFillColor(255,255,255);
    ...
    doc.text(budgetTxt, L.PAGE_W/2, y, { align:"center" }); y += 22;
  ...
  function totalsCompat(project, budgetKey){
    const p = buildProjectForCalc(project, budgetKey);
    const t = (window.Calc && typeof Calc.calcTotals === "function") ? (Calc.calcTotals(p) || {}) : {};
  ...
  async function getAPUForProjectItem(project, it, budgetKey){
    const key = normalizeBudgetKey(budgetKey);
    const p = projectForBudget(project, key);
    ...
    async function tryByCode(apuCode){
      ...
      try{
        const ovMap = getApuOverridesForBudget(p, key);
        const ov = ovMap ? ovMap[apuCode] : null;
        if(ov){
      ...
      try{
        const custom = getBudgetCustomAPU(project, apuCode, key);
        if(custom){
  ...
  async function calcDesagregadoByItemFromAPU(project, items, budgetKey){
    const out = [];
    const totals = {
      MATERIALES:0,
      "EQUIPO Y HERRAM":0,
    ...
    for(const it of (items||[])){
      ...
      const apuObj = await getAPUForProjectItem(project, it, budgetKey);
  ...
  /* =========================================================
     ...
     - TRANSPORTES  => IVA 0%
     ========================================================= */
  async function calcIvaProyectoByCategoria(project, items, budgetKey){
    const agg = await calcDesagregadoByItemFromAPU(project, items || [], budgetKey);
    const baseTotals = agg.totals || {};
    const ivaPct = Number(pct(project, "ivaUtilPct", "ivaPct") || 0);

    function makeRow(label, key, appliesIva){
      const base = Number(baseTotals[key] || 0);
      const appliedPct = appliesIva ? ivaPct : 0;
      const iva = base * (appliedPct / 100);
      return {
        label,
        key,
        base,
    ...
    const ivaRows = [
      makeRow("MATERIALES", "MATERIALES", true),
      makeRow("EQUIPO Y HERRAM", "EQUIPO Y HERRAM", true),
      makeRow("MANO DE OBRA", "MANO DE OBRA", false),
      makeRow("SUBCONTRATOS", "SUBCONTRATOS", true),
      makeRow("TRANSPORTES", "TRANSPORTES", false),
      makeRow("OTROS", "OTROS", true)
    ];

    const baseTotal = ivaRows.reduce((s,r)=> s + Number(r.base||0), 0);
    const baseGravada = ivaRows.reduce((s,r)=> s + (r.appliesIva ? Number(r.base||0) : 0), 0);
    const ivaTotal = ivaRows.reduce((s,r)=> s + Number(r.iva||0), 0);
  ...
  async function exportIvaProyectoPDF(project, opts){
    const budgetKey = resolveBudgetKey(project, opts);
    const p = projectForBudget(project, budgetKey);

    const doc = newDoc();
    const L = mkLayout(doc);
    const { items } = groupByChaptersCompat(p, budgetKey);

    drawInstitutionalCover(doc, L, p, "PDF IVA PROYECTO", budgetKey);

    doc.addPage();
    L.setY(PDF_THEME.safe.top);

    const data = await calcIvaProyectoByCategoria(p, items || [], budgetKey);
    const rows = data.rows || [];
    const ivaPct = Number(data.ivaPct||0);
    const baseTotal = Number(data.baseTotal||0);
    const baseGravada = Number(data.baseGravada||0);
    ...
    L.p(`Proyecto: ${p.name || "—"}`);
    L.p(`Presupuesto analizado: ${budgetLabel(budgetKey)}`);
    L.p(`Entidad: ${p.entity || "—"}`);
    L.p(`Ubicación: ${p.location || "—"}`);
    L.p(`Fecha generación: ${new Date().toLocaleString()}`);
    L.p(`Tarifa general configurada de IVA: ${fmt2(ivaPct)}%`);
    L.p(`Criterio aplicado: NO se liquida IVA sobre MANO DE OBRA ni TRANSPORTES.`);
    L.p(" ");

    const margin = L.margin;
    const contentW = L.contentW;

    drawBand(doc, margin, L.getY(), contentW, 18, "IVA DISCRIMINADO POR CATEGORÍA", { fontSize:9.8, bold:true });
    L.setY(L.getY()+24);

    const cols = fitColsToWidth([
      { k:"cat",  label:"CATEGORÍA", w:180, align:"left" },
      { k:"base", label:"BASE", w:130, align:"right" },
      { k:"pct",  label:"IVA %", w:80, align:"right" },
      { k:"obs",  label:"OBSERVACIÓN", w:120, align:"left" },
      { k:"iva",  label:"VALOR IVA", w:120, align:"right" }
    ], contentW, 24);
    ...
    function row(r, isTotal=false){
      const obsText = isTotal ? "" : (r.appliesIva ? "Gravado" : "No aplica");
      ...
      drawCellText(doc, x, yTop, cols[0].w, rowH, r.label, "left", { bold:isTotal, fontSize:8.1 }); x += cols[0].w;
      drawCellText(doc, x, yTop, cols[1].w, rowH, moneyCOP0(r.base||0), "right", { bold:isTotal, fontSize:8.1 }); x += cols[1].w;
      drawCellText(doc, x, yTop, cols[2].w, rowH, fmt2(r.appliedPct||0) + "%", "right", { bold:isTotal, fontSize:8.1 }); x += cols[2].w;
      drawCellText(doc, x, yTop, cols[3].w, rowH, obsText, "left", { bold:isTotal, fontSize:8.1 }); x += cols[3].w;
      drawCellText(doc, x, yTop, cols[4].w, rowH, moneyCOP0(r.iva||0), "right", { bold:isTotal, fontSize:8.1 });

      hLine(doc, margin, margin+contentW, yTop + rowH, isTotal ? PDF_THEME.lines.band.w : PDF_THEME.lines.row.w, isTotal ? PDF_THEME.lines.band.c : PDF_THEME.lines.row.c);
      L.setY(yTop + rowH + 2);
    }
    ...
    if(!rows.length){
      L.p("Sin información para calcular IVA del proyecto.");
    }else{
      for(const r of rows){
        row(r, false);
      }
    ...
    L.setY(L.getY()+14);
    drawBand(doc, margin, L.getY(), contentW, 18, "RESUMEN", { fontSize:9.8, bold:true });
    L.setY(L.getY()+24);

    L.row("BASE TOTAL ANALIZADA:", moneyCOP0(baseTotal));
    L.row("BASE GRAVADA CON IVA:", moneyCOP0(baseGravada));
    L.row("BASE NO GRAVADA (M.O. + TRANSPORTES):", moneyCOP0(baseTotal - baseGravada));
    L.row(`IVA TOTAL DEL PROYECTO:`, moneyCOP0(ivaTotal));

    await appendElaboradorFirma(doc, L);

    stampHeaderFooter(doc, {
      docType:"PDF IVA PROYECTO",
      projectName: p.name || "",
      logoDataUrl: p.logoDataUrl || "",
      budgetKey
    });

    const filename = buildFilenameIvaProyecto(p, budgetKey);
  ...
  async function exportPresupuestoObraDesagregadoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "PRESUPUESTO DE OBRA DESAGREGADO", budgetKey);
    ...
    const agg = await calcDesagregadoByItemFromAPU(p, items || [], budgetKey);
    const rows = agg.rows || [];
    ...
    const filename = buildFilenameDesagregado(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportResumenPresupuestoObraDesagregadoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "RESUMEN PRESUPUESTO DE OBRA DESAGREGADO", budgetKey);
    ...
    const filename = buildFilenameResumenDesagregado(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportDistribucionPorcentualCostosDirectosPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "DISTRIBUCIÓN PORCENTUAL DE COSTOS DIRECTOS", budgetKey);
    ...
    const filename = buildFilenameDistribucionPctDirectos(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportRendimientoEquipoYManoDeObraPorActividadPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "RENDIMIENTO EQUIPO Y MANO DE OBRA POR ACTIVIDAD", budgetKey);
    ...
    async function itemBlock(it){
      const qtyItem = Number(it.qty||0);
      const apuObj = await getAPUForProjectItem(p, it, budgetKey);
      const lines = (apuObj && Array.isArray(apuObj.lines)) ? apuObj.lines : [];
    ...
    const filename = buildFilenameRendimientos(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportResumenMaterialesPorActividadPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "RESUMEN MATERIALES POR ACTIVIDAD", budgetKey);
    ...
    const filename = buildFilenameResumenMaterialesActividad(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportCantidadRecursosEInsumosPresupuestoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "CANTIDAD DE RECURSOS E INSUMOS DEL PRESUPUESTO", budgetKey);
    ...
    const filename = buildFilenameCantRecursosInsumos(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  async function exportPresupuestoPDF(project, opts){
    ...
    const { groups, items } = groupByChaptersCompat(p, budgetKey);

    const totals = totalsCompat(p, budgetKey);
    const adminPct = pct(p, "adminPct", "aiuPct");
    const imprevPct = pct(p, "imprevPct", null);
    const utilPct = pct(p, "utilPct", null);
    const ivaUtilPct = pct(p, "ivaUtilPct", "ivaPct");

    drawInstitutionalCover(doc, L, p, "PRESUPUESTO DE OBRA", budgetKey);

    doc.addPage();
    L.setY(PDF_THEME.safe.top);

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, `RESUMEN ECONÓMICO — ${budgetLabel(budgetKey)}`, { fontSize:10, bold:true });
    L.setY(L.getY()+24);
    ...
    const filename = buildFilename(p, "DETALLE", budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, p, "PRESUPUESTO DE OBRA + APUs", budgetKey);
    ...
    const filename = buildFilename(p, "APUS", budgetKey);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportEspecificacionesTecnicasPDF(project, opts){
    ...
    const entidadContratante = p?.entity ? String(p.entity) : "—";
    const ubicacion = p?.location ? String(p.location) : "—";

    drawInstitutionalCover(doc, L, p, "ESPECIFICACIONES TÉCNICAS DEL PROYECTO", budgetKey);
    ...
    L.p(
      ...
      "dentro del presupuesto analizado del proyecto. Las especificaciones se estructuran con base en el " +
    ...
    const filename = buildFilenameSpecs(p, budgetKey);
    return await finalizePDF(doc, filename, opts);
  }

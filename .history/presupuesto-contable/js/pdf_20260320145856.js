(function(){
  /* =========================================================
     ...
     ✅ AJUSTE NUEVO (2026-03 textos largos completos):
     - En todos los PDF, los títulos y descripciones de ítems deben salir completos.
     - Se aumenta el número de líneas permitidas y el alto dinámico de fila.
     - Se evita recorte artificial en descripciones extensas.

     ✅ AJUSTE NUEVO (2026-03 facturación categorías):
     - Se preparan categorías extendidas para reportes:
         * MATERIALES
  ...
  function splitFullLines(doc, text, maxW){
  ...
  function mkLayout(doc){
    ...
    function p(t){
      ...
      doc.setFontSize(10);
      const lines = splitFullLines(doc, t, contentW);
    ...
    function tableRowPresupuesto(it){
      ...
      // AJUSTE: descripción completa, hasta 8 líneas si es necesario
      const descLines = splitFullLines(doc, safe(it.desc), DESC_W).slice(0, 8);
    ...
    function tableRowAPU(lineObj){
      ...
      // AJUSTE: descripción completa, hasta 8 líneas
      const descLines = splitFullLines(doc, safe(lineObj.desc), A_DESC_W).slice(0, 8);
  ...
  async function getAPUForProjectItem(project, it){
   ...
   async function getAPULinesForProjectItem(project, it){
    const apu = await getAPUForProjectItem(project, it);
    return Array.isArray(apu?.lines) ? apu.lines : [];
  }

  // =========================================================
  // CLASIFICACIÓN FACTURACIÓN (AJUSTADA)
  // =========================================================
  ...
  // =========================================================
  // FACTURACIÓN PROYECTADA (NUEVA LÓGICA COMPLETA)
  // =========================================================
  ...
  async function calcProjectedFacturacion(project){
    ...
    for(const it of items){
      ...
      for(const ln of lines){
        ...
        const qtyLn = Number(ln.qty || 0);
        const puLn  = Number(ln.pu  || 0);

        const parcial = Number(ln.parcial || 0) || (qtyLn * puLn);

        projected[cat] = Number(projected[cat] || 0) + (parcial * qtyItem);
      }
    }
  ...
  // =========================================================
  // FACTURACIÓN REGISTRADA (CON UTILIDAD AUTOMÁTICA)
  // =========================================================

  function calcFacturacionRegistrada(project){

    const rows = (window.getProjectFacturacion)
      ? getProjectFacturacion(project)
      : [];
    ...
    for(const r of rows){
      ...
      // 🔴 IMPORTANTE: UTILIDAD NO SE SUMA DIRECTO
      if(cat !== "UTILIDAD"){
        out[cat] = Number(out[cat] || 0) + val;
      }
    }

    // 🔴 UTILIDAD = SOBRANTE REAL
    const sumaSinUtilidad = cats
      .filter(c => c !== "UTILIDAD")
      .reduce((s,c)=> s + Number(out[c]||0), 0);

    out["UTILIDAD"] = totalFacturado - sumaSinUtilidad;

    return out;
  }

  // =========================================================
  // TABLA FACTURACIÓN PARA PDF
  // =========================================================

  async function buildFacturacionTableData(project){

    const projected = await calcProjectedFacturacion(project);
    const facturado = calcFacturacionRegistrada(project);
  ...
  // =========================================================
  // EXPORTAR PDF FACTURACIÓN (AJUSTADO)
  // =========================================================

  async function exportarFacturacionPDF(project){
    ...
    drawInstitutionalCover(doc, L, project, "RESUMEN DE FACTURACIÓN REGISTRADA");

    doc.addPage();

    const data = await buildFacturacionTableData(project);

    L.setY(80);

    doc.setFont("helvetica","bold");
    doc.setFontSize(12);
    doc.text("RESUMEN DE FACTURACIÓN REGISTRADA", L.margin, L.getY());
    ...
    const headerY = L.getY();

    doc.setFont("helvetica","bold");
    doc.setFontSize(9);

    doc.text("CATEGORÍA", x0, headerY);
    doc.text("PROYECTADO", x0 + cols[0].w + cols[1].w - 4, headerY, {align:"right"});
    doc.text("FACTURADO", x0 + cols[0].w + cols[1].w + cols[2].w - 4, headerY, {align:"right"});
    doc.text("DIFERENCIA", x0 + cols[0].w + cols[1].w + cols[2].w + cols[3].w - 4, headerY, {align:"right"});

    L.setY(headerY + 10);
    hLine(doc, x0, x0 + L.contentW, L.getY());
    ...
    for(const r of data){
      ...
      doc.text(r.categoria, x0, y);

      doc.text(moneyCOP(r.proyectado), x0 + cols[0].w + cols[1].w - 4, y, {align:"right"});
      doc.text(moneyCOP(r.facturado), x0 + cols[0].w + cols[1].w + cols[2].w - 4, y, {align:"right"});
      doc.text(moneyCOP(r.diferencia), x0 + cols[0].w + cols[1].w + cols[2].w + cols[3].w - 4, y, {align:"right"});

      L.setY(y + 12);
      hLine(doc, x0, x0 + L.contentW, L.getY(), PDF_THEME.lines.row.w, PDF_THEME.lines.row.c);
      L.setY(L.getY() + 4);
    }
    ...
    if(isMobileLike()){
      ...
      doc.save(buildFilename(project, "facturacion"));
    }
  }
  ...
  async function calcIvaProyectoByCategoria(project, items){
  ...
  async function exportIvaProyectoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, project, "PDF IVA PROYECTO");

    doc.addPage();
    L.setY(PDF_THEME.safe.top);

    const data = await calcIvaProyectoByCategoria(project, items || []);
    ...
    function row(r, isTotal=false){
      const obsText = isTotal
        ? ""
        : (r.appliesIva ? "Gravado" : "No aplica");
    ...
    const filename = buildFilenameIvaProyecto(project);
  ...
  async function exportPresupuestoObraDesagregadoPDF(project, opts){
    ...
    function dataRow(r){
      ...
      const descLines = splitFullLines(doc, r.desc, cols[1].w - 6).slice(0, 8);
      const lineH = 9.2;
  ...
  async function exportRendimientoEquipoYManoDeObraPorActividadPDF(project, opts){
    ...
    async function itemBlock(it){
      ...
      for(let i=0;i<maxRows;i++){
        ...
        const eqLines = splitFullLines(doc, eq?.desc||"", cols[3].w - 6).slice(0,8);
        const moLines = splitFullLines(doc, mo?.desc||"", cols[6].w - 6).slice(0,8);
  ...
  async function exportResumenMaterialesPorActividadPDF(project, opts){
    ...
    async function itemMaterials(it){
      ...
      if(!mats.length){
        ...
        const itDesc = splitFullLines(doc, safe(it.desc||""), cols[1].w-6).slice(0,8);
        ...
        for(let i=0;i<mats.length;i++){
          ...
          doc.setFont("helvetica","normal"); doc.setFontSize(7.2);
          const itDesc = splitFullLines(doc, safe(it.desc||""), cols[1].w-6).slice(0,8);
          const insDesc = splitFullLines(doc, safe(ln.desc||""), cols[4].w-6).slice(0,8);
  ...
  async function exportCantidadRecursosEInsumosPresupuestoPDF(project, opts){
    ...
    function row(rec){
      doc.setFont("helvetica","normal"); doc.setFontSize(7.2);
      const descLines = splitFullLines(doc, rec.desc, cols[0].w - 6).slice(0,8);
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    for(const it of items){
      ...
      const subLines = splitFullLines(doc, safe(apuObj.subtitle || it.desc || ""), L.contentW);
      doc.text(subLines, L.margin, L.getY());
      L.setY(L.getY() + (subLines.length * 12) + 4);

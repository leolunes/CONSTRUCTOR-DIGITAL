(function(){
  /* =========================================================
     ...
     - Textos y filas NUNCA se montan (alto de fila dinámmico)
  ...
  async function calcIvaProyectoByCategoria(project, items){
    ...
    const ivaRows = [
      { label:"MATERIALES", base: Number(baseTotals.MATERIALES||0) },
      { label:"EQUIPO Y HERRAM", base: Number(baseTotals["EQUIPO Y HERRAM"]||0) },
      { label:"MANO DE OBRA", base: Number(baseTotals["MANO DE OBRA"]||0) },
      { label:"SUBCONTRATOS", base: Number(baseTotals.SUBCONTRATOS||0) },
      { label:"TRANSPORTES", base: Number(baseTotals.TRANSPORTES||0) },
      { label:"OTROS", base: Number(baseTotals.OTROS||0) }
    ].map(r=>{
      const iva = r.base * (ivaPct/100);
      return { ...r, iva };
    });
  ...
  async function exportIvaProyectoPDF(project, opts){
    ...
    L.p(`Tarifa de IVA aplicada: ${fmt2(ivaPct)}%`);
    L.p(" ");
    ...
    const cols = fitColsToWidth([
      { k:"cat", label:"CATEGORÍA", w:220, align:"left" },
      { k:"base", label:"BASE", w:150, align:"right" },
      { k:"pct", label:"IVA %", w:80, align:"right" },
      { k:"iva", label:"VALOR IVA", w:150, align:"right" }
    ], contentW, 24);
    ...
    function row(r, isTotal=false){
      ...
      drawCellText(doc, x, yTop, cols[0].w, rowH, r.label, "left", { bold:isTotal, fontSize:8.2 }); x += cols[0].w;
      drawCellText(doc, x, yTop, cols[1].w, rowH, moneyCOP0(r.base||0), "right", { bold:isTotal, fontSize:8.2 }); x += cols[1].w;
      drawCellText(doc, x, yTop, cols[2].w, rowH, fmt2(ivaPct) + "%", "right", { bold:isTotal, fontSize:8.2 }); x += cols[2].w;
      drawCellText(doc, x, yTop, cols[3].w, rowH, moneyCOP0(r.iva||0), "right", { bold:isTotal, fontSize:8.2 });
    ...
    L.row(`IVA TOTAL DEL PROYECTO (${fmt2(ivaPct)}%):`, moneyCOP0(ivaTotal));

...
function loadStore(){
  ...
  try{
    ...
    if(!db.meta) db.meta = { createdAt:nowISO(), version:12, lastSavedAt: nowISO() };
    if(!Number.isFinite(Number(db.meta.version))) db.meta.version = 12;
    ...
    if(db.meta.version < 12) db.meta.version = 12;
    ...
    if(rawMain && rawBackup && rawMain !== rawBackup){
      try{
        ...
        if(!recovered.meta) recovered.meta = { createdAt:nowISO(), version:12, lastSavedAt: nowISO() };
...
async function importBackupFromFile(file){
  ...
  if(!data.meta) data.meta = { createdAt:nowISO(), version:12, lastSavedAt: nowISO() };

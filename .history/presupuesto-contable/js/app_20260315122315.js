...
function getProjectExpuria(project){
  return Array.isArray(project?.facturacionExpuria) ? project.facturacionExpuria : [];
}
...
async function renderFacturacionTables(project){
  ...
  factBody.innerHTML = rows.map(r=>`
    <tr>
      <td>${UI.esc(String(r.fecha||"").replace("T"," ").slice(0,16))}</td>
  ...
  factBody.querySelectorAll("[data-delfact]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      renderFacturacionTables(updated);
      renderExpuriaTable(updated);
    });
  });
}

function renderExpuriaTable(project){
  const body = UI.qs("#expuriaBody");
  const empty = UI.qs("#expuriaEmpty");
  if(!body) return;

  const rows = getProjectExpuria(project).slice().sort((a,b)=> String(b.fecha||"").localeCompare(String(a.fecha||"")));
  ...
  body.innerHTML = rows.map(r=>`
    <tr>
      ...
      <td>${UI.esc(r.categoria||"")}</td>
      <td style="text-align:right">${UI.fmtMoney(Number(r.valor||0), project.currency||"COP")}</td>
      <td>${UI.esc(r.descripcion||"")}</td>
      <td>
        <button class="btn danger" type="button" data-delexp="${UI.esc(r.id)}">Eliminar</button>
      </td>
    </tr>
  `).join("");
  ...
  body.querySelectorAll("[data-delexp]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const id = btn.getAttribute("data-delexp");
      const fresh = StorageAPI.getProjectById(project.id);
      if(!fresh) return;

      const next = getProjectExpuria(fresh).filter(x => String(x.id) !== String(id));
      StorageAPI.updateProject(fresh.id, { facturacionExpuria: next });
...
function bindFacturacionUI(projectId){
  ...
  const formExp = UI.qs("#formFacturacionExpuria");
  ...
  formExp?.addEventListener("submit", (e)=>{
    ...
    if(!categoria){
      alert("Seleccione la categoría asociada.");
      return;
    }
    if(!(valor > 0)){
      alert("El valor observado debe ser mayor a 0.");
      return;
    }
    if(!descripcion){
      alert("Describa el hallazgo.");
      return;
    }

    const next = getProjectExpuria(fresh).slice();
    next.push({
      id: makeId("exp"),
      fecha: new Date().toISOString(),
      categoria,
    ...
    StorageAPI.updateProject(projectId, { facturacionExpuria: next });
    formExp.reset();
...
function openAddApuModal(project, apu){
  ...
  if(!modal) {
    ...
    renderFacturacionTables(fresh);
    renderExpuriaTable(fresh);

...
function normalizeFactCategory(v){
  ...
  if(s === "EQUIPO_Y_HERRAM" || s === "EQUIPO_HERRAMIENTAS" || s === "EQUIPO_Y_HERRAMIENTA") return "EQUIPO_Y_HERRAMIENTAS";
  if(s === "MANO_OBRA") return "MANO_DE_OBRA";
  if(s === "TRANSPORTE") return "TRANSPORTES";
  if(FACT_CATEGORIES.includes(s)) return s;
  return "OTROS";
}

function buildEmptyFacturacion(){
  return {
    registros: [],
    hallazgos: []
  };
...
function normalizeFacturacionRegistro(r){
  ...
  const cantidad = toNum(r?.cantidad, 0);
  const valorUnitario = toNum(r?.valorUnitario, 0);
  const valorFacturado = toNum(
    r?.valorFacturado,
    Number((cantidad * valorUnitario).toFixed(2))
  );

  return {
    id: safeStr(r?.id) || uid("fac"),
    categoria,
    fecha: safeStr(r?.fecha),
    numeroFactura: safeStr(r?.numeroFactura),
    tercero: safeStr(r?.tercero),
    nit: safeStr(r?.nit),
    concepto: safeStr(r?.concepto),
    unidad: safeStr(r?.unidad),
    cantidad,
    valorUnitario,
    valorFacturado,
    observacion: safeStr(r?.observacion),
    soporteDocId: safeStr(r?.soporteDocId),
    createdAt: safeStr(r?.createdAt) || nowISO(),
    updatedAt: safeStr(r?.updatedAt) || nowISO()
  };
}

function normalizeFacturacionHallazgo(h){
  return {
    id: safeStr(h?.id) || uid("hall"),
    fecha: safeStr(h?.fecha) || nowISO().slice(0,10),
    tipo: safeStr(h?.tipo) || "FACTURACION_EXPUREA_O_FICTICIA",
    severidad: safeStr(h?.severidad) || "ALTA",
    categoria: normalizeFactCategory(h?.categoria),
    facturaId: safeStr(h?.facturaId),
    numeroFactura: safeStr(h?.numeroFactura),
    tercero: safeStr(h?.tercero),
    concepto: safeStr(h?.concepto),
    valorObservado: toNum(h?.valorObservado, 0),
    motivo: safeStr(h?.motivo),
    analisis: safeStr(h?.analisis),
    estado: safeStr(h?.estado) || "PENDIENTE",
    createdAt: safeStr(h?.createdAt) || nowISO(),
    updatedAt: safeStr(h?.updatedAt) || nowISO()
  };
}

function normalizeFacturacion(raw){
  const base = raw && typeof raw === "object" ? raw : buildEmptyFacturacion();

  const registros = Array.isArray(base.registros) ? base.registros : [];
  const hallazgos = Array.isArray(base.hallazgos) ? base.hallazgos : [];

  return {
    registros: registros.map(normalizeFacturacionRegistro),
    hallazgos: hallazgos.map(normalizeFacturacionHallazgo)
  };
}
...
function createProject(payload){
  ...
  const p = normalizeProject({
    ...
    items: [],
    apuOverrides: {},
    facturacion: buildEmptyFacturacion(),
    ...payload
  });
...
function listFacturacion(projectId, categoria=""){
  const p = getProjectById(projectId);
  if(!p) return [];
  const rows = Array.isArray(p.facturacion?.registros) ? p.facturacion.registros : [];
...
function addFacturacion(projectId, payload){
  ...
  const p = db.projects[pidx];
  if(!p.facturacion) p.facturacion = buildEmptyFacturacion();
  if(!Array.isArray(p.facturacion.registros)) p.facturacion.registros = [];

  const rec = normalizeFacturacionRegistro(payload || {});
  p.facturacion.registros.unshift(rec);
  p.updatedAt = nowISO();
...
function updateFacturacion(projectId, facturaId, patch){
  ...
  const idx = p.facturacion.registros.findIndex(r => String(r.id) === String(facturaId));
  if(idx === -1) return null;

  const merged = {
    ...p.facturacion.registros[idx],
    ...(patch || {}),
    updatedAt: nowISO()
  };
  p.facturacion.registros[idx] = normalizeFacturacionRegistro(merged);
  p.updatedAt = nowISO();

  db.projects[pidx] = normalizeProject(p);
  saveStore(db);
  return p.facturacion.registros[idx];
}
...
function deleteFacturacion(projectId, facturaId){
  ...
  if(!p.facturacion) p.facturacion = buildEmptyFacturacion();
  if(!Array.isArray(p.facturacion.registros)) p.facturacion.registros = [];
  if(!Array.isArray(p.facturacion.hallazgos)) p.facturacion.hallazgos = [];

  const before = p.facturacion.registros.length;
  p.facturacion.registros = p.facturacion.registros.filter(r => String(r.id) !== String(facturaId));

  const changed = p.facturacion.registros.length !== before;
  if(changed){
    p.facturacion.hallazgos = p.facturacion.hallazgos.filter(h => String(h.facturaId||"") !== String(facturaId));
    p.updatedAt = nowISO();
    db.projects[pidx] = normalizeProject(p);
    saveStore(db);
  }
...
function sumFacturacionByCategoria(projectId){
  ...
  const out = {};
  for(const cat of FACT_CATEGORIES) out[cat] = 0;
  for(const r of rows){
    const cat = normalizeFactCategory(r.categoria);
    out[cat] = Number(out[cat]||0) + Number(r.valorFacturado||0);
  }
  return out;
}

/* =========================
   HALLAZGOS FACTURACIÓN EXPÚREA / FICTICIA
   ========================= */
function listFacturacionHallazgos(projectId){
  const p = getProjectById(projectId);
  if(!p) return [];
  return Array.isArray(p.facturacion?.hallazgos) ? p.facturacion.hallazgos.slice() : [];
}

function addFacturacionHallazgo(projectId, payload){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return null;
  ...
  const rec = normalizeFacturacionHallazgo(payload || {});
  p.facturacion.hallazgos.unshift(rec);
  p.updatedAt = nowISO();
...
function updateFacturacionHallazgo(projectId, hallazgoId, patch){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return null;
  ...
  const idx = p.facturacion.hallazgos.findIndex(h => String(h.id) === String(hallazgoId));
  if(idx === -1) return null;

  const merged = {
    ...p.facturacion.hallazgos[idx],
    ...(patch || {}),
    updatedAt: nowISO()
  };
  p.facturacion.hallazgos[idx] = normalizeFacturacionHallazgo(merged);
  p.updatedAt = nowISO();

  db.projects[pidx] = normalizeProject(p);
  saveStore(db);
  return p.facturacion.hallazgos[idx];
}

function deleteFacturacionHallazgo(projectId, hallazgoId){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return false;
  ...
  const before = p.facturacion.hallazgos.length;
  p.facturacion.hallazgos = p.facturacion.hallazgos.filter(h => String(h.id) !== String(hallazgoId));

  const changed = p.facturacion.hallazgos.length !== before;
  if(changed){
    p.updatedAt = nowISO();
    db.projects[pidx] = normalizeProject(p);
    saveStore(db);
...
window.StorageAPI = {
  ...
  listFacturacionHallazgos, addFacturacionHallazgo, updateFacturacionHallazgo, deleteFacturacionHallazgo,

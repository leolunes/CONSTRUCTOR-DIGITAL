...
function importarExcel() {
  ...
  reader.onload = function (e) {
    try {
      ...
      // Elimina encabezado y filas vacías
      const filas = json
        .slice(1)
        .filter(fila => Array.isArray(fila) && (String(fila[0] || "").trim() !== "" || String(fila[1] || "").trim() !== ""));

      if (!filas.length) {
        alert("No se encontraron filas con proponente y oferta.");
        return;
      }

      // Genera filas y usa los IDs reales creados por db.js
      const nuevasOfertas = generarLineasOfertas(filas.length);

      filas.forEach((fila, index) => {
        const empresa = String(fila[0] || "").trim();

        let valorTexto = String(fila[1] || "0").trim();

        // Limpia símbolos monetarios, espacios y separadores
      ...
      // Actualiza automáticamente el número de propuestas habilitadas
      if ($("#procesoNumeroPropuestas")) {
        $("#procesoNumeroPropuestas").value = filas.length;
      }
      ...
      alert(`Importación exitosa: ${filas.length} oferta(s) cargada(s).`);
    } catch (error) {
      console.error(error);

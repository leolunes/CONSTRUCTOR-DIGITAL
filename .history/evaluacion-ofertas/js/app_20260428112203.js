...
async function agregarArchivosAnexosAlProyecto() {
  ...
  guardarDB();
  guardarProyectoConNombreDeProceso();
  guardarDB();
...
async function extraerInformacionPDF() {
  ...
  resultadoBox.innerHTML = "Leyendo PDF(s), por favor espere...";
  ...
  if (autollenado) {
    ...
    html += `
      <strong>No se encontraron datos suficientes para autollenar el formulario.</strong><br>
...
// ===============================
// NOMBRES DE PROYECTOS - FUENTE: INFORMACIÓN BÁSICA DEL PROCESO
// ===============================
function textoLimpioProyecto(valor = "") {
  return String(valor || "")
    .replace(/\s+/g, " ")
    .trim();
}
...
function esNombreProyectoGenerico(nombre = "") {
  const limpio = textoLimpioProyecto(nombre);
  if (!limpio) return true;

  return (
    /^proyecto(\s+sin\s+nombre)?$/i.test(limpio) ||
    /^proyecto\s+\d+$/i.test(limpio) ||
    /^nuevo\s+proyecto$/i.test(limpio)
  );
}

function obtenerProcesoDeProyectoGuardado(proyecto = {}) {
  return proyecto?.data?.proceso || proyecto?.proceso || {};
}

function obtenerNombreProyectoDesdeProceso(proceso = {}) {
  const nombreProceso = textoLimpioProyecto(proceso?.nombre);
  if (nombreProceso) return nombreProceso;

  const entidad = textoLimpioProyecto(proceso?.entidad);
  if (entidad) return entidad;

  return "Proyecto sin nombre";
}

function obtenerNombreVisibleProyecto(proyecto = {}) {
  const proceso = obtenerProcesoDeProyectoGuardado(proyecto);
  const nombreDesdeProceso = obtenerNombreProyectoDesdeProceso(proceso);

  if (nombreDesdeProceso && nombreDesdeProceso !== "Proyecto sin nombre") {
    return nombreDesdeProceso;
  }

  const nombreGuardado = textoLimpioProyecto(proyecto?.nombre);
  if (nombreGuardado && !esNombreProyectoGenerico(nombreGuardado)) {
...
function obtenerNombreProyectoDesdeFormulario() {
  const nombreFormulario = textoLimpioProyecto($("#procesoNombre")?.value || DB.proceso?.nombre || "");
  if (nombreFormulario) return nombreFormulario;

  const entidadFormulario = textoLimpioProyecto($("#procesoEntidad")?.value || DB.proceso?.entidad || "");
  if (entidadFormulario) return entidadFormulario;

  return "Proyecto sin nombre";
}

function guardarProyectoConNombreDeProceso() {
  const nombreProyecto = obtenerNombreProyectoDesdeFormulario();
  guardarProyectoActual(nombreProyecto);
}
...
function inicializarGestionProyectos() {
  ...
  function obtenerListaProyectosFiltrada(filtro = "") {
    const texto = textoLimpioProyecto(filtro).toLowerCase();
    const lista = listarProyectos();

    if (!texto) return lista;

    return lista.filter(p => {
      const nombreVisible = obtenerNombreVisibleProyecto(p).toLowerCase();
      const nombreGuardado = textoLimpioProyecto(p?.nombre).toLowerCase();
      const proceso = obtenerProcesoDeProyectoGuardado(p);
      const entidad = textoLimpioProyecto(proceso?.entidad).toLowerCase();
      return nombreVisible.includes(texto) || nombreGuardado.includes(texto) || entidad.includes(texto);
    });
  }
  ...
  function renderLista(filtro = "") {
    ...
    const lista = obtenerListaProyectosFiltrada(filtro);
    select.innerHTML = "";

    if (!lista.length) {
      const opt = document.createElement("option");
      opt.value = "";
      opt.textContent = "No hay proyectos guardados";
    ...
    lista.forEach(p => {
      const opt = document.createElement("option");
      opt.value = p.id;
      opt.textContent = obtenerNombreVisibleProyecto(p);
      if (Number(p.id) === Number(PROYECTOS.activoId)) opt.selected = true;
      select.appendChild(opt);
    });
  }

  btnNuevo?.addEventListener("click", () => {
    const nombreInicial = prompt("Nombre o número del proceso:", "");
    if (nombreInicial === null) return;

    crearNuevoProyecto(textoLimpioProyecto(nombreInicial) || "Proyecto sin nombre");

    // El usuario diligencia manualmente la información básica del proceso.
    actualizarProceso({
      nombre: textoLimpioProyecto(nombreInicial),
      objeto: "",
      entidad: "",
      presupuesto: 0,
      trm: 0,
  ...
  btnGuardar?.addEventListener("click", () => {
    ...
    if (!nombreProyecto || nombreProyecto === "Proyecto sin nombre") {
      alert("Digite primero el nombre o número del proceso en Información básica del proceso.");
      return;
    }

    guardarProyectoActual(nombreProyecto);
    guardarDB();
    renderLista(inputBuscar?.value || "");
    alert("Proyecto guardado correctamente como: " + nombreProyecto);
  });

...
export function calcularValorCompetitivoHistorico() {
  ...
  const muestras = [];
  const proyectosDescartados = [];

  // Rango prudente para validar que el presupuesto oficial sea coherente
  // con las ofertas. Si la mediana de las ofertas está muy lejos del
  // presupuesto, el proyecto no se usa para el histórico porque generaría
  // desviaciones falsas como 199%, 250%, etc.
  const RATIO_MINIMO_OFERTA_PRESUPUESTO = 0.50;
  const RATIO_MAXIMO_OFERTA_PRESUPUESTO = 1.30;
  const DESVIACION_MINIMA_PERMITIDA = -50;
  const DESVIACION_MAXIMA_PERMITIDA = 30;
  ...
  function validarCoherenciaProyectoHistorico(proyecto, proceso, presupuesto, ofertasValidas) {
    if (!presupuesto || presupuesto <= 0) {
      return { valido: false, motivo: "sin presupuesto oficial válido" };
    }

    if (!ofertasValidas.length) {
      return { valido: false, motivo: "sin ofertas válidas" };
    }
    ...
    if (!valores.length) {
      return { valido: false, motivo: "sin valores de oferta válidos" };
    }

    const medianaOfertas = mediana(valores);
    const ratio = medianaOfertas / presupuesto;

    if (ratio < RATIO_MINIMO_OFERTA_PRESUPUESTO || ratio > RATIO_MAXIMO_OFERTA_PRESUPUESTO) {
      return {
        valido: false,
        motivo: `presupuesto incoherente con las ofertas: mediana/presupuesto = ${ratio.toFixed(2)}`
      };
    }
  ...
  PROYECTOS.lista.forEach(proyecto => {
    ...
    const presupuesto = Number(proceso.presupuesto || 0);
    const ofertas = Array.isArray(data.ofertas) ? data.ofertas : [];

    const ofertasBase = ofertas.filter(esOfertaHistoricaValida);
    const validacion = validarCoherenciaProyectoHistorico(proyecto, proceso, presupuesto, ofertasBase);

    if (!validacion.valido) {
      proyectosDescartados.push({
    ...
    const ofertasValidas = ofertasBase
      .map(o => {
        const desviacion = ((Number(o.oferta || 0) - presupuesto) / presupuesto) * 100;
      ...
      .filter(o =>
        Number.isFinite(Number(o.desviacion)) &&
        Number(o.desviacion) >= DESVIACION_MINIMA_PERMITIDA &&
        Number(o.desviacion) <= DESVIACION_MAXIMA_PERMITIDA
      );
    ...
    if (!ofertasValidas.length) {
      proyectosDescartados.push({
        proyectoId: proyecto.id,
        proyectoNombre: proyecto.nombre || proceso.nombre || "Proyecto sin nombre",
        motivo: "desviaciones fuera de rango razonable"
      });
      return;
    }

    // 🔹 Prioridad 1: rankingFinal real
    let top = ofertasValidas
      .filter(o => Number(o.rankingFinal || 0) > 0)
      .sort((a, b) => Number(a.rankingFinal || 0) - Number(b.rankingFinal || 0))
      .slice(0, 3);

    // 🔹 Prioridad 2: puntajeTotal
    if (!top.length) {
      top = [...ofertasValidas]
        .sort((a, b) => Number(b.puntajeTotal || 0) - Number(a.puntajeTotal || 0))
        .slice(0, 3);
    }

    // 🔹 Prioridad 3: puntaje económico
    if (!top.length) {
      top = [...ofertasValidas]
        .sort((a, b) => Number(b.puntaje || 0) - Number(a.puntaje || 0))
        .slice(0, 3);
    }

    top.forEach((oferta, index) => {
      // TOP 3 ponderado: 1° pesa 3, 2° pesa 2, 3° pesa 1
  ...
  if (procesosAnalizados <= 1) {
    lectura = "El histórico todavía es limitado porque solo existe un proceso válido con presupuesto coherente. El resultado debe leerse como referencia inicial y mejorará cuando se guarden más proyectos correctamente parametrizados.";
  } else if (procesosAnalizados < 5) {
    lectura = "El histórico empieza a mostrar tendencia, pero aún cuenta con pocos procesos válidos. Se recomienda usarlo como apoyo y contrastarlo con el valor competitivo del proceso actual.";
  } else {
    lectura = "El histórico tiene una base más representativa y permite identificar una zona recurrente de competitividad a partir del TOP 3 ponderado de los proyectos guardados.";
  }

  if (proyectosDescartados.length) {
    lectura += ` Se descartaron ${proyectosDescartados.length} proyecto(s) del histórico por presupuesto incoherente, ofertas inválidas o desviaciones fuera de rango razonable.`;
  }
  ...
  return {
    ...
    confianza,
    metodo: "TOP 3 ponderado histórico validado",
    pesos: {
      primero: 3,
      segundo: 2,
      tercero: 1
...
function calcularAnalisisFactoresParaOfertas(ofertas = [], topN = 3) {
  ...
  const factores = FACTORES_ESTRATEGICOS.map(factor => {
    const cumplen = validas.filter(o => Boolean(o[factor.key])).length;
    const cumplenTop = top.filter(o => Boolean(o[factor.key])).length;
    const cumpleGanador = ganador ? Boolean(ganador[factor.key]) : false;
...
export function calcularFactoresEstrategicosHistorico() {
  if (!PROYECTOS || !Array.isArray(PROYECTOS.lista)) return null;
  ...
  if (!procesosAnalizados) return null;
  ...
  return {
    ...
    lecturaGeneral:
      procesosAnalizados <= 1
        ...
        : "Este análisis consolida el comportamiento de los factores adicionales en los proyectos guardados, compar&&o cumplimiento general, TOP 3 y ganadores."
  };
}

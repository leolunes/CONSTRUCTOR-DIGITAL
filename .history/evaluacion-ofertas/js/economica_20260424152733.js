...
// ===============================
// 🔥 VALOR COMPETITIVO SUGERIDO (CORREGIDO)
// ===============================
export function calcularValorCompetitivo() {
  ...
  const p50 = ordenados[Math.floor(ordenados.length*0.50)];
  const p75 = ordenados[Math.floor(ordenados.length*0.75)];

  const minReal = Math.min(...porcentajes);
  const maxReal = Math.max(...porcentajes);

  // 🔥 CLAVE: limitar simulación al rango real observado
  const rangoInferior = Math.max(minReal, p25 - 1);
  const rangoSuperior = Math.min(maxReal, p75 + 1);

  let mejor = null;

  for (let p = rangoSuperior; p >= rangoInferior; p -= 0.1) {
    ...
    if (!mejor || promedioPos < mejor.promedioPos) {
      mejor = { porcentaje: p, promedioPos };
    }
  }

  if (!mejor) return null;

  const desviacion = mejor.porcentaje - 100;

  return {
    desviacionSugerida: Number(desviacion.toFixed(2)),
    valorPesos: Math.round((mejor.porcentaje / 100) * presupuesto),
    rango: [
      Number((desviacion - 0.5).toFixed(2)),
      Number((desviacion + 0.5).toFixed(2))
    ],
    robustez: mejor.promedioPos <= 2 ? "ALTA" : mejor.promedioPos <= 4 ? "MEDIA" : "BAJA",
    riesgo: desviacion < (minReal-100) ? "ALTO" : "BAJO"
  };
}

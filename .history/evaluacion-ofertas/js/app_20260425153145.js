...
function agregarDocumentoAnexoDesdeInput() {
  ...
  const nuevoValor = String(nuevoDoc.url || nuevoDoc.nombre || "").trim().toLowerCase();

  const existe = DB.proceso.documentosAnexos.some(doc => {
    const actual = typeof doc === "string"
      ? doc
      : (doc?.url || doc?.nombre || "");

    return String(actual || "").trim().toLowerCase() === nuevoValor;
  });
...
function inicializarSecopYDocumentos() {
  ...
  // Evita comportamiento submit si el navegador lo interpreta así
  ...
  btnAgregarDoc?.addEventListener("click", e => {
    e.preventDefault();
    agregarDocumentoAnexoDesdeInput();
  });

  // Evento delegado de respaldo: funciona aunque el botón se haya recreado o no haya enlazado
  document.addEventListener("click", e => {
    if (e.target?.id === "btnAgregarDocumento") {
      e.preventDefault();
      agregarDocumentoAnexoDesdeInput();
    }

    if (e.target?.classList?.contains("btn-abrir-doc")) {
      const index = Number(e.target.dataset.index);
      asegurarArrayDocumentosAnexos();
      abrirDocumentoAnexo(DB.proceso.documentosAnexos[index]);
    }
  });
...
function renderDocumentosAnexos() {
  ...
  docs.forEach((doc, index) => {
    ...
    if (url) {
      ...
      abrir.type = "button";
      abrir.className = "btn secondary btn-abrir-doc";

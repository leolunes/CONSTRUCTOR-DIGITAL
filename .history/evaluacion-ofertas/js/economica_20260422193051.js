...
function obtenerValoresAceptados() {
  return obtenerOfertasAceptadas()
    .filter(o =>
      String(o.empresa || "").trim() !== "" &&
      Number(o.oferta || 0) > 0 &&
      String(o.estado || "").trim() !== "RECHAZADA" &&
      String(o.observacion || "").toUpperCase().trim() !== "ELIMINADA"
    )
...
export function calcularPuntajeMediana(oferta, puntajeMaximo, baseMediana, vme, totalAceptadas) {
  ...
  // Si número de ofertas es par, usa valor inmediatamente inferior a la mediana
...
export function ejecutarEvaluacionEconomica() {
  ...
  if (!bases.totalAceptadas) {
    ...
    return {
      metodo: "",
      nombreMetodo: "No existen ofertas aceptadas para evaluar",
      ...bases,
      mejorOferta: null,
      mejorPuntaje: 0
    };
  ...
  const ranking = obtenerOfertasAceptadas()
    .filter(o =>
      String(o.empresa || "").trim() !== "" &&
      Number(o.oferta || 0) > 0 &&
      String(o.estado || "").trim() !== "RECHAZADA" &&
...
export function obtenerLecturaEconomica() {
  ...
  if (!resumen.totalAceptadas) {
    return "No existen ofertas aceptadas para aplicar la evaluación económica.";
  }
  ...
  return `La evaluación económica se realizó mediante el método ${obtenerNombreMetodo(resumen.metodo)}. Se evaluaron ${resumen.totalAceptadas} propuesta(s) aceptada(s). El proponente ubicado en el primer orden de elegibilidad económico es ${resumen.mejorOferta.empresa}, con una oferta de ${resumen.mejorOferta.oferta} y un puntaje de ${resumen.mejorPuntaje}.`;
}
...
export function generarTextoEvaluacionEconomica() {
  ...
  if (!resumen.totalAceptadas) {
    return "No existen ofertas aceptadas para la ponderación económica.";
  }

...
function normalizarNombreProponenteSECOP(nombre = "") {
  let limpio = limpiarTextoCampoPDF(nombre)
    .replace(/\s+/g, " ")
    .replace(/^(nombre\s+del\s+proponente|proponente|oferente|participante)\s*/i, "")
...
function esNombreProponenteValidoSECOP(nombre = "") {
  ...
  const indicadores = [
    "consorcio", "union temporal", "unión temporal", "ut ", "sas", "s.a.s",
    "ltda", "construct", "ingenier", "campus", "santander", "proyectos",
    "edificaciones", "soluciones", "temporal", "caipro", "icssa", "met", "grupo"
  ];

  return indicadores.some(p => clave.includes(p)) || n.split(" ").length >= 2;
}
...
function extraerOfertasDesdeTextoSECOP(texto = "") {
  const textoNormalizado = normalizarLineaTablaSECOP(texto);
  const resultados = [];

  // Caso frecuente: NOMBRE $ 3,598,241,365 10.00 9.00 20.00 1.00 0.25 0.25 0.25
  const patronPrincipal = /([A-ZÁÉÍÓÚÑ0-9][A-ZÁÉÍÓÚÑ0-9 .,&()\/\-]{3,120}?)\s+\$?\s*([0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,})([^A-ZÁÉÍÓÚÑ$]{0,120})/g;
  let match;

  while ((match = patronPrincipal.exec(textoNormalizado)) !== null) {
    const empresa = normalizarNombreProponenteSECOP(match[1]);
    const oferta = limpiarValorOfertaSECOP(match[2]);
    const nums = obtenerNumerosPosterioresSECOP(match[3] || "");

    if (!esNombreProponenteValidoSECOP(empresa) || !oferta) continue;
  ...
  // Caso alterno: valor en una línea y nombre en la línea cercana.
  const lineas = String(texto || "")
    .split(/\n|\r|(?<=\d)\s{2,}(?=[A-ZÁÉÍÓÚÑ])/)
    .map(normalizarLineaTablaSECOP)
    .filter(Boolean);

  const patronValor = /\$?\s*[0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,}/;

  for (let i = 0; i < lineas.length; i++) {
    const linea = lineas[i];
    const valorMatch = linea.match(patronValor);
    if (!valorMatch) continue;

    const oferta = limpiarValorOfertaSECOP(valorMatch[0]);
    if (!oferta) continue;

    let antesValor = linea.slice(0, valorMatch.index).trim();
    let empresa = normalizarNombreProponenteSECOP(antesValor);

    if (!esNombreProponenteValidoSECOP(empresa)) {
      empresa = normalizarNombreProponenteSECOP(lineas[i - 1] || "");
    }
    ...
    if (!esNombreProponenteValidoSECOP(empresa)) continue;

    const despues = linea.slice((valorMatch.index || 0) + valorMatch[0].length);
    const nums = obtenerNumerosPosterioresSECOP(despues);
  ...
  resultados.forEach(item => {
    ...
    const key = `${empresa}__${item.oferta}`;
    ...
    // Conserva la fila con más puntajes detectados.
...
function renderTablaOfertasDetectadasSECOP(ofertas = []) {
  ...
  return `
    ...
    <p>
      Revise la tabla antes de cargarla. La extracción depende de la calidad del texto interno del PDF.
    </p>

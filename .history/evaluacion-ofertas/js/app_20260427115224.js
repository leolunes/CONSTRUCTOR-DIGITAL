...
async function leerTextoPDF(docMeta) {
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      // Conserva estructura de tabla: agrupa por coordenada Y y ordena por X.
      // Esto evita que PDF.js mezcle nombres y valores de distintas filas.
      items.sort((a, b) => {
        const diffY = b.y - a.y;
        if (Math.abs(diffY) > 2) return diffY;
        return a.x - b.x;
      });
      ...
      items.forEach(item => {
        if (yActual === null || Math.abs(item.y - yActual) <= 2) {
          filaActual.push(item);
          yActual = yActual === null ? item.y : yActual;
        } else {
      ...
      textoCompleto += `\n--- PÁGINA ${i} ---\n`;
      textoCompleto += filas
        .map(fila => fila.sort((a, b) => a.x - b.x).map(x => x.texto).join("    "))
        .join("\n");
      textoCompleto += "\n";
    }
...
function esNombreProponenteValidoSECOP(nombre = "") {
  ...
  const palabrasBloqueadas = [
    ...
    "representante legal", "firma", "folio", "anexo", "documento"
  ];
...
function filtrarClusterValoresSECOP(candidatos = []) {
  const filas = candidatos.filter(o =>
    o &&
  ...
  // Si ya hay presupuesto oficial, descarta montos que no tienen escala de oferta.
  // Esto evita tomar garantías, pólizas, estampillas u otros valores pequeños del acta.
  let filtradas = presupuesto > 0
    ? filas.filter(o => Number(o.oferta || 0) >= presupuesto * 0.35 && Number(o.oferta || 0) <= presupuesto * 1.25)
    : filas;

  if (!filtradas.length) filtradas = filas;

  const conteoPorMagnitud = {};
  filtradas.forEach(o => {
    const magnitud = Math.floor(Math.log10(Number(o.oferta || 1)));
    conteoPorMagnitud[magnitud] = (conteoPorMagnitud[magnitud] || 0) + 1;
  });

  const magnitudes = Object.entries(conteoPorMagnitud)
    .map(([k, v]) => ({ magnitud: Number(k), total: v }))
    .sort((a, b) => b.total - a.total || b.magnitud - a.magnitud);

  const magnitudPrincipal = magnitudes[0]?.magnitud;

  if (magnitudPrincipal !== undefined && magnitudes[0].total >= 3) {
    filtradas = filtradas.filter(o => Math.floor(Math.log10(Number(o.oferta || 1))) === magnitudPrincipal);
  }

  // Cuando conviven valores en millones y miles de millones, conserva el bloque alto con más filas.
  const maximo = Math.max(...filtradas.map(o => Number(o.oferta || 0)));
  if (maximo >= 1000000000) {
    filtradas = filtradas.filter(o => Number(o.oferta || 0) >= 1000000000);
  } else if (maximo >= 100000000) {
    filtradas = filtradas.filter(o => Number(o.oferta || 0) >= 100000000);
  }

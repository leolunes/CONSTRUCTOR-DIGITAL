...
// ===============================
// 🔥 EXTRACCIÓN REAL DE TEXTO DESDE PDF - ROBUSTA
// Insertado después de Documentos anexos
// ===============================
...
function limpiarValorMonedaPDF(valor = "") {
  ...
  // Si hay varios puntos, conserva solo separadores de miles eliminándolos
...
function detectarDatosClave(texto = "") {
  ...
  const proceso = textoPlano.match(/\b(LP|LPN|SA|SAMC|MC|CD|CI|IP)-?\d{1,4}-?\d{4}\b/i);
  if (proceso) {
    resultados.proceso = proceso[0].toUpperCase();
  }

  const presupuestoRegex = /(presupuesto\s+oficial|valor\s+estimado|presupuesto)[^$0-9]{0,100}(\$?\s?[0-9]{1,3}(?:[.,]?[0-9]{3}){2,})/i;
  const presupuesto = textoPlano.match(presupuestoRegex);
  if (presupuesto) {
    resultados.presupuestoTexto = presupuesto[2];
    resultados.presupuestoNumero = limpiarValorMonedaPDF(presupuesto[2]);
  }

  const entidadRegex = /(entidad\s+(contratante)?|contratante)[:\s-]{1,20}([A-ZÁÉÍÓÚÑ0-9 .,&()/-]{8,140})/i;
  const entidad = textoPlano.match(entidadRegex);
  if (entidad) {
    resultados.entidad = String(entidad[3] || "").trim();
  }

  const objetoRegex = /(objeto|objeto\s+contractual)[:\s-]{1,20}(.{30,450}?)(presupuesto|valor\s+estimado|plazo|modalidad|fecha|forma\s+de\s+pago|$)/i;
  const objeto = textoPlano.match(objetoRegex);
  if (objeto) {
    resultados.objeto = String(objeto[2] || "").trim();
  }

  const propuestas = textoPlano.match(/(n[uú]mero\s+de\s+propuestas|propuestas\s+habilitadas|proponentes\s+habilitados)[^0-9]{0,50}([0-9]{1,3})/i);
  if (propuestas) {
    resultados.numeroPropuestas = Number(propuestas[2] || 0);
  }

  const numeros = textoPlano.match(/\$?\s?[0-9]{1,3}(?:[.,]?[0-9]{3}){2,}/g);
  if (numeros && numeros.length) {
    resultados.posiblesValores = [...new Set(numeros)].slice(0, 10);
  }
...
// ===============================
// NOMBRES DE PROYECTOS - ESTABLE
// ===============================
function obtenerProyectoActivo() {
  return PROYECTOS.lista.find(p => Number(p.id) === Number(PROYECTOS.activoId)) || null;
}

function nombreProyectoActualSeguro() {
  const proyectoActual = obtenerProyectoActivo();
  const nombreActual = String(proyectoActual?.nombre || "").trim();
...
function nombreSugeridoParaGuardarProyecto() {
  const proyectoActual = obtenerProyectoActivo();
  const nombreActual = String(proyectoActual?.nombre || "").trim();
  const nombreProceso = String(DB.proceso?.nombre || "").trim();

  // Si ya existe un nombre real de proyecto, se conserva como sugerencia.
  // Solo evitamos sugerir nombres demasiado cortos tipo "LP", "SA", "MC", etc.
  if (nombreActual && nombreActual.length > 4 && nombreActual !== nombreProceso) {
    return nombreActual;
  }

  return "Proyecto " + (PROYECTOS.lista.length || 1);
}

function guardarProyectoSinRenombrar() {
  guardarProyectoActual(nombreProyectoActualSeguro());
}
...
// Regla estable:
// - La información básica del proceso se diligencia manualmente.
// - El nombre visible del proyecto se toma del campo "Nombre o número del proceso".
// - Guardar proyecto actualiza el selector con ese nombre.
// - Cargar proyecto trae TODO el proyecto guardado: información básica + ofertas + resultados.
// ===============================
function inicializarGestionProyectos() {
  ...
  function textoValidoNombreProyecto(texto = "") {
    ...
    const prohibidos = [
      ...
      "observaciones que sobre la evaluacion",
      "observaciones que sobre la evaluación",
      "a petición de los proponentes",
      "la calificación definitiva"
    ];

    const normal = limpio.toLowerCase();
    return !prohibidos.some(p => normal.includes(p));
  }

  function obtenerNombreDesdeInformacionBasica() {
    const desdeInput = String($("#procesoNombre")?.value || "").trim();
    const desdeDB = String(DB.proceso?.nombre || "").trim();

    if (textoValidoNombreProyecto(desdeInput)) return desdeInput;
    if (textoValidoNombreProyecto(desdeDB)) return desdeDB;

    return "";
  }

  function obtenerNombreVisibleProyecto(proyecto = {}) {
    const nombreGuardado = String(proyecto?.nombre || "").trim();
    const nombreProceso = String(proyecto?.data?.proceso?.nombre || "").trim();

    if (textoValidoNombreProyecto(nombreProceso)) return nombreProceso;
    if (textoValidoNombreProyecto(nombreGuardado)) return nombreGuardado;
  ...
  function renderLista(filtro = "") {
    ...
    const listaBase = listarProyectos();
    const lista = textoFiltro
      ? listaBase.filter(p => obtenerNombreVisibleProyecto(p).toLowerCase().includes(textoFiltro))
      : listaBase;
  ...
  function limpiarInformacionBasicaParaNuevoProyecto() {
    actualizarProceso({
      nombre: "",
      objeto: "",
      entidad: "",
      ...
      trm: 0,
      puntajeMaximo: 100,
      puntajeCalidad: Number(DB.proceso?.puntajeCalidad || 0),
      puntajeIndustriaNacional: Number(DB.proceso?.puntajeIndustriaNacional || 0),
      puntajeDiscapacidad: Number(DB.proceso?.puntajeDiscapacidad || 0),
      puntajeMujer: Number(DB.proceso?.puntajeMujer || 0),
      puntajeMipyme: Number(DB.proceso?.puntajeMipyme || 0),
    ...
    DB.ofertas = [];
    if (DB.resultadosOAB) Object.keys(DB.resultadosOAB).forEach(k => DB.resultadosOAB[k] = 0);
    if (DB.resultadosEconomicos) Object.keys(DB.resultadosEconomicos).forEach(k => DB.resultadosEconomicos[k] = 0);
  }

  btnNuevo?.addEventListener("click", () => {
    crearNuevoProyecto("Proyecto sin nombre");
    limpiarInformacionBasicaParaNuevoProyecto();
    guardarProyectoActual("Proyecto sin nombre");
    guardarDB();
    renderLista();
    cargarProcesoEnUI();
    renderTodo();
  ...
  btnGuardar?.addEventListener("click", () => {
    ...
    const nombreProyecto = obtenerNombreDesdeInformacionBasica();

    if (!nombreProyecto) {
      alert("Diligencie el campo 'Nombre o número del proceso' antes de guardar el proyecto.");
      $("#procesoNombre")?.focus();
      return;
    }
    ...
    alert(`Proyecto guardado como: ${nombreProyecto}`);
  });

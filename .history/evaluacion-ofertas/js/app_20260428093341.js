...
function calcularPuntajesFactores(oferta) {
  // Si la oferta fue importada desde una matriz oficial de Excel, se respetan los puntajes exactos del archivo.
  if (oferta?.puntajesImportados) {
    return {
      puntajeCalidad: Number(oferta.puntajeCalidad || 0),
      puntajeSostenibilidad: Number(oferta.puntajeSostenibilidad || 0),
      puntajeIndustria: Number(oferta.puntajeIndustria || 0),
      puntajeDiscapacidad: Number(oferta.puntajeDiscapacidad || 0),
      puntajeMujer: Number(oferta.puntajeMujer || 0),
      puntajeMipyme: Number(oferta.puntajeMipyme || 0)
    };
  }

  const puntajeCalidad = oferta.cumpleCalidad ? Number(DB.proceso.puntajeCalidad || 0) : 0;
  const puntajeSostenibilidad = oferta.cumpleSostenibilidad ? Number(DB.proceso.puntajeSostenibilidad || 0) : 0;
...
function normalizarFactoresDesdePuntajesEnOferta(oferta = {}) {
  if (!oferta || typeof oferta !== "object") return oferta;

  const puntajeCalidad = Number(oferta.puntajeCalidad || 0);
  const puntajeSostenibilidad = Number(oferta.puntajeSostenibilidad || 0);
  const puntajeIndustria = Number(oferta.puntajeIndustria || 0);
  const puntajeDiscapacidad = Number(oferta.puntajeDiscapacidad || 0);
  const puntajeMujer = Number(oferta.puntajeMujer || 0);
  const puntajeMipyme = Number(oferta.puntajeMipyme || 0);

  oferta.cumpleCalidad = Boolean(oferta.cumpleCalidad) || puntajeCalidad > 0;
  oferta.cumpleSostenibilidad = Boolean(oferta.cumpleSostenibilidad) || puntajeSostenibilidad > 0;
  oferta.cumpleIndustria = Boolean(oferta.cumpleIndustria) || puntajeIndustria > 0;
  oferta.cumpleDiscapacidad = Boolean(oferta.cumpleDiscapacidad) || puntajeDiscapacidad > 0;
  oferta.cumpleMujer = Boolean(oferta.cumpleMujer) || puntajeMujer > 0;
  oferta.cumpleMipyme = Boolean(oferta.cumpleMipyme) || puntajeMipyme > 0;

  const tienePuntajesFactor =
    puntajeCalidad > 0 ||
    puntajeSostenibilidad > 0 ||
    puntajeIndustria > 0 ||
    puntajeDiscapacidad > 0 ||
    puntajeMujer > 0 ||
    puntajeMipyme > 0;

  if (tienePuntajesFactor && oferta.puntajesImportados === undefined) {
    oferta.puntajesImportados = true;
  }

  if (tienePuntajesFactor && Number(oferta.puntajeTotal || 0) === 0) {
    oferta.puntajeTotal = Number((
      Number(oferta.puntaje || 0) +
      puntajeCalidad +
      puntajeSostenibilidad +
      puntajeIndustria +
      puntajeDiscapacidad +
...
function normalizarFactoresHistoricosDesdePuntajes() {
  let huboCambios = false;

  if (Array.isArray(DB.ofertas)) {
    DB.ofertas.forEach(oferta => {
      const antes = JSON.stringify({
        cumpleCalidad: oferta.cumpleCalidad,
        cumpleSostenibilidad: oferta.cumpleSostenibilidad,
        cumpleIndustria: oferta.cumpleIndustria,
        cumpleDiscapacidad: oferta.cumpleDiscapacidad,
        cumpleMujer: oferta.cumpleMujer,
        cumpleMipyme: oferta.cumpleMipyme,
        puntajesImportados: oferta.puntajesImportados,
        puntajeTotal: oferta.puntajeTotal
      });

      normalizarFactoresDesdePuntajesEnOferta(oferta);
  ...
  if (PROYECTOS && Array.isArray(PROYECTOS.lista)) {
    PROYECTOS.lista.forEach(proyecto => {
      const ofertas = proyecto?.data?.ofertas;
...
function recalcularPuntajeTotalOferta(id) {
  ...
  const puntajeTotal =
    ...
    factores.puntajeSostenibilidad +
...
// Mantiene separado el nombre del proyecto del nombre/número del proceso.
// ===============================
function construirNombreProyectoSugerido() {
  const entidad = String(DB.proceso?.entidad || "").trim();
  const objeto = String(DB.proceso?.objeto || "").trim();
  const proceso = String(DB.proceso?.nombre || "").trim();

  if (entidad && objeto) return `${entidad} - ${objeto}`.slice(0, 120);
  if (objeto) return objeto.slice(0, 120);
  if (entidad) return entidad.slice(0, 120);

  // Solo usa el proceso como último recurso, para evitar que proyectos queden
  // nombrados como "LP" cuando sí existe información más descriptiva.
  if (proceso && proceso.length > 3) return proceso.slice(0, 120);

  return "Proyecto sin nombre";
}

function esNombreProyectoGenerico(nombre = "") {
  const n = String(nombre || "").trim();
  const proceso = String(DB.proceso?.nombre || "").trim();

  if (!n) return true;
  if (n.toLowerCase() === "proyecto") return true;
  if (n.toLowerCase() === "proyecto sin nombre") return true;
  if (proceso && n.toLowerCase() === proceso.toLowerCase() && n.length <= 12) return true;

  // Casos comunes cuando el campo proceso contiene solo modalidad: LP, SA, MC, CD, etc.
  if (/^(LP|LPN|SA|SAMC|MC|CD|CI|IP)$/i.test(n)) return true;

  return false;
}

function obtenerNombreProyectoActivoSeguro() {
  const proyectoActual = PROYECTOS?.lista?.find(p => Number(p.id) === Number(PROYECTOS?.activoId));
  const sugerido = construirNombreProyectoSugerido();

  if (proyectoActual?.nombre && String(proyectoActual.nombre).trim() && !esNombreProyectoGenerico(proyectoActual.nombre)) {
    return String(proyectoActual.nombre).trim();
  }

  return sugerido;
}

function repararNombresGenericosDeProyectos() {
  if (!PROYECTOS || !Array.isArray(PROYECTOS.lista)) return false;

  let huboCambios = false;

  PROYECTOS.lista.forEach(proyecto => {
    const esActivo = Number(proyecto.id) === Number(PROYECTOS.activoId);
    const data = proyecto?.data || {};
    const procesoProyecto = data.proceso || {};

    const entidad = String(procesoProyecto.entidad || (esActivo ? DB.proceso?.entidad : "") || "").trim();
    const objeto = String(procesoProyecto.objeto || (esActivo ? DB.proceso?.objeto : "") || "").trim();
    const proceso = String(procesoProyecto.nombre || (esActivo ? DB.proceso?.nombre : "") || "").trim();

    let sugerido = "";
    if (entidad && objeto) sugerido = (entidad + " - " + objeto).slice(0, 120);
    else if (objeto) sugerido = objeto.slice(0, 120);
    else if (entidad) sugerido = entidad.slice(0, 120);
    else if (proceso && proceso.length > 3) sugerido = proceso.slice(0, 120);
    else sugerido = "Proyecto sin nombre";

    if (esNombreProyectoGenerico(proyecto.nombre) && sugerido && sugerido !== "Proyecto sin nombre") {
      proyecto.nombre = sugerido;
      huboCambios = true;
    }
  });

  if (huboCambios) guardarDB();
  return huboCambios;
}
...
document.addEventListener("DOMContentLoaded", () => {
  cargarDB();
  normalizarFactoresHistoricosDesdePuntajes();
  repararNombresGenericosDeProyectos();
...
function actualizarProcesoDesdeUI() {
  actualizarProceso({
    ...
    puntajeSostenibilidad: Number($("#procesoPuntajeSostenibilidad")?.value || 0),
...
function cargarProcesoEnUI() {
  ...
  if ($("#procesoPuntajeSostenibilidad")) $("#procesoPuntajeSostenibilidad").value = DB.proceso.puntajeSostenibilidad || "";
...
function renderResumenProceso() {
  ...
  box.innerHTML = `
    ...
    <div><strong>Sostenibilidad:</strong> ${DB.proceso.puntajeSostenibilidad || 0}</div>
...
function renderOfertas() {
  ...
  ofertas.forEach(oferta => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align:center;">
        <input data-id="${oferta.id}" class="check-sostenibilidad" type="checkbox" ${oferta.cumpleSostenibilidad ? "checked" : ""} />
      </td>
      <td style="text-align:center;">
...
function bindTablaOfertas() {
  ...
  $all(".check-calidad").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleCalidad: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  $all(".check-sostenibilidad").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleSostenibilidad: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  $all(".check-industria").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleIndustria: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  $all(".check-discapacidad").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleDiscapacidad: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  $all(".check-mujer").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleMujer: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  $all(".check-mipyme").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleMipyme: e.target.checked, puntajesImportados: false });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
  ...
  if (tbody) {
    ...
    tabla.forEach(item => {
      ...
      tr.innerHTML = `
        ...
        <td>${Number(item.puntajeSostenibilidad || 0).toFixed(2)}</td>
...
async function agregarArchivosAnexosAlProyecto() {
  ...
  guardarDB();
  guardarProyectoActual(obtenerNombreProyectoActivoSeguro());
  guardarDB();
...
function extraerNumeroExcel(valor) {
  ...
  if (tienePunto && (texto.match(/\./g) || []).length > 1 && !tieneComa) {
    texto = texto.replace(/\./g, "");
  } else if (tieneComa && (texto.match(/,/g) || []).length > 1 && !tienePunto) {
    texto = texto.replace(/,/g, "");
...
function obtenerIndiceColumnaPorEncabezadoExcel(filaEncabezados = [], nombres = []) {
  const normalizados = nombres.map(n => normalizarTextoExcel(n));

  for (let i = 0; i < filaEncabezados.length; i++) {
    const texto = normalizarTextoExcel(filaEncabezados[i]);
    if (!texto) continue;

    if (normalizados.some(n => texto.includes(n))) {
      return i;
    }
  }
...
function tieneColumnaExcel(indice) {
  return Number(indice) >= 0;
}

function maximoFilasImportadas(filas = [], campo = "") {
  return filas.reduce((max, fila) => Math.max(max, Number(fila?.[campo] || 0)), 0);
}

function sumarPuntajeTotalImportado(fila = {}) {
  return Number((
    Number(fila.puntajeEconomico || 0) +
    Number(fila.puntajeCalidad || 0) +
    Number(fila.puntajeSostenibilidad || 0) +
    Number(fila.puntajeIndustria || 0) +
    Number(fila.puntajeDiscapacidad || 0) +
    Number(fila.puntajeMujer || 0) +
    Number(fila.puntajeMipyme || 0)
  ).toFixed(4));
}

function aplicarMaximosDetectadosDesdeExcel(filas = [], columnas = {}) {
  const cambios = {};

  if (tieneColumnaExcel(columnas.colPuntajeEconomico)) {
    const max = maximoFilasImportadas(filas, "puntajeEconomico");
    if (max > 0) cambios.puntajeMaximo = max;
  }
  if (tieneColumnaExcel(columnas.colCalidad)) {
    const max = maximoFilasImportadas(filas, "puntajeCalidad");
    if (max > 0) cambios.puntajeCalidad = max;
  }
  if (tieneColumnaExcel(columnas.colSostenibilidad)) {
    const max = maximoFilasImportadas(filas, "puntajeSostenibilidad");
    if (max > 0) cambios.puntajeSostenibilidad = max;
  }
  if (tieneColumnaExcel(columnas.colIndustria)) {
    const max = maximoFilasImportadas(filas, "puntajeIndustria");
    if (max > 0) cambios.puntajeIndustriaNacional = max;
  }
  if (tieneColumnaExcel(columnas.colDiscapacidad)) {
    const max = maximoFilasImportadas(filas, "puntajeDiscapacidad");
    if (max > 0) cambios.puntajeDiscapacidad = max;
  }
  if (tieneColumnaExcel(columnas.colMujer)) {
    const max = maximoFilasImportadas(filas, "puntajeMujer");
    if (max > 0) cambios.puntajeMujer = max;
  }
  if (tieneColumnaExcel(columnas.colMipyme)) {
    const max = maximoFilasImportadas(filas, "puntajeMipyme");
    if (max > 0) cambios.puntajeMipyme = max;
  }
...
function importarExcel() {
  ...
  reader.onload = function (e) {
    try {
      ...
      const encabezados = Array.isArray(json[columnas.headerRowIndex]) ? json[columnas.headerRowIndex] : [];

      const colPuntajeEconomico = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje oferta economica", "puntaje valor oferta", "puntaje economico", "puntaje de la oferta economica"]);
      const colCalidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje factor calidad", "puntaje factor de calidad", "factor de calidad", "calidad"]);
      const colSostenibilidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje factor de sostenibilidad", "puntaje factor sostenibilidad", "puntaje sostenibilidad", "sostenibilidad"]);
      const colIndustria = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje apoyo industria nacional", "apoyo industria nacional", "industria nacional"]);
      const colDiscapacidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje vinculacion personas con discapacidad", "puntaje vinculacion discapacidad", "vinculacion personas con discapacidad", "vinculacion discapacidad", "discapacidad"]);
      const colMujer = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje emprendimiento empresas de mujeres", "puntaje emprendimientos y empresas de mujeres", "emprendimientos y empresas de mujeres", "empresas de mujeres", "mujeres"]);
      const colMipyme = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje mipyme", "mipyme", "mi pyme", "mipymes"]);
      ...
      json.slice(inicioDatos).forEach((fila, idx) => {
        ...
        const pareceEncabezado =
          ...
          empresaNormalizada.includes("puntaje");

        if (pareceEncabezado) return;

        if (empresa && valor > 0) {
          const puntajeEconomico = colPuntajeEconomico >= 0 ? extraerNumeroExcel(fila[colPuntajeEconomico]) : 0;
          const puntajeCalidad = colCalidad >= 0 ? extraerNumeroExcel(fila[colCalidad]) : 0;
          const puntajeSostenibilidad = colSostenibilidad >= 0 ? extraerNumeroExcel(fila[colSostenibilidad]) : 0;
          const puntajeIndustria = colIndustria >= 0 ? extraerNumeroExcel(fila[colIndustria]) : 0;
          const puntajeDiscapacidad = colDiscapacidad >= 0 ? extraerNumeroExcel(fila[colDiscapacidad]) : 0;
          const puntajeMujer = colMujer >= 0 ? extraerNumeroExcel(fila[colMujer]) : 0;
          const puntajeMipyme = colMipyme >= 0 ? extraerNumeroExcel(fila[colMipyme]) : 0;
          ...
          filasLimpias.push({
            ...
            puntajeMujer,
            puntajeMipyme,
            tienePuntajesImportados:
              colPuntajeEconomico >= 0 || colCalidad >= 0 || colSostenibilidad >= 0 ||
              colIndustria >= 0 || colDiscapacidad >= 0 || colMujer >= 0 || colMipyme >= 0
          });
        } else if (empresa || String(valorOriginal || "").trim()) {
          filasOmitidas.push({ fila: inicioDatos + idx + 1, empresa, valorOriginal: String(valorOriginal || "") });
        }
      });
      ...
      const confirmar = confirm(`Se importarán ${filasLimpias.length} oferta(s).\n\n¿Desea reemplazar el listado actual de ofertas?`);
      if (!confirmar) return;
      ...
      filasLimpias.forEach((fila, index) => {
        ...
        actualizarOferta(ofertaCreada.id, {
          ...
          oferta: fila.oferta,
          puntaje: fila.puntajeEconomico || 0,
          puntajeCalidad: fila.puntajeCalidad || 0,
          puntajeSostenibilidad: fila.puntajeSostenibilidad || 0,
          puntajeIndustria: fila.puntajeIndustria || 0,
          puntajeDiscapacidad: fila.puntajeDiscapacidad || 0,
          puntajeMujer: fila.puntajeMujer || 0,
          puntajeMipyme: fila.puntajeMipyme || 0,
          puntajeTotal: sumarPuntajeTotalImportado(fila),
          puntajesImportados: Boolean(fila.tienePuntajesImportados),
          cumpleCalidad: Number(fila.puntajeCalidad || 0) > 0,
          cumpleSostenibilidad: Number(fila.puntajeSostenibilidad || 0) > 0,
          cumpleIndustria: Number(fila.puntajeIndustria || 0) > 0,
          cumpleDiscapacidad: Number(fila.puntajeDiscapacidad || 0) > 0,
          cumpleMujer: Number(fila.puntajeMujer || 0) > 0,
          cumpleMipyme: Number(fila.puntajeMipyme || 0) > 0
        });
      });

      aplicarMaximosDetectadosDesdeExcel(filasLimpias, { colPuntajeEconomico, colCalidad, colSostenibilidad, colIndustria, colDiscapacidad, colMujer, colMipyme });

      actualizarProceso({ numeroPropuestasHabilitadas: filasLimpias.length });
      if ($("#procesoNumeroPropuestas")) $("#procesoNumeroPropuestas").value = filasLimpias.length;
      ...
      const columnasDetectadas = [
        colPuntajeEconomico >= 0 ? "puntaje económico" : null,
        colCalidad >= 0 ? "calidad" : null,
        colSostenibilidad >= 0 ? "sostenibilidad" : null,
        colIndustria >= 0 ? "industria nacional" : null,
        colDiscapacidad >= 0 ? "discapacidad" : null,
        colMujer >= 0 ? "empresas de mujeres" : null,
        colMipyme >= 0 ? "Mipyme" : null
      ].filter(Boolean).join(", ");

      if (filasOmitidas.length > 0) {
        const detalle = filasOmitidas.slice(0, 10).map(x => `Fila ${x.fila}: ${x.empresa || "(sin nombre)"} | ${x.valorOriginal || "(sin valor)"}`).join("\n");
        alert(`Importación parcial: ${filasLimpias.length} oferta(s) cargada(s).\n${filasOmitidas.length} fila(s) fueron omitida(s).\nColumnas adicionales detectadas: ${columnasDetectadas || "ninguna"}.\n\n${detalle}`);
      } else {
...
function inicializarGestionProyectos() {
  ...
  btnNuevo?.addEventListener("click", () => {
    const nombre = prompt("Nombre del nuevo proyecto:", construirNombreProyectoSugerido());
    if (!nombre || !String(nombre).trim()) return;
    crearNuevoProyecto(String(nombre).trim());
    guardarDB();
    renderLista();
    cargarProcesoEnUI();
    renderTodo();
  ...
  btnGuardar?.addEventListener("click", () => {
    ...
    const nombreSugerido =
      proyectoActual?.nombre && !esNombreProyectoGenerico(proyectoActual.nombre)
        ? proyectoActual.nombre
        : construirNombreProyectoSugerido();

    const nombre = prompt("Nombre del proyecto:", nombreSugerido);

    if (!nombre || !String(nombre).trim()) return;

    guardarProyectoActual(String(nombre).trim());
    guardarDB();
    renderLista();
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>PUNTAJE<br>SOSTENIBILIDAD</th>
...
function inicializarSimulacionFormulas() {
  ...
  function ejecutarSimulacionSeleccionada() {
    ...
    if (!resultado.tabla.length) {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td colspan="11">No existen ofertas habilitadas para simular.</td>`;
      tbody.appendChild(tr);
      return;
    }

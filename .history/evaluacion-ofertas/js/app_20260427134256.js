...
function bindTablaOfertas() {
  ...
  $all(".check-sostenibilidad").forEach(input => {
    input.addEventListener("change", e => {
      const id = Number(e.target.dataset.id);
      actualizarOferta(id, { cumpleSostenibilidad: e.target.checked });
      recalcularPuntajeTotalOferta(id);
      guardarDB();
      renderOfertas();
    });
...
function importarExcel() {
  ...
  reader.onload = function (e) {
    try {
      ...
      const colPuntajeEconomico = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje oferta economica", "puntaje valor oferta"]);
      const colCalidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje factor calidad", "factor de calidad", "calidad"]);
      const colSostenibilidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje factor de sostenibilidad", "puntaje sostenibilidad", "sostenibilidad"]);
      ...
      const colDiscapacidad = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje vinculacion personas con discapacidad", "vinculacion discapacidad", "discapacidad"]);
      const colMujer = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje emprendimiento empresas de mujeres", "emprendimientos y empresas de mujeres", "empresas de mujeres", "mujeres"]);
      const colMipyme = obtenerIndiceColumnaPorEncabezadoExcel(encabezados, ["puntaje mipyme", "mipyme"]);

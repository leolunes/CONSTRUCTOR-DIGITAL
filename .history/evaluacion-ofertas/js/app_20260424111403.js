...
function inicializarAnalisisEstrategico() {
  ...
  function cargarProponentes() {
    ...
    DB.ofertas
      .filter(o => o.empresa && o.puntajeTotal)
      .forEach(o => {
        const opt = document.createElement("option");
        opt.value = o.id;
        opt.textContent = o.empresa;
        select.appendChild(opt);
      });
  }
  ...
  function analizar() {
    ...
    if (resultado) {
      if (resultadoSim.yaEsGanador) {
        resultado.innerHTML = `<strong>Ya eres el ganador del proceso</strong>`;
      } else if (resultadoSim.encontrado) {
        const nombres = resultadoSim.escenario.excluidos.map(x => x.empresa).join(", ");

        resultado.innerHTML = `
          <strong>ESCENARIO GANADOR IDENTIFICADO</strong><br><br>
          <strong>Proponente:</strong> ${DB.ofertas.find(o=>o.id==id)?.empresa || ""}<br>
          <strong>Posición actual:</strong> ${resultadoSim.posicionActual}<br>
          <strong>Viabilidad:</strong> ${resultadoSim.viabilidad}<br><br>
          <strong>Para ser No.1 se requeriría que no sean considerados:</strong><br>
          ${nombres}<br><br>
          <strong>Total exclusiones:</strong> ${resultadoSim.escenario.totalExcluidos}<br>
          <strong>Nueva posición:</strong> 1<br>
          <strong>Nueva base (mediana):</strong> ${resultadoSim.escenario.bases.mediana}
        `;
      } else {
        resultado.innerHTML = `
          <strong>No se encontró escenario ganador</strong><br>
          Viabilidad: ${resultadoSim.viabilidad}<br>
          ${resultadoSim.motivo || ""}
        `;
      }
    }
    ...
    if (tbody) {
      ...
      if (resultadoSim.encontrado && resultadoSim.escenario) {
        resultadoSim.escenario.excluidos.forEach(o => {
          ...
          tr.innerHTML = `
            <td>${o.empresa}</td>
            <td>${Number(o.puntajeTotal || 0).toFixed(4)}</td>
            <td>${Number(o.diferenciaContraMi || 0).toFixed(4)}</td>
            <td>${o.rankingActual}</td>
          `;

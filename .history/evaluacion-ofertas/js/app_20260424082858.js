...
function inicializarAnalisisEstrategico() {
  ...
  function analizar() {
    ...
    const mi = DB.ofertas.find(o => o.id === id);
    if (!mi) return;

    const superiores = DB.ofertas
      .filter(o => o.rankingFinal > 0 && o.rankingFinal < mi.rankingFinal)
      .sort((a, b) => a.rankingFinal - b.rankingFinal);
    ...
    if (resultado) {
      if (!superiores.length) {
        resultado.innerHTML = `<strong>Ya ocupas la posición No. 1</strong>`;
      } else {
        const nombres = superiores.map(o => o.empresa).join(", ");

        resultado.innerHTML = `
          <strong>Para que ${mi.empresa} ocupe la posición No. 1:</strong><br>
          Los siguientes proponentes no deberían estar por encima:<br>
          <strong>${nombres}</strong><br><br>
          <strong>Posición actual:</strong> ${mi.rankingFinal}<br>
          <strong>Posición proyectada:</strong> 1
        `;
      }
    }
    ...
    if (tbody) {
      ...
      superiores.forEach(o => {
        ...
        const diff = Number(o.puntajeTotal || 0) - Number(mi.puntajeTotal || 0);

        tr.innerHTML = `
          <td>${o.empresa}</td>
          <td>${Number(o.puntajeTotal || 0).toFixed(4)}</td>
          <td>${diff.toFixed(4)}</td>
          <td>${o.rankingFinal}</td>
        `;

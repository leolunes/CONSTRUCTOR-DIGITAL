...
// ===============================
// 🔥 EXTRACCIÓN REAL DE TEXTO DESDE PDF + AUTOLLENADO FASE 3
// ===============================
...
function normalizarTextoExtraccionPDF(texto = "") {
  return String(texto || "")
    .replace(/\u00A0/g, " ")
    .replace(/[\t\r\n]+/g, " ")
    .replace(/\s+/g, " ")
...
function normalizarClavePDF(texto = "") {
  return String(texto || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function limpiarValorMonedaPDF(valor = "") {
...
function limpiarTextoCampoPDF(valor = "") {
  return String(valor || "")
    .replace(/\s+/g, " ")
    .replace(/^[\s:;,.\-–—]+/g, "")
    .replace(/[\s:;,.\-–—]+$/g, "")
...
async function leerTextoPDF(docMeta) {
  try {
    if (!configurarPDFJS()) {
      throw new Error("PDF.js no está cargado en index.html.");
    }
    ...
    if (!registro || !registro.blob) {
      console.warn("No se encontró blob del documento:", docMeta);
      return "";
    }
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      // Reconstrucción por coordenadas: conserva filas reales de tablas SECOP.
      // Antes se unía todo con espacios y eso mezclaba apoderados, valores y puntajes.
      ...
      const grupos = [];
      const toleranciaY = 2.5;

      items.forEach(item => {
        let grupo = grupos.find(g => Math.abs(g.y - item.y) <= toleranciaY);
...
function detectarProcesoPDF(textoPlano = "") {
  const patrones = [
    /\b(?:proceso|convocatoria|licitaci[oó]n|selecci[oó]n)[^A-Z0-9]{0,80}([A-Z]{1,8}[\s.-]*\d{1,6}[\s.-]*\d{2,4}(?:[\s.-]*[A-Z0-9]{1,6})?)/i,
    /\b(LP|LPN|SA|SAMC|MC|CD|CI|IP|LP-DO|LPDO|SASI|CM|CP)[\s.-]*\d{1,6}[\s.-]*\d{2,4}\b/i,
    /\b[A-Z]{2,8}[\s.-]*\d{1,6}[\s.-]*\d{4}\b/i
  ];

  for (const patron of patrones) {
    const match = textoPlano.match(patron);
    if (match) return limpiarTextoCampoPDF(match[1] || match[0]).toUpperCase();
  }

  return "";
}

function detectarPresupuestoPDF(textoPlano = "") {
  const patrones = [
    /(presupuesto\s+oficial|presupuesto\s+estimado|valor\s+estimado|valor\s+del\s+presupuesto|valor\s+oficial|valor\s+total\s+estimado)[^$0-9]{0,160}(\$?\s?[0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,}(?:[.,][0-9]{1,2})?)/i,
    /(presupuesto|valor)[^$0-9]{0,100}(\$?\s?[0-9][0-9.,\s]{8,})/i
  ];
  ...
  for (const patron of patrones) {
    ...
    const numero = limpiarValorMonedaPDF(match[2]);
    if (numero > 1000000) {
      return {
        texto: match[2],
        numero
  ...
  const todos = textoPlano.match(/\$?\s?[0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,}(?:[.,][0-9]{1,2})?/g) || [];
  const valores = todos
    .map(v => ({ texto: v, numero: limpiarValorMonedaPDF(v) }))
    .filter(v => v.numero > 1000000)
    .sort((a, b) => b.numero - a.numero);

  return valores[0] || null;
}

function detectarEntidadPDF(textoPlano = "") {
  const patrones = [
    /(entidad\s+contratante|entidad|contratante|nombre\s+de\s+la\s+entidad)[:\s\-–—]{1,30}([A-ZÁÉÍÓÚÑ0-9 .,&()\/-]{8,180}?)(?:\s+(?:objeto|proceso|modalidad|presupuesto|valor|direcci[oó]n|nit)\b|$)/i,
    /\b(ALCALD[IÍ]A|GOBERNACI[OÓ]N|MUNICIPIO|DEPARTAMENTO|INSTITUTO|UNIVERSIDAD|EMPRESA|UNIDAD|AGENCIA|MINISTERIO)[A-ZÁÉÍÓÚÑ0-9 .,&()\/-]{8,160}/i
  ];

  for (const patron of patrones) {
    const match = textoPlano.match(patron);
    if (match) {
      const valor = limpiarTextoCampoPDF(match[2] || match[0]);
...
function detectarObjetoPDF(textoPlano = "") {
  const patrones = [
    /(objeto\s+contractual|objeto\s+del\s+contrato|objeto\s+del\s+proceso|objeto)[:\s\-–—]{1,30}(.{30,700}?)(?:\s+(?:presupuesto|valor\s+estimado|valor\s+oficial|plazo|modalidad|fecha|forma\s+de\s+pago|clasificaci[oó]n|cronograma)\b|$)/i,
    /(contratar|seleccionar)[\s\S]{20,500}?(?:\.|presupuesto|valor|plazo|modalidad)/i
  ];

  for (const patron of patrones) {
    const match = textoPlano.match(patron);
    if (match) {
      let valor = limpiarTextoCampoPDF(match[2] || match[0]);
      valor = valor.replace(/\s{2,}/g, " ");
      if (valor.length >= 20) return valor.slice(0, 900);
    }
  }
...
function detectarNumeroPropuestasPDF(textoPlano = "") {
  const patrones = [
    /(n[uú]mero\s+de\s+propuestas|propuestas\s+habilitadas|proponentes\s+habilitados|propuestas\s+recibidas|ofertas\s+recibidas|proponentes)[^0-9]{0,80}([0-9]{1,3})/i,
    /([0-9]{1,3})\s+(propuestas|proponentes|ofertas)\b/i
  ];

  for (const patron of patrones) {
    const match = textoPlano.match(patron);
    if (match) {
      const numero = Number(match[2] || match[1] || 0);
      if (numero > 0 && numero < 500) return numero;
    }
  }
...
function detectarDatosClave(texto = "") {
  const textoPlano = normalizarTextoExtraccionPDF(texto);
  const resultados = {};

  const proceso = detectarProcesoPDF(textoPlano);
  if (proceso) resultados.proceso = proceso;

  const presupuesto = detectarPresupuestoPDF(textoPlano);
  if (presupuesto?.numero) {
    resultados.presupuestoTexto = presupuesto.texto;
    resultados.presupuestoNumero = presupuesto.numero;
  }

  const entidad = detectarEntidadPDF(textoPlano);
  if (entidad) resultados.entidad = entidad;

  const objeto = detectarObjetoPDF(textoPlano);
  if (objeto) resultados.objeto = objeto;

  const numeroPropuestas = detectarNumeroPropuestasPDF(textoPlano);
  if (numeroPropuestas) resultados.numeroPropuestas = numeroPropuestas;

  const numeros = textoPlano.match(/\$?\s?[0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,}(?:[.,][0-9]{1,2})?/g);
  if (numeros && numeros.length) {
    resultados.posiblesValores = [...new Set(numeros.map(v => limpiarTextoCampoPDF(v)))].slice(0, 12);
  }
...
// ===============================
// 🔥 EXTRACCIÓN INTELIGENTE SECOP - MATRIZ DE EVALUACIÓN / ADJUDICACIÓN
// Detecta proponentes, valores y puntajes desde actas o resoluciones PDF con texto.
// ===============================
function normalizarNombreProponenteSECOP(nombre = "") {
  let limpio = String(nombre || "")
    .replace(/\u00A0/g, " ")
    .replace(/\s+/g, " ")
    .replace(/^[0-9]+[\s.)-]+/g, "")
    .replace(/^(no\.?|nro\.?|item|orden)\s*[0-9]{1,3}\s*/i, "")
    .replace(/\b(valor\s+oferta|valor\s+de\s+la\s+oferta|puntaje|factor|calidad|sostenibilidad)\b.*$/i, "")
    .replace(/\b\d{1,3}(?:[.,]\d{3}){2,}.*$/g, "")
    .trim();

  const prefijos = [
    "CONSORCIO", "UNIÓN TEMPORAL", "UNION TEMPORAL", "U.T.", "UT ",
    "CAIPRO", "ICSSA", "REDEY", "METSAP", "GRU", "EAR S.A.S", "ARTPECO",
    "ESTRUCA", "PROAINCOL", "CONSTRUCCOL", "CONSTRUCCIONES", "VERTICAL"
  ];
  ...
  for (const prefijo of prefijos) {
    const pos = upper.indexOf(prefijo);
    if (pos >= 0 && (idx === -1 || pos < idx)) idx = pos;
  }

  if (idx > 0) limpio = limpio.slice(idx).trim();
...
function esNombreProponenteValidoSECOP(nombre = "") {
  const n = normalizarNombreProponenteSECOP(nombre);
  const clave = normalizarClavePDF(n);

  if (!n || n.length < 4 || n.length > 120) return false;
  if (/^[0-9.,$\s]+$/.test(n)) return false;

  const palabrasBloqueadas = [
    "pagina", "acta de audiencia", "resolucion", "audiencia", "adjudicacion",
    "evaluacion", "presupuesto", "valor total", "valores posibles", "texto detectado",
    "puntaje oferta", "puntaje factor", "sostenibilidad", "industria nacional",
    "discapacidad", "empresas de mujeres", "mipyme", "observaciones", "fecha",
    "objeto", "entidad", "proceso", "contrato", "de acuerdo", "se procede",
    "garantia", "poliza", "multa", "anticipo", "plazo", "cedula", "identificacion",
    "representante legal", "apoderado", "apoderada", "c.c", "cc", "cedula", "cédula", "firma", "folio", "anexo", "documento"
  ];

  if (palabrasBloqueadas.some(p => clave.includes(p))) return false;
  if (/APODERAD[OA]/i.test(n)) return false;
  if (/C\.?\s*C\.?/i.test(n)) return false;
  if (/(?:CC|NIT)\s*\d{3,}/i.test(n)) return false;

  const indicadores = [
    "consorcio", "union temporal", "unión temporal", "ut ", "u.t.",
    "sas", "s.a.s", "ltda", "construct", "ingenier", "campus", "santander",
    "proyectos", "edificaciones", "soluciones", "temporal", "caipro", "icssa",
    "met", "grupo", "proaincol", "red", "vertical", "artpeco", "estruca"
  ];

  return indicadores.some(p => clave.includes(p));
}

function extraerNumeroDecimalSECOP(valor = "") {
  const limpio = String(valor || "").replace(",", ".").replace(/[^0-9.\-]/g, "");
  const numero = Number(limpio);
  return Number.isFinite(numero) ? numero : 0;
}

function limpiarValorOfertaSECOP(valor = "") {
  const numero = limpiarValorMonedaPDF(valor);
  return numero > 1000000 ? numero : 0;
}

function obtenerNumerosPosterioresSECOP(segmento = "") {
  const matches = String(segmento || "").match(/[0-9]+(?:[.,][0-9]+)?/g) || [];
  return matches.map(extraerNumeroDecimalSECOP).filter(n => Number.isFinite(n));
}

function normalizarLineaTablaSECOP(texto = "") {
  return String(texto || "")
    .replace(/\u00A0/g, " ")
    .replace(/[|;]/g, " ")
    .replace(/\s+/g, " ")
...
function obtenerLineasCandidatasSECOP(texto = "") {
  const lineas = String(texto || "")
    .split(/\n|\r/)
    .map(normalizarLineaTablaSECOP)
    .filter(Boolean);

  const combinadas = [];

  for (let i = 0; i < lineas.length; i++) {
    combinadas.push(lineas[i]);

    if (lineas[i + 1]) {
      combinadas.push(`${lineas[i]} ${lineas[i + 1]}`);
    }

    if (lineas[i + 1] && lineas[i + 2]) {
      combinadas.push(`${lineas[i]} ${lineas[i + 1]} ${lineas[i + 2]}`);
    }
  }
...
function contieneIndicadorProponenteSECOP(texto = "") {
  return /(CONSORCIO|UNIÓN\s+TEMPORAL|UNION\s+TEMPORAL|U\.?\s*T\.?|S\.?\s*A\.?\s*S\.?|SAS|LTDA|CAIPRO|ICSSA|REDEY|METSAP|PROAINCOL|CONSTRUCCOL|GRU|EAR|ARTPECO|ESTRUCA|VERTICAL)/i.test(texto);
}

function extraerCandidatosOfertasSECOP(texto = "") {
  const lineas = obtenerLineasCandidatasSECOP(texto);
  const resultados = [];
  const patronValor = /\$?\s*[0-9]{1,3}(?:[.,\s]?[0-9]{3}){2,}(?:[.,][0-9]{1,2})?/g;

  lineas.forEach(lineaOriginal => {
    const linea = normalizarLineaTablaSECOP(lineaOriginal);
    if (!contieneIndicadorProponenteSECOP(linea)) return;

    const valores = [...linea.matchAll(patronValor)];
    if (!valores.length) return;

    valores.forEach(valorMatch => {
      const oferta = limpiarValorOfertaSECOP(valorMatch[0]);
      if (!oferta) return;

      const antesValor = linea.slice(0, valorMatch.index).trim();
      const despuesValor = linea.slice((valorMatch.index || 0) + valorMatch[0].length).trim();

      let empresa = normalizarNombreProponenteSECOP(antesValor);

      if (!esNombreProponenteValidoSECOP(empresa)) {
        const posible = linea.match(/((?:CONSORCIO|UNIÓN\s+TEMPORAL|UNION\s+TEMPORAL|U\.?\s*T\.?|CAIPRO|ICSSA|REDEY|METSAP|PROAINCOL|CONSTRUCCOL|GRU|EAR|ARTPECO|ESTRUCA|VERTICAL)[A-ZÁÉÍÓÚÑ0-9 .,&()\/-]{2,100})/i);
        if (posible) empresa = normalizarNombreProponenteSECOP(posible[1]);
      }

      if (!esNombreProponenteValidoSECOP(empresa)) return;

      const nums = obtenerNumerosPosterioresSECOP(despuesValor)
        .filter(n => n >= 0 && n <= 100);

      resultados.push({
        empresa,
        oferta,
        puntajeEconomicoPDF: nums[0] ?? null,
        puntajeCalidadPDF: nums[1] ?? null,
        puntajeSostenibilidadPDF: nums[2] ?? null,
        puntajeIndustriaPDF: nums[3] ?? null,
        puntajeDiscapacidadPDF: nums[4] ?? null,
        puntajeMujerPDF: nums[5] ?? null,
        puntajeMipymePDF: nums[6] ?? null,
        fuente: "PDF SECOP",
        confianza: nums.length >= 5 ? "ALTA" : "MEDIA"
      });
    });
  });
...
function filtrarClusterValoresSECOP(candidatos = []) {
  let filas = candidatos.filter(o =>
    o &&
    esNombreProponenteValidoSECOP(o.empresa) &&
    Number(o.oferta || 0) > 1000000
  );

  if (!filas.length) return [];

  // Descarta filas típicas de tablas de apoderados/identificación.
  filas = filas.filter(o => {
    const empresa = String(o.empresa || "").toUpperCase();
    return !/APODERAD[OA]|C\.?\s*C\.?|CEDULA|CÉDULA|REPRESENTANTE/.test(empresa);
  });
  ...
  // Si existe presupuesto, tomar solo escala de ofertas reales.
  let filtradas = presupuesto > 0
    ? filas.filter(o => Number(o.oferta || 0) >= presupuesto * 0.55 && Number(o.oferta || 0) <= presupuesto * 1.10)
    : filas;

  if (!filtradas.length) filtradas = filas;

  // Agrupa por bandas de 500 millones. En actas SECOP suele haber varios bloques de valores:
  // garantías/anticipos/adjudicados parciales (~1.0B) y tabla económica real (~3.5B).
  // Se prioriza la banda alta que tenga suficientes filas, no simplemente la más repetida.
  const grupos = new Map();
  filtradas.forEach(o => {
    const oferta = Number(o.oferta || 0);
    const banda = Math.floor(oferta / 500000000); // 0.5B
    if (!grupos.has(banda)) grupos.set(banda, []);
    grupos.get(banda).push(o);
  });
  ...
  const gruposOrdenados = [...grupos.entries()]
    .map(([banda, items]) => ({
      ...
      items,
      total: items.length,
      mediana: medianaSim(items.map(x => Number(x.oferta || 0)).sort((a, b) => a - b))
    }))
    .sort((a, b) => {
      // Primero preferimos más filas; ante empate o volumen aceptable, la banda más alta.
      const minFilas = Math.min(Number(DB.proceso?.numeroPropuestasHabilitadas || 0) || 8, 8);
      const aFuerte = a.total >= minFilas;
      const bFuerte = b.total >= minFilas;
      if (aFuerte !== bFuerte) return aFuerte ? -1 : 1;
      if (Math.abs(a.total - b.total) <= 3) return b.mediana - a.mediana;
      return b.total - a.total;
    });

  filtradas = gruposOrdenados[0]?.items || filtradas;

  // Si existe otra banda más alta con al menos 6 filas, normalmente corresponde a la tabla completa real.
  const bandaActual = gruposOrdenados[0]?.banda;
  const bandaAlta = gruposOrdenados
    .filter(g => g.banda > bandaActual && g.total >= 6)
    .sort((a, b) => b.mediana - a.mediana)[0];

  if (bandaAlta) filtradas = bandaAlta.items;

  // Elimina duplicados exactos por empresa/valor conservando la fila con más puntajes.
  const mapa = new Map();
  filtradas.forEach(item => {
    const empresa = normalizarNombreProponenteSECOP(item.empresa);
    const key = `${empresa}__${Number(item.oferta || 0)}`;
    const score = Object.values(item).filter(v => typeof v === "number" && v > 0).length;

    if (!mapa.has(key)) {
      mapa.set(key, { ...item, empresa, _score: score });
      return;
    }

    if (score > (mapa.get(key)._score || 0)) {
      mapa.set(key, { ...item, empresa, _score: score });
    }
  });

  return [...mapa.values()].map(({ _score, ...item }) => item);
}

function extraerOfertasDesdeTextoSECOP(texto = "") {
  const candidatos = extraerCandidatosOfertasSECOP(texto);
  const filtradas = filtrarClusterValoresSECOP(candidatos);
  const mapa = new Map();
  ...
  filtradas.forEach(item => {
    ...
    if (!mapa.has(key)) {
      mapa.set(key, { ...item, empresa });
      return;
    }

    const actual = mapa.get(key);
    const scoreActual = Object.values(actual).filter(v => typeof v === "number" && v > 0).length;
    const scoreNuevo = Object.values(item).filter(v => typeof v === "number" && v > 0).length;
    if (scoreNuevo > scoreActual) mapa.set(key, { ...item, empresa });
  });

  return [...mapa.values()]
    .filter(o => esNombreProponenteValidoSECOP(o.empresa) && o.oferta > 1000000)
    .sort((a, b) => Number(a.oferta || 0) - Number(b.oferta || 0));
}

function fusionarOfertasSECOP(base = [], nuevas = []) {
  const mapa = new Map();

  [...base, ...nuevas].forEach(item => {
    if (!item?.empresa || !item?.oferta) return;
    const empresa = normalizarNombreProponenteSECOP(item.empresa);
    const key = `${empresa}__${Number(item.oferta || 0)}`;
  ...
  return filtrarClusterValoresSECOP([...mapa.values()])
    .sort((a, b) => Number(a.oferta || 0) - Number(b.oferta || 0));
}

function calcularMaximoDetectadoSECOP(filas = [], campo = "") {
  const valores = filas
    .map(f => Number(f[campo] || 0))
    .filter(v => Number.isFinite(v) && v > 0);

  return valores.length ? Math.max(...valores) : 0;
}

function aplicarOfertasDetectadasSECOP(ofertasDetectadas = []) {
  const filas = Array.isArray(ofertasDetectadas) ? ofertasDetectadas : [];

  if (!filas.length) {
    alert("No hay ofertas detectadas para cargar.");
    return;
  }

  const confirmar = confirm(
    `Se cargarán ${filas.length} oferta(s) detectada(s) desde los PDF.\n\n` +
    "Esto reemplazará el listado actual de ofertas. ¿Desea continuar?"
  );

  if (!confirmar) return;

  const maxCalidad = calcularMaximoDetectadoSECOP(filas, "puntajeCalidadPDF");
  const maxIndustria = calcularMaximoDetectadoSECOP(filas, "puntajeIndustriaPDF");
  const maxDiscapacidad = calcularMaximoDetectadoSECOP(filas, "puntajeDiscapacidadPDF");
  const maxMujer = calcularMaximoDetectadoSECOP(filas, "puntajeMujerPDF");
  const maxMipyme = calcularMaximoDetectadoSECOP(filas, "puntajeMipymePDF");

  actualizarProceso({
    numeroPropuestasHabilitadas: filas.length,
    ...(maxCalidad ? { puntajeCalidad: maxCalidad } : {}),
    ...(maxIndustria ? { puntajeIndustriaNacional: maxIndustria } : {}),
    ...(maxDiscapacidad ? { puntajeDiscapacidad: maxDiscapacidad } : {}),
    ...(maxMujer ? { puntajeMujer: maxMujer } : {}),
    ...(maxMipyme ? { puntajeMipyme: maxMipyme } : {})
  });

  DB.ofertas = [];
  const nuevas = generarLineasOfertas(filas.length);

  filas.forEach((fila, index) => {
    const ofertaCreada = nuevas[index];
    if (!ofertaCreada) return;

    actualizarOferta(ofertaCreada.id, {
      empresa: fila.empresa,
      oferta: Number(fila.oferta || 0),
      puntaje: Number(fila.puntajeEconomicoPDF || 0),
      cumpleCalidad: Number(fila.puntajeCalidadPDF || 0) > 0,
      cumpleIndustria: Number(fila.puntajeIndustriaPDF || 0) > 0,
      cumpleDiscapacidad: Number(fila.puntajeDiscapacidadPDF || 0) > 0,
      cumpleMujer: Number(fila.puntajeMujerPDF || 0) > 0,
      cumpleMipyme: Number(fila.puntajeMipymePDF || 0) > 0,
      puntajeCalidad: Number(fila.puntajeCalidadPDF || 0),
      puntajeIndustria: Number(fila.puntajeIndustriaPDF || 0),
      puntajeDiscapacidad: Number(fila.puntajeDiscapacidadPDF || 0),
      puntajeMujer: Number(fila.puntajeMujerPDF || 0),
      puntajeMipyme: Number(fila.puntajeMipymePDF || 0),
      observacion: "Cargada automáticamente desde matriz PDF SECOP"
    });
  });
  ...
  alert("Ofertas cargadas automáticamente desde PDF.");
}

function renderTablaOfertasDetectadasSECOP(ofertas = []) {
  if (!ofertas.length) return "";

  const filas = ofertas.map((o, index) => `
    <tr>
      <td style="text-align:center;">${index + 1}</td>
      <td><strong>${o.empresa || ""}</strong></td>
      <td>${formatearPesos(o.oferta || 0)}</td>
      <td style="text-align:center;">${o.puntajeEconomicoPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeCalidadPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeSostenibilidadPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeIndustriaPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeDiscapacidadPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeMujerPDF ?? "-"}</td>
      <td style="text-align:center;">${o.puntajeMipymePDF ?? "-"}</td>
    </tr>
  `).join("");

  return `
    <hr>
    <h3>Matriz de evaluación detectada desde PDF</h3>
    <p>
      Revise la tabla antes de cargarla. Ahora se descartan valores pequeños como garantías,
      pólizas o registros que no tienen la escala de una oferta económica del proceso.
    </p>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Valor oferta económica</th>
            <th>Puntaje oferta económica</th>
            <th>Puntaje factor calidad</th>
            <th>Puntaje sostenibilidad</th>
            <th>Apoyo industria nacional</th>
            <th>Vinculación discapacidad</th>
            <th>Empresas de mujeres</th>
            <th>Mipyme</th>
          </tr>
        </thead>
        <tbody>${filas}</tbody>
...
function fusionarDatosPDF(base = {}, nuevos = {}) {
  const salida = { ...base };

  if (!salida.proceso && nuevos.proceso) salida.proceso = nuevos.proceso;
  if (!salida.entidad && nuevos.entidad) salida.entidad = nuevos.entidad;
  if (!salida.objeto && nuevos.objeto) salida.objeto = nuevos.objeto;
  if (!salida.numeroPropuestas && nuevos.numeroPropuestas) salida.numeroPropuestas = nuevos.numeroPropuestas;

  if (!salida.presupuestoNumero && nuevos.presupuestoNumero) {
    salida.presupuestoNumero = nuevos.presupuestoNumero;
    salida.presupuestoTexto = nuevos.presupuestoTexto;
  }

  if (Array.isArray(nuevos.posiblesValores)) {
    salida.posiblesValores = [...new Set([...(salida.posiblesValores || []), ...nuevos.posiblesValores])].slice(0, 12);
  }

  return salida;
}

function autollenarFormularioDesdePDF(datos = {}) {
  const cambios = {};

  if (datos.proceso) cambios.nombre = datos.proceso;
  if (datos.entidad) cambios.entidad = datos.entidad;
  if (datos.objeto) cambios.objeto = datos.objeto;
  if (datos.presupuestoNumero) cambios.presupuesto = Number(datos.presupuestoNumero || 0);
  if (datos.numeroPropuestas) cambios.numeroPropuestasHabilitadas = Number(datos.numeroPropuestas || 0);

  if (!Object.keys(cambios).length) return false;

  actualizarProceso(cambios);
  guardarDB();
  cargarProcesoEnUI();
  renderResumenProceso();
  renderOfertas();
...
function renderDatoPDF(label, valor) {
  if (valor === undefined || valor === null || valor === "") return "";
  return `<div><strong>${label}:</strong> ${valor}</div>`;
}
...
async function extraerInformacionPDF() {
  ...
  if (!resultadoBox) {
    alert("No se encontró el bloque resultadoExtraccionPDF en index.html.");
    return;
  }

  if (!configurarPDFJS()) {
    resultadoBox.innerHTML = `
      <strong>No se pudo iniciar PDF.js.</strong><br>
      Verifique que index.html tenga cargada la librería pdf.min.js antes de app.js.
    `;
    return;
  }
  ...
  if (!Array.isArray(DB.proceso.documentosAnexos) || DB.proceso.documentosAnexos.length === 0) {
    resultadoBox.innerHTML = "No hay documentos anexos para analizar.";
    return;
  }

  const proyectoId = obtenerProyectoActivoIdSeguro();

  const documentosFiltrados = proyectoId
    ? DB.proceso.documentosAnexos.filter(doc =>
        !doc.proyectoId || String(doc.proyectoId) === String(proyectoId)
      )
    : DB.proceso.documentosAnexos;

  const pdfs = documentosFiltrados.filter(doc =>
    (doc.mime || "").toLowerCase().includes("pdf") ||
    (doc.nombre || "").toLowerCase().endsWith(".pdf")
  );

  if (!pdfs.length) {
    resultadoBox.innerHTML = "No hay documentos PDF para analizar.";
    return;
  }

  resultadoBox.innerHTML = "Leyendo PDF(s) y buscando matriz de evaluación SECOP, por favor espere...";

  let html = "<strong>Resultados extracción PDF:</strong><br><br>";
  let totalConTexto = 0;
  let datosAcumulados = {};
  let ofertasDetectadasGlobal = [];

  for (const doc of pdfs) {
    const texto = await leerTextoPDF(doc);
    const textoLimpio = normalizarTextoExtraccionPDF(texto);
    const datos = detectarDatosClave(textoLimpio);
    const ofertasDetectadas = extraerOfertasDesdeTextoSECOP(texto);

    datosAcumulados = fusionarDatosPDF(datosAcumulados, datos);
    ofertasDetectadasGlobal = fusionarOfertasSECOP(ofertasDetectadasGlobal, ofertasDetectadas);

    html += `<div style="margin-bottom:12px;">`;
    html += `<b>📄 ${doc.nombre || "Documento PDF"}</b><br>`;

    if (!textoLimpio) {
      html += `
        <em>No se detectó texto interno en este PDF.</em><br>
        <small>
          Posibles causas: PDF escaneado como imagen, documento protegido o archivo sin capa de texto.
          Para este caso se requiere OCR.
        </small>
      `;
      html += `</div><hr>`;
      continue;
    ...
    html += renderDatoPDF("Proceso detectado", datos.proceso);
    html += renderDatoPDF("Entidad detectada", datos.entidad);
    html += renderDatoPDF("Presupuesto detectado", datos.presupuestoNumero ? formatearPesos(datos.presupuestoNumero) : "");
    html += renderDatoPDF("No. propuestas detectado", datos.numeroPropuestas);
    html += renderDatoPDF("Objeto detectado", datos.objeto);

    if (ofertasDetectadas.length) {
      html += `<div><strong>Ofertas/matriz detectadas en este PDF:</strong> ${ofertasDetectadas.length}</div>`;
    }

    if (datos.posiblesValores?.length) {
      html += `<div><strong>Valores posibles:</strong> ${datos.posiblesValores.join(", ")}</div>`;
    }

    html += `
      <br>
      <strong>Texto detectado (primeros 600 caracteres):</strong><br>
      <small>${textoLimpio.slice(0, 600)}</small>
    `;

    html += `</div><hr>`;
  }

  const autollenado = autollenarFormularioDesdePDF(datosAcumulados);

  if (autollenado) {
    html += `
      <strong>✔ Formulario autollenado desde PDF.</strong><br>
      Revise y ajuste manualmente los campos si el documento tiene textos ambiguos.
    `;
  } else if (totalConTexto > 0) {
    html += `
      <strong>No se encontraron datos suficientes para autollenar el formulario básico.</strong><br>
      El PDF tiene texto, pero no se identificaron patrones claros de proceso, entidad, objeto o presupuesto.
    `;
  }

  if (ofertasDetectadasGlobal.length) {
    html += renderTablaOfertasDetectadasSECOP(ofertasDetectadasGlobal);
    html += `
      <div class="actions" style="margin-top:10px;">
        <button id="btnCargarOfertasPDF" type="button" class="btn primary">
          Cargar matriz detectada como ofertas
        </button>
      </div>
    `;
  } else if (totalConTexto > 0) {
    html += `
      <hr>
      <strong>No se detectó una matriz de ofertas/proponentes.</strong><br>
      Posibles causas: tabla partida en el PDF, columnas sin texto seleccionable, o documento escaneado.
    `;
  }
  ...
  if (totalConTexto === 0) {
    html += `
      ...
      Los PDF fueron encontrados, pero no contienen texto extraíble.
      El siguiente paso sería agregar OCR para documentos escaneados.
    `;
  }

  resultadoBox.innerHTML = html;

  const btnCargar = document.getElementById("btnCargarOfertasPDF");
  if (btnCargar) {
    btnCargar.addEventListener("click", () => {
      aplicarOfertasDetectadasSECOP(ofertasDetectadasGlobal);
    });
  }
}

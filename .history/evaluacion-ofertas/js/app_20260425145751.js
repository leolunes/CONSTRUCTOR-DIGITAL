...
// ===============================
// 🔥 HERRAMIENTAS DEL OFERENTE
// Reubica botones estratégicos en el módulo Oferente
// ===============================
function moverBotonesAOferente() {
  const contenedor = document.getElementById("accionesOferente");
  if (!contenedor) return;

  const idsBotones = [
    "btnValorCompetitivo",
    "btnValorCompetitivoHistorico",
    "btnFactoresEstrategicos",
    "btnSimulacionFormulas"
  ];
  ...
  [
    "bloqueValorCompetitivo",
    "bloqueValorCompetitivoHistorico",
    "bloqueFactoresEstrategicos",
    "bloqueSimulacion"
  ].forEach(id => {
    const bloque = document.getElementById(id);
    if (!bloque) return;

    bloque.classList.add("modulo-oferente");
    bloque.classList.add("bloque-dinamico-oferente");

    if (!bloque.dataset.mostradoPorUsuario) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔥 Ubica las herramientas estratégicas dentro del módulo Oferente
  setTimeout(moverBotonesAOferente, 500);
...
function obtenerTextoDocumento(doc) {
  if (typeof doc === "string") return doc;
  return doc?.nombre || doc?.url || "";
}

function extraerNombreDocumentoDesdeUrl(url = "") {
  try {
    const limpio = String(url || "").trim();
    const obj = new URL(limpio);
    ...
    return "Documento SECOP II";
  } catch (error) {
    return "Documento SECOP II";
  }
}

function normalizarDocumentoAnexo(entrada = "") {
  ...
  if (esUrl(texto)) {
    ...
    return {
      nombre: extraerNombreDocumentoDesdeUrl(url),
      url,
      tipo: "ENLACE",
...
function abrirDocumentoAnexo(doc) {
  const url = typeof doc === "string" ? doc : doc?.url;

  if (!url || !esUrl(url)) {
    alert("Este documento no tiene un enlace válido para abrir.");
    return;
  }

  const normalizada = normalizarUrl(url);
  const nuevaVentana = window.open(normalizada, "_blank", "noopener,noreferrer");

  if (!nuevaVentana || nuevaVentana.closed || typeof nuevaVentana.closed === "undefined") {
    window.location.assign(normalizada);
  }
}
...
function inicializarSecopYDocumentos() {
  ...
  btnAgregarDoc?.addEventListener("click", e => {
    e.preventDefault();
    ...
    const nuevoDoc = normalizarDocumentoAnexo(texto);
    ...
    const existe = DB.proceso.documentosAnexos.some(doc => {
      const actual = typeof doc === "string" ? doc : (doc?.url || doc?.nombre || "");
      const nuevo = nuevoDoc.url || nuevoDoc.nombre;
      return String(actual || "").trim().toLowerCase() === String(nuevo || "").trim().toLowerCase();
    });
    ...
    DB.proceso.documentosAnexos.push(nuevoDoc);
...
function renderDocumentosAnexos() {
  ...
  docs.forEach((doc, index) => {
    ...
    const esObjeto = typeof doc === "object" && doc !== null;
    const nombre = esObjeto
      ? (doc.nombre || doc.url || "Documento anexo")
      : obtenerTextoDocumento(doc);

    const url = esObjeto ? doc.url : (esUrl(doc) ? normalizarUrl(doc) : "");
    const tipo = esObjeto ? (doc.tipo || (url ? "ENLACE" : "REFERENCIA")) : (url ? "ENLACE" : "REFERENCIA");

    const etiqueta = document.createElement("span");
    etiqueta.innerHTML = `📄 <strong>${nombre}</strong> <small style="color:#64748b;">(${tipo})</small>`;
    ...
    if (url) {
      ...
      abrir.textContent = "Abrir / Descargar";
      abrir.dataset.index = index;
      abrir.addEventListener("click", () => abrirDocumentoAnexo(doc));

      const enlace = document.createElement("a");
      enlace.href = normalizarUrl(url);
      enlace.target = "_blank";
      enlace.rel = "noopener noreferrer";
      enlace.textContent = "Enlace directo";
    ...
    eliminar.className = "btn-eliminar-doc btn danger";
...
function inicializarValorCompetitivo() {
  ...
  btn.addEventListener("click", () => {
    ...
    if (!visible) {
      bloque.dataset.mostradoPorUsuario = "SI";

...
export function clasificarOfertaInicial(oferta, resultadosRelativos, presupuesto, umbral = 0.2) {
  ...
  const minimo = Number(resultadosRelativos.minimoAceptable || 0);

  const bajoMinimoRelativo = ofertaNum > 0 && ofertaNum < minimo;

  let clasificacionInicial = "SIGUE EN CURSO";
  let requiereSustentacion = false;
  let estadoInicial = "PENDIENTE";

  // REGLA CLAVE:
  // 1. Si está por debajo del mínimo aceptable → RECHAZO INICIAL
  if (bajoMinimoRelativo) {
    clasificacionInicial = "RECHAZADA INICIAL (POR DEBAJO DEL MÍNIMO ACEPTABLE)";
    requiereSustentacion = false;
    estadoInicial = "RECHAZADA";
  }
  // 2. Si no está por debajo del mínimo pero presenta alerta → pedir sustentación
  else if (absoluta.alertaAbsoluta) {
    clasificacionInicial = "POSIBLE OAB - REQUERIR SUSTENTACIÓN";
    requiereSustentacion = true;
  }
  // 3. Caso normal
  else {
    clasificacionInicial = "SIGUE EN CURSO";
    requiereSustentacion = false;
  }

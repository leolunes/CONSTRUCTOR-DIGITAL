...
/**
 * Carga únicamente ofertas desde el proyecto seleccionado.
 * NO reemplaza la información básica del proceso.
 */
function cargarSoloOfertasDesdeProyecto(id) {
  const proyecto = PROYECTOS.lista.find(p => Number(p.id) === Number(id));
  ...
  const ofertasFuente = Array.isArray(proyecto?.data?.ofertas)
    ? proyecto.data.ofertas
    : Array.isArray(proyecto?.ofertas)
      ? proyecto.ofertas
      : [];

  DB.ofertas = JSON.parse(JSON.stringify(ofertasFuente));
  PROYECTOS.activoId = Number(id);
...
function inicializarGestionProyectos() {
  ...
  btnGuardar?.addEventListener("click", () => {
    const nombre = prompt("Nombre del proyecto:", nombreSugeridoParaGuardarProyecto());
    if (!nombre || !String(nombre).trim()) return;
  ...
  btnCargar?.addEventListener("click", () => {
    ...
    const cargado = cargarSoloOfertasDesdeProyecto(id);
    if (!cargado) return;

    renderLista();

    // La información básica del proceso queda manual.
    // Por eso NO se llama cargarProcesoEnUI() ni renderTodo().
    renderResumenProceso();
    renderOfertas();
    renderOAB();
    renderEconomica();
    ...
    alert("Proyecto cargado: se importaron las ofertas. La información básica del proceso se mantiene manual.");
  });

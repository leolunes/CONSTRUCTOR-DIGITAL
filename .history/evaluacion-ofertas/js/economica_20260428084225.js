...
export function calcularValorCompetitivoHistorico() {
  ...
  function obtenerBaseReferenciaHistorica(presupuesto, ofertasValidas) {
    ...
    if (!medianaOfertas) {
      return { base: 0, tipo: "SIN_BASE", razon: "No hay valores de oferta suficientes para calcular una base." };
    }

    if (presupuestoNum > 0) {
      const relacion = medianaOfertas / presupuestoNum;

      if (relacion >= 0.50 && relacion <= 1.50) {
        return { base: presupuestoNum, tipo: "PRESUPUESTO_OFICIAL", razon: "Presupuesto oficial consistente con las ofertas." };
      }
    }

    return {
      base: medianaOfertas,
      tipo: "MEDIANA_OFERTAS",
      razon: "Presupuesto oficial ausente o inconsistente; se usó la mediana de ofertas como base referencial."
    };
  }
  ...
  PROYECTOS.lista.forEach(proyecto => {
    ...
    const baseInfo = obtenerBaseReferenciaHistorica(presupuesto, ofertasValidas);

    if (!baseInfo.base || baseInfo.base <= 0) {
      proyectosOmitidos.push({
        proyecto: proyecto.nombre || proceso.nombre || "Proyecto sin nombre",
        razon: baseInfo.razon
      });
      return;
    }
    ...
    top.forEach((oferta, index) => {
      ...
      const desviacion = ((Number(oferta.oferta || 0) - baseInfo.base) / baseInfo.base) * 100;
      ...
      muestras.push({
        ...
        baseReferencia: baseInfo.base,
        tipoBase: baseInfo.tipo,
        notaBase: baseInfo.razon,
        oferta: oferta.oferta
      });
    });
  });
  ...
  function obtenerBaseActualParaValorEstimado() {
    const presupuestoActual = Number(DB.proceso.presupuesto || 0);
    const ofertasActuales = Array.isArray(DB.ofertas) ? DB.ofertas.filter(esOfertaHistoricaValida) : [];

    if (!ofertasActuales.length) return presupuestoActual;

    const baseInfo = obtenerBaseReferenciaHistorica(presupuestoActual, ofertasActuales);
    return Number(baseInfo.base || presupuestoActual || 0);
  }

  const baseActual = obtenerBaseActualParaValorEstimado();
  const valorPesos = baseActual
    ? Math.round(baseActual * ((100 + promedioPonderado) / 100))
    : 0;

  const usaBaseReferencial = muestras.some(m => m.tipoBase !== "PRESUPUESTO_OFICIAL");

  let lectura = "";

  if (procesosAnalizados <= 1) {
    lectura = "El histórico todavía es limitado porque solo existe un proceso con información completa. El resultado debe leerse como una referencia inicial que mejorará a medida que se guarden más proyectos.";
  } else if (procesosAnalizados < 5) {
    lectura = "El histórico empieza a mostrar tendencia, pero aún cuenta con pocos procesos. Se recomienda usarlo como apoyo y contrastarlo con el valor competitivo del proceso actual.";
  } else {
  ...
  if (usaBaseReferencial) {
    lectura += " Nota: uno o más proyectos tenían presupuesto oficial ausente o inconsistente frente a sus ofertas; para evitar desviaciones irreales, el sistema usó una base referencial calculada con la mediana de ofertas.";
  }

  return {
    desviacionHistoricaSugerida: Number(promedioPonderado.toFixed(2)),
    valorPesos,
    rango: [Number(p25.toFixed(2)), Number(p75.toFixed(2))],
    ...
    confianza,
    metodo: usaBaseReferencial ? "TOP 3 ponderado histórico con base referencial" : "TOP 3 ponderado histórico",
    pesos: { primero: 3, segundo: 2, tercero: 1 },
    lectura,
    proyectosOmitidos,
    detalle: muestras.map(m => ({
      ...
      peso: m.peso,
      baseReferencia: Math.round(Number(m.baseReferencia || 0)),
      tipoBase: m.tipoBase
    }))
  };
}

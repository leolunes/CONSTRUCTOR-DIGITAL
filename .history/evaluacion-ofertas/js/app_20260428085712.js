...
// ===============================
function renderTodo() {
  normalizarCumplimientoFactoresDesdePuntajesDB();
  recalcularPuntajesTotalesTodasLasOfertas();
  cargarProcesoEnUI();
  renderResumenProceso();
  renderOfertas();
...
function normalizarCumplimientoFactoresDesdePuntajesOferta(oferta = {}) {
  return {
    cumpleCalidad: Boolean(oferta.cumpleCalidad) || Number(oferta.puntajeCalidad || 0) > 0,
    cumpleSostenibilidad: Boolean(oferta.cumpleSostenibilidad) || Number(oferta.puntajeSostenibilidad || 0) > 0,
    cumpleIndustria: Boolean(oferta.cumpleIndustria) || Number(oferta.puntajeIndustria || 0) > 0,
    cumpleDiscapacidad: Boolean(oferta.cumpleDiscapacidad) || Number(oferta.puntajeDiscapacidad || 0) > 0,
    cumpleMujer: Boolean(oferta.cumpleMujer) || Number(oferta.puntajeMujer || 0) > 0,
    cumpleMipyme: Boolean(oferta.cumpleMipyme) || Number(oferta.puntajeMipyme || 0) > 0
  };
}

function normalizarCumplimientoFactoresDesdePuntajesDB() {
  if (!Array.isArray(DB.ofertas)) return;

  DB.ofertas.forEach(oferta => {
    if (!oferta || !oferta.id) return;

    const cambios = normalizarCumplimientoFactoresDesdePuntajesOferta(oferta);

    const requiereCambio = Object.keys(cambios).some(key => Boolean(oferta[key]) !== Boolean(cambios[key]));
    if (requiereCambio) {
      actualizarOferta(oferta.id, cambios);
    }
  });
}
  ...
  reader.onload = function (e) {
    try {
      ...
      json.slice(inicioDatos).forEach((fila, idx) => {
        ...
        // ============================================================
        // AJUSTE 37 PROPUESTAS
        // Antes se usaba: if (empresa && valor > 0)
        // Eso podía omitir filas válidas cuando el valor venía como 0,
        // vacío temporalmente, o con formato no leído correctamente.
        // Ahora se importa toda fila que tenga nombre de proponente,
        // y solo se omiten filas realmente vacías o encabezados.
        // ============================================================
        const filaTieneDatosDePuntaje = [
          colPuntajeEconomico,
          colCalidad,
          colSostenibilidad,
          colIndustria,
          ...
          colMujer,
          colMipyme
        ].some(col => col >= 0 && String(fila[col] || "").trim() !== "");

        const filaTieneValorOriginal = String(valorOriginal || "").trim() !== "";
        const filaTieneNombre = String(empresa || "").trim() !== "";
        ...
        } else if (filaTieneValorOriginal || filaTieneDatosDePuntaje) {

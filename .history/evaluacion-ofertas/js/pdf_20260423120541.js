...
export function generarPDFResultadosEconomicos() {
  try {
    ...
    doc.text(`Puntaje máximo valor oferta: ${DB.proceso.puntajeMaximo || 0}`, 15, 42);

    const total = DB.ofertas.length;
    const alerta = DB.ofertas.filter(o => o.requiereSustentacion).length;
    const eliminadas = DB.ofertas.filter(
      o => String(o.observacion || "").trim().toUpperCase() === "ELIMINADA"
    ).length;
    const habilitadas = DB.ofertas.filter(
      o => String(o.observacion || "").trim().toUpperCase() !== "ELIMINADA"
    ).length;
...
TOTAL"
      ...
      doc.text("La librería autoTable no está cargada. No se pudo crear la tabla.", 15, 60);
    }

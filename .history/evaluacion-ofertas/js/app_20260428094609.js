...
async function agregarArchivosAnexosAlProyecto() {
  ...
  guardarDB();
  guardarProyectoActual(obtenerNombreProyectoActualSeguro());
  guardarDB();
...
// ===============================
// NOMBRE SEGURO DE PROYECTOS
// Evita usar automáticamente el número del proceso u objeto como nombre visible.
// ===============================
function esNombreProyectoInvalido(nombre = "", data = null) {
  const n = String(nombre || "").trim();
  if (!n) return true;

  const proceso = data?.proceso || DB.proceso || {};
  const nombreProceso = String(proceso.nombre || "").trim();
  const objetoProceso = String(proceso.objeto || "").trim();

  if (n.toLowerCase() === "proyecto") return true;
  if (n.toLowerCase() === "proyecto sin nombre") return true;

  // Caso detectado: el proyecto quedó nombrado como el número del proceso, por ejemplo "LP".
  if (nombreProceso && n.toLowerCase() === nombreProceso.toLowerCase()) return true;

  // Caso detectado: el proyecto quedó nombrado con el objeto completo o un texto demasiado largo.
  if (objetoProceso && n.toLowerCase() === objetoProceso.toLowerCase()) return true;
  if (n.length > 70) return true;

  return false;
}

function obtenerNombreProyectoActualSeguro() {
  const proyectoActual = PROYECTOS.lista.find(p => Number(p.id) === Number(PROYECTOS.activoId));

  if (proyectoActual && !esNombreProyectoInvalido(proyectoActual.nombre, proyectoActual.data)) {
...
function obtenerNombreProyectoVisible(proyecto = {}, index = 0) {
  if (!esNombreProyectoInvalido(proyecto?.nombre, proyecto?.data)) {
    return proyecto.nombre;
  }

  const numero = Number(index || 0) + 1;
  return `Proyecto guardado ${numero}`;
}

function solicitarNombreProyecto(titulo = "Nombre del proyecto:") {
  const proyectoActual = PROYECTOS.lista.find(p => Number(p.id) === Number(PROYECTOS.activoId));
  const sugerido = obtenerNombreProyectoActualSeguro();
  const nombre = prompt(titulo, sugerido);
...
function inicializarGestionProyectos() {
  ...
  function renderLista(filtro = "") {
    ...
    lista.forEach((p, index) => {
      ...
      opt.value = p.id;
      opt.textContent = obtenerNombreProyectoVisible(p, index);
  ...
  btnNuevo?.addEventListener("click", () => {
    const nombre = prompt("Nombre del nuevo proyecto:", "Proyecto guardado");
  ...
  btnGuardar?.addEventListener("click", () => {
    const nombre = solicitarNombreProyecto("Nombre del proyecto:");
    if (!nombre) return;

    const proyecto = guardarProyectoActual(nombre);

    // Asegura que el nombre visible del proyecto no vuelva a tomar el nombre del proceso.
    if (proyecto) {

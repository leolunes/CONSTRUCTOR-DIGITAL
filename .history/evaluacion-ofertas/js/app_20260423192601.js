...
import { obtenerNombreMetodo } from "./trm.js";
...
// ===============================
// PRESERVAR DECISIONES MANUALES (OAB FINAL)
// ===============================
function capturarDecisionesManuales() {
  const mapa = new Map();
  DB.ofertas.forEach(o => {
    mapa.set(o.id, {
      estado: o.estado,
      respondio: o.respondio,
      sustentoValido: o.sustentoValido,
...
function restaurarDecisionesManuales(mapa) {
  DB.ofertas.forEach(o => {
    const prev = mapa.get(o.id);
    if (!prev) return;

    let obs = prev.observacion || "";
    if (prev.estado === "ACEPTADA") obs = "";
    if (prev.estado === "RECHAZADA") obs = "ELIMINADA";

    actualizarOferta(o.id, {
      estado: prev.estado || o.estado,
      respondio: (prev.respondio ?? o.respondio),
      sustentoValido: (prev.sustentoValido ?? o.sustentoValido),
      observacion: obs
    });
  });
}
...
// No afecta la evaluación económica oficial
// ===============================
...
function simularEvaluacionEconomicaLocal(metodoSimulado) {
  ...
  const tabla = obtenerOfertasSimulables()
    .map(oferta => ({
      ...
      puntaje: calcularPuntajeSimulado(oferta.oferta, metodoSimulado, puntajeMaximo, bases)
    }))
    .sort((a, b) => {
...
function inicializarAccionesEvaluacion() {
  ...
  $("#btnRecalcularTodo")?.addEventListener("click", () => {
    const snapshot = capturarDecisionesManuales();
    ejecutarEvaluacionOAB(0.2);
    restaurarDecisionesManuales(snapshot);
...
function bindTablaOfertas() {
  ...
  $all(".select-estado").forEach(select => {
    select.addEventListener("change", e => {
      ...
      let observacionFinal = oferta.observacion || "";

      // ===============================
      // 🔥 LÓGICA FINAL OAB
      // ===============================
      // Si la propuesta pasa a ACEPTADA:
      // - se limpia la observación ELIMINADA
      // - queda habilitada para evaluación económica
      if (estado === "ACEPTADA") {
        observacionFinal = "";
      }

      // Si la propuesta pasa a RECHAZADA:
      // - queda marcada automáticamente como ELIMINADA
      // - sale de la evaluación económica
      if (estado === "RECHAZADA") {
        observacionFinal = "ELIMINADA";
      }
...
function inicializarSimulacionFormulas() {
  ...
  const bloque = $("#bloqueSimulacion");
  const select = $("#selectSimulacionMetodo");
  const btnSimular = $("#btnEjecutarSimulacion");
  ...
  function cargarOpcionesSimulacion() {
    ...
    const metodoActual =
      DB.resultadosEconomicos?.metodo ||
      "";
    ...
    metodos
      ...
      .forEach(m => {
        const opt = document.createElement("option");
        opt.value = m;
        opt.textContent = obtenerNombreMetodo(m);
        select.appendChild(opt);
      });
  }
  ...
  function ejecutarSimulacionSeleccionada() {
    ...
    if (!resultado.tabla.length) {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td colspan="4">No existen ofertas habilitadas para simular.</td>`;
      tbody.appendChild(tr);
      return;
    }
  ...
  btn?.addEventListener("click", () => {
    if (!bloque) {
      alert("No se encontró el bloque de simulación. Revise index.html.");
      return;
    }
  ...
  btnSimular?.addEventListener("click", ejecutarSimulacionSeleccionada);

  select?.addEventListener("change", ejecutarSimulacionSeleccionada);
}

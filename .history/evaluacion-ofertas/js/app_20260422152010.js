...
function inicializarFormularioOfertas() {
  ...
  const btnLimpiar = $("#btnLimpiarListadoOfertas");
  ...
  if (btnLimpiar) {
    btnLimpiar.addEventListener("click", () => {
      const confirmar = confirm("¿Desea eliminar todas las ofertas cargadas?");
      if (!confirmar) return;
...
function importarExcel() {
  ...
  reader.onload = function (e) {
    try {
      ...
      // Detectar encabezado de forma inteligente
      let filasBase = [...json];
      ...
      // Conservar filas con nombre, aunque vengan con formatos extraños
      const filasLimpias = [];
      const filasOmitidas = [];

...
/* =========================================================
   ✅ BIND DETALLE PROYECTO + EXPORTS + APU VIEW FIX
   ========================================================= */
async function bindProjectDetailPage(){
  ...
  UI.qs("#btnExportExcel")?.addEventListener("click", ()=>{
    ...
    exportProjectExcel(p);
  });

  UI.qs("#btnPDF")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportPresupuesto(p);
  });

  UI.qs("#btnPDFAPU")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportPresupuestoConAPU(p);
  });

  UI.qs("#btnPDFDesagregado")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportDesagregado(p);
  });

  UI.qs("#btnPDFResumenDesagregado")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportResumenDesagregado(p);
  });

  UI.qs("#btnPDFDistribucion")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportDistribucion(p);
  });

  UI.qs("#btnPDFRendimientos")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportRendimientos(p);
  });

  UI.qs("#btnPDFMateriales")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportMateriales(p);
  });

  UI.qs("#btnPDFInsumos")?.addEventListener("click", ()=>{
    const p = StorageAPI.getProjectById(projectId);
    PDF.exportInsumos(p);
  });
  ...
  /* =========================
     BUSCADOR APU
     ========================= */
  UI.qs("#btnBuscarAPU")?.addEventListener("click", async ()=>{
    const q = UI.qs("#searchAPU")?.value || "";
    if(!q.trim()){
      alert("Escribe un código o descripción");
      return;
    }

    try{
      const results = await APUBase.search(q);
      renderApuResults(projectId, results);
    }catch(err){
      alert("Error buscando APU: " + err.message);
    }
  });

  UI.qs("#searchAPU")?.addEventListener("keydown", (e)=>{
    if(e.key === "Enter"){
      e.preventDefault();
      UI.qs("#btnBuscarAPU")?.click();
    }
  });
  ...
  UI.qs("#btnConfirmAddApu")?.addEventListener("click", ()=>{
    confirmAddApuToProject({ keepOpen:false });
  });

  UI.qs("#btnConfirmAddApuKeep")?.addEventListener("click", ()=>{
    confirmAddApuToProject({ keepOpen:true });
  });

  /* =========================
     DOCUMENTOS
     ========================= */
  UI.qs("#btnAddDoc")?.addEventListener("click", ()=>{
    UI.qs("#fileDoc")?.click();
  });
...
/* =========================================================
   ✅ FIX CRÍTICO: VISOR APU (ITEM + SUBPRODUCTO)
   ========================================================= */
async function bindApuPage(){
  ...
  async function renderLines(lines){
    ...
    tbody.innerHTML = lines.map(l=>`
      <tr>
        <td>${UI.esc(l.group||"")}</td>
        <td>${UI.esc(l.desc||"")}</td>
        <td>${UI.esc(l.unit||"")}</td>
        <td style="text-align:right">${UI.esc(String(l.qty||0))}</td>
        <td style="text-align:right"><b>${UI.fmtMoney(l.pu||0,"COP")}</b></td>
        <td style="text-align:right"><b>${UI.fmtMoney(l.parcial||0,"COP")}</b></td>
      </tr>
    `).join("");
  }

  /* =========================
     🔥 CASO SUBPRODUCTO (FIX)
     ========================= */
  if(sub){
    try{
      const apu = await APUBase.getSubproducto(sub);

      if(!apu){
        alert("Subproducto no encontrado");
        return;
      }

      if(title) title.textContent = apu.subName || "SUBPRODUCTO";
      if(subTitle) subTitle.textContent = "Fuente: Subproductos";
      ...
      alert("Error cargando subproducto: " + err.message);
      return;
    }
  }
  ...
  if(code){
    try{
      const resolved = await tryResolveAPUFromProjectItem(projectId, code);

      if(!resolved || !resolved.data){
        alert("No se encontró el APU. Instala la base XLSX.");
        return;
      }

      const apu = resolved.data;

      if(title) title.textContent = apu.desc || code;
      if(subTitle) subTitle.textContent = `Fuente: ${resolved.source}`;
      ...
      alert("Error cargando APU: " + err.message);
    }
  }
}

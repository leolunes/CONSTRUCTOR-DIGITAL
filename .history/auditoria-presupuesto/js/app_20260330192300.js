...
function ensureIndirectTableMount(form){
  ...
  host.innerHTML = `
    <div class="cardhead">
      ...
      <p class="muted small">La app mostrará el mismo desglose base del PDF: ÍTEM, GRUPO, DESCRIPCIÓN, UNID, CANT, VR UNIT, % DIR y VR PARCIAL.</p>
    </div>
    ...
    <div id="indirectTableWrap" style="display:none">
      ...
      <div style="overflow:auto">
        <table class="table" style="min-width:1180px">
          <thead>
            <tr>
              <th style="min-width:90px">ÍTEM</th>
              <th style="min-width:110px">GRUPO</th>
              <th style="min-width:320px">DESCRIPCIÓN</th>
              <th style="min-width:90px">UNID</th>
              <th style="min-width:90px; text-align:right">CANT</th>
              <th style="min-width:140px; text-align:right">VR UNIT</th>
              <th style="min-width:90px; text-align:right">% DIR</th>
              <th style="min-width:150px; text-align:right">VR PARCIAL</th>
...
function renderIndirectRows(projectId){
  ...
  const manualWrap = UI.qs("#manualIndirectWrap");
  if(manualWrap) manualWrap.style.display = mode === "table" ? "none" : "";
  ...
  tbody.innerHTML = rows.map((row, idx)=>{
    ...
    const unitVal = isPercentCd ? Number(directo || 0) : Number(r.unitValue || 0);
    const itemTxt = UI.esc(r.item || "");
    const groupTxt = UI.esc(r.group || "");
    const descTxt = UI.esc(r.desc || "");
    const unitTxt = UI.esc(isPercentCd ? "%CD" : (r.unit || ""));
    const qtyTxt = UI.esc(String(isPercentCd ? 1 : (r.qty||0)));
    const pctTxt = Number(r.percent||0).toFixed(2);
...
async function bindProjectDetailPage(){
  ...
  UI.qs("#formAjustes")?.addEventListener("submit", (e)=>{
    ...
    const indirectMode = normalizeIndirectModeValue(UI.qs("#indirectMode")?.value || project.indirectMode || "manual");
  ...
  if(form){
    ...
    if(form.indirectMode) form.indirectMode.value = String(cfg.indirectMode || "manual");
    const manualWrap = UI.qs("#manualIndirectWrap");
    if(manualWrap) manualWrap.style.display = (cfg.indirectMode === "table") ? "none" : "";

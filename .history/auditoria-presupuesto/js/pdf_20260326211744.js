(function(){
  ...
  async function getAPUForProjectItem(project, it){
    ...
    if(!hasRealRef && c2 && !candidates.includes(c2)){
      candidates.push(c2);
    }
    ...
    async function tryByCode(apuCode, matchType){
    ...
    if(c1){
      const got = await tryByCode(c1, "apuRef");
      if(got) return got;
      return null;
    }

    if(c2){
      const got = await tryByCode(c2, "code");
      if(got) return got;
    }
    if(!hasRealRef){
      ...
      if(desc){
        ...
        if(codeByDesc){
          const got = await tryByCode(codeByDesc, "desc");
          if(got){
            got._match = "desc";
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    // ✅ ACUMULADOR DE SUBPRODUCTOS USADOS EN TODO EL PRESUPUESTO
    ...
    for(const it of items){
      ...
      // ✅ SOLO IMPRIMIR LÍNEAS NORMALES DEL APU
      // ✅ LOS SUBPRODUCTOS SE ACUMULAN PARA IMPRIMIRLOS AL FINAL
      const normales = [];
    ...
    if(subproductosUsados.size){
      ...
      for(const info of subproductosUsados.values()){
        ...
        // ✅ prioridad: override del proyecto
        let subAPU = null;
        try{

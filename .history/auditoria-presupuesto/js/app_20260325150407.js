...
async function bindApuPage(){
  ...
  const goProjectBtn = qsAny(["#btnGoProject"]);
  if(goProjectBtn){
    goProjectBtn.href = `proyecto-detalle.html?projectId=${encodeURIComponent(projectId)}`;
  }

  const infoEl = qsAny(["#apuInfo"]);
  if(infoEl){
    infoEl.innerHTML = `
      <div class="cardhead">
        <h2>Información del APU</h2>
        <p class="muted small">Proyecto: ${UI.esc(project.name || "-")} · Modo: ${UI.esc(budgetLabel(mode))}</p>
      </div>
    `;
  }

  const editorEl = qsAny(["#apuEditor"]);
  if(editorEl){
  ...
  if(!apu || (!apu.desc && (!apu.lines || !apu.lines.length))){
    const loadEl = qsAny(["#apuLoading", "#loadingText", ".loading-text", "#apuSub"]);
  ...
  function redraw(currentApu){
    ...
    if(infoEl){
      infoEl.innerHTML = `
        <div class="cardhead">
          <h2>${UI.esc(currentApu.code || apuCode)} · ${UI.esc(currentApu.desc || "Sin descripción")}</h2>
          <p class="muted small">Unidad: ${UI.esc(currentApu.unit || "-")} · PU actual: <b>${UI.fmtMoney(currentApu.pu || 0, project?.currency || "COP")}</b></p>
        </div>
      `;
    }
    ...
    tbody.querySelectorAll("[data-apu-edit-line]").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        ...
        currentApu.lines = lines;
        currentApu.pu = calcApuPU(lines);
        saveProjectApuOverride(projectId, apuCode, currentApu, mode);
        updateItemsPUByApuCompat(projectId, apuCode, currentApu.pu, mode);
        redraw(currentApu);
      });
    });
  ...
  if(addBtn){
    addBtn.addEventListener("click", ()=>{
      ...
      updateItemsPUByApuCompat(projectId, apuCode, apu.pu, mode);
      redraw(apu);
    });
  }
}

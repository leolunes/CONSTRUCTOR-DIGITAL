...
function detectBudgetItemAlerts(row){
  ...
  const maxPct = Math.max(Math.abs(qtyPct || 0), Math.abs(puPct || 0));
  let severity = "ok";
  let severityLabel = "OK";

  const hasStructuralAlert =
    (baseQty === 0 && officialQty > 0) ||
    (baseQty > 0 && officialQty === 0) ||
    (basePU === 0 && officialPU > 0) ||
    (basePU > 0 && officialPU === 0);

  if(hasStructuralAlert || maxPct > 20){
    severity = "bad";
    severityLabel = "CRÍTICO";
  }else if(alerts.length || maxPct > 10){
    severity = "warn";
    severityLabel = "ALERTA";
  }

  return { qtyPct, puPct, alerts, severity, severityLabel, maxPct };
}
...
function renderBudgetItemComparison(project){
  ...
  const audits = rows.map(r => ({ row: r, audit: detectBudgetItemAlerts(r) }));
  const okCount = audits.filter(x => x.audit.severity === "ok").length;
  const warnCount = audits.filter(x => x.audit.severity === "warn").length;
  const badCount = audits.filter(x => x.audit.severity === "bad").length;
  ...
  wrap.innerHTML = `
    ...
    <div class="cardhead">
      ...
      <p class="muted small">Modo auditoría activo. Umbrales de alerta: PU 10% y Cantidad 10%. Crítico: variaciones mayores al 20% o ítems nuevos/eliminados.</p>
    </div>

    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
      ${UI.chip(`OK ${okCount}`, "ok")}
      ${UI.chip(`ALERTA ${warnCount}`, "warn")}
      ${UI.chip(`CRÍTICO ${badCount}`, "bad")}
    </div>
    ...
    <div class="tablewrap">
      <table class="table" style="min-width:1560px">
        ...
        <tbody>
          ${audits.map(({ row:r, audit })=>{
            const diffQty = Number(r.officialQty || 0) - Number(r.baseQty || 0);
            const diffPU = Number(r.officialPU || 0) - Number(r.basePU || 0);
            const bg =
              audit.severity === "bad" ? "rgba(239,68,68,.10)" :
              audit.severity === "warn" ? "rgba(245,158,11,.10)" :
              "transparent";

            const qtyChipType = Math.abs(Number(audit.qtyPct || 0)) > 20 ? "bad" : (Math.abs(Number(audit.qtyPct || 0)) >= 10 ? "warn" : "ok");
            const puChipType = Math.abs(Number(audit.puPct || 0)) > 20 ? "bad" : (Math.abs(Number(audit.puPct || 0)) >= 10 ? "warn" : "ok");

            return `
              <tr style="background:${bg}">
                <td>${UI.chip(audit.severityLabel || "OK", audit.severity || "ok")}</td>
                ...
                <td style="text-align:right">${UI.chip(`${Number(audit.qtyPct || 0).toFixed(2)}%`, qtyChipType)}</td>
                <td style="text-align:right">${UI.chip(`${Number(audit.puPct || 0).toFixed(2)}%`, puChipType)}</td>
                <td>${audit.alerts.length ? audit.alerts.map(a=>UI.chip(a, audit.severity === "bad" ? "bad" : "warn")).join(" ") : UI.chip("Sin alerta", "ok")}</td>
              </tr>
            `;
          }).join("")}
        </tbody>
...
function exportAuditPdf(project){
  ...
  addTitle("2. Comparación por capítulos");
  ...
  addTitle("4. Historial reciente de cambios");
  ...
  addTitle("5. Conclusión automática");
  ...
  addParagraph(
    ...
    + `Se identificaron ${highAlerts} ítems con alerta automática y ${positiveChapters} capítulos con variación positiva frente al presupuesto base. `

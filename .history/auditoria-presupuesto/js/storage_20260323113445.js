...
/* =========================
   ✅ NUEVO: helpers presupuesto oficial
   ========================= */
function getProjectOfficialView(project){
  const p = project || {};
  return {
    items: Array.isArray(p.oficialItems) ? p.oficialItems : [],
    chapters: Array.isArray(p.oficialChapters) ? p.oficialChapters : [],
    apuOverrides: p.oficialApuOverrides || {}
  };
}

function findOfficialChapterByCode(project, code){
  const cc = normalizeChapterCode(code);
  if(!cc) return null;
  const chs = Array.isArray(project?.oficialChapters) ? project.oficialChapters : [];
  return chs.find(c => String(c.chapterCode) === cc) || null;
}

function resolveOfficialChapterCodeForItem(project, itemCode, explicitChapterCode){
  const exp = normalizeChapterCode(explicitChapterCode);
  if(exp) return exp;
...
function resolveOfficialChapterNameForItem(project, chapterCode, fallbackName){
  const cc = normalizeChapterCode(chapterCode);
  const fb = safeStr(fallbackName);
  const ch = cc ? findOfficialChapterByCode(project, cc) : null;
  if(ch && ch.chapterName) return String(ch.chapterName);
  return fb;
}
...
function normalizeProject(p){
  ...
  // ✅ NUEVO: capítulos del presupuesto oficial
  proj.oficialChapters = normalizeChapters(proj.oficialChapters);

  // =========================
  // PRESUPUESTO BASE (existente)
  // =========================
  if(!Array.isArray(proj.items)) proj.items = [];
  if(!proj.apuOverrides) proj.apuOverrides = {};

  // =========================
  // PRESUPUESTO OFICIAL (nuevo)
  // =========================
  if(!Array.isArray(proj.oficialItems)) proj.oficialItems = [];
  if(!proj.oficialApuOverrides) proj.oficialApuOverrides = {};

  // ✅ Normalización suave de items BASE:
  // - completa chapterName desde chapters
  // - garantiza apuRefCode para NO romper PDFs al renumerar item.code
  try{
  ...
  // ✅ Normalización suave de items OFICIALES:
  // - completa chapterName desde oficialChapters
  // - garantiza apuRefCode
  try{
    const chs = Array.isArray(proj.oficialChapters) ? proj.oficialChapters : [];
    if(Array.isArray(proj.oficialItems)){
      for(const it of proj.oficialItems){
        if(!normalizeApuRefCode(it?.apuRefCode)){
          it.apuRefCode = safeStr(it?.code || "");
        }else{
          it.apuRefCode = normalizeApuRefCode(it.apuRefCode);
...
function loadStore(){
  ...
  try{
    ...
    if(!db.meta) db.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO() };
    ...
    // ✅ Normalizar proyectos (incluye base + oficial + chapters + nuevos % + logo + inst* + apuRefCode)
    ...
    if(rawMain && rawBackup && rawMain !== rawBackup){
      try{
        ...
        if(!recovered.meta) recovered.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO() };
...
async function importBackupFromFile(file){
  ...
  if(!data.meta) data.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO() };
...
/* =========================
   Chapters OFICIAL (por proyecto)
   ========================= */
function listOfficialProjectChapters(projectId){
  const p = getProjectById(projectId);
  return (p && Array.isArray(p.oficialChapters)) ? p.oficialChapters : [];
}

function upsertOfficialProjectChapter(projectId, chap){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return null;
  ...
  const chapterCode = normalizeChapterCode(chap?.chapterCode ?? chap?.code);
  const chapterName = normalizeChapterName(chap?.chapterName ?? chap?.name);
  if(!chapterCode) throw new Error("Capítulo oficial: falta chapterCode");

  const p = db.projects[pidx];
  if(!Array.isArray(p.oficialChapters)) p.oficialChapters = [];

  const id = safeStr(chap?.id);
  let idx = id ? p.oficialChapters.findIndex(c => String(c.id) === id) : -1;

  if(idx === -1){
    idx = p.oficialChapters.findIndex(c => String(c.chapterCode) === chapterCode);
  }

  const rec = {
    id: (idx >= 0 ? p.oficialChapters[idx].id : (id || uid("ochap"))),
    chapterCode,
    chapterName
  };

  if(idx >= 0) p.oficialChapters[idx] = rec;
  else p.oficialChapters.push(rec);

  p.oficialChapters = normalizeChapters(p.oficialChapters);

  try{
    for(const it of (p.oficialItems||[])){
      if(String(it.chapterCode||"") === chapterCode && !safeStr(it.chapterName)){
        it.chapterName = chapterName;
      }
    }
...
function deleteOfficialProjectChapter(projectId, chapIdOrCode){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return false;
  ...
  const before = p.oficialChapters.length;
  p.oficialChapters = p.oficialChapters.filter(c => String(c.id) !== key && String(c.chapterCode) !== key);

  const changed = p.oficialChapters.length !== before;
  if(changed){
    p.updatedAt = nowISO();
    db.projects[pidx] = normalizeProject(p);
    saveStore(db);
...
/* ===== Items OFICIALES ===== */
function addOfficialItem(projectId, item){
  const db = loadStore();
  const idx = db.projects.findIndex(p=>p.id===projectId);
  if(idx===-1) return null;
  ...
  const code = safeStr(item?.code);
  const chapCode = resolveOfficialChapterCodeForItem(proj, code, item?.chapterCode);
  const chapName = resolveOfficialChapterNameForItem(proj, chapCode, item?.chapterName);
  ...
  db.projects[idx].oficialItems.push(it);
  db.projects[idx].updatedAt = nowISO();
  saveStore(db);
  return it;
}

function updateOfficialItem(projectId, itemId, patch){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return null;
  ...
  const iidx = (proj.oficialItems||[]).findIndex(i=>i.id===itemId);
  if(iidx===-1) return null;

  const cur = proj.oficialItems[iidx];
  const next = { ...cur, ...patch };
  ...
  if(patch.code && patch.chapterCode == null){
    const resolved = resolveOfficialChapterCodeForItem(proj, String(patch.code), "");
    if(resolved) next.chapterCode = resolved;
  }

  if((patch.chapterCode != null) || next.chapterCode){
    if(!safeStr(next.chapterName)){
      next.chapterName = resolveOfficialChapterNameForItem(proj, next.chapterCode, next.chapterName);
    }
  }

  proj.oficialItems[iidx] = next;
  proj.updatedAt = nowISO();
  db.projects[pidx] = normalizeProject(proj);
...
function deleteOfficialItem(projectId, itemId){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return false;
  db.projects[pidx].oficialItems = (db.projects[pidx].oficialItems||[]).filter(i=>i.id!==itemId);
  db.projects[pidx].updatedAt = nowISO();
  saveStore(db);
  return true;
}

function updateOfficialItemsPUByCode(projectId, code, newPU){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return 0;
  ...
  const c = String(code||"").trim();
  let count = 0;
  for(const it of (db.projects[pidx].oficialItems||[])){
    const lookup = getItemApuLookupCode(it);
    if(String(lookup||"").trim() === c){
      it.pu = Number(newPU||0);
      count++;
...
/* =========================
   Override APU BASE por proyecto
   ========================= */
...
/* =========================
   Override APU OFICIAL por proyecto
   ========================= */
function getOfficialApuOverride(projectId, code){
  const p = getProjectById(projectId);
  if(!p) return null;
  const c = String(code||"").trim();
  return (p.oficialApuOverrides && p.oficialApuOverrides[c]) ? p.oficialApuOverrides[c] : null;
}

function setOfficialApuOverride(projectId, code, lines){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return null;

  const c = String(code||"").trim();
  if(!db.projects[pidx].oficialApuOverrides) db.projects[pidx].oficialApuOverrides = {};
  db.projects[pidx].oficialApuOverrides[c] = {
    code: c,
    updatedAt: nowISO(),
    lines: Array.isArray(lines) ? lines : []
  };
  db.projects[pidx].updatedAt = nowISO();
  saveStore(db);
  return db.projects[pidx].oficialApuOverrides[c];
}

function clearOfficialApuOverride(projectId, code){
  const db = loadStore();
  const pidx = db.projects.findIndex(p=>p.id===projectId);
  if(pidx===-1) return false;

  const c = String(code||"").trim();
  if(db.projects[pidx].oficialApuOverrides && db.projects[pidx].oficialApuOverrides[c]){
    delete db.projects[pidx].oficialApuOverrides[c];
    db.projects[pidx].updatedAt = nowISO();
    saveStore(db);
    return true;
  }
...
function importProjectAsNew(projectObj){
  ...
  const base = createProject({
    ...
    oficialChapters: Array.isArray(payload.oficialChapters) ? payload.oficialChapters : []
  });
  ...
  const oficialItems = Array.isArray(payload.oficialItems) ? payload.oficialItems : [];
  const mappedOfficial = oficialItems.map(it => ({
    id: uid("oit"),
    chapterCode: safeStr(it.chapterCode || ""),
    chapterName: safeStr(it.chapterName || ""),
    apuRefCode: normalizeApuRefCode(it.apuRefCode) || safeStr(it.code || ""),
...
window.StorageAPI = {
  ...
  // OFICIAL chapters
  listOfficialProjectChapters, upsertOfficialProjectChapter, deleteOfficialProjectChapter,
  ...
  // OFICIAL items
  addOfficialItem, updateOfficialItem, deleteOfficialItem,
  updateOfficialItemsPUByCode,

  // BASE overrides
  getApuOverride, setApuOverride, clearApuOverride,

  // OFICIAL overrides
  getOfficialApuOverride, setOfficialApuOverride, clearOfficialApuOverride,

  importProjectAsNew,

  // helpers útiles
  getProjectOfficialView
};
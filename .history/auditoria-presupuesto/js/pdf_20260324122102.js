(function(){
  ...
  async function getAPUForProjectItem(project, it){
    ...
    const budgetMode = String(project?.__budgetMode || "base").toLowerCase();
    ...
    async function tryByCode(apuCode){
      ...
      try{
        if(budgetMode === "oficial" && window.StorageAPI?.getOfficialApuOverride){
          const ov = StorageAPI.getOfficialApuOverride(project.id, apuCode);
          if(ov){
        ...
        if(budgetMode !== "oficial" && window.StorageAPI?.getApuOverride){
  ...
  async function finalizePDF(doc, filename, opts){
    ...
    if(o.share){
      const blobShare = doc.output("blob");
      const shared = await shareBlobAsFile(blobShare, filename);
      forcePersistAppState();

      if(shared) return true;

      if(isMobileLike()){
        const opened = openBlobInNewTab(blobShare, filename);
        await sleep(40);
        forcePersistAppState();
        if(opened) return true;
      }

      downloadBlob(blobShare, filename);
      await sleep(40);
      forcePersistAppState();
      return true;
    }
    ...
    try{
      if(doc && typeof doc.save === "function" && !isMobileLike()){
      ...
      console.warn("[PDF] doc.save falló, usando fallback blob:", err);
    }

    const blob = doc.output("blob");

    if(isMobileLike()){
      const opened = openBlobInNewTab(blob, filename);
      await sleep(40);
      forcePersistAppState();
      if(opened) return true;
    }
  ...
  async function exportPresupuestoObraDesagregadoPDF(project, opts){
    const pview = getProjectForPDF(project, opts?.mode);
    const doc = newDoc({ orientation: "landscape" });
    const L = mkLayout(doc);
    const { items } = Calc.groupByChapters(pview);

    drawInstitutionalCover(doc, L, pview, "PRESUPUESTO DE OBRA DESAGREGADO");
    ...
    L.p(`Proyecto: ${pview.name || "—"}`);
    L.p(`Tipo de presupuesto: ${getBudgetLabelPDF(pview.__budgetMode)}`);
    L.p(`Entidad: ${pview.entity || "—"}`);
    L.p(`Ubicación: ${pview.location || "—"}`);
    L.p(`Fecha generación: ${new Date().toLocaleString()}`);
    L.p(" ");

    const agg = await calcDesagregadoByItemFromAPU(pview, items || []);
    const rows = agg.rows || [];
    ...
    stampHeaderFooter(doc, {
      docType:`PRESUPUESTO DE OBRA DESAGREGADO · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameDesagregado(pview);
  ...
  async function exportResumenPresupuestoObraDesagregadoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "RESUMEN PRESUPUESTO DE OBRA DESAGREGADO");
    ...
    stampHeaderFooter(doc, {
      docType:`RESUMEN PRESUPUESTO OBRA DESAGREGADO · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameResumenDesagregado(pview);
    return await finalizePDF(doc, filename, opts);
  }
  async function exportDistribucionPorcentualCostosDirectosPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "DISTRIBUCIÓN PORCENTUAL DE COSTOS DIRECTOS");
    ...
    stampHeaderFooter(doc, {
      docType:`DISTRIBUCIÓN PORCENTUAL COSTOS DIRECTOS · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameDistribucionPctDirectos(pview);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportRendimientoEquipoYManoDeObraPorActividadPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "RENDIMIENTO EQUIPO Y MANO DE OBRA POR ACTIVIDAD");
    ...
    async function itemBlock(it){
      const qtyItem = Number(it.qty||0);
      const apuObj = await getAPUForProjectItem(pview, it);
      const lines = (apuObj && Array.isArray(apuObj.lines)) ? apuObj.lines : [];
    ...
    stampHeaderFooter(doc, {
      docType:`RENDIMIENTO EQUIPO Y MANO DE OBRA · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameRendimientos(pview);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportResumenMaterialesPorActividadPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "RESUMEN MATERIALES POR ACTIVIDAD");
    ...
    stampHeaderFooter(doc, {
      docType:`RESUMEN MATERIALES POR ACTIVIDAD · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameResumenMaterialesActividad(pview);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportCantidadRecursosEInsumosPresupuestoPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "CANTIDAD DE RECURSOS E INSUMOS DEL PRESUPUESTO");
    ...
    stampHeaderFooter(doc, {
      docType:`CANTIDAD DE RECURSOS E INSUMOS · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameCantRecursosInsumos(pview);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportPresupuestoPDF(project, opts){
    ...
    const { groups, items } = Calc.groupByChapters(pview);

    const totals = totalsCompat(pview);
    const adminPct = pct(pview, "adminPct", "aiuPct");
    const imprevPct = pct(pview, "imprevPct", null);
    const utilPct = pct(pview, "utilPct", null);
    const ivaUtilPct = pct(pview, "ivaUtilPct", "ivaPct");

    drawInstitutionalCover(doc, L, pview, "PRESUPUESTO DE OBRA");
    ...
    L.row("TIPO DE PRESUPUESTO:", getBudgetLabelPDF(pview.__budgetMode));
    ...
    stampHeaderFooter(doc, {
      docType:`PRESUPUESTO DE OBRA · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilename(pview, pview.__budgetMode === "oficial" ? "OFICIAL_DETALLE" : "DETALLE");
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    drawInstitutionalCover(doc, L, pview, "PRESUPUESTO DE OBRA + APUs");
    ...
    stampHeaderFooter(doc, {
      docType:`PRESUPUESTO DE OBRA + APUs · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilename(pview, pview.__budgetMode === "oficial" ? "OFICIAL_APUS" : "APUS");
    return await finalizePDF(doc, filename, opts);
  }
  ...
  async function exportEspecificacionesTecnicasPDF(project, opts){
    ...
    const entidadContratante = pview?.entity ? String(pview.entity) : "—";
    const ubicacion = pview?.location ? String(pview.location) : "—";

    drawInstitutionalCover(doc, L, pview, "ESPECIFICACIONES TÉCNICAS DEL PROYECTO");
    ...
    L.p(
      ...
      "dentro del presupuesto visible del proyecto. Las especificaciones se estructuran con base en el " +
    ...
    stampHeaderFooter(doc, {
      docType:`ESPECIFICACIONES TÉCNICAS · ${getBudgetLabelPDF(pview.__budgetMode)}`,
      projectName: getProjectNameWithBudgetLabel(pview, pview.__budgetMode),
      logoDataUrl: pview.logoDataUrl || ""
    });

    const filename = buildFilenameSpecs(pview);
    return await finalizePDF(doc, filename, opts);
  }

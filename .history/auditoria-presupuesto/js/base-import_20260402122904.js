(function(){
  ...
  async function listSubproductosDeep(query="", limit){
    const q = normalizeText(query);
    const st = await store(STORE_SUB, "readonly");
    ...
    await new Promise((res, rej)=>{
      ...
      r.onsuccess = ()=>{
        ...
        if(!cur) return res(true);
        const v = cur.value || {};
        const key = String(v.subKey || "").trim();
        if(key){
          if(!grouped.has(key)){
            grouped.set(key, {
              subKey: key,
              subName: String(v.subName || key).trim(),
              groups: new Set(),
              units: new Set(),
              descs: []
            });
          ...
          if(v.group) g.groups.add(String(v.group).trim());
          if(v.unit) g.units.add(String(v.unit).trim());
          if(v.desc) g.descs.push(String(v.desc).trim());
        }
        cur.continue();
      };
      r.onerror = ()=>rej(r.error || new Error("No se pudo recorrer subproductos"));
    });

    let out = Array.from(grouped.values()).map(g=>{
    ...
    if(q){
      out = out.filter(x => String(x.searchText || "").includes(q));
    }

    out.sort((a,b)=> String(a.subName||"").localeCompare(String(b.subName||""), 'es', {numeric:true, sensitivity:'base'}));
    if(Number.isFinite(limit) && limit > 0) out = out.slice(0, limit);
    return out.map(x=>({ subKey: x.subKey, subName: x.subName }));
  }

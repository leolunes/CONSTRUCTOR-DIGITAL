(function(){
  /* =========================================================
     ...
     - El PDF “Presupuesto + APUs” ahora también muestra los subproductos reales
       referenciados desde Costos_Directos mediante subRef
     ========================================================= */
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    for(const it of items){
      ...
      if(!lines.length){
        ...
        const normales = [];
        const subRefs = [];
        const seenSubRefs = new Set();

        for(const ln of lines){
          const subRef = String(ln?.subRef || "").trim();
          if(subRef){
            if(!seenSubRefs.has(subRef)){
              seenSubRefs.add(subRef);
              subRefs.push(subRef);
            }
          }else{
            normales.push(ln);
        ...
        // =========================
        // 2) SUBPRODUCTOS REALES
        // =========================
        if(subRefs.length){
          for(const subKey of subRefs){
            const subAPU = await (window.APUBase?.getSubAPU ? APUBase.getSubAPU(subKey) : null);
            if(!subAPU || !Array.isArray(subAPU.lines) || !subAPU.lines.length) continue;

...
function getBudgetItemComparisonRows(project){
  ...
  baseItems.forEach(it=>{
    const key = String(it.code || it.descripcion || "").trim();
    map.set(key, {
      code: it.code || "",
      desc: it.descripcion || "",
...
function renderBudgetItemComparison(project){
  ...
  wrap.innerHTML = `
    ...
    <div class="cardhead">
      ...
      <p class="muted small">Diferencias en cantidades y precios unitarios.</p>
    </div>
    ...
    <div class="tablewrap">
      <table class="table" style="min-width:1100px">
        ...
        <tbody>
          ${rows.map(r=>{
            const diffQty = (r.officialQty||0) - (r.baseQty||0);
            const diffPU = (r.officialPU||0) - (r.basePU||0);
            return `
              <tr>
                <td><b>${UI.esc(r.code)}</b></td>
                <td>${UI.esc(r.desc)}</td>
                <td style="text-align:right">${r.baseQty}</td>
                <td style="text-align:right">${UI.fmtMoney(r.basePU, currency)}</td>
                <td style="text-align:right">${r.officialQty}</td>
                <td style="text-align:right">${UI.fmtMoney(r.officialPU, currency)}</td>
                <td style="text-align:right"><b>${diffQty}</b></td>
                <td style="text-align:right"><b>${UI.fmtMoney(diffPU, currency)}</b></td>
              </tr>
            `;
          }).join("")}
        </tbody>

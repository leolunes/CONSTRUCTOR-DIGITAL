...
function exportAuditPdf(project){
  ...
  function drawAuditCover(){
    ...
    doc.setFontSize(12);
    doc.text(country, W/2, y, { align:"center" }); y += 18;
    doc.text(deptLine, W/2, y, { align:"center" }); y += 18;
    doc.text(muniLine, W/2, y, { align:"center" }); y += 30;

    doc.setFont("helvetica","bold");
    doc.setFontSize(14);
    doc.text("INFORME DE AUDITORÍA PRESUPUESTAL", W/2, y, { align:"center" }); y += 22;

    doc.setFont("helvetica","bold");
    doc.setFontSize(11);
    doc.text("COMPARATIVO PRESUPUESTO BASE VS PRESUPUESTO OFICIAL", W/2, y, { align:"center" }); y += 22;

    doc.setFont("helvetica","bold");
    doc.setFontSize(12);
    doc.text(`PROYECTO: ${up(projectLabel)}`, W/2, y, { align:"center" }); y += 40;

    doc.setFont("helvetica","normal");
    doc.setFontSize(11);
    doc.text(`Entidad Contratante: ${entidadContratante}`, M.left, y); y += 16;
    doc.text(`Ubicación: ${ubicacion}`, M.left, y); y += 16;
    doc.text(`Fecha de Elaboración: ${fechaElab}`, M.left, y); y += 16;

    if(elaborador && (elaborador.nombre || elaborador.profesion || elaborador.matricula)){
      y += 6;
      if(elaborador.nombre){ doc.text(`Elaboró: ${elaborador.nombre}`, M.left, y); y += 14; }
      if(elaborador.profesion){ doc.text(`Profesión: ${elaborador.profesion}`, M.left, y); y += 14; }
      if(elaborador.matricula){ doc.text(`Matrícula Profesional: ${elaborador.matricula}`, M.left, y); y += 14; }
    }
  }
  ...
  async function buildApuComponentComparisons(limit=8){
    ...
    officialItems.forEach(it=>{
      const key = String(it?.apuRefCode || it?.code || it?.desc || "").trim();
      if(!key) return;
      officialMap.set(key, it);
    });

    const candidates = riskTopRows.length
      ? riskTopRows.slice(0, limit)
      : itemRows.slice(0, limit);

    for(const row of candidates){
      const key = String(row?.code || row?.desc || "").trim();
      const baseItem = baseItems.find(it => String(it?.code || "").trim() === String(row?.code || "").trim())
        || baseItems.find(it => String(it?.apuRefCode || it?.code || "").trim() === key);
      const officialItem = officialItems.find(it => String(it?.code || "").trim() === String(row?.code || "").trim())
        || officialMap.get(key)
        || officialItems.find(it => String(it?.apuRefCode || it?.code || "").trim() === String(baseItem?.apuRefCode || baseItem?.code || "").trim());
  ...
  function addSimpleTable(headers, rows, widths, opts={}){
    ...
    ensureSpace(headerH + calcTableRowsHeight(rows.slice(0, Math.min(2, rows.length)), widths, fontSize, lineHeight) + 12);
  ...
  function addSignatureBlock(){
    ...
    if(String(elaborador?.firmaDataUrl || "").trim()){
      try{
        doc.addImage(elaborador.firmaDataUrl, "JPEG", sigX + 8, sigY + 8, sigW - 16, sigH - 16);
      }catch(_){
        try{
          doc.addImage(elaborador.firmaDataUrl, "PNG", sigX + 8, sigY + 8, sigW - 16, sigH - 16);
        }catch(__){}
      }
    }
  ...
  const apuComparisons = [];
  try{
    // jsPDF generation is sync, but we can still use Promise via loop in async-like chain not possible here.
  }catch(_){}

  // build synchronous placeholder from available top rows if detailed APU comparison cannot be built inline
  ...
  // Note: this function remains synchronous; if there is no precomputed async source, attempt a conservative comparison using available item totals.
  addParagraph(
    "El comparativo detallado por componente se presentará cuando los APUs del Presupuesto Base y del Presupuesto Oficial tengan referencia comparable en la base activa y/o en sus overrides de proyecto."
  );

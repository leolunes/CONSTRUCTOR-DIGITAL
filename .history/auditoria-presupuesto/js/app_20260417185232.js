...
function getProjectBaseId(project){
  try{
    if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      return String(project?.baseId || StorageAPI.getDefaultBaseId() || "default").trim() || "default";
    }
  }catch(_){}
...
async function resolvePUForProjectItem(projectId, item){
  ...
  try{
    if(typeof tryResolveAPUFromProjectItem === "function"){
      const resolved = await tryResolveAPUFromProjectItem(projectId, lookupCode);
      if(resolved?.data){
  ...
  try{
    if(window.APUBase && typeof APUBase.getAPU === "function"){
      const apu = await APUBase.getAPU(lookupCode);
      if(apu){
...
async function recalcProjectsForActiveBase(){
  ...
  const activeBaseId = getActiveBaseIdForUi();
  ...
  async function getAPUForBudgetItem(projectView, item){
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const apu = await APUBase.getAPU(itemCode, getActiveBaseIdForUi());
...
async function tryResolveAPUFromProjectItem(projectId, requestedCode){
  ...
  const baseHit = await getBaseAPUByAnyLookup(refCode, visibleCode, baseId);
  if(baseHit){
    return {
      source: "base",
...
async function bindProjectsPage(){
  ...
  UI.qs("#fileBase")?.addEventListener("change", async (e)=>{
    ...
    try{
      ...
      setActiveBaseIdForUi(meta?.baseId || meta?.id || getActiveBaseIdForUi());
  ...
  setActiveBaseIdForUi(getActiveBaseIdForUi());
  renderBaseSelector();
  await renderKpis();
  renderProjects();
}
  ...
  if(apuCode){
    ...
    if(!apu){
      ...
      if(custom){
        ...
        apu = await APUBase.getAPU(apuCode, getActiveBaseIdForUi());

...
/* ==========================================
   ✅ Catálogo personalizado de APUs e Insumos
   - Guarda en localStorage
   - Permite volver a buscarlos y reutilizarlos
   ========================================== */
...
function installCustomCatalogFallbacks(){
  ...
  function normalizeApuRow(payload){
    ...
    return {
      code: normCode(payload?.code),
      desc: String(payload?.desc || "").trim(),
      unit: String(payload?.unit || "").trim(),
      pu: Number(payload?.pu || 0) || computeApuLinesDirecto(lines),
  ...
  function normalizeInsumoRow(payload){
    return {
      id: String(payload?.id || `ins_${Date.now()}_${Math.random().toString(16).slice(2,8)}`),
      tipo: String(payload?.tipo || "MATERIALES").trim(),
      desc: String(payload?.desc || "").trim(),
      unit: String(payload?.unit || payload?.unidad || "").trim(),
  ...
  function dedupeBy(items, getKey){
    ...
    safeArr(items).forEach(item=>{
      ...
      const key = String(getKey(normalized) || "").trim().toLowerCase();
      if(!key) return;
  ...
  StorageAPI.listCustomAPUs = function(){
    return dedupeBy(
      safeArr(readJson(KEY_APUS, [])).map(normalizeApuRow).filter(x=>x.code),
      x => x.code
    );
  };

  StorageAPI.getCustomAPU = function(code){
    const key = normCode(code).toLowerCase();
    if(!key) return null;
    return StorageAPI.listCustomAPUs().find(x => String(x?.code || "").trim().toLowerCase() === key) || null;
  };

  StorageAPI.saveCustomAPU = function(payload){
    const row = normalizeApuRow(payload);
    if(!row.code) throw new Error("El APU nuevo debe tener código.");
    const all = StorageAPI.listCustomAPUs().filter(x => String(x?.code || "").trim().toLowerCase() !== row.code.toLowerCase());
    all.push(row);
    writeJson(KEY_APUS, all);
    return StorageAPI.getCustomAPU(row.code);
  };

  StorageAPI.listCustomInsumos = function(){
    return dedupeBy(
      safeArr(readJson(KEY_INSUMOS, [])).map(normalizeInsumoRow).filter(x=>x.id || x.desc),
      x => x.id || `${x.desc}__${x.unit}`
    );
  };

  StorageAPI.saveCustomInsumo = function(payload){
    const row = normalizeInsumoRow(payload);
    if(!row.desc) throw new Error("El insumo nuevo debe tener descripción.");
    if(!row.unit) throw new Error("El insumo nuevo debe tener unidad.");
    const all = StorageAPI.listCustomInsumos().filter(x => String(x?.id || "").trim() !== row.id);
    all.push(row);
    writeJson(KEY_INSUMOS, all);
    return all.find(x => String(x?.id || "").trim() === row.id) || row;
  };
}

function dedupeCustomApuRows(rows){
  const map = new Map();
...
async function getUnifiedApuSearchResults(term, limit=30){
  ...
  const customCatalog = (window.StorageAPI && typeof StorageAPI.listCustomAPUs === "function")
    ? (StorageAPI.listCustomAPUs() || [])
    : [];

  const nq = normalizeTextForMatch(q);
  const customRows = dedupeCustomApuRows(customCatalog)
    .filter(r=>{
      if(!nq) return true;
      return [
        r?.code,
    ...
    .map(r=>{
      ...
      return {
        ...
        desc: String(r?.desc || "").trim(),
        unit: String(r?.unit || "").trim(),
        pu: Number(r?.pu || 0) || computeApuLinesDirecto(lines),
...
function refreshCreateApuChapterOptions(project){
  ...
  sel.innerHTML = chapters.length
    ? chapters.map(c=>{
        ...
        return `<option value="${UI.esc(code)}" data-name="${UI.esc(name)}">${UI.esc(`${code} — ${name}`)}</option>`;
      }).join("")
    : `<option value="">(Sin capítulos definidos)</option>`;
...
/* ===== Crear APU nuevo desde insumos / subproductos ===== */
  ...
  async function searchCreateApuInputs(){
    ...
    let baseInsumos = [];
    let subproductos = [];
    try{ if(window.APUBase && typeof APUBase.listInsumos === "function") baseInsumos = await APUBase.listInsumos(""); }catch(_){}
    try{ if(window.APUBase && typeof APUBase.listSubproductos === "function") subproductos = await APUBase.listSubproductos(); }catch(_){}
    ...
    for(const r of (subproductos || [])){
      ...
      let subApu = null;
      try{ if(window.APUBase && typeof APUBase.getSubAPU === "function") subApu = await APUBase.getSubAPU(r.subKey); }catch(_){}
  ...
  formCreateApu?.addEventListener("submit", (e)=>{
    ...
    try{
      ...
      StorageAPI.saveCustomAPU({
        ...
        desc,
        unit,
        pu: computeApuLinesDirecto(createApuLines),
        chapterCode,
        chapterName,
        lines: createApuLines
      });

...
async function getSubproductoAPU(project, subKey){
  ...
  function isAccesoriosPercentLine(ln){
    /* =========================================================
   🔴 NUEVO (2026-03):
   SUBPRODUCTOS + MANO DE OBRA PARA PDF APU COMPLETO
   ========================================================= */

// Obtener subproductos usados en un APU
function getUsedSubRefsFromAPU(apu){
  const used = [];
  const seen = new Set();
...
// Obtener líneas de mano de obra de un APU
function getManoObraLines(apu){
  return (apu?.lines || []).filter(ln=>{
    const g = String(ln?.group || ln?.tipo || "").toUpperCase();
    return g.includes("MANO") && g.includes("OBRA");
...
async function getSubproductoAPU(project, subKey){
  ...
  // base
  try{
    return await (window.APUBase?.getSubAPU ? APUBase.getSubAPU(key) : null);
  }catch(_){
    return null;
  }
}

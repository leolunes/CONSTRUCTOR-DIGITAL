...
async function drawFormalSubproductBlock(doc, L, subAPU, options){
  ...
  function buildFilename(project, suffix=""){
    ...
    const budgetTag = getBudgetFileTag(project);
    return `Presupuesto_${budgetTag}_${name}${suffix ? "_"+suffix : ""}_${Date.now()}.pdf`;
  }
  function buildFilenameSpecs(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Especificaciones_Tecnicas_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameDesagregado(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Presupuesto_Obra_Desagregado_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenDesagregado(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Resumen_Presupuesto_Obra_Desagregado_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameDistribucionPctDirectos(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Distribucion_Porcentual_Costos_Directos_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameRendimientos(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Rendimiento_Equipo_Mano_Obra_Por_Actividad_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenMaterialesActividad(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Resumen_Materiales_Por_Actividad_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
  function buildFilenameCantRecursosInsumos(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Cantidad_Recursos_Insumos_Presupuesto_${budgetTag}_${name}_${Date.now()}.pdf`;
  }

  function buildFilenameCalculoIndirectos(project){
    const name = sanitizeName(project?.name || "proyecto");
    const budgetTag = getBudgetFileTag(project);
    return `Calculo_Costos_Indirectos_${budgetTag}_${name}_${Date.now()}.pdf`;
  }
...
doc.setFontSize(9.2);
  ...
  function getBudgetModeCode(project){
    const mode = String(project?.activeBudget || "").trim().toLowerCase();
    return mode === "official" ? "official" : "base";
  }

  function getBudgetModeLabel(project){
    return getBudgetModeCode(project) === "official" ? "Presupuesto Oficial" : "Presupuesto Base";
  }

  function getBudgetModeLabelUpper(project){
    return getBudgetModeCode(project) === "official" ? "PRESUPUESTO OFICIAL" : "PRESUPUESTO BASE";
  }

  function getBudgetFileTag(project){
    return getBudgetModeCode(project) === "official" ? "OFICIAL" : "BASE";
  }

  function withBudgetDocType(project, baseTitle){
    const budget = getBudgetModeLabelUpper(project);
    const title = String(baseTitle || "").trim();
    if(!title) return budget;
    if(title.includes("PRESUPUESTO DE OBRA")){
      return title.replace("PRESUPUESTO DE OBRA", budget);
    }
    if(title.includes("RESUMEN PRESUPUESTO")){
      return `${title} (${budget})`;
    }
    return `${title} (${budget})`;
  }

  function getProjectForPdf(project){
    try{
      if(project?.id && window.StorageAPI?.getProjectById){
  ...
  async function exportPresupuestoObraDesagregadoPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "PRESUPUESTO DE OBRA DESAGREGADO");
    const doc = newDoc({ orientation: "landscape" });
    const L = mkLayout(doc);
    const { items } = Calc.groupByChapters(project);

    drawInstitutionalCover(doc, L, project, budgetDocTitle);
    ...
    L.p(`Versión presupuestal: ${budgetLabel}`);
    L.p(" ");
  ...
  async function exportResumenPresupuestoObraDesagregadoPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "RESUMEN PRESUPUESTO DE OBRA DESAGREGADO");
    const doc = newDoc();
    const L = mkLayout(doc);
    const { items } = Calc.groupByChapters(project);
  ...
  async function exportDistribucionPorcentualCostosDirectosPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "DISTRIBUCIÓN PORCENTUAL DE COSTOS DIRECTOS");
    const doc = newDoc();
    const L = mkLayout(doc);
  ...
  async function exportRendimientoEquipoYManoDeObraPorActividadPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "RENDIMIENTO EQUIPO Y MANO DE OBRA POR ACTIVIDAD");
    const doc = newDoc();
    const L = mkLayout(doc);
  ...
  async function exportResumenMaterialesPorActividadPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "RESUMEN MATERIALES POR ACTIVIDAD");
    const doc = newDoc();
    const L = mkLayout(doc);
  ...
  async function exportCantidadRecursosEInsumosPresupuestoPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "CANTIDAD DE RECURSOS E INSUMOS DEL PRESUPUESTO");
    const doc = newDoc();
    const L = mkLayout(doc);
...
const rows = Array.from(map.values()).sort((a,b)=> a.desc.localeCompare(b.desc, "es"));
  ...
  async function exportPresupuestoPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "PRESUPUESTO DE OBRA");
    const doc = newDoc();
    const L = mkLayout(doc);
  ...
  project = getProjectForPdf(project);
  const budgetLabel = getBudgetModeLabel(project);
  const budgetDocTitle = withBudgetDocType(project, "PRESUPUESTO DE OBRA + APUs");
  const doc = newDoc();
  const L = mkLayout(doc);
  ...
  drawBand(doc, L.margin, L.getY(), L.contentW, 18, "RESUMEN ECONÓMICO", { fontSize:10, bold:true });
  L.setY(L.getY()+24);
  L.row("Versión presupuestal:", budgetLabel);
  ...
  drawBand(doc, L.margin, L.getY(), L.contentW, 18, `PRESUPUESTO (${budgetLabel})`, { fontSize:10, bold:true });
  L.setY(L.getY()+24);
  ...
  async function exportEspecificacionesTecnicasPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "ESPECIFICACIONES TÉCNICAS DEL PROYECTO");
    const doc = newDoc();
    const L = mkLayout(doc);
  ...
  async function exportCalculoCostosIndirectosPDF(project, opts){
    project = getProjectForPdf(project);
    const budgetLabel = getBudgetModeLabel(project);
    const budgetDocTitle = withBudgetDocType(project, "CÁLCULO DE COSTOS INDIRECTOS");
    const doc = newDoc();
    const L = mkLayout(doc);
    ...
    let cols = [
      ...
      { key:"desc", label:"DESCRIPCIÓN", w:220, align:"left" },
      { key:"unit", label:"UNID", w:46, align:"center" },
      { key:"qty", label:"CANT", w:56, align:"right" },
      { key:"unitValue", label:"VR UNIT", w:78, align:"right" },
      { key:"percent", label:"% DIR", w:52, align:"right" },
      { key:"partial", label:"VR PARCIAL", w:92, align:"right" }
    ];
    ...
    function dataRow(r){
      ...
      if(r.rowType === "header"){
        ...
        drawCellText(doc, x, yTop, cols[6].w, rowH, "", "right", { fontSize:7.4 }); x += cols[6].w;
        drawCellText(doc, x, yTop, cols[7].w, rowH, "", "right", { fontSize:7.4 });
      }else{
        ...
        drawCellText(doc, x, yTop, cols[6].w, rowH, Number(r.percent||0).toFixed(2), "right", { fontSize:7.4 }); x += cols[6].w;
        drawCellText(doc, x, yTop, cols[7].w, rowH, moneyCOP0(part), "right", { bold:true, fontSize:7.4 });
      }

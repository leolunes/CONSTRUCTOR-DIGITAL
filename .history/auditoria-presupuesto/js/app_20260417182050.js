...
function getActiveUiBaseId(){
  try{
    const fromUi = String(localStorage.getItem("activeBaseId") || "").trim();
    if(fromUi) return fromUi;
  }catch(_){}
  try{
    if(window.APUBase && typeof APUBase.getDefaultBaseId === "function"){
      const fromApu = String(APUBase.getDefaultBaseId() || "").trim();
      if(fromApu) return fromApu;
    }
  }catch(_){}
  try{
    if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      const fromStore = String(StorageAPI.getDefaultBaseId() || "").trim();
...
function setActiveUiBaseId(baseId){
  ...
  try{ localStorage.setItem("apu_base_default_id", next); }catch(_){ }
  return next;
}

function getProjectBaseId(project){
  const selected = getActiveUiBaseId();
...
function renderBaseSelector(){
  ...
  const current = getActiveUiBaseId();
  ...
  async function getAPUForBudgetItem(projectView, item){
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const apu = await APUBase.getAPU(itemCode, getProjectBaseId(projectView));
...
async function getBaseAPUByAnyLookup(codeA, codeB){
  ...
  for(const key of tries){
    try{
      const apu = await APUBase.getAPU(key, getActiveUiBaseId());
      if(apu) return { key, apu };
    }catch(_){}
  }
  return null;
...
async function renderKpis(){
  ...
  try{
    ...
    try{
      ...
      try{ meta = await APUBase.getMeta(getActiveUiBaseId()); }catch(__){}
    }
...
async function bindProjectsPage(){
  UI.qs("#btnNew")?.addEventListener("click", ()=>{
    ...
    const baseId = selectedBaseId || getActiveUiBaseId();
...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  ...
  let meta = null;
  try{ meta = await APUBase.getMeta(getActiveUiBaseId()); }catch(_){}
  if(!meta){
  ...
  if(type === "insumo"){
    let rows = [];
    try{
      rows = await APUBase.listInsumos(term, 30, getActiveUiBaseId());
    }catch(err){
  ...
  let list = [];
  try{
    list = await APUBase.listSubproductos(500, getActiveUiBaseId());
  }catch(err){
...
function bindBaseViewer(projectId){
  ...
  async function loadRows(q){
    ...
    try{
      ...
      if(mode === "sub"){
        const list = await APUBase.listSubproductos(500, getActiveUiBaseId());
      ...
      if(mode === "insumos"){
        const rows = await APUBase.listInsumos(q || "", 300, getActiveUiBaseId());
        if(!rows.length){ showEmpty(true); return; }
...
async function bindProjectDetailPage(){
  ...
  UI.qs("#btnApuSearch")?.addEventListener("click", async ()=>{
    try{
      const meta = await APUBase.getMeta(getActiveUiBaseId());
      if(!meta){
        alert("Primero instala la Base APU (XLSX) desde Proyectos.");
        return;
      }
      ...
      const results = await APUBase.search(q, 30, getActiveUiBaseId());
  ...
  if(apuCode){
    ...
    if(!apu){
      ...
      if(custom){
        ...
        apu = await APUBase.getAPU(apuCode, getProjectBaseId(project));
  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        let meta = null;
        try{
          meta = await APUBase.getMeta(getActiveUiBaseId());
        }catch(_){}
        ...
        if(mode === "2"){
          ...
          let rows = [];
          try{
            rows = await APUBase.listInsumos("", 300, getActiveUiBaseId());
          }catch(err){
            alert("Error consultando insumos: " + (err?.message || err));
            return;
          }

...
function getBudgetIndirectMethodAudit(project){
  const cmp = getBudgetComparison(project);
  const baseModeRaw = safeStr(cmp.baseView?.indirectMode || "manual").toLowerCase() === "table" ? "table" : "manual";
  const officialModeRaw = safeStr(cmp.officialView?.indirectMode || "manual").toLowerCase() === "table" ? "table" : "manual";
...
function renderBudgetComparisonCard(project){
  ...
  body.innerHTML = `
    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
      ${UI.chip(`Base indirectos: ${methodAudit.baseModeLabel}`, methodAudit.baseModeRaw === "table" ? "warn" : "ok")}
      ${UI.chip(`Oficial indirectos: ${methodAudit.officialModeLabel}`, methodAudit.officialModeRaw === "table" ? "warn" : "ok")}

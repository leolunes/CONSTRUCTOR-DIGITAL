...
function withProjectBaseId(project, fallbackProjectId){
  const directProject = project && typeof project === "object" ? project : null;
  const resolvedProject = directProject || (
    fallbackProjectId && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
      ? StorageAPI.getProjectById(fallbackProjectId)
      : null
  );
  return getProjectBaseId(resolvedProject || {});
}

async function apuBaseGetMetaForProject(project){
  if(!(window.APUBase && typeof APUBase.getMeta === "function")) return null;
  return await APUBase.getMeta(withProjectBaseId(project));
}

async function apuBaseSearchForProject(project, query, limit){
  if(!(window.APUBase && typeof APUBase.search === "function")) return [];
  return await APUBase.search(query, limit, withProjectBaseId(project));
}

async function apuBaseGetByCodeForProject(project, code){
  if(!(window.APUBase && typeof APUBase.getByCode === "function")) return null;
  return await APUBase.getByCode(code, withProjectBaseId(project));
}

async function apuBaseGetAPUForProject(project, code){
  if(!(window.APUBase && typeof APUBase.getAPU === "function")) return null;
  return await APUBase.getAPU(code, withProjectBaseId(project));
}

async function apuBaseGetSubAPUForProject(project, subKey){
  if(!(window.APUBase && typeof APUBase.getSubAPU === "function")) return null;
  return await APUBase.getSubAPU(subKey, withProjectBaseId(project));
}

async function apuBaseListInsumosForProject(project, query="", limit){
  if(!(window.APUBase && typeof APUBase.listInsumos === "function")) return [];
  return await APUBase.listInsumos(query, limit, withProjectBaseId(project));
}

async function apuBaseListSubproductosForProject(project, limit){
  if(!(window.APUBase && typeof APUBase.listSubproductos === "function")) return [];
  return await APUBase.listSubproductos(limit, withProjectBaseId(project));
}

async function apuBaseListBaseChaptersForProject(project){
  if(!(window.APUBase && typeof APUBase.listBaseChapters === "function")) return [];
  return await APUBase.listBaseChapters(withProjectBaseId(project));
}
  ...
  async function getAPUForBudgetItem(projectView, item){
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const apu = await apuBaseGetAPUForProject(projectView || project, itemCode);
...
async function getBaseAPUByAnyLookup(project, codeA, codeB){
  const tries = [normalizeLookupCode(codeA), normalizeLookupCode(codeB)].filter(Boolean);
  if(!tries.length || !window.APUBase || typeof APUBase.getAPU !== "function") return null;

  for(const key of tries){
    try{
      const apu = await apuBaseGetAPUForProject(project, key);
      if(apu) return { key, apu };
    }catch(_){}
  }
  return null;
...
async function tryResolveAPUFromProjectItem(projectId, requestedCode){
  ...
  const baseHit = await getBaseAPUByAnyLookup(project, refCode, visibleCode);
  if(baseHit){
    return {
      source: "base",
...
async function renderKpis(){
  ...
  try{
    const meta = await apuBaseGetMetaForProject(project);
  ...
  tbody.innerHTML = ps.map(p=>{
    ...
    return `
      <tr>
        ...
        <td>${UI.esc(getProjectBaseId(p))}</td>
...
function getInstalledBasesForUi(){
  try{
    if(window.StorageAPI && typeof StorageAPI.listInstalledBases === "function"){
      const list = StorageAPI.listInstalledBases() || [];
      if(Array.isArray(list) && list.length) return list;
    }
  }catch(_){}
  try{
    if(window.APUBase && typeof APUBase.listInstalledBases === "function"){
      const list = APUBase.listInstalledBases() || [];
      if(Array.isArray(list) && list.length) return list;
    }
  }catch(_){}
...
function refreshProjectBaseSelector(){
  ...
  const current = (window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
    ? StorageAPI.getDefaultBaseId()
    : ((window.APUBase && typeof APUBase.getDefaultBaseId === "function") ? APUBase.getDefaultBaseId() : "default");

  sel.innerHTML = bases.map(b=>{
    const id = String(b?.id || "default").trim() || "default";
    const label = String(b?.label || b?.name || b?.fileName || id).trim() || id;
    return `<option value="${UI.esc(id)}" ${id === current ? "selected" : ""}>${UI.esc(label)}</option>`;
  }).join("");
}
...
async function bindProjectsPage(){
  ...
  refreshProjectBaseSelector();

  UI.qs("#selectBase")?.addEventListener("change", async (e)=>{
    const nextBaseId = String(e.target?.value || "").trim() || "default";
    try{
      if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
        StorageAPI.setDefaultBaseId(nextBaseId);
      }
      if(window.APUBase && typeof APUBase.setDefaultBaseId === "function"){
        APUBase.setDefaultBaseId(nextBaseId);
      }
      await renderKpis();
      renderProjects();
    }catch(err){
      alert("No se pudo cambiar la base por defecto: " + (err?.message || err));
    }
  });
...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  ...
  let meta = null;
  try{ meta = await apuBaseGetMetaForProject(project); }catch(_){}
  if(!meta){
  ...
  let list = [];
  try{
    list = await apuBaseListSubproductosForProject(project);
  }catch(err){
...
function bindBaseViewer(projectId){
  ...
  async function loadRows(q){
    ...
    try{
      ...
      if(mode === "sub"){
        const list = await apuBaseListSubproductosForProject(project);
  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        let meta = null;
        try{
          meta = await apuBaseGetMetaForProject(project);
        }catch(_){}
        ...
        if(mode === "2"){
          ...
          let rows = [];
          try{
            rows = await apuBaseListInsumosForProject(project, "");
          }catch(err){
            alert("Error consultando insumos: " + (err?.message || err));
            return;
          }

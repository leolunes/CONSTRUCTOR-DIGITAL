(function(){
  ...
  function buildSuggestedBaseId(input, fallback="base"){
    const raw = String(input || "").trim();
    if(!raw) return safeBaseId(fallback || "base");
    const clean = raw
      .replace(/\.[^.]+$/, "")
      .normalize("NFD").replace(/[\u0300-\u036f]/g,"")
      .replace(/[^A-Za-z0-9_-]+/g, "_")
      .replace(/^_+|_+$/g, "");
    return clean || safeBaseId(fallback || "base");
  }
  ...
  async function installFromFile(file, opts={}){
    ...
    const rawRequestedBaseId = String(opts?.baseId || opts?.id || "").trim();
    const requestedBaseId = rawRequestedBaseId ? safeBaseId(rawRequestedBaseId) : "";
    const derivedBaseId = buildSuggestedBaseId(String(file.name || "base"), "base");
    const baseId = requestedBaseId || derivedBaseId;
    ...
    if(!catalog.length || !catalog.some(x => safeBaseId(x.id) === currentDefault)){
  ...
  async function clearAll(baseId="default"){
    const cacheKey = safeBaseId(baseId);
    await clearStore(STORE_META, cacheKey);
    await clearStore(STORE_ITEMS, cacheKey);
    await clearStore(STORE_CD, cacheKey);
    await clearStore(STORE_SUB, cacheKey);
    await clearStore(STORE_INSUM, cacheKey);

...
function refreshCreateApuChapterOptions(project){
  ...
  const pick = ()=>{
    const code = String(sel.value || "").trim();
    const name = code
      ? ((sel.options && sel.options[sel.selectedIndex]?.getAttribute("data-name")) || lookupChapterName(project, code) || "")
      : "";
    if(inpName) inpName.value = name;
  };
...
async function bindProjectDetailPage(){
  ...
  refreshProjectBudgetAwareUI(project.id);
  refreshCreateApuChapterOptions(StorageAPI.getProjectById(project.id) || project);

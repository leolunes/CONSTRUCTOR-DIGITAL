(function(){
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    // ✅ ACUMULADOR GLOBAL SOLO DE SUBPRODUCTOS REALMENTE USADOS EN APUs
    ...
    for(const it of items){
      ...
      // ✅ SUBPRODUCTOS USADOS SOLO EN ESTE APU, TOMADOS EXCLUSIVAMENTE DE line.subRef
      const subRefsDelApu = collectUsedSubRefsFromLines(lines);

      // ✅ Guardar en acumulador global solo los realmente usados
      ...
      // ✅ Imprimir solo líneas normales; NO imprimir líneas de subproducto aquí
      const normales = [];
      for(const ln of lines){
        const subRef = String(ln?.subRef || "").trim();
        if(subRef) continue;
      ...
      if(!normales.length){
        L.p("Sin líneas normales en la descomposición.");
        continue;
      }

      L.tableHeaderAPU();

      for(const ln of normales){
        L.tableRowAPU({
    ...
    // =========================================================
    // SUBPRODUCTOS DEL PRESUPUESTO
    // ✅ SOLO LOS QUE REALMENTE FUERON USADOS EN LOS APUs
    // =========================================================
    if(subproductosUsados.size){
      ...
      for(const info of subproductosUsados.values()){
        ...
        const subAPU = await resolveSubproductoForPDF(project, subKey);
        ...
        const fuente = safe(
          subAPU.metaLine ||
          (subAPU.mode === "sub_override" ? "Fuente: Override del proyecto" : "Fuente: Subproductos")
        );
        L.p(fuente);

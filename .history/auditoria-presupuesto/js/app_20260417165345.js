...
function getSelectedBaseIdForProjectsPage(){
  const sel = UI.qs("#selectBase");
  const selValue = String(sel?.value || "").trim();
  ...
  const list = getInstalledBasesForUi();
  return String(list?.[0]?.id || "default").trim() || "default";
}

function getProjectsForSelectedBase(){
  const activeBaseId = getSelectedBaseIdForProjectsPage();
  const all = (window.StorageAPI && typeof StorageAPI.listProjects === "function")
    ? (StorageAPI.listProjects() || [])
    : [];

  return all.filter(p => String(getProjectBaseId(p) || "default").trim() === activeBaseId);
}

async function refreshProjectsPageBySelectedBase(){
  renderBaseSelector();
  await renderKpis();
  renderProjects();
}
...
function renderBaseSelector(){
  ...
  let current = (window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
    ? String(StorageAPI.getDefaultBaseId() || "default").trim() || "default"
    : "default";

  if(!bases.some(b => String(b.id || "").trim() === current)){
    current = String(bases?.[0]?.id || "default").trim() || "default";
    if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
      try{ StorageAPI.setDefaultBaseId(current); }catch(_){ }
    }
  }

  sel.innerHTML = bases.map(b => `
    <option value="${UI.esc(b.id)}" ${String(b.id).trim() === current ? "selected" : ""}>${UI.esc(b.name)}</option>
  `).join("");
...
async function renderKpis(){
  ...
  el.innerHTML = `
    <div class="card item"><div class="topline"><div class="name">Proyectos</div><div class="chips">${UI.chip(String(ps.length),"ok")}</div></div><div class="muted small">De la base seleccionada.</div></div>
...
function renderProjects(){
  ...
  let ps = getProjectsForSelectedBase();
  ...
  tbody.querySelectorAll("[data-del]").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      await refreshProjectsPageBySelectedBase();
    });
  });
}
...
async function bindProjectsPage(){
  UI.qs("#btnNew")?.addEventListener("click", ()=>{
    ...
    if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
      try{ StorageAPI.setDefaultBaseId(baseId); }catch(_){ }
    }

...
async function getSubproductoAPU(project, subKey){
  ...
  async function findLaborRateByRole(role){
    try{
      const q = String(role || "").trim();
      if(!q) return 0;

      if(window.APUBase?.getInsumoByDesc){
        const exact = await APUBase.getInsumoByDesc(q);
        const rate = Number(exact?.pu || exact?.precio || exact?.precioComercial || 0);
        if(rate > 0) return rate;
      }

      if(window.APUBase?.searchInsumos){
        const found = await APUBase.searchInsumos(q, 20);
        const arr = Array.isArray(found) ? found : [];
        const roleNorm = q.toLowerCase();

        const exact = arr.find(x => String(x?.desc || "").toLowerCase() === roleNorm);
        if(exact){
          const rate = Number(exact?.pu || exact?.precio || exact?.precioComercial || 0);
          if(rate > 0) return rate;
        }

        const contains = arr.find(x => String(x?.desc || "").toLowerCase().includes(roleNorm));
        if(contains){
          const rate = Number(contains?.pu || contains?.precio || contains?.precioComercial || 0);
          if(rate > 0) return rate;
        }
      }
    }catch(_){ }
  ...
  function parseLaborCrewFromDesc(desc){
    ...
    else if(/OFICIAL/.test(s)) add("OFICIAL", 1);

    m = s.match(/(\d+)\s*AY/);
    if(m) add("AYUDANTE", m[1]);
    else if(/AYUDANTE/.test(s)) add("AYUDANTE", 1);

    m = s.match(/(\d+)\s*OP/);
    if(m) add("OPERADOR", m[1]);
    else if(/OPERADOR/.test(s)) add("OPERADOR", 1);
    ...
    try{
      if(window.APUBase?.listSubproductos){
        ...
        if(hit && window.APUBase?.getSubAPU){
          const sub = await APUBase.getSubAPU(String(hit.subKey || hit.subName || "").trim());
  ...
  async function buildLaborBreakdownForLine(ln){
    ...
    const totalQty = crew.reduce((s, x) => s + Number(x.qty || 0), 0) || 0;
    if(!totalQty) return [];
    ...
    for(const part of crew){
      const sub = await resolveLaborRoleSubproduct(part.role);
      let rate = Number(sub?.directo || 0);
      if(!(rate > 0)) rate = await findLaborRateByRole(part.role);
      if(sub && rate > 0) rowsWithSub++;

      if(rate > 0){
        knownTotal += part.qty * rate;
      }else{
        unknownQty += part.qty;
      }
      ...
      rows.push({
        ...
        qty: part.qty,
        pu: rate,
        parcial: part.qty * rate,
        subKey: String(sub?.subtitle || sub?.header || part.role || "").trim(),
        subObj: sub || null
      });
    }

    // Si encontramos subproductos reales, respetar esos valores para evitar inconsistencias.
    if(rowsWithSub === 0 && lineUnitValue > 0){
      if(knownTotal > 0 && Math.abs(knownTotal - lineUnitValue) / lineUnitValue > 0.03){
        const factor = lineUnitValue / knownTotal;
        for(const r of rows){
          r.pu = Number(r.pu || 0) * factor;
          r.parcial = Number(r.qty || 0) * Number(r.pu || 0);
        }
      }else if(knownTotal === 0){
        const unitPerPerson = lineUnitValue / totalQty;
        for(const r of rows){
          r.pu = unitPerPerson;
          r.parcial = Number(r.qty || 0) * unitPerPerson;
        }
      }else if(unknownQty > 0){
        const remainder = Math.max(0, lineUnitValue - knownTotal);
        const unitPerUnknown = unknownQty ? (remainder / unknownQty) : 0;
        for(const r of rows){
          if(!(Number(r.pu || 0) > 0)){
            r.pu = unitPerUnknown;
            r.parcial = Number(r.qty || 0) * unitPerUnknown;
          }
        }
      }
    }
  ...
  async function drawFormalSubproductBlock(doc, L, subAPU, options){
    ...
    const forceNewPage = !!o.forceNewPage;

    if(forceNewPage){
      doc.addPage();
      L.setY(PDF_THEME.safe.top);
    }else{
      const approxRows = Array.isArray(subAPU?.lines) ? subAPU.lines.length : 0;
      const need = 90 + (approxRows * 20);
      const jumped = L.ensure(need);
      if(jumped){
        L.setY(PDF_THEME.safe.top);
      }
    }
  ...
  async function drawLaborBreakdownUnderRow(doc, L, ln, sharedRolesMap){
    const rows = await buildLaborBreakdownForLine(ln);
    if(!rows.length) return;

    L.ensure(20 + rows.length * 14);
    doc.setFont("helvetica","bold");
    doc.setFontSize(8.5);
    doc.text("Desglose cuadrilla:", L.margin + 18, L.getY());
    L.setY(L.getY() + 10);

    for(const r of rows){
      doc.setFont("helvetica","normal");
      doc.setFontSize(8.3);
      const txt = `${safe(r.desc)} · Cant: ${fmt2(r.qty)} · Vr Unit: ${moneyCOP0(r.pu)} · Parcial: ${moneyCOP0(r.parcial)}`;
      doc.text(txt, L.margin + 28, L.getY());
      L.setY(L.getY() + 12);

      const roleKey = laborRoleNorm(r.desc || r.subKey || "");
      if(sharedRolesMap && roleKey && r.subObj && !sharedRolesMap.has(roleKey)){
        sharedRolesMap.set(roleKey, {
          role: safe(r.desc || r.subKey || roleKey),
          subAPU: r.subObj,
          origin: ln?.__moCtx ? `Primera aparición en ítem ${safe(ln.__moCtx.itemCode || "")} — ${safe(ln.__moCtx.itemDesc || "")}` : ""
        });
      }
    }
 ...
 if(!map.has(key)){
  ...
  for(const it of items){
    ...
    // ✅ Subproductos usados por este APU
    ...
    // ✅ AHORA EL APU SALE COMPLETO, incluyendo subproductos
    L.tableHeaderAPU();
  ...
  // =========================================================
  // SUBPRODUCTOS DEL PRESUPUESTO (SECCIÓN INDEPENDIENTE)
  // =========================================================
  if(subproductosUsados.size){
    let firstSubPage = true;
  ...
  // =========================================================
  // MANO DE OBRA DEL PRESUPUESTO (SECCIÓN INDEPENDIENTE)
  // =========================================================
  const uniqueLaborRoleSubs = new Map();

  if(manoObraPorItem.length){
    let firstMoPage = true;
    ...
    for(const mo of manoObraPorItem){
      ...
      for(const ln of lines){
        ln.__moCtx = mo;
        const breakdownRows = await buildLaborBreakdownForLine(ln);
        const displayPu = breakdownRows.length
          ? breakdownRows.reduce((s, r) => s + Number(r.parcial || 0), 0)
          : Number(ln.pu || 0);
        ...
        await drawLaborBreakdownUnderRow(doc, L, { ...ln, pu: displayPu }, uniqueLaborRoleSubs);
      }
  ...
  // =========================================================
  // SUBPRODUCTOS BASE DE MANO DE OBRA (ÚNICOS)
  // =========================================================
  if(uniqueLaborRoleSubs.size){
    let firstRolePage = true;
    for(const info of uniqueLaborRoleSubs.values()){
      if(firstRolePage){
        doc.addPage();
        L.setY(PDF_THEME.safe.top);

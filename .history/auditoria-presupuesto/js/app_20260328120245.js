...
/* =========================
   ✅ NUEVO: Indirectos por tabla (UI flexible)
   - Soporta modo manual / tabla
   - Filas libres + subtítulos
...
function normalizeIndirectUiRow(row){
  ...
  const calcType = String(r.calcType || "").trim().toLowerCase() === "percent_direct" ? "percent_direct" : "fixed";
  return {
...
function makeIndirectRow(rowType="detail", group="ADMIN"){
...
function ensureIndirectTableMount(form){
  ...
  host.innerHTML = `
    <div class="cardhead">
      ...
      <p class="muted small">Modo flexible con líneas libres, subtítulos y cálculo automático sobre costos directos.</p>
    </div>
    ...
    <div class="grid two" style="margin-bottom:12px">
      ...
      <div id="indirectModeHint" class="card item">
        ...
        <div class="muted small">En modo manual se usan Administración, Imprevistos, Utilidad e IVA. En modo tabla se usa el cuadro editable inferior.</div>
      </div>
    </div>
    ...
    <div id="indirectTableWrap" style="display:none">
      ...
      <div id="indirectTableEmpty" class="muted small" style="display:none; margin-top:10px">
        Aún no hay líneas. Usa “Agregar línea” o “Agregar subtítulo”.
      </div>
...
function renderIndirectRows(projectId){
  ...
  tbody.innerHTML = rows.map(row=>{
    ...
    if(r.rowType === "header"){
      return `
        <tr data-indirect-id="${UI.esc(r.id)}" data-row-type="header" class="indirect-header-row">
          <td><div class="chip"><b>${UI.esc(r.item || "—")}</b></div></td>
          <td><div class="chip">${UI.esc(r.group || "OTRO")}</div></td>
          <td colspan="5">
            <div style="font-weight:700; letter-spacing:.2px; padding:10px 12px; border:1px solid var(--border); border-radius:14px; background:rgba(255,255,255,.04)">
              ${UI.esc(r.desc || "SUBTÍTULO")}
            </div>
            <div class="muted small" style="margin-top:6px">Subtítulo (no calcula)</div>
          </td>
    ...
    return `
      <tr data-indirect-id="${UI.esc(r.id)}" data-row-type="detail">
        <td><input class="input" data-field="item" value="${UI.esc(r.item)}" placeholder="1.1.1"></td>
        <td>
        ...
        <td><input class="input" data-field="desc" value="${UI.esc(r.desc)}" placeholder="Descripción"></td>
        <td><input class="input" data-field="unit" value="${UI.esc(r.unit)}" placeholder="MES"></td>
        <td><input class="input" data-field="qty" type="number" step="any" value="${UI.esc(String(r.qty||0))}"></td>
        <td><input class="input" data-field="unitValue" type="number" step="any" value="${UI.esc(String(r.unitValue||0))}"></td>
        <td>
          <div class="row" style="gap:6px; flex-wrap:wrap">
            <select class="input" data-field="calcType" style="min-width:120px">
        ...
        <td style="text-align:right"><b>${UI.fmtMoney(partial, project.currency||"COP")}</b></td>
        <td>
          <button type="button" class="btn danger" data-indirect-del="${UI.esc(r.id)}">Quitar</button>
        </td>
      </tr>
...
function renderSingleIndirectRowPreview(tr, projectId){
  ...
  const row = normalizeIndirectUiRow({
    ...
    unitValue: tr.querySelector('[data-field="unitValue"]')?.value || 0,
  ...
  const lastTd = tr.children?.[7];
  if(lastTd){
    lastTd.innerHTML = `<b>${UI.fmtMoney(partial, project.currency||"COP")}</b>`;
  }
}
...
function editIndirectHeaderRow(projectId, rowId){
  ...
  const item = prompt("Ítem / numeración del subtítulo:", cur.item || "") ?? cur.item;
  const group = prompt("Grupo del subtítulo (ADMIN / IMPREV / UTIL / OTRO):", cur.group || "OTRO") ?? cur.group;
  const desc = prompt("Texto del subtítulo:", cur.desc || "") ?? cur.desc;

  rows[idx] = normalizeIndirectUiRow({
    ...cur,
    rowType: "header",
    item: String(item || "").trim(),
    group: String(group || "OTRO").trim().toUpperCase(),
...
function bindIndirectTableUI(projectId){
  ...
  btnLine.onclick = ()=>{
    const fresh = StorageAPI.getProjectById(projectId);
    const rows = getIndirectRows(fresh);
    rows.push(makeIndirectRow("detail", "ADMIN"));
    saveIndirectRows(projectId, rows);
    renderIndirectRows(projectId);
  };
  ...
  btnHeader.onclick = ()=>{
    ...
    const item = String(prompt("Ítem / numeración del subtítulo:", "1.1") || "").trim();
    const groupRaw = String(prompt("Grupo del subtítulo (ADMIN / IMPREV / UTIL / OTRO):", "ADMIN") || "ADMIN").trim().toUpperCase();
    const desc = String(prompt("Texto del subtítulo:", "SUBTÍTULO") || "").trim();

    if(!desc){
      alert("Debes escribir el texto del subtítulo.");
      return;
    }

    const allowed = ["ADMIN", "IMPREV", "UTIL", "OTRO"];
    const group = allowed.includes(groupRaw) ? groupRaw : "OTRO";

    const rows = getIndirectRows(fresh);
    const nextHeader = makeIndirectRow("header", group);
    nextHeader.item = item;
    nextHeader.group = group;
    nextHeader.desc = desc;
    rows.push(nextHeader);
    saveIndirectRows(projectId, rows);
    renderIndirectRows(projectId);
  };

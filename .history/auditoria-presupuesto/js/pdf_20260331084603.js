...
async function drawFormalSubproductBlock(doc, L, subAPU, options){
  ...
  function getBudgetPdfLabel(project){
    const mode = String(project?.activeBudget || "base").trim().toLowerCase();
    return (mode === "oficial" || mode === "official") ? "PRESUPUESTO OFICIAL" : "PRESUPUESTO BASE";
  }

  function getBudgetFileSuffix(project){
    const mode = String(project?.activeBudget || "base").trim().toLowerCase();
    return (mode === "oficial" || mode === "official") ? "OFICIAL" : "BASE";
  }

  function buildFilename(project, suffix=""){
    const name = sanitizeName(project?.name || "proyecto");
    return `Presupuesto_${name}_${getBudgetFileSuffix(project)}${suffix ? "_"+suffix : ""}_${Date.now()}.pdf`;
  }
  function buildFilenameSpecs(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Especificaciones_Tecnicas_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameDesagregado(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Presupuesto_Obra_Desagregado_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenDesagregado(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Resumen_Presupuesto_Obra_Desagregado_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameDistribucionPctDirectos(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Distribucion_Porcentual_Costos_Directos_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameRendimientos(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Rendimiento_Equipo_Mano_Obra_Por_Actividad_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameResumenMaterialesActividad(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Resumen_Materiales_Por_Actividad_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  function buildFilenameCantRecursosInsumos(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Cantidad_Recursos_Insumos_Presupuesto_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }

  function buildFilenameCalculoIndirectos(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Calculo_Costos_Indirectos_${name}_${getBudgetFileSuffix(project)}_${Date.now()}.pdf`;
  }
  ...
  function stampHeaderFooter(doc, { docType, projectName, logoDataUrl, budgetLabel }){
    ...
    for(let p=1; p<=pageCount; p++){
      ...
      doc.setFont("helvetica","normal");
      doc.setFontSize(8.8);
      if(budgetLabel) doc.text(safe(budgetLabel), textStartX, 38);

      doc.setFont("helvetica","normal");
      doc.setFontSize(9.2);
      if(projectName) doc.text(safe(projectName), textStartX, budgetLabel ? 48 : 38);
  ...
  function drawInstitutionalCover(doc, L, project, docTitle){
    ...
    doc.text(up(docTitle||""), L.PAGE_W/2, y, { align:"center" }); y += 18;

    doc.setFont("helvetica","bold"); doc.setFontSize(11);
    doc.text(getBudgetPdfLabel(project), L.PAGE_W/2, y, { align:"center" }); y += 20;
  ...
  async function exportPresupuestoObraDesagregadoPDF(project, opts){
    ...
    stampHeaderFooter(doc, {
      ...
      budgetLabel: getBudgetPdfLabel(project)
    });

...
async function bindProjectDetailPage(){
  ...
  (UI.qs("#btnPdfCalculoIndirectos") || UI.qs("#btnPdfCalcIndirectos"))?.addEventListener("click", async ()=>{
    try{
      ...
      await (function(){
const __proj = fresh, { share: isIOS(;
const fresh = StorageAPI.getProjectById(__proj.id) || __proj;
const activeMode = (StorageAPI.getProjectActiveBudgetMode)
  ? StorageAPI.getProjectActiveBudgetMode(__proj.id)
  : (fresh.activeBudget || "base");
const pdfProject = (typeof getProjectViewForBudget === "function")
  ? getProjectViewForBudget(fresh, activeMode)
  : fresh;
return PDF.exportCalculoCostosIndirectosPDF(pdfProject);
})() });
    }catch(err){

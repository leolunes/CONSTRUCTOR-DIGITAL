...
/* ==========================================
   Compat para presupuesto base / oficial
   ========================================== */
...
/* ==========================================
   Capítulos manuales por proyecto
   ========================================== */
...
/* ==========================================
   Helper: actualizar PU por APU real
   ========================================== */
...
function exportProjectExcel(project, mode="base"){
  ...
  const grouped = Calc.groupByChapters(view);
...
function confirmAddApuToProject(params){
  const keepOpen = !!params?.keepOpen;
...
async function bindAPUPage(){
  ...
  if(projectId && apuCode){
    ...
    const sourceItems = budgetMode === "oficial"
      ? (proj?.oficialItems || [])
      : (proj?.items || []);

    const hit = sourceItems.find(x => String(x.code || "").trim() === apuCode);
  ...
  if(apuCode){
    if(projectId){
      if(budgetMode === "oficial" && StorageAPI.getOfficialApuOverride){
        const ov = StorageAPI.getOfficialApuOverride(projectId, apuCode);
        if(ov && Array.isArray(ov.lines) && ov.lines.length){
          const title = (displayCode && displayCode !== apuCode)
            ? `APU ${displayCode} (Ref: ${apuCode})`
            : `APU ${apuCode}`;

          apu = {
            title,
            subtitle: "(Override del presupuesto oficial)",
            header: `${apuCode} — Override del presupuesto oficial`,
            metaLine: `Fuente: override del presupuesto oficial · Actualizado: ${(ov.updatedAt || "").slice(0,19).replace("T"," ")}`,
            unit: "",
      ...
      if(!apu && budgetMode !== "oficial" && StorageAPI.getApuOverride){
    ...
    if(!apu){
      const custom = StorageAPI.getCustomAPU ? StorageAPI.getCustomAPU(apuCode) : null;
      if(custom){

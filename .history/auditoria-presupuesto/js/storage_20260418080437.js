...
function normalizeBaseId(v){
  const id = safeStr(v);
  return id || "default";
}

function getStoreDefaultBaseId(dbLike){
  try{
    const db = dbLike || loadStore();
    const fromMeta = normalizeBaseId(db?.meta?.defaultBaseId || "");
...
function normalizeComparisonBaseId(v, fallbackBaseId){
  const id = safeStr(v);
  if(id) return id;
  return normalizeBaseId(fallbackBaseId || "default");
}
...
/* =========================
   ✅ FASE 1 — DOBLE PRESUPUESTO (estructura segura)
   - NO cambia la lógica actual de la app
   - Mantiene el presupuesto actual como raíz del proyecto
   - Prepara budgets.base / budgets.oficial / activeBudget
   - En esta fase budgets.oficial se deja listo, pero vacío
   ========================= */
...
function cloneBudgetMapObject(obj){
  const out = {};
...
function buildBudgetBranchFromProjectRoot(projectLike){
  const p = projectLike || {};
  return {
    items: cloneBudgetLines(p.items),
    chapters: cloneBudgetLines(p.chapters),
    apuOverrides: cloneBudgetMapObject(p.apuOverrides),
    subApuOverrides: cloneBudgetMapObject(p.subApuOverrides),
    indirectMode: String(p.indirectMode || "manual"),
    indirectTable: cloneBudgetLines(p.indirectTable)
  };
}

function ensureProjectBudgetStructure(proj){
  const p = proj || {};
  ...
  p.activeBudget = String(p.activeBudget).trim().toLowerCase() === "oficial" ? "oficial" : "base";

  if(!p.budgets.base || typeof p.budgets.base !== "object" || Array.isArray(p.budgets.base)){
    p.budgets.base = buildBudgetBranchFromProjectRoot(p);
  }else{
    p.budgets.base.items = cloneBudgetLines(p.budgets.base.items);
    p.budgets.base.chapters = cloneBudgetLines(p.budgets.base.chapters);
    p.budgets.base.apuOverrides = cloneBudgetMapObject(p.budgets.base.apuOverrides);
    p.budgets.base.subApuOverrides = cloneBudgetMapObject(p.budgets.base.subApuOverrides);
    p.budgets.base.indirectMode = String(p.budgets.base.indirectMode || "manual");
    p.budgets.base.indirectTable = cloneBudgetLines(p.budgets.base.indirectTable);
  }

  if(p.budgets.oficial == null){
    p.budgets.oficial = null;
  }else if(typeof p.budgets.oficial === "object" && !Array.isArray(p.budgets.oficial)){
    p.budgets.oficial = {
      items: cloneBudgetLines(p.budgets.oficial.items),
      chapters: cloneBudgetLines(p.budgets.oficial.chapters),
      apuOverrides: cloneBudgetMapObject(p.budgets.oficial.apuOverrides),
      subApuOverrides: cloneBudgetMapObject(p.budgets.oficial.subApuOverrides),
      indirectMode: String(p.budgets.oficial.indirectMode || "manual"),
      indirectTable: cloneBudgetLines(p.budgets.oficial.indirectTable)
    };
  }else{
    p.budgets.oficial = null;
  }
...
/* =========================
   Proyecto
   ========================= */
function normalizeProject(p, fallbackDefaultBaseId){
  const proj = p || {};

  // ✅ Defaults robustos
  proj.currency = String(proj.currency || "COP");
  proj.baseId = normalizeBaseId(proj.baseId || fallbackDefaultBaseId || "default");
  proj.comparisonBaseId = normalizeComparisonBaseId(proj.comparisonBaseId || "", proj.baseId);
  ...
  if(!proj.subApuOverrides) proj.subApuOverrides = {};
  ...
  // ✅ FASE 1 — preparar doble presupuesto sin tocar la raíz actual
  ensureProjectBudgetStructure(proj);
  ...
  return {
    meta:{
      ...
      defaultBaseId: normalizeBaseId(
        safeLocalGet("activeComparisonBaseId") ||
        safeLocalGet("activeBaseId") ||
        safeLocalGet("apu_base_default_id") ||
        "default"
      ),
...
function loadStore(){
  ...
  try{
    ...
    if(!db.meta) db.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO(), defaultBaseId: "default" };
    if(!Number.isFinite(Number(db.meta.version))) db.meta.version = 10;
    if(!db.meta.lastSavedAt) db.meta.lastSavedAt = nowISO();
    db.meta.defaultBaseId = normalizeBaseId(
      safeLocalGet("activeComparisonBaseId") ||
      safeLocalGet("activeBaseId") ||
      safeLocalGet("apu_base_default_id") ||
      db.meta.defaultBaseId ||
      "default"
    );
    ...
    // ✅ Normalizar proyectos (incluye chapters + nuevos % + logo + inst* + apuRefCode + subApuOverrides)
    db.projects = db.projects.map(p => normalizeProject(p, db.meta.defaultBaseId));

    if(db.meta.version < 10) db.meta.version = 10;
    ...
    if(rawMain && rawBackup && rawMain !== rawBackup){
      try{
        ...
        if(!recovered.meta) recovered.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO(), defaultBaseId: "default" };
        if(!recovered.projects) recovered.projects = [];
        recovered.meta.defaultBaseId = normalizeBaseId(recovered.meta.defaultBaseId || "default");
        recovered.projects = recovered.projects.map(p => normalizeProject(p, recovered.meta.defaultBaseId));
        recovered.meta.version = 10;
...
function saveStore(db){
  ...
  out.meta.version = 10;
  out.meta.defaultBaseId = normalizeBaseId(out.meta.defaultBaseId || "default");
...
async function importBackupFromFile(file){
  ...
  if(!data.meta) data.meta = { createdAt:nowISO(), version:10, lastSavedAt: nowISO(), defaultBaseId: "default" };
  ...
  // ✅ base por defecto
  data.meta.defaultBaseId = normalizeBaseId(data.meta.defaultBaseId || "default");

  // ✅ Normalizar proyectos importados (incluye chapters + apuRefCode + subApuOverrides + baseId)
  data.projects = data.projects.map(p => normalizeProject(p, data.meta.defaultBaseId));
...
function createProject(payload){
  ...
  const defaultBaseId = getStoreDefaultBaseId(db);
  ...
  const p = normalizeProject({
    ...
    updatedAt: nowISO(),
    currency: "COP",
    baseId: normalizeBaseId(payload?.baseId || defaultBaseId),
    comparisonBaseId: normalizeComparisonBaseId(payload?.comparisonBaseId || "", payload?.baseId || defaultBaseId),
    ...
    apuOverrides: {},
    subApuOverrides: {},
    budgets: { base: null, oficial: null },
    activeBudget: "base",
    ...payload
  });
...
function updateProject(id, patch){
  ...
  db.projects[idx] = normalizeProject(merged, getStoreDefaultBaseId(db));
...
/* =========================
   ✅ NUEVO: Override SUBPRODUCTO por proyecto
   ========================= */
function getSubApuOverride(projectId, code){
  const p = getProjectById(projectId);
  if(!p) return null;
  const c = String(code||"").trim();
  return (p.subApuOverrides && p.subApuOverrides[c]) ? p.subApuOverrides[c] : null;
}
...
function setSubApuOverride(projectId, code, lines){
  ...
  const c = String(code||"").trim();
  if(!db.projects[pidx].subApuOverrides) db.projects[pidx].subApuOverrides = {};

  db.projects[pidx].subApuOverrides[c] = {
    code: c,
    updatedAt: nowISO(),
    lines: Array.isArray(lines) ? lines : []
  };
  ...
  db.projects[pidx].updatedAt = nowISO();
  saveStore(db);
  return db.projects[pidx].subApuOverrides[c];
}
...
function clearSubApuOverride(projectId, code){
  ...
  const c = String(code||"").trim();
  if(db.projects[pidx].subApuOverrides && db.projects[pidx].subApuOverrides[c]){
    delete db.projects[pidx].subApuOverrides[c];
    db.projects[pidx].updatedAt = nowISO();
    saveStore(db);
    return true;
  }
...
function importProjectAsNew(projectObj){
  ...
  const base = createProject({
    ...
    baseId: normalizeBaseId(payload.baseId || getStoreDefaultBaseId()),
    comparisonBaseId: normalizeComparisonBaseId(payload.comparisonBaseId || "", payload.baseId || getStoreDefaultBaseId()),
  ...
  updateProject(base.id, {
    items: mapped,
    apuOverrides: {},
    subApuOverrides: {} // ✅ nuevos overrides de subproductos tampoco se importan por defecto
  });
...
function getProjectComparisonBaseId(projectId){
  const p = getProjectById(projectId);
  if(!p) return null;
  return normalizeComparisonBaseId(p.comparisonBaseId || "", p.baseId);
}
...
function setProjectComparisonBaseId(projectId, baseId){
  ...
  current.comparisonBaseId = normalizeComparisonBaseId(baseId, current.baseId || db.meta.defaultBaseId || "default");
  current.updatedAt = nowISO();
  db.projects[idx] = normalizeProject(current, getStoreDefaultBaseId(db));

  saveStore(db);
  return db.projects[idx].comparisonBaseId;
}


function getDefaultBaseId(){
  try{
    const fromActive = safeStr(
      safeLocalGet("activeComparisonBaseId") ||
      safeLocalGet("activeBaseId") ||
      safeLocalGet("apu_base_default_id") ||
      ""
    );
    if(fromActive) return normalizeBaseId(fromActive);
  }catch(_){}
  const db = loadStore();
  return getStoreDefaultBaseId(db);
}

function setDefaultBaseId(baseId){
  const db = loadStore();
  db.meta.defaultBaseId = normalizeBaseId(baseId);
  saveStore(db);
  try{
    safeLocalSet("activeComparisonBaseId", db.meta.defaultBaseId);
    safeLocalSet("activeBaseId", db.meta.defaultBaseId);
    safeLocalSet("apu_base_default_id", db.meta.defaultBaseId);
  }catch(_){}
  return db.meta.defaultBaseId;
}

function listInstalledBases(){
  try{
    if(window.APUBase && typeof APUBase.listInstalledBases === "function"){
      return APUBase.listInstalledBases();
    }
  }catch(_){}
  return [{ id: getDefaultBaseId(), name: "Base actual", isDefault: true }];
}
...
window.StorageAPI = {
  ...
  getDefaultBaseId, setDefaultBaseId, listInstalledBases,
  getProjectComparisonBaseId, setProjectComparisonBaseId,
  ...
  // ✅ NUEVO: Subproductos por proyecto

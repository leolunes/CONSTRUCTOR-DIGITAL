(function(){
  ...
  async function getByCode(code){
    ...
    // 2) intento por código normalizado
    const items = await loadItemsCache();
  ...
  async function listFormularioItems(query="", limit=200){
    ...
    // ✅ SIN búsqueda: devolver TODO
    if(!q) return arr;

    // ✅ CON búsqueda: sí limitar
  ...
  async function listCostosDirectosAll(query="", limit=250){
    ...
    await new Promise((res, rej)=>{
      ...
      r.onsuccess = ()=>{
        ...
        // ✅ SOLO limitar cuando sí hay búsqueda
        if(q && Number.isFinite(limit) && limit > 0 && out.length >= limit) return res(true);
  ...
  // Lista insumos (si no hay query devuelve TODOS)

...
Escribe 1, 2 o 3:",
      ...
      if(mode === "2"){
        ...
        rows = (rows || []).filter(r => {
          const hay = normalizeTextForMatch(`${r?.tipo||""} ${r?.desc||""} ${r?.unit||""}`);
        ...
        const menu = shown.map((r, i)=>`${i+1}) ${r.tipo || "-"} | ${r.desc || ""} | ${r.unit || ""} | ${UI.fmtMoney(r.pu || 0, "COP")}`).join("
");
        const pick = Number(prompt(`Selecciona el insumo:

${menu}`, "1") || "0");
        if(!(pick >= 1 && pick <= shown.length)) return;

        const sel = shown[pick - 1];
        const qty = Number(prompt("Cant/Rend:", "1") || "0");
        if(!(qty > 0)) return;
        ...
        localLines.push({
          ...
          subRef: "",
          source: "base_insumo",
          baseCode: String(sel.key || sel.code || sel.id || sel.desc || "").trim()
        });
        renderLines();
        return;
      }
      ...
      if(mode === "3"){
        ...
        let list = [];
        try{
          if(window.APUBase && typeof APUBase.listSubproductosDeep === "function"){
            list = await APUBase.listSubproductosDeep(term);
          }else{
            list = await APUBase.listSubproductos();
            const q = normalizeTextForMatch(term);
            list = (list || []).filter(x => normalizeTextForMatch(`${x?.subName||""} ${x?.subKey||""}`).includes(q));
          }
        }catch(err){
          alert("Error consultando subproductos: " + (err?.message || err));
          return;
        ...
        const menu = shown.map((r, i)=>`${i+1}) ${r.subName || ""} | ${r.subKey || ""}`).join("
");
        const pick = Number(prompt(`Selecciona el subproducto:
...
${menu}`, "1") || "0");
        ...
        if(!subApu){
          alert("No se pudo leer el subproducto seleccionado.");
          return;
        }
      ...
      if(!desc.trim()) return;
      if(!(qty>0) || !(pu>=0)) return;
      localLines.push({ group, desc, unit, qty, pu, parcial: qty*pu, subRef: "", source: "manual", baseCode: "" });
      renderLines();
    });

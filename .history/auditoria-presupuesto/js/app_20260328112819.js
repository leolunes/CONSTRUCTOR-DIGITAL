...
function renderIndirectRows(projectId){
  ...
  tbody.innerHTML = rows.map(row=>{
    ...
    if(r.rowType === "header"){
      return `
        <tr data-indirect-id="${UI.esc(r.id)}" data-row-type="header">
          <td><input class="input" data-field="item" value="${UI.esc(r.item)}" placeholder="1.1"></td>
          <td>
          ...
          <td colspan="5"><input class="input" data-field="desc" value="${UI.esc(r.desc)}" placeholder="Subtítulo o encabezado"></td>
...
function persistIndirectRowsFromDom(projectId, opts={}){
...
function updateIndirectRowPreview(tr, projectId){
  ...
  if(rowType === "header"){
    const partialCell = tr.children?.[7];
    if(partialCell){
      partialCell.innerHTML = `<b>${UI.fmtMoney(0, project.currency||"COP")}</b>`;
    }
    return;
  }
  ...
  const partialCell = tr.children?.[7];
  if(partialCell){
    partialCell.innerHTML = `<b>${UI.fmtMoney(partial, project.currency||"COP")}</b>`;
  }
}
...
function bindIndirectTableUI(projectId){
  ...
  btnHeader.onclick = ()=>{
    const fresh = StorageAPI.getProjectById(projectId);
    const rows = getIndirectRows(fresh);
    rows.push(makeIndirectRow("header", "ADMIN"));
    saveIndirectRows(projectId, rows);
    renderIndirectRows(projectId);
  };
  ...
  tbody.oninput = (e)=>{
    ...
    updateIndirectRowPreview(tr, projectId);
  };

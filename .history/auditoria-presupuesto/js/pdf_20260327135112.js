...
async function getSubproductoAPU(project, subKey){
  ...
  async function drawLaborRoleSubproductBreakdown(doc, L, roleRow){
    const sub = await resolveLaborRoleSubproduct(roleRow?.desc || "");
    if(!sub || !Array.isArray(sub.lines) || !sub.lines.length) return;

    L.ensure(18 + sub.lines.length * 12);
    doc.setFont("helvetica","bold");
    doc.setFontSize(8.1);
    doc.text(`Subproducto ${safe(roleRow.desc)}:`, L.margin + 40, L.getY());
    L.setY(L.getY() + 9);

    for(const sl of sub.lines){
      doc.setFont("helvetica","normal");
      doc.setFontSize(7.8);
      const txt = `${safe(sl.group || sl.tipo || "-")} · ${safe(sl.desc || "")} · Cant: ${fmtQty(sl.qty || 0)} · Vr Unit: ${moneyCOP0(sl.pu || 0)} · Parcial: ${moneyCOP0(sl.parcial || 0)}`;
      const parts = doc.splitTextToSize(txt, L.contentW - 70);
      for(const p of parts){
        L.ensure(10);
        doc.text(p, L.margin + 48, L.getY());
        L.setY(L.getY() + 9);
      }
    }
  ...
  async function drawLaborBreakdownUnderRow(doc, L, ln){
    ...
    for(const r of rows){
      ...
      await drawLaborRoleSubproductBreakdown(doc, L, r);
    }

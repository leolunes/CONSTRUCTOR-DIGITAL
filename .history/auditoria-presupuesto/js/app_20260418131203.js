...
function getActiveReadBaseIdForProject(projectOrId){
  ...
  const compareBase = getProjectComparisonBaseId(projectId || project);
  if(String(compareBase || "").trim()) return String(compareBase).trim();

  const activeBase = getActiveBaseIdForUi();
  if(String(activeBase || "").trim()) return String(activeBase).trim();

  const originBase = getProjectBaseId(project);
  if(String(originBase || "").trim()) return String(originBase).trim();
...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  ...
  if(type === "insumo"){
    ...
    try{
      const activeBaseId = getActiveReadBaseIdForProject(projectId);
      rows = await APUBase.listInsumos(term, 50, activeBaseId);
    }catch(err){
...
async function searchCreateApuInputs(project){
  ...
  try{
    ...
    try{
      if(typeof APUBase.listInsumos === "function"){
        baseInsumos = await APUBase.listInsumos(raw, 40);
      }
    }catch(__){}
  }
  ...
  try{
    ...
    try{
      if(typeof APUBase.listSubproductos === "function"){
        subproductos = await APUBase.listSubproductos(200);
      }
    }catch(__){}
  }
  ...
  for(const r of (Array.isArray(subproductos) ? subproductos : [])){
    ...
    try{
      ...
      try{
        if(typeof APUBase.getSubAPU === "function"){
          subApu = await APUBase.getSubAPU(r.subKey);
        }
      }catch(__){}
    }
  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        const selectedSubKey = String(sel.subKey || sel.key || sel.code || "").trim();
        if(!selectedSubKey){
          alert("El subproducto seleccionado no tiene clave válida.");
          return;
        }

        let subApu = null;
        try{
          subApu = await APUBase.getSubAPU(selectedSubKey, activeBaseId);
        }catch(err){
          alert("Error leyendo subproducto: " + (err?.message || err));
          return;
        }
        ...
        localLines.push({
          ...
          subRef: selectedSubKey,
          source: "base_subproducto",
          baseCode: selectedSubKey
        });

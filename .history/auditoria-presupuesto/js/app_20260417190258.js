...
function getProjectBaseId(project){
  try{
    const active = getActiveBaseIdForUi();
    if(active) return String(active).trim() || "default";
  }catch(_){}
  try{
    if(window.APUBase && typeof APUBase.getDefaultBaseId === "function"){
      return String(APUBase.getDefaultBaseId() || "default").trim() || "default";
    }
  }catch(_){}
  try{
    if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      return String(StorageAPI.getDefaultBaseId() || "default").trim() || "default";
    }
  }catch(_){}
  return "default";
}
...
function getActiveBaseIdForUi(){
  try{
    const local = String(localStorage.getItem("activeBaseId") || "").trim();
...
function setActiveBaseIdForUi(baseId){
  ...
  if(window.APUBase && typeof APUBase.setDefaultBaseId === "function"){
    try{ APUBase.setDefaultBaseId(next); }catch(_){}
  }
  try{
...
async function resolvePUForProjectItem(projectId, item, baseId){
  const lookupCode = String(item?.apuRefCode || item?.code || "").trim();
  if(!lookupCode) return Number(item?.pu || 0);

  const chosenBaseId = String(baseId || getActiveBaseIdForUi() || "default").trim() || "default";

  try{
    if(typeof tryResolveAPUFromProjectItem === "function"){
      const resolved = await tryResolveAPUFromProjectItem(projectId, lookupCode, chosenBaseId);
      if(resolved?.data){
  ...
  try{
    if(window.APUBase && typeof APUBase.getAPU === "function"){
      const apu = await APUBase.getAPU(lookupCode, chosenBaseId);
      if(apu){
...
async function recalcItemListAgainstActiveBase(projectId, items, baseId){
  const arr = Array.isArray(items) ? deepCloneBudgetRuntime(items) : [];
  const chosenBaseId = String(baseId || getActiveBaseIdForUi() || "default").trim() || "default";
  for(let i=0; i<arr.length; i++){
    arr[i].pu = await resolvePUForProjectItem(projectId, arr[i], chosenBaseId);
  }
  return arr;
}
...
async function recalcProjectsForActiveBase(){
  ...
  const activeBaseId = String(getActiveBaseIdForUi() || "default").trim() || "default";
  setActiveBaseIdForUi(activeBaseId);
  ...
  for(const p of projects){
    ...
    const recalculatedBaseItems = await recalcItemListAgainstActiveBase(fresh.id, baseSourceItems, activeBaseId);
    ...
    const patch = {
      budgets: nextBudgets,
      items: deepCloneBudgetRuntime(recalculatedBaseItems),
      baseId: activeBaseId
    };
...
function renderBaseSelector(){
  ...
  sel.onchange = async ()=>{
    ...
    try{
      ...
      console.warn("[BaseActiva] No se pudieron recalcular los proyectos con la nueva base:", err);
    }
  ...
  async function getAPUForBudgetItem(projectView, item){
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const apu = await APUBase.getAPU(itemCode);
...
async function getBaseAPUByAnyLookup(codeA, codeB, baseId){
  ...
  for(const key of tries){
    try{
      const apu = await APUBase.getAPU(key, chosenBaseId);
      if(apu) return { key, apu };
    }catch(_){}
  }
  return null;
...
async function tryResolveAPUFromProjectItem(projectId, requestedCode, baseId){
  ...
  const baseHit = await getBaseAPUByAnyLookup(refCode, visibleCode, chosenBaseId);
  if(baseHit){
    return {
      source: "base",
...
async function renderKpis(){
  ...
  try{
    ...
    const currentBaseId = (window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
...
function renderProjectDetail(project){
  ...
  UI.qs("#projSub") && (UI.qs("#projSub").textContent = `${project.entity || "—"} · ${project.location || "—"} · ${project.currency || "COP"} · Base: ${getProjectBaseId(project)} · Presupuesto activo: ${budgetInfo.label}`);
  ...
  if(info){
    info.innerHTML = `
      <div class="cardhead">
        ...
        <p class="muted small">Entidad: ${UI.esc(project.entity||"-")} · Ubicación: ${UI.esc(project.location||"-")} · Base: ${UI.esc(getProjectBaseId(project))} · Presupuesto activo: ${UI.esc(budgetInfo.label)}</p>
      </div>
      <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
...
async function searchCreateApuInputs(project){
  ...
  const baseId = getProjectBaseId(project);

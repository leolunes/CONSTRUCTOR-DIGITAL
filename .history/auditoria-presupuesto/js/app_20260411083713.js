...
function renderBudgetToolbar(project){
  ...
  host.innerHTML = `
    ...
    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
      ...
      ${getProjectBasePeriodo(project) ? UI.chip(`Base ${UI.esc(getProjectBasePeriodo(project))}`, "ok") : ""}
  ...
  async function getAPUForBudgetItem(projectView, item){
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const apu = await APUBase.getAPU(itemCode, getProjectBasePeriodo(projectView));
...
async function tryResolveAPUFromProjectItem(projectId, requestedCode){
  ...
  const basePeriodo = getProjectBasePeriodo(project);
  ...
  const baseHit = await getBaseAPUByAnyLookup(refCode, visibleCode, basePeriodo);
  if(baseHit){
    return {
      source: "base",
...
function exportProjectExcel(project){
  ...
  resumenAOA.splice(9, 0, ["Base trimestral", project.basePeriodo || ""]);
...
async function bindProjectsPage(){
  UI.qs("#btnNew")?.addEventListener("click", async ()=>{
    ...
    const basePeriodo = await getCurrentActiveBasePeriodo();
    if(!basePeriodo){
      alert("Primero instala o activa una base trimestral antes de crear el proyecto.");
      return;
    }

    const p = StorageAPI.createProject({ name, entity, location: ubicacion, basePeriodo });
...
function renderProjectDetail(project){
  ...
  const basePeriodo = getProjectBasePeriodo(project);
  UI.qs("#projSub") && (UI.qs("#projSub").textContent = `${project.entity || "—"} · ${project.location || "—"} · ${project.currency || "COP"} · Base: ${basePeriodo || "—"} · Presupuesto activo: ${budgetInfo.label}`);
  ...
  if(info){
    info.innerHTML = `
      <div class="cardhead">
        ...
        <p class="muted small">Entidad: ${UI.esc(project.entity||"-")} · Ubicación: ${UI.esc(project.location||"-")} · Base trimestral: ${UI.esc(basePeriodo||"-")} · Presupuesto activo: ${UI.esc(budgetInfo.label)}</p>
      </div>
      <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  const basePeriodo = getProjectBasePeriodo(projectId);
  ...
  let list = [];
  try{
    list = await APUBase.listSubproductos(undefined, basePeriodo);
  }catch(err){
  ...
  let subApu = null;
  try{
    subApu = await APUBase.getSubAPU(sel.subKey, basePeriodo);
  }catch(err){
...
function bindBaseViewer(projectId){
  ...
  async function loadRows(q){
    ...
    try{
      if(mode === "formulario"){
        const rows = await APUBase.listFormularioItems(q || "", undefined, basePeriodo);
      ...
      if(mode === "cd"){
        const rows = await APUBase.listCostosDirectosAll(q || "", undefined, basePeriodo);
        if(!rows.length){ showEmpty(true); return; }
      ...
      if(mode === "sub"){
        const list = await APUBase.listSubproductos(undefined, basePeriodo);
      ...
      if(mode === "insumos"){
        const rows = await APUBase.listInsumos(q || "", undefined, basePeriodo);
        if(!rows.length){ showEmpty(true); return; }
...
async function bindProjectDetailPage(){
  ...
  project = await ensureProjectBaseBinding(project, { persist:true }) || project;
  ...
  UI.qs("#btnExportProj")?.addEventListener("click", async ()=>{
    try{
      ...
      if(!p) return;
      await ensureProjectBaseBinding(p.id);
  ...
  UI.qs("#btnApuSearch")?.addEventListener("click", async ()=>{
    try{
      const meta = await APUBase.getMeta(getProjectBasePeriodo(projectId));
      if(!meta){
        alert("Primero instala la Base APU (XLSX) desde Proyectos.");
        return;
      }
      ...
      const results = await APUBase.search(q, 30, getProjectBasePeriodo(projectId));
...
async function bindAPUPage(){
  ...
  const projectId = UI.getParam("projectId");
  const projectBasePeriodo = getProjectBasePeriodo(projectId);
  if(projectId) await ensureProjectBaseBinding(projectId);
  ...
  if(apuCode){
    ...
    if(!apu){
      ...
      if(custom){
        ...
        apu = await APUBase.getAPU(apuCode, projectBasePeriodo);
    ...
    if(!subApu){
      subApu = await APUBase.getSubAPU(subKey, projectBasePeriodo);
    }
  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        let meta = null;
        try{
          meta = await APUBase.getMeta(basePeriodo);
        }catch(_){}
        ...
        if(mode === "2"){
          ...
          let rows = [];
          try{
            rows = await APUBase.listInsumos("", undefined, basePeriodo);
          }catch(err){
            alert("Error consultando insumos: " + (err?.message || err));
            return;
          }

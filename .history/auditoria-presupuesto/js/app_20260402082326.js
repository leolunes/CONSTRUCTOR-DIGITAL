...
function exportAuditPdf(project){
  ...
  addTitle("2. Hallazgos automáticos");
  addSimpleTable(
    ["Estado", "Hallazgo", "Detalle", "Trazabilidad"],
    findings.map(f=>[
      f.severity === "bad" ? "CRÍTICO" : (f.severity === "warn" ? "ALERTA" : "OK"),
      f.title || "",
      f.detail || "",
  ...
  addTitle("3. Comparación de costos indirectos");
  ...
  addTitle("4. Comparación por capítulos");
  ...
  addTitle("6. Top de riesgos presupuestales");
  ...
  addTitle("7. Historial reciente de cambios");
  ...
  addTitle("8. Conclusión automática");
  ...
  addParagraph(
    ...
    + `La auditoría automática generó ${findings.length} hallazgos para revisión técnica. `
...
function normalizeSubSearchText(v){
  return String(v || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
async function searchSubproductosDeep(term){
  if(!window.APUBase || typeof APUBase.listSubproductos !== "function") return [];
  const q = normalizeSubSearchText(term);
  if(!q) return [];
  ...
  function addHit(baseRow, subApu){
    const key = String(baseRow?.subKey || subApu?.subKey || "").trim();
    if(!key) return;
    if(byKey.has(key)) return;
    byKey.set(key, {
      ...baseRow,
      subKey: key,
      subName: String(baseRow?.subName || subApu?.subtitle || subApu?.header || key).trim(),
      unit: String(subApu?.unit || baseRow?.unit || "").trim(),
      directo: Number(subApu?.directo || baseRow?.directo || 0)
    });
  }

  for(const row of all){
    const hay =
      normalizeSubSearchText(row?.subName).includes(q) ||
      normalizeSubSearchText(row?.subKey).includes(q);
    if(hay) addHit(row, null);
  }

  for(const row of all){
    const key = String(row?.subKey || "").trim();
    if(!key) continue;

    let subApu = null;
    try{
      subApu = await APUBase.getSubAPU(key);
    }catch(_){
      continue;
    }
    if(!subApu) continue;
    ...
    const hay =
      normalizeSubSearchText(row?.subName).includes(q) ||
      normalizeSubSearchText(row?.subKey).includes(q) ||
      normalizeSubSearchText(subApu?.title).includes(q) ||
      normalizeSubSearchText(subApu?.subtitle).includes(q) ||
      normalizeSubSearchText(subApu?.header).includes(q) ||
      normalizeSubSearchText(subApu?.metaLine).includes(q) ||
      normalizeSubSearchText(subApu?.unit).includes(q) ||
      (Array.isArray(subApu?.lines) && subApu.lines.some(l =>
        normalizeSubSearchText(l?.group || l?.tipo).includes(q) ||
        normalizeSubSearchText(l?.desc).includes(q) ||
        normalizeSubSearchText(l?.unit).includes(q) ||
        normalizeSubSearchText(l?.subRef).includes(q)
      ));
  ...
  return Array.from(byKey.values()).sort((a,b)=>
    String(a.subName || a.subKey || "").localeCompare(
      String(b.subName || b.subKey || ""),
      "es",
      ...
      if(mode === "2" || mode === "3"){
        ...
        if(mode === "2"){
          ...
          const menu = rows.map((r, i)=>
            `${i+1}) ${r.tipo || "-"} | ${r.desc || ""} | ${r.unit || ""} | ${UI.fmtMoney(r.pu || 0, "COP")}`
          ).join("\n");
          ...
          localLines.push({
            ...
            unit: String(sel.unit || "UND").trim(),
            qty,
            pu: Number(sel.pu || 0),
            parcial: qty * Number(sel.pu || 0),
            subRef: "",
            source: "base_insumo",
            baseCode: String(sel.code || sel.id || sel.desc || "").trim()
          });
        ...
        list = await searchSubproductosDeep(term);
        ...
        localLines.push({
          group: "SUBPRODUCTO",
          desc: String(sel.subName || sel.subKey || "").trim(),
          unit: String(subApu?.unit || "GLB").trim(),
          qty,
          pu: unitCost,

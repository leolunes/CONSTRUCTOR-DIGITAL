...
async function exportAuditPdf(project){
  ...
  const riskTopRows = (typeof getBudgetRiskTopRows === "function")
    ? getBudgetRiskTopRows(project, 15)
    : [];
  ...
  function riskLevelFromPct(pct){
    ...
    if(v >= 20) return { label:"ALTO", code:"R3", note:"Revisión inmediata" };
    if(v >= 10) return { label:"MEDIO", code:"R2", note:"Revisión prioritaria" };
    return { label:"BAJO", code:"R1", note:"Seguimiento rutinario" };
  }

  function riskText(level){
    if(level.label === "ALTO") return "🔴 ALTO";
    if(level.label === "MEDIO") return "🟡 MEDIO";
    return "🟢 BAJO";
  }

  function recommendationForFinding(f){
    const id = String(f?.id || "");
    if(id === "criterio_indirectos") return "Unificar el criterio de cálculo de indirectos y dejar trazabilidad de la decisión.";
    if(id === "variacion_global") return "Validar soporte técnico y financiero de la variación global antes de aprobación.";
    if(id === "items_alerta") return "Revisar precios unitarios, cantidades y soportes de mercado de los ítems alertados.";
    if(id === "capitulos_variacion") return "Priorizar verificación de capítulos con mayor impacto en la desviación.";
    if(id === "indirectos_variacion") return "Revisar la consistencia de administración, imprevistos, utilidad y otros.";
    if(id === "presupuesto_igual") return "Validar si la capa oficial realmente fue intervenida o si sigue replicando la base.";
    return "Mantener trazabilidad, soporte y justificación técnica de los cambios detectados.";
  }
  ...
  function drawCover(){
    const logo = String(project.logoDataUrl || "").trim();
    const risk = riskLevelFromPct(cmp.diffPct || 0);
    ...
    doc.text(`NIVEL DE RIESGO GLOBAL: ${riskText(risk)}`, W/2, y, { align:"center" });
    y += 24;
    ...
    const coverRows = [
      ...
      ["Moneda", currency],
      ["Variación total", `${money(cmp.diff || 0)} (${Number(cmp.diffPct || 0).toFixed(2)}%)`]
    ];
    ...
    const intro = doc.splitTextToSize(
      "El presente informe consolida un análisis técnico de auditoría presupuestal entre el Presupuesto Base y el Presupuesto Oficial, con énfasis en variaciones globales, costos indirectos, capítulos críticos, ranking de riesgo e identificación de diferencias internas por APU y por componente de costo directo.",
      usableW
    );
  ...
  function addBulletLines(lines){
    (lines || []).forEach(t => addParagraph(`• ${safeText(t)}`));
  }
  ...
  function classifyApuCostComponent(groupRaw){
  ...
  async function getApuComponentBuckets(projectView, item){
    const zero = {
      "MATERIALES": 0,
    ...
    if(!projectView || !item || typeof getAPUForProjectItem !== "function"){
      return { ...zero };
    }

    try{
      const apuObj = await getAPUForProjectItem(projectView, item);
      const lines = Array.isArray(apuObj?.lines) ? apuObj.lines : [];
      if(!lines.length) return { ...zero };

      const out = { ...zero };
      lines.forEach(ln=>{
        const key = classifyApuCostComponent(ln?.group || ln?.tipo || "");
        const parcial = Number(ln?.parcial || 0) || (Number(ln?.qty || 0) * Number(ln?.pu || 0));
        out[key] = Number(out[key] || 0) + parcial;
      });
      return out;
    }catch(_){
      return { ...zero };
  ...
  async function buildApuComparisonRows(limitItems=12){
    const baseItems = Array.isArray(cmp.baseView?.items) ? cmp.baseView.items : [];
    const officialItems = Array.isArray(cmp.officialView?.items) ? cmp.officialView.items : [];
    const baseMap = new Map(baseItems.map(it => [itemCompareKey(it), it]));
    const officialMap = new Map(officialItems.map(it => [itemCompareKey(it), it]));

    const ranked = riskTopRows.length
      ? riskTopRows
      : itemRows
          .map(r=>{
            const basePartial = Number(r.baseQty || 0) * Number(r.basePU || 0);
            const officialPartial = Number(r.officialQty || 0) * Number(r.officialPU || 0);
            return { ...r, impact: officialPartial - basePartial, impactAbs: Math.abs(officialPartial - basePartial) };
          })
          .sort((a,b)=> Number(b.impactAbs || 0) - Number(a.impactAbs || 0))
          .slice(0, limitItems);

    const selected = ranked.slice(0, limitItems);
    const rows = [];
    const labels = ["MATERIALES","EQUIPO Y HERRAMIENTA","MANO DE OBRA","TRANSPORTE","SUBCONTRATOS","OTROS"];

    for(const rr of selected){
      const key = itemCompareKey(rr);
      const baseItem = baseMap.get(key) || null;
      const officialItem = officialMap.get(key) || null;
      const code = String(rr.code || baseItem?.code || officialItem?.code || "").trim();
      const desc = String(rr.desc || baseItem?.desc || officialItem?.desc || "").trim();

      const baseBuckets = await getApuComponentBuckets(cmp.baseView, baseItem);
      const officialBuckets = await getApuComponentBuckets(cmp.officialView, officialItem);

      for(const comp of labels){
        const baseVal = Number(baseBuckets[comp] || 0);
        const offVal = Number(officialBuckets[comp] || 0);
        const diff = offVal - baseVal;
        const pct = baseVal > 0 ? (diff / baseVal) * 100 : (offVal > 0 ? 100 : 0);
        if(baseVal === 0 && offVal === 0) continue;
  ...
  function stampAllPages(){
    ...
    for(let p=1; p<=totalPages; p++){
      ...
      if(p > 1){
        ...
        doc.setFontSize(8.4);
        doc.text(nowTxt, W - M.right, 28, { align:"right" });
        doc.text(`Página ${p} de ${totalPages}`, W - M.right, 39, { align:"right" });
        doc.setLineWidth(0.35);
      ...
      doc.setFontSize(8.2);
      doc.setLineWidth(0.35);
      doc.line(M.left, H - 28, W - M.right, H - 28);
      doc.text(`Página ${p} de ${totalPages}`, W/2, H - 14, { align:"center" });
    }
  }
  ...
  const globalRisk = riskLevelFromPct(cmp.diffPct || 0);
  const topRisk = riskTopRows.length ? riskTopRows[0] : null;
  const strongestChapter = chapterRows
    .map(r=>{
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return { ...r, diff, pct, impactAbs: Math.abs(diff) };
    })
    .sort((a,b)=> Number(b.impactAbs || 0) - Number(a.impactAbs || 0))[0] || null;
  ...
  addParagraph(
    ...
    + `El análisis identifica una variación total de ${money(cmp.diff || 0)}, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, `
    + `clasificada en nivel de riesgo ${riskText(globalRisk)}. `
    + `${methodAudit.sameMethod ? "Ambos presupuestos mantienen el mismo criterio de cálculo de indirectos." : `Se detectó una diferencia metodológica en el cálculo de indirectos (Base: ${methodAudit.baseModeLabel}; Oficial: ${methodAudit.officialModeLabel}).`}`
  );
  addBulletLines([
    ...
    strongestChapter
      ? `Capítulo con mayor incidencia: ${safeText(strongestChapter.chapterCode || "")} ${safeText(strongestChapter.chapterName || "")} con variación de ${Number(strongestChapter.pct || 0).toFixed(2)}%.`
      : "No se detectó un capítulo predominante en la variación.",
    topRisk
      ? `Ítem crítico con mayor impacto: ${safeText(topRisk.code || "")} – ${safeText(topRisk.desc || "")} con impacto de ${money(topRisk.impact || 0)}.`
      : "No se detectaron ítems críticos priorizados en el ranking."
  ]);

  addTitle("2. Semáforo y lectura de riesgo");
  addRow("Semáforo global", riskText(globalRisk));
  addRow("Código interno", globalRisk.code);
  addRow("Criterio de lectura", globalRisk.note);
  ...
  if(!indirectCmp.rows.length){
    ...
    const maxIndirect = indirectCmp.rows
      .filter(r => ["Administración","Imprevistos","Utilidad","Otros"].includes(String(r.label || "")))
      .sort((a,b)=> Math.abs(Number(b.diffPct || 0)) - Math.abs(Number(a.diffPct || 0)))[0];
    if(maxIndirect){
      addParagraph(
        `El componente indirecto de mayor variación es ${safeText(maxIndirect.label || "")}, con diferencia de ${money(maxIndirect.diff || 0)} (${Number(maxIndirect.diffPct || 0).toFixed(2)}%).`
      );
    }
  }
  ...
  if(!chapterRows.length){
    ...
    if(strongestChapter){
      addParagraph(
        `El capítulo con mayor impacto económico corresponde a ${safeText(strongestChapter.chapterCode || "")} ${safeText(strongestChapter.chapterName || "")}, con una diferencia de ${money(strongestChapter.diff || 0)} y variación relativa de ${Number(strongestChapter.pct || 0).toFixed(2)}%.`
      );
    }
  }
  ...
  if(!itemAlerts.length){
    addParagraph("No se detectaron ítems con alerta automática.");
  }else{
    addSimpleTable(
  ...
  const apuComparisonRows = await buildApuComparisonRows(12);
  if(!apuComparisonRows.length){
    addParagraph("No fue posible construir comparativos internos de APU para los ítems evaluados.");
  }else{
    addSimpleTable(
      ["Código", "Descripción / Componente", "Base", "Oficial", "Dif.", "%"],
      apuComparisonRows.map(r=>[
        r.code || "",
        `${r.desc || ""} / ${r.component || ""}`,
        money(r.baseValue || 0),
        money(r.officialValue || 0),
  ...
  addTitle("9. Historial reciente de cambios");
  ...
  addTitle("10. Conclusión técnica");
  ...
  const conclusionRisk = riskText(globalRisk);

  addParagraph(
    `Se concluye que el Presupuesto Oficial presenta una variación de ${money(cmp.diff || 0)} respecto del Presupuesto Base, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, con clasificación de riesgo ${conclusionRisk}. `
    + `Se identificaron ${highAlerts} ítems con alerta automática, ${riskTopRows.length} riesgos priorizados por impacto económico y ${positiveChapters} capítulos con incremento frente al presupuesto base. `
    + `${methodAudit.sameMethod ? "No se evidencia cambio en el criterio de cálculo de indirectos entre ambos presupuestos. " : `Se evidencia una diferencia metodológica en el cálculo de indirectos (Base: ${methodAudit.baseModeLabel}; Oficial: ${methodAudit.officialModeLabel}). `}`

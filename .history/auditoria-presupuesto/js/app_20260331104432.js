...
function ensureBudgetComparisonMount(){
  ...
  const totalsSection = UI.qs("#totalsRows")?.closest?.(".card");
  ...
  host.innerHTML = `
    <div class="cardhead">
      ...
      <p class="muted small">Resumen comparativo entre ambos presupuestos del mismo proyecto.</p>
    </div>
    <div id="budgetComparisonBody"></div>
  `;
  totalsSection.insertAdjacentElement("afterend", host);
  return host;
}
...
function renderBudgetChapterComparison(project){
  ...
  let wrap = UI.qs("#budgetComparisonChapters");
  if(!wrap){
    wrap = document.createElement("div");
    wrap.id = "budgetComparisonChapters";
...
function renderBudgetItemComparison(project){
  ...
  let wrap = UI.qs("#budgetComparisonItems");
  if(!wrap){
    wrap = document.createElement("div");
    wrap.id = "budgetComparisonItems";
    wrap.style.marginTop = "14px";
    host.appendChild(wrap);
  }
...
function confirmAddApuToProject({ keepOpen=false }){
  ...
  pushProjectAuditEntry(projectId, {
    type: "ITEM_AGREGADO",
    code: String(apu.code || ""),
    desc: String(apu.desc || ""),
    detail: "Nuevo ítem desde APU base",
    beforeValue: "",
    afterValue: `Cant ${qty} · PU ${UI.fmtMoney(Number(apu.pu||0), fresh.currency||"COP")}`
  });
...
function renderItemsTable(project){
  ...
  tbody.querySelectorAll("[data-del]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      const it = (project.items || []).find(x=>String(x.id) === String(id));
      if(it){
        pushProjectAuditEntry(project.id, {
          type: "ITEM_ELIMINADO",
          code: String(it.code || ""),
          desc: String(it.desc || ""),
          detail: "Ítem eliminado",
          beforeValue: `Cant ${Number(it.qty||0)} · PU ${UI.fmtMoney(Number(it.pu||0), project.currency||"COP")}`,
          afterValue: ""
        });
      }
...
const curApuRef = String(it.apuRefCode || it.code || "").trim();
      ...
      const changes = [];
      if(String(it.code||"") !== String(code||"")) changes.push({ detail:"Código visible", beforeValue:String(it.code||""), afterValue:String(code||"") });
      if(String(it.apuRefCode||"") !== String(apuRefCode||"")) changes.push({ detail:"Código APU ref", beforeValue:String(it.apuRefCode||""), afterValue:String(apuRefCode||"") });
      if(String(it.desc||"") !== String(desc||"")) changes.push({ detail:"Descripción", beforeValue:String(it.desc||""), afterValue:String(desc||"") });
      if(String(it.unit||"") !== String(unit||"")) changes.push({ detail:"Unidad", beforeValue:String(it.unit||""), afterValue:String(unit||"") });
      if(Number(it.pu||0) !== Number(pu||0)) changes.push({ detail:"PU", beforeValue:UI.fmtMoney(Number(it.pu||0), fresh.currency||"COP"), afterValue:UI.fmtMoney(Number(pu||0), fresh.currency||"COP") });
      if(Number(it.qty||0) !== Number(qty||0)) changes.push({ detail:"Cantidad", beforeValue:String(Number(it.qty||0)), afterValue:String(Number(qty||0)) });
      if(String(curChapterCode||"") !== String(chapterCode||"")) changes.push({ detail:"Capítulo", beforeValue:String(curChapterCode||""), afterValue:String(chapterCode||"") });

      changes.forEach(ch=>{
        pushProjectAuditEntry(fresh.id, {
          type: "ITEM_EDITADO",
          code: String(code || it.code || ""),
          desc: String(desc || it.desc || ""),
          detail: ch.detail,
...
async function bindProjectDetailPage(){
  ...
  UI.qs("#formAjustes")?.addEventListener("submit", (e)=>{
    ...
    const before = StorageAPI.getProjectById(projectId);
    ...
    [
      ["Administración %", Number(before?.adminPct ?? before?.aiuPct ?? 0), adminPct],
      ["Imprevistos %", Number(before?.imprevPct ?? 0), imprevPct],
      ["Utilidad %", Number(before?.utilPct ?? 0), utilPct],
      ["IVA utilidad %", Number(before?.ivaUtilPct ?? before?.ivaPct ?? 0), ivaUtilPct],
      ["Modo indirectos", String(before?.indirectMode || "manual"), String(indirectMode || "manual")]
    ].forEach(([label, prev, next])=>{
      if(String(prev) !== String(next)){
        pushProjectAuditEntry(projectId, {
          type: "AJUSTE_EDITADO",
          code: "",
          desc: "Configuración del presupuesto",
          detail: String(label),
          beforeValue: String(prev),
  ...
  UI.qs("#formAddManual")?.addEventListener("submit", (e)=>{
    ...
    pushProjectAuditEntry(projectId, {
      ...
      code,
      desc,
      detail: "Nuevo ítem manual",
      beforeValue: "",
      afterValue: `Cant ${qty} · PU ${UI.fmtMoney(pu, fresh.currency||"COP")}`
    });

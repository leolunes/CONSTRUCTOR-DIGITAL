...
async function exportAuditPdf(project){
  ...
  const M = { top: 42, right: 44, bottom: 52, left: 44 };
  ...
  const topRows = itemRows
    .map(r=>{
      const audit = detectBudgetItemAlerts(r);
      const basePartial = Number(r.baseQty || 0) * Number(r.basePU || 0);
      const officialPartial = Number(r.officialQty || 0) * Number(r.officialPU || 0);
      const impact = officialPartial - basePartial;
      return { ...r, audit, basePartial, officialPartial, impact, impactAbs: Math.abs(impact) };
    })
    .sort((a,b)=> Number(b.impactAbs||0) - Number(a.impactAbs||0))
    .slice(0, 10);

  const absDiffPct = Math.abs(Number(cmp.diffPct || 0));
  const riskInfo = (() => {
    const criticalFindings = findings.filter(f => String(f.severity || "") === "bad").length;
    const alertItems = itemAlerts.length;
    const chapterAlerts = chapterRows.filter(r => {
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return Math.abs(pct) >= 10;
    }).length;
    if(absDiffPct >= 20 || criticalFindings >= 2 || alertItems >= 5){
      return { code:"R3", label:"ALTO", read:"Revisión inmediata" };
    }
    if(absDiffPct >= 10 || criticalFindings >= 1 || chapterAlerts >= 1){
      return { code:"R2", label:"MEDIO", read:"Revisión prioritaria" };
    }
    return { code:"R1", label:"BAJO", read:"Seguimiento ordinario" };
  })();
  ...
  function splitText(txt, width){
    return doc.splitTextToSize(safeText(txt), width);
  }

  function addParagraph(text, opts){
    const o = opts || {};
    const fs = o.fontSize || 10;
    const leading = o.leading || (fs + 2);
    doc.setFont("helvetica", o.bold ? "bold" : "normal");
    doc.setFontSize(fs);
    const lines = splitText(text, o.width || usableW);
    ensureSpace(lines.length * leading + 6);
    doc.text(lines, o.x || M.left, y);
    y += lines.length * leading + (o.after || 6);
  }

  function addBullets(lines){
    (lines || []).forEach(txt=>{
      addParagraph(`- ${safeText(txt)}`, { fontSize: 10, leading: 12, after: 4 });
    });
  }
  ...
  function addSectionTitle(number, title){
    ...
    doc.setFontSize(14);
    doc.text(`${number}. ${safeText(title)}`, M.left, y);
    y += 16;
    line(y);
    y += 14;
  }
  ...
  function drawSimpleTable(headers, rows, widths, aligns){
    const colAligns = Array.isArray(aligns) ? aligns : headers.map(()=> "left");
    ...
    function drawHeader(){
      ...
      doc.setFontSize(8.5);
      headers.forEach((h, i)=>{
        const tx = colAligns[i] === "right" ? x + widths[i] - 2 : x + 2;
        doc.text(safeText(h), tx, y, { align: colAligns[i] === "right" ? "right" : "left" });
        x += widths[i];
      });
      y += 8;
      line(y);
    ...
    rows.forEach(row=>{
      const prepared = row.map((cell, i)=>{
        doc.setFont("helvetica","normal");
        doc.setFontSize(8.2);
        return splitText(cell, Math.max(24, widths[i] - 6));
      });
      const maxLines = Math.max(...prepared.map(arr => arr.length || 1), 1);
      const rowH = maxLines * 9 + 6;

      if(y + rowH > H - M.bottom){
        addPage();
        drawHeader();
      }

      let x = M.left;
      prepared.forEach((arr, i)=>{
        const tx = colAligns[i] === "right" ? x + widths[i] - 2 : x + 2;
        doc.text(arr, tx, y, { align: colAligns[i] === "right" ? "right" : "left" });
        x += widths[i];
      });
  ...
  function drawCover(){
    ...
    doc.setFont("helvetica","bold");
    doc.setFontSize(17);
    doc.text("INFORME DE AUDITORIA PRESUPUESTAL", W/2, y, { align:"center" });
    y += 22;
    ...
    const coverRows = [
      ...
      ["Pais", project.instPais || ""],
      ["Departamento", project.instDepto || ""],
      ["Municipio", project.instMunicipio || project.location || ""],
      ["Fecha elaboracion", project.instFechaElab || new Date().toLocaleDateString()],
      ["Moneda", currency]
    ];
    ...
    coverRows.forEach(([label, value])=>{
      ...
      doc.text(safeText(label), M.left, y);
      doc.setFont("helvetica","normal");
      const lines = splitText(value || "-", usableW - 120);
    ...
    addParagraph(
      "El presente informe consolida el analisis comparativo entre el Presupuesto Base y el Presupuesto Oficial del proyecto, incluyendo variaciones generales, hallazgos automaticos, comparacion por capitulos, ranking de items criticos y comparativo tecnico por APU y componente.",
      { fontSize: 10, leading: 12, after: 18 }
    );

    if(elaborador?.nombre || elaborador?.profesion || elaborador?.matricula){
      doc.setFont("helvetica","bold");
      doc.setFontSize(10);
      doc.text("Elaboro:", M.left, y);
      y += 14;
      doc.setFont("helvetica","normal");
      ...
      if(elaborador?.matricula){ doc.text(`Matricula: ${safeText(elaborador.matricula)}`, M.left, y); y += 12; }
    }
  }
  ...
  function stampAllPages(){
    ...
    for(let p=2; p<=totalPages; p++){
      ...
      const logo = String(project.logoDataUrl || "").trim();
      if(logo){
        try{ doc.addImage(logo, "PNG", M.left, 18, 36, 36); }catch(_){}
      }
      const textX = logo ? M.left + 46 : M.left;

      doc.setFont("helvetica","bold");
      doc.setFontSize(9.4);
      doc.text("INFORME DE AUDITORIA PRESUPUESTAL", textX, 28);

      doc.setFont("helvetica","normal");
      doc.setFontSize(8.8);
      doc.text(safeText(project.instEntidad || project.entity || ""), textX, 39);
      doc.text(safeText(project.instProyectoLabel || project.name || ""), textX, 50);

      doc.text(nowTxt, W - M.right, 28, { align:"right" });
      doc.text(`Pagina ${p} de ${totalPages}`, W - M.right, 40, { align:"right" });

      doc.setLineWidth(0.45);
      doc.line(M.left, 60, W - M.right, 60);

      doc.line(M.left, H - 36, W - M.right, H - 36);
      doc.setFontSize(8.6);
      doc.text(`Pagina ${p} de ${totalPages}`, W/2, H - 22, { align:"center" });
    }
    if(totalPages >= 1){
      doc.setPage(1);
    }
  ...
  function classifyComponent(groupRaw){
    const up = String(groupRaw || "").toUpperCase();
    if(up.includes("SUBCONTRAT")) return "SUBCONTRATOS";
    if(up.includes("MATERIAL")) return "MATERIALES";
    if(up.includes("MANO") && up.includes("OBRA")) return "MANO DE OBRA";
    if(up.includes("EQUIPO") || up.includes("HERRAM") || up.includes("MAQUIN")) return "EQUIPO Y HERRAMIENTA";
    if(up.includes("TRANSP")) return "TRANSPORTE";
    return "OTROS";
  }

  function sumDirectFromLines(lines){
  ...
  async function resolveBudgetApu(view, item){
    if(!view || !item) return null;
    const keyA = String(item.apuRefCode || item.code || "").trim();
    const keyB = String(item.code || "").trim();

    const map = (view.apuOverrides && typeof view.apuOverrides === "object" && !Array.isArray(view.apuOverrides))
      ? view.apuOverrides
      : {};

    const ov = map[keyA] || map[keyB];
    if(ov && Array.isArray(ov.lines) && ov.lines.length){
      return {
        code: keyA || keyB,
        unit: String(ov.unit || item.unit || "").trim(),
        directo: sumDirectFromLines(ov.lines),
        lines: ov.lines
      };
    }
    ...
    try{
      if(window.APUBase && typeof APUBase.getAPU === "function"){
        const apu = await APUBase.getAPU(keyA || keyB);
  ...
  function bucketApu(apu){
    const out = {
      "MATERIALES": 0,
      "EQUIPO Y HERRAMIENTA": 0,
      "MANO DE OBRA": 0,
      "TRANSPORTE": 0,
      "SUBCONTRATOS": 0,
      "OTROS": 0
    ...
    lines.forEach(ln=>{
      const k = classifyComponent(ln.group || ln.tipo || "");
      out[k] += Number(ln.parcial || 0) || (Number(ln.qty || 0) * Number(ln.pu || 0));
    });
    return out;
  }

  function itemPairRows(baseViewArg, officialViewArg){
    function itemKey(it){
      return String(it?.apuRefCode || it?.code || it?.desc || "").trim();
    }
    const map = new Map();
    (baseViewArg?.items || []).forEach(it=>{
      const key = itemKey(it);
      if(!key) return;
      map.set(key, { key, baseItem: it, officialItem: null });
    });
    (officialViewArg?.items || []).forEach(it=>{
      const key = itemKey(it);
      if(!key) return;
      if(!map.has(key)){
        map.set(key, { key, baseItem: null, officialItem: it });
      }else{
        map.get(key).officialItem = it;
      }
    });
    return Array.from(map.values())
      .filter(x => x.baseItem && x.officialItem)
      .sort((a,b)=> String(a.baseItem?.code || "").localeCompare(String(b.baseItem?.code || ""), "es", { numeric:true, sensitivity:"base" }));
  }

  async function buildApuComparisonRows(){
    const pairs = itemPairRows(baseView, officialView);
    const out = [];
    for(const pair of pairs){
      const baseApu = await resolveBudgetApu(baseView, pair.baseItem);
      const officialApu = await resolveBudgetApu(officialView, pair.officialItem);
      if(!baseApu || !officialApu) continue;

      const baseBuckets = bucketApu(baseApu);
      const officialBuckets = bucketApu(officialApu);
      ...
      const rows = components.map(label=>{
        const baseVal = Number(baseBuckets[label] || 0);
        const officialVal = Number(officialBuckets[label] || 0);
        const diff = officialVal - baseVal;
        const pct = baseVal > 0 ? (diff / baseVal) * 100 : (officialVal > 0 ? 100 : 0);
        return { label, baseVal, officialVal, diff, pct };
      });

      const lead = rows.slice().sort((a,b)=> Math.abs(Number(b.diff||0)) - Math.abs(Number(a.diff||0)))[0];

      out.push({
        code: String(pair.baseItem?.code || pair.officialItem?.code || "").trim(),
        desc: String(pair.baseItem?.desc || pair.officialItem?.desc || "").trim(),
        unit: String(pair.baseItem?.unit || pair.officialItem?.unit || "").trim(),
        rows,
        lead
      });
    }
  ...
  async function drawAuditSignature(){
    const sigNeed = 140;
    ensureSpace(sigNeed);
    ...
    const imgData = String(elaborador?.firmaDataUrl || "").trim();
    if(imgData){
      try{
        doc.addImage(imgData, "PNG", M.left, y - 6, 140, 46);
      }catch(_){
        try{ doc.addImage(imgData, "JPEG", M.left, y - 6, 140, 46); }catch(__){}
      }
    }
    ...
    y += 13;
    if(elaborador?.matricula) doc.text(`Matricula: ${safeText(elaborador.matricula)}`, M.left, y);
  }
  ...
  const mostSensitiveChapter = chapterRows
    .map(r=>{
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return { ...r, diff, pct, absDiff: Math.abs(diff) };
    })
    .sort((a,b)=> Number(b.absDiff||0) - Number(a.absDiff||0))[0];

  const topImpact = topRows[0] || null;

  addParagraph(
    `Se evaluó la consistencia del Presupuesto Oficial frente al Presupuesto Base del proyecto "${safeText(project.name || "")}". El análisis identifica una variación total de ${money(cmp.diff || 0)}, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, clasificada en nivel de riesgo ${riskInfo.code} ${riskInfo.label}. ${methodAudit.sameMethod ? "Ambos presupuestos mantienen el mismo criterio de cálculo de indirectos." : `Se detectó diferencia en el criterio de cálculo de indirectos: Base ${methodAudit.baseModeLabel} y Oficial ${methodAudit.officialModeLabel}.`}`,
    { fontSize: 10.2, leading: 12, after: 10 }
  );
  ...
  addBullets([
    ...
    mostSensitiveChapter
      ? `Capítulo con mayor incidencia: ${safeText(mostSensitiveChapter.chapterCode || "")} ${safeText(mostSensitiveChapter.chapterName || "")} con variación de ${Number(mostSensitiveChapter.pct || 0).toFixed(2)}%.`
      : "No se detectaron capítulos con incidencia relevante.",
    topImpact
      ? `Ítem crítico con mayor impacto: ${safeText(topImpact.code || "")} - ${safeText(topImpact.desc || "")} con impacto de ${money(topImpact.impact || 0)}.`
      : "No se identificaron ítems críticos de alto impacto."
  ]);

  addSectionTitle(2, "Semáforo y lectura de riesgo");
  addKeyValueRow("Semáforo global", `${riskInfo.code} ${riskInfo.label}`);
  addKeyValueRow("Código interno", riskInfo.code);
  addKeyValueRow("Criterio de lectura", riskInfo.read);
  addKeyValueRow("Método indirectos Base", methodAudit.baseModeLabel);
  addKeyValueRow("Método indirectos Oficial", methodAudit.officialModeLabel);
  addParagraph(
    ...
    { fontSize: 9.8, leading: 11, after: 8 }
  );

  addSectionTitle(3, "Matriz de hallazgos");
  const findingRows = findings.map(f=>{
    const recommendation =
      String(f.id || "") === "variacion_global" ? "Validar soporte técnico y financiero de la variación global antes de aprobación." :
      String(f.id || "") === "items_alerta" ? "Revisar precios unitarios, cantidades y soportes de mercado de los ítems alertados." :
      String(f.id || "") === "capitulos_variacion" ? "Priorizar verificación de capítulos con mayor impacto en la desviación." :
      String(f.id || "") === "indirectos_variacion" ? "Revisar la consistencia de administración, imprevistos, utilidad y otros." :
      String(f.id || "") === "criterio_indirectos" ? "Homologar el método de cálculo de indirectos antes del cierre del análisis." :
      "Documentar la revisión técnica correspondiente.";
    return [
      String(f.severity || "") === "bad" ? "CRITICO" : (String(f.severity || "") === "warn" ? "ALERTA" : "OK"),
      f.title || "",
      f.detail || "",
      recommendation
    ];
  ...
  addSectionTitle(4, "Comparación de costos indirectos");
  if(!indirectCmp.rows.length){
    ...
    drawSimpleTable(
      ...
      [115, 86, 52, 86, 56, 86, 52],
      ["left","right","right","right","right","right","right"]
    );
    const leadIndirect = indirectCmp.rows
      .filter(r => ["Administración","Imprevistos","Utilidad","Otros"].includes(String(r.label || "")))
      .sort((a,b)=> Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)))[0];
    if(leadIndirect){
      addParagraph(
        `El componente indirecto de mayor variación es ${safeText(leadIndirect.label || "")}, con diferencia de ${money(leadIndirect.diff || 0)} (${Number(leadIndirect.diffPct || 0).toFixed(2)}%).`,
        { fontSize: 9.8, leading: 11, after: 8 }
      );
    }
  }

  addSectionTitle(5, "Comparación por capítulos");
  if(!chapterRows.length){
    ...
    if(mostSensitiveChapter){
      addParagraph(
        `El capítulo con mayor impacto económico corresponde a ${safeText(mostSensitiveChapter.chapterCode || "")} ${safeText(mostSensitiveChapter.chapterName || "")}, con una diferencia de ${money(mostSensitiveChapter.diff || 0)} y variación relativa de ${Number(mostSensitiveChapter.pct || 0).toFixed(2)}%.`,
        { fontSize: 9.8, leading: 11, after: 8 }
      );
    }
  }

  addSectionTitle(6, "Ranking de ítems críticos por impacto económico");
  if(!topRows.length){
    addParagraph("No se identificaron ítems con impacto económico relevante.");
  }else{
    drawSimpleTable(
      ["#", "Estado", "Código", "Descripción", "Base", "Oficial", "Impacto"],
      topRows.map((r, idx)=>[
        String(idx + 1),
        (Math.abs(Number(r.audit?.puPct || 0)) >= 20 || Math.abs(Number(r.audit?.qtyPct || 0)) >= 20) ? "CRITICO" :
          ((r.audit?.alerts || []).length ? "ALERTA" : "OK"),
        r.code || "",
        r.desc || "",
      ...
      [24, 50, 50, 220, 70, 70, 72],
      ["left","left","left","left","right","right","right"]
    );
  }

  addSectionTitle(7, "Ítems con alerta automática");
  addParagraph("Umbrales usados en la auditoría automática: Precio Unitario 10% y Cantidad 10%.", { fontSize: 9.8, leading: 11, after: 8 });
  if(!itemAlerts.length){
    ...
    drawSimpleTable(
      ...
      [52, 230, 48, 66, 48, 46, 110],
      ["left","left","right","right","right","right","left"]
    );
  }

  addSectionTitle(8, "Comparativo técnico por APU y componente");
  addParagraph(
    "La siguiente matriz compara la estructura interna de los APUs entre Presupuesto Base y Presupuesto Oficial, discriminando materiales, equipo y herramienta, mano de obra, transporte, subcontratos y otros. Los valores corresponden al costo directo unitario de cada APU.",
    { fontSize: 9.8, leading: 11, after: 10 }
  );

  const apuComparisons = await buildApuComparisonRows();

  if(!apuComparisons.length){
    addParagraph("No fue posible construir comparativos internos de APU para los ítems evaluados.");
  }else{
    for(const blk of apuComparisons){
      const blockNeed = 132;
      ensureSpace(blockNeed);

      doc.setFont("helvetica","bold");
      doc.setFontSize(10.2);
      doc.text(`${safeText(blk.code)} - ${safeText(blk.desc)}`, M.left, y);
      y += 14;
      ...
      if(blk.lead){
        addParagraph(
          `Componente con mayor variación en este APU: ${safeText(blk.lead.label || "")}, con diferencia de ${money(blk.lead.diff || 0)} (${Number(blk.lead.pct || 0).toFixed(2)}%).`,
          { fontSize: 9.6, leading: 10.5, after: 10 }
        );
      }
    }
  }

  addSectionTitle(9, "Conclusión técnica");
  const positiveChapters = chapterRows.filter(r => (Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0)) > 0).length;
  const apuComparable = apuComparisons.length;

  addParagraph(
    `Se concluye que el Presupuesto Oficial presenta una variación de ${money(cmp.diff || 0)} respecto del Presupuesto Base, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, con clasificación de riesgo ${riskInfo.code} ${riskInfo.label}. Se identificaron ${itemAlerts.length} ítems con alerta automática, ${topRows.length} ítems priorizados por impacto económico y ${positiveChapters} capítulos con incremento frente al presupuesto base. ${methodAudit.sameMethod ? "No se evidencia cambio en el criterio de cálculo de indirectos entre ambos presupuestos." : `Se detecta diferencia en el criterio de cálculo de indirectos (Base ${methodAudit.baseModeLabel} / Oficial ${methodAudit.officialModeLabel}).`} ${apuComparable > 0 ? `Adicionalmente, se construyeron ${apuComparable} comparativos técnicos por APU y componente.` : "No fue posible consolidar comparativos completos por APU en todos los ítems."} Desde una perspectiva de auditoría forense, los resultados recomiendan validar soportes técnicos, trazabilidad de cantidades, análisis de precios unitarios y justificación contractual de los componentes que explican la desviación observada antes de aprobar el presupuesto oficial.`,
    { fontSize: 10.1, leading: 12, after: 12 }
  );

  await drawAuditSignature();
  stampAllPages();

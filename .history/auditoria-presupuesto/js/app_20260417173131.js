...
function renderBaseSelector(){
  ...
  try{
    if(window.APUBase && typeof APUBase.getDefaultBaseId === "function"){
      current = String(APUBase.getDefaultBaseId() || "default").trim() || "default";
    }else if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      current = String(StorageAPI.getDefaultBaseId() || "default").trim() || "default";
    }
  }catch(_){
    current = "default";
  }
  ...
  sel.onchange = async ()=>{
    ...
    try{ localStorage.setItem("activeBaseId", next); }catch(_){ }

    if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
      try{ StorageAPI.setDefaultBaseId(next); }catch(_){ }
    }
...
async function renderKpis(){
  ...
  const allProjects = StorageAPI.listProjects();
  const currentBaseId = (window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
    ...
    : "default";
  const projectsInActiveBase = allProjects.filter(p => String(getProjectBaseId(p) || "default").trim() === currentBaseId);
  const total = projectsInActiveBase.reduce((s,p)=> s + (totalsCompat(p).total||0), 0);

  let baseChip = UI.chip("NO","bad");
  let activeLabel = currentBaseId;
  try{
    const installed = getInstalledBasesForUi();
    activeLabel = getBaseLabelById(currentBaseId);
  ...
  el.innerHTML = `
    <div class="card item"><div class="topline"><div class="name">Proyectos</div><div class="chips">${UI.chip(String(projectsInActiveBase.length),"ok")}</div></div><div class="muted small">Asociados a la base activa: ${UI.esc(activeLabel)}.</div></div>
    <div class="card item"><div class="topline"><div class="name">Total presupuestado</div><div class="chips">${UI.chip(UI.fmtMoney(total,"COP"))}</div></div><div class="muted small">Suma de totales de la base activa.</div></div>
...
async function bindProjectsPage(){
  ...
  try{
    const initialBaseId = (window.APUBase && typeof APUBase.getDefaultBaseId === "function")
      ? String(APUBase.getDefaultBaseId() || "default").trim() || "default"
      : ((window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
          ? String(StorageAPI.getDefaultBaseId() || "default").trim() || "default"
          : "default");
    try{ localStorage.setItem("activeBaseId", initialBaseId); }catch(_){ }
    if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
      try{ StorageAPI.setDefaultBaseId(initialBaseId); }catch(_){ }
    }
    if(window.APUBase && typeof APUBase.setDefaultBaseId === "function"){
      try{ APUBase.setDefaultBaseId(initialBaseId); }catch(_){ }
    }
  }catch(_){ }

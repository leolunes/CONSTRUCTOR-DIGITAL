...
function getGlobalComparisonBaseId(){
  try{
    const v = String(
      localStorage.getItem("activeComparisonBaseId") ||
      localStorage.getItem("activeBaseId") ||
      localStorage.getItem("apu_base_default_id") ||
      ""
    ).trim();
    if(v) return v;
  }catch(_){ }
...
function setGlobalComparisonBaseId(baseId){
  ...
  try{
    ...
    localStorage.setItem("apu_base_default_id", next);
  }catch(_){ }
  try{
    if(window.StorageAPI && typeof StorageAPI.setDefaultBaseId === "function"){
      StorageAPI.setDefaultBaseId(next);
    }
  }catch(_){ }
  try{

...
function refreshProjectBudgetAwareUI(projectId){
  ...
  applyWorkspaceCollapsibles(projectId);
}


function getWorkspaceCollapseStorageKey(projectId, sectionKey){
  return `app_auditoria_workspace_collapse__${String(projectId||"global")}__${String(sectionKey||"section")}`;
}

function readWorkspaceCollapsed(projectId, sectionKey, defaultCollapsed=false){
  try{
    const raw = localStorage.getItem(getWorkspaceCollapseStorageKey(projectId, sectionKey));
...
function writeWorkspaceCollapsed(projectId, sectionKey, collapsed){
  try{
    localStorage.setItem(getWorkspaceCollapseStorageKey(projectId, sectionKey), collapsed ? "1" : "0");
  }catch(_){}
}

function ensureCardCollapsible(card, opts){
  const el = card instanceof Element ? card : null;
  if(!el) return;

  const sectionKey = String(opts?.sectionKey || "").trim() || "section";
  const title = String(opts?.title || "").trim() || "Sección";
  const projectId = String(opts?.projectId || "").trim() || "global";
  const defaultCollapsed = !!opts?.defaultCollapsed;
  const contentSelector = String(opts?.contentSelector || ":scope > *:not(.workspace-collapsible-head)");
  const forceCardClass = !!opts?.forceCardClass;

  if(forceCardClass && !el.classList.contains("card")) el.classList.add("card");

  let header = el.querySelector(':scope > .workspace-collapsible-head');
  let body = el.querySelector(':scope > .workspace-collapsible-body');

  if(!header){
    header = document.createElement("div");
    header.className = "workspace-collapsible-head row";
    ...
    p.textContent = "Click para minimizar o expandir.";
  ...
  if(!body){
    ...
    body.className = "workspace-collapsible-body";
    const nodes = Array.from(el.querySelectorAll(contentSelector)).filter(node => node !== header && node !== body);
    nodes.forEach(node => body.appendChild(node));
    el.appendChild(body);
  }

  const toggleBtn = header.querySelector("[data-collapse-toggle]");
  function applyState(collapsed){
    body.style.display = collapsed ? "none" : "";
    if(toggleBtn) toggleBtn.textContent = collapsed ? "► Expandir" : "▼ Minimizar";
    el.setAttribute("data-collapsed", collapsed ? "1" : "0");
  }

  let collapsed = readWorkspaceCollapsed(projectId, sectionKey, defaultCollapsed);
  applyState(collapsed);

  if(!header.dataset.boundCollapse){
    ...
    header.addEventListener("click", (e)=>{
      ...
      collapsed = !collapsed;
      writeWorkspaceCollapsed(projectId, sectionKey, collapsed);
      applyState(collapsed);
    });
  }
}

function resolveSectionCardFromControl(controlId){
  const ctrl = UI.qs("#" + controlId);
  if(!ctrl) return null;
  return ctrl.closest(".card") || ctrl.parentElement?.closest?.(".card") || ctrl.parentElement || null;
}

function applyWorkspaceCollapsibles(projectId){
  const pid = String(projectId || "global");

  const budgetToolbar = UI.qs("#budgetToolbar");
  if(budgetToolbar){
    ensureCardCollapsible(budgetToolbar, {
      projectId: pid,
      sectionKey: "doble_presupuesto",
      title: "Doble presupuesto",
      defaultCollapsed: false,
      contentSelector: ":scope > *:not(.workspace-collapsible-head)"
    });
  }

  const generationCard = resolveSectionCardFromControl("btnPdfPresupuesto");
  if(generationCard){
    ensureCardCollapsible(generationCard, {
      projectId: pid,
      sectionKey: "generacion",
      title: "GENERACIÓN",
      defaultCollapsed: false,
      contentSelector: ":scope > *:not(.workspace-collapsible-head)"
    });
  ...
  const analysisCard =
    resolveSectionCardFromControl("btnVerFormulario") ||
    UI.qs("#budgetComparisonMount");
  if(analysisCard){
    ensureCardCollapsible(analysisCard, {
      projectId: pid,
      sectionKey: "analisis",
      title: "ANÁLISIS",
      defaultCollapsed: false,
  ...
  const exportCard =
    resolveSectionCardFromControl("btnExportExcel") ||
    resolveSectionCardFromControl("btnExportProj");
  if(exportCard){
    ensureCardCollapsible(exportCard, {
      projectId: pid,
      sectionKey: "exportacion",
      title: "EXPORTACIÓN",

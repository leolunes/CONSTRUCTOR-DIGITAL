...
function normalizeProject(p){
  ...
  /* =========================
     DOBLE PRESUPUESTO (FASE 1 SEGURA)
     ========================= */
  try{
    if(!proj.activeBudget) proj.activeBudget = "base";

    if(!proj.budgetBase){
      proj.budgetBase = {
        items: Array.isArray(proj.items) ? proj.items : [],
        apuOverrides: proj.apuOverrides || {},
        subApuOverrides: proj.subApuOverrides || {},
        indirectMode: proj.indirectMode || "manual",
        indirectTable: proj.indirectTable || []
      };
    }

    if(!proj.budgetOfficial){
      proj.budgetOfficial = null;
    }

    if(proj.budgetBase && Array.isArray(proj.budgetBase.items)){
      proj.items = proj.budgetBase.items;
    }

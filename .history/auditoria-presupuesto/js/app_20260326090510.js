...
async function bindAPUPage(){
  ...
  let apu = null;
  let apuCode = normalizeLookupCode(codeParam);
  let displayCode = apuCode;
  let resolvedVisibleCode = apuCode;
  let resolvedRefCode = apuCode;
  let resolvedItem = null;

  if(apuCode){
    const resolved = await tryResolveAPUFromProjectItem(projectId, apuCode);

    resolvedItem = resolved?.item || null;
    resolvedVisibleCode = normalizeLookupCode(resolved?.visibleCode || apuCode);
    resolvedRefCode = normalizeLookupCode(resolved?.refCode || apuCode);

    if(resolved?.source === "override" && resolved?.data){
      const ov = resolved.data;
      apuCode = normalizeLookupCode(resolved.resolvedCode || resolvedRefCode || resolvedVisibleCode || apuCode);
      displayCode = resolvedVisibleCode || apuCode;

      apu = {
        title: (displayCode && displayCode !== apuCode) ? `APU ${displayCode} (Ref: ${apuCode})` : `APU ${apuCode}`,
        subtitle: "(Override del proyecto)",
        header: `${displayCode || apuCode} — Override del proyecto`,
    ...
    if(!apu && resolved?.source === "custom" && resolved?.data){
      ...
      apuCode = normalizeLookupCode(resolved.resolvedCode || resolvedRefCode || resolvedVisibleCode || apuCode);
      displayCode = resolvedVisibleCode || custom.code || apuCode;

      const directo = (custom.lines||[]).reduce((s,l)=> s + Number(l.parcial||0), 0);
      apu = {
        title: (displayCode && displayCode !== apuCode) ? `APU ${displayCode} (Ref: ${apuCode})` : `APU ${custom.code || apuCode}`,
        subtitle: custom.desc || "",
        header: `${displayCode || custom.code || apuCode} — ${custom.desc||""}`,
        metaLine: `APU creado en la app · Capítulo ${custom.chapterCode||"-"} ${custom.chapterName||""}`,
        unit: custom.unit || resolvedItem?.unit || "",
        directo,
    ...
    if(!apu && resolved?.source === "base" && resolved?.data){
      ...
      if(apu){
        if(displayCode && displayCode !== apuCode){
          apu.title = `APU ${displayCode} (Ref: ${apuCode})`;
          apu.header = `${displayCode} — ${apu.header || apu.desc || ""}`;
        }
      }
    }

    if(!apu && resolvedRefCode && resolvedVisibleCode && resolvedRefCode !== resolvedVisibleCode){
      try{
        const fallbackBase = await APUBase.getAPU(resolvedVisibleCode);
        if(fallbackBase){
          apuCode = resolvedVisibleCode;
          displayCode = resolvedVisibleCode;
          apu = fallbackBase;
        }
      }catch(_){}
    }
  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", ()=>{
      ...
      localLines.push({ group, desc, unit, qty, pu, parcial: qty*pu, subRef:"" });
      renderLines();
    });
    ...
    btnSaveOverride?.addEventListener("click", ()=>{
      ...
      const saveKey = normalizeLookupCode(resolvedRefCode || apuCode || resolvedVisibleCode || codeParam);

      StorageAPI.setApuOverride(projectId, saveKey, cleaned);

      const newPU = computeDirecto(cleaned);
      const count = updateItemsPUByApuCompat(projectId, saveKey, newPU);
    ...
    btnResetOverride?.addEventListener("click", async ()=>{
      ...
      const clearKeys = [
        normalizeLookupCode(resolvedRefCode),
        normalizeLookupCode(apuCode),
        normalizeLookupCode(resolvedVisibleCode),
        normalizeLookupCode(codeParam)
      ].filter(Boolean);

      const uniqueKeys = [...new Set(clearKeys)];
      uniqueKeys.forEach(k=>{
        try{ StorageAPI.clearApuOverride(projectId, k); }catch(_){}
      });

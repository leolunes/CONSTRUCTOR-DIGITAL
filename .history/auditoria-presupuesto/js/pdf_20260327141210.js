...
async function getSubproductoAPU(project, subKey){
  ...
  async function buildLaborBreakdownForLine(ln){
    ...
    for(const part of crew){
      const rate = await findLaborRateByRole(part.role);
      if(rate > 0){
  ...
  async function drawLaborRoleSubproductBreakdown(doc, L, roleRow, moCtx){
    const sub = await resolveLaborRoleSubproduct(roleRow?.desc || "");
    if(!sub || !Array.isArray(sub.lines) || !sub.lines.length) return;

    L.ensure(48);

    doc.setFont("helvetica","bold");
    doc.setFontSize(9.8);
    doc.text(`SUBPRODUCTO: ${safe(sub.subtitle || sub.header || roleRow?.desc || "")}`, L.margin + 18, L.getY());
    L.setY(L.getY() + 14);

    doc.setFont("helvetica","normal");
    doc.setFontSize(10);
    if(moCtx){
      L.p(`Ítem origen: ${safe(moCtx.itemCode || "")} — ${safe(moCtx.itemDesc || "")}`);
    }

    const fuente = safe(sub.metaLine || (sub.mode === "sub_override" ? "Fuente: Override del proyecto" : "Fuente: Subproductos"));
    L.p(fuente);
    L.p(`Costo directo subproducto: ${moneyCOP(sub.directo||0)}`);
  ...
  async function drawLaborBreakdownUnderRow(doc, L, ln){
    ...
    for(const r of rows){
      ...
      await drawLaborRoleSubproductBreakdown(doc, L, r, ln.__moCtx || null);
    }

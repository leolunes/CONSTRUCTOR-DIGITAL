...
function editIndirectDetailRow(projectId, rowId){
  ...
  const sourceTypeRaw = String(prompt(
    "Origen de la línea:\nmanual / insumo / subproducto / percent_cd",
    cur.sourceType || "manual"
  ) ?? cur.sourceType).trim().toLowerCase();

  const sourceType = ["manual","insumo","subproducto","percent_cd"].includes(sourceTypeRaw)
    ? sourceTypeRaw
    : "manual";

  let unit = cur.unit || "";
  let qty = Number(cur.qty || 0);
  let unitValue = Number(cur.unitValue || 0);
  let percent = Number(cur.percent || 0);
  let calcType = cur.calcType || "fixed";
  let sourceRef = cur.sourceRef || "";
  let sourceLabel = cur.sourceLabel || "";
  ...
  if(sourceType === "percent_cd"){
    ...
    qty = 1;
    unitValue = directo;
    percent = Number(prompt("Porcentaje sobre costo directo (%CD):", String(cur.percent || 0)) ?? cur.percent || 0);
    calcType = "percent_direct";
    sourceRef = "__CD__";
    sourceLabel = "Costo Directo del proyecto";
  }else{
    unit = String(prompt("Unidad:", cur.unit || "") ?? cur.unit).trim();
    qty = Number(prompt("Cantidad:", String(cur.qty || 0)) ?? cur.qty);
    unitValue = Number(prompt("VR Unitario:", String(cur.unitValue || 0)) ?? cur.unitValue);

    const calcTypeRaw = String(prompt(
      "Tipo de cálculo:\nfixed / percent_direct",
      cur.calcType || "fixed"
    ) ?? cur.calcType).trim().toLowerCase();

    calcType = (calcTypeRaw === "percent_direct") ? "percent_direct" : "fixed";
    percent = Number(prompt("Porcentaje (% directo):", String(cur.percent || 0)) ?? cur.percent);

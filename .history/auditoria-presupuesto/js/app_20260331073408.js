...
function ensureIndirectTableMount(form){
  ...
  host.innerHTML = `
    ...
    <div id="indirectTableWrap" style="display:none">
      ...
      <div id="indirectGroupTotals" class="grid two" style="margin-top:12px"></div>
    </div>
  `;
...
function renderIndirectSummary(projectId){
  ...
  const totalsBox = UI.qs("#indirectGroupTotals");
    ...
    box.innerHTML = `
      <div class="grid two">
        <div>
          ...
          <div class="row space"><div class="name">% sobre costos directos</div><div><b>${Number(s.totalPct||0).toFixed(2)}%</b></div></div>
          <div class="row space"><div class="name">Costos directos base</div><div><b>${UI.fmtMoney(totals.directo||0, moneda)}</b></div></div>
        </div>
        <div>
  ...
  if(totalsBox){
    totalsBox.innerHTML = `
      <div class="card item">
        ...
        <div style="margin-top:6px"><b>${UI.fmtMoney(adminVal, moneda)}</b></div>
        <div class="muted small" style="margin-top:4px">${Number(s.adminPct||0).toFixed(2)}% del costo directo</div>
      </div>

      <div class="card item">
        <div class="name">Imprevistos</div>
        <div style="margin-top:6px"><b>${UI.fmtMoney(imprevVal, moneda)}</b></div>
        <div class="muted small" style="margin-top:4px">${Number(s.imprevPct||0).toFixed(2)}% del costo directo</div>
      </div>

      <div class="card item">
        <div class="name">Utilidad</div>
        <div style="margin-top:6px"><b>${UI.fmtMoney(utilVal, moneda)}</b></div>
        <div class="muted small" style="margin-top:4px">${Number(s.utilPct||0).toFixed(2)}% del costo directo</div>
      </div>

      <div class="card item">
        <div class="name">Otros</div>
        <div style="margin-top:6px"><b>${UI.fmtMoney(otrosVal, moneda)}</b></div>
        <div class="muted small" style="margin-top:4px">${Number(s.othersPct||0).toFixed(2)}% del costo directo</div>
      </div>
    `;
  }
}

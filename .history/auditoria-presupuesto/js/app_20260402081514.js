  ...
  if(projectId && apuCode){
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        let filtered = baseList.filter(x =>
          normalizeTextForMatch(x?.subName || "").includes(q) ||
          normalizeTextForMatch(x?.subKey || "").includes(q)
        );
        ...
        if(!filtered.length){
          ...
          for(const item of baseList){
            try{
              const subDet = await APUBase.getSubAPU(item.subKey);
              const haystack = [
                item?.subName || "",
                item?.subKey || "",
                subDet?.title || "",
                ...
                subDet?.metaLine || "",
                subDet?.unit || "",
                ...(Array.isArray(subDet?.lines)
                  ? subDet.lines.flatMap(l => [l?.group || l?.tipo || "", l?.desc || "", l?.unit || ""])
                  : [])
              ].map(v => normalizeTextForMatch(v)).join(" | ");

              if(haystack.includes(q)){
                deepMatches.push(item);
              }
            }catch(_){ }
          }

          filtered = deepMatches;
        }

        filtered = filtered
          .filter((x, i, arr) => arr.findIndex(y => String(y?.subKey || "") === String(x?.subKey || "")) === i)
          .sort((a,b)=> String(a?.subName || a?.subKey || "").localeCompare(String(b?.subName || b?.subKey || ""), "es", { numeric:true, sensitivity:"base" }));

        if(!filtered.length){
          alert(`No se encontraron subproductos para la búsqueda: ${term}`);
          return;
        }

        const limited = filtered.slice(0, 30);

        const menu = limited.map((r, i)=>
...
Mostrando ${limited.length} de ${filtered.length} coincidencias.`, "1") || "0");
        if(!(pick >= 1 && pick <= limited.length)) return;

        const sel = limited[pick - 1];

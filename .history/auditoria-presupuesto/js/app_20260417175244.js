...
function getEffectiveActiveBaseId(){
  try{
    const selected = String(UI.qs("#selectBase")?.value || "").trim();
    if(selected) return selected;
  }catch(_){}

  try{
    const ls = String(localStorage.getItem("activeBaseId") || "").trim();
    if(ls) return ls;
  }catch(_){}

  try{
    if(window.APUBase && typeof APUBase.getDefaultBaseId === "function"){
      const apuId = String(APUBase.getDefaultBaseId() || "").trim();
      if(apuId) return apuId;
    }
  }catch(_){}

  try{
    if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      const storeId = String(StorageAPI.getDefaultBaseId() || "").trim();
...
function syncActiveBaseSelection(baseId, opts={}){
  const next = String(baseId || "default").trim() || "default";
  const shouldReload = opts && opts.reload === true;
...
function renderBaseSelector(){
  ...
  const current = getEffectiveActiveBaseId();
  ...
  sel.value = current;
  syncActiveBaseSelection(current);

  sel.onchange = ()=>{
    const next = String(sel.value || "default").trim() || "default";
    syncActiveBaseSelection(next, { reload:true });
  };
}

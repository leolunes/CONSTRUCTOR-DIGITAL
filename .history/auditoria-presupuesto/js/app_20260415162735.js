...
function getBudgetItemComparisonRowsByModes(project, leftMode, rightMode){
  ...
  function itemCompareKey(it){
    return String(it?.apuRefCode || it?.code || it?.desc || it?.descripcion || "").trim();
  }
  ...
  leftItems.forEach(it=>{
    ...
    map.set(key, {
      ...
      desc: itemDesc(it),
      leftQty: Number(it?.qty || 0),
      leftPU: Number(it?.pu || 0),
      rightQty: 0,
      rightPU: 0
    });
  });
  rightItems.forEach(it=>{
    ...
    if(!map.has(key)){
      map.set(key, {
        ...
        leftQty: 0,
        leftPU: 0,
        rightQty: Number(it?.qty || 0),
        rightPU: Number(it?.pu || 0)
      });
      return;
    }
    const row = map.get(key);
    row.rightQty = Number(it?.qty || 0);
    row.rightPU = Number(it?.pu || 0);
    if(!row.code) row.code = String(it?.code || "").trim();
    if(!row.desc) row.desc = itemDesc(it);
  });

  return Array.from(map.values()).map(r=>{
    const leftPartial = Number(r.leftQty || 0) * Number(r.leftPU || 0);
    const rightPartial = Number(r.rightQty || 0) * Number(r.rightPU || 0);
    const diff = rightPartial - leftPartial;
    const pct = Number(leftPartial || 0) > 0 ? (diff / Number(leftPartial || 0)) * 100 : (Number(rightPartial || 0) > 0 ? 100 : 0);
    return { ...r, leftPartial, rightPartial, diff, pct };
  }).sort((a,b)=> Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)));
}
...
(function boot(){
  ...
  /* =========================
     ✅ BRIDGE FINAL: APUs e insumos personalizados sí deben aparecer en búsquedas
     ========================= */
  function __pproInstallCustomCatalogFallbacks(){
    if(!window.StorageAPI || window.__pproCustomCatalogReady) return;
    window.__pproCustomCatalogReady = true;
    ...
    function computePu(lines){
      return safeArr(lines).reduce((sum, l)=>{
        const qty = Number(l?.qty || 0);
        const pu = Number(l?.pu || 0);
        const parcial = Number(l?.parcial || 0) || (qty * pu);
    ...
    if(typeof StorageAPI.getCustomAPU !== "function"){
      StorageAPI.getCustomAPU = function(code){
        const key = norm(code);
        return StorageAPI.listCustomAPUs().find(x => norm(x?.code) === key) || null;
      };
    }
    if(typeof StorageAPI.saveCustomAPU !== "function"){
      StorageAPI.saveCustomAPU = function(payload){
        ...
        const all = StorageAPI.listCustomAPUs().filter(x => norm(x?.code) !== code);
        const lines = safeArr(payload?.lines).map(l => ({
        ...
        all.push({
          ...
          updatedAt: new Date().toISOString(),
          pu: Number(payload?.pu || 0) || computePu(lines),
          lines
        });
    ...
    window.__pproCustomCatalogHelpers = {
      normTextLocal,
      computePu
    };
  }

  async function __pproRunCustomApuSearch(){
    __pproInstallCustomCatalogFallbacks();
    const tbody = UI.qs("#apuResultsBody");
    const empty = UI.qs("#apuResultsEmpty");
    const input = UI.qs("#apuSearch");
    const projectId = UI.getParam("projectId");
    const project = projectId ? StorageAPI.getProjectById(projectId) : null;
    if(!tbody || !project) return;

    const term = String(input?.value || "").trim();
    const q = (window.__pproCustomCatalogHelpers?.normTextLocal || (v => String(v||"").toLowerCase().trim()))(term);

    let baseRows = [];
    try{
      if(window.APUBase?.listFormularioItems){
        baseRows = await APUBase.listFormularioItems(term || "");
      }
    }catch(_){}

    baseRows = (Array.isArray(baseRows) ? baseRows : [])
      .filter(r => !r?.isChapter)
      .map(r => ({
        sourceTag: "BASE",
        sourceType: "base",
        code: String(r.code || "").trim(),
        desc: String(r.desc || r.descripcion || "").trim(),
        unit: String(r.unit || r.unidad || "").trim(),
        pu: Number(r.pu || 0),
    ...
    let customRows = [];
    try{
      customRows = (typeof StorageAPI.listCustomAPUs === "function" ? (StorageAPI.listCustomAPUs() || []) : []);
    }catch(_){
      customRows = [];
    }

    customRows = customRows.map(r => ({
      sourceTag: "PERSONALIZADO",
      sourceType: "custom",
      code: String(r.code || "").trim(),
      desc: String(r.desc || "").trim(),
      unit: String(r.unit || "").trim(),
      pu: Number(r.pu || 0) || Number(window.__pproCustomCatalogHelpers?.computePu?.(r.lines) || 0),
      chapterCode: String(r.chapterCode || "").trim(),
      chapterName: String(r.chapterName || "").trim(),
    ...
    if(q){
      rows = rows.filter(r =>
        [r.code, r.desc, r.unit, r.chapterCode, r.chapterName, r.sourceTag]
          .some(v => (window.__pproCustomCatalogHelpers?.normTextLocal || (x => String(x||"").toLowerCase().trim()))(v).includes(q))
      );
    }

    rows.sort((a,b)=>{
      if(a.sourceType !== b.sourceType) return a.sourceType === "custom" ? -1 : 1;
      return String(a.code || "").localeCompare(String(b.code || ""), "es", { numeric:true, sensitivity:"base" });
    });
    ...
    tbody.innerHTML = rows.map((r, idx)=>`
      <tr>
        <td><b>${UI.esc(r.code || "")}</b></td>
        <td>${UI.esc(r.desc || "")}<div class="muted small">[${UI.esc(r.sourceTag)}]</div></td>
        <td>${UI.esc(r.unit || "")}</td>
        <td style="text-align:right"><b>${UI.fmtMoney(r.pu || 0, project.currency || "COP")}</b></td>
        <td><button class="btn primary" type="button" data-custom-apu-pick="${idx}">Agregar</button></td>
      </tr>
    `).join("");

    tbody.querySelectorAll("[data-custom-apu-pick]").forEach(btn=>{
      btn.onclick = ()=>{
        const idx = Number(btn.getAttribute("data-custom-apu-pick"));
        const apu = rows[idx];
        if(!apu) return;

        if(typeof openAddApuModal === "function"){
          openAddApuModal(project, {
            code: apu.code,
            apuRefCode: apu.code,
            desc: apu.desc,
            unit: apu.unit,
        ...
        const fresh = StorageAPI.getProjectById(project.id);
        if(typeof refreshProjectBudgetAwareUI === "function" && fresh) refreshProjectBudgetAwareUI(fresh.id);
        alert("Ítem agregado al presupuesto.");
      };
    });
  }

  async function __pproRunCustomInsumoSearch(){
    __pproInstallCustomCatalogFallbacks();
    const tbody = UI.qs("#insumoResultsBody");
    const empty = UI.qs("#insumoResultsEmpty");
    const input = UI.qs("#insumoSearch");
    if(!tbody) return;

    const term = String(input?.value || "").trim();
    const normTextLocal = window.__pproCustomCatalogHelpers?.normTextLocal || (v => String(v||"").toLowerCase().trim());
    const q = normTextLocal(term);

    let baseInsumos = [];
    let baseSubs = [];
    try{ if(window.APUBase?.listInsumos) baseInsumos = await APUBase.listInsumos(""); }catch(_){}
    try{ if(window.APUBase?.listSubproductos) baseSubs = await APUBase.listSubproductos(); }catch(_){}

    let customInsumos = [];
    try{ customInsumos = (typeof StorageAPI.listCustomInsumos === "function" ? (StorageAPI.listCustomInsumos() || []) : []); }catch(_){}

    const rows = [
      ...customInsumos.map(r => ({
        sourceTag: "PERSONALIZADO",
        kind: "INSUMO",
        tipo: String(r.tipo || "MATERIALES").trim(),
        desc: String(r.desc || "").trim(),
        unit: String(r.unit || "").trim(),
        pu: Number(r.pu || 0)
      })),
      ...
      ...(Array.isArray(baseSubs) ? baseSubs : []).map(r => ({
        sourceTag: "BASE",
        kind: "SUBPRODUCTO",
        tipo: "SUBPRODUCTO",
        desc: String(r.subName || r.name || r.subKey || "").trim(),
        unit: String(r.unit || "GLB").trim(),
        pu: Number(r.pu || 0),
        subRef: String(r.subKey || "").trim()
      }))
    ].filter(r => !q || [r.kind, r.tipo, r.desc, r.unit, r.sourceTag, r.subRef].some(v => normTextLocal(v).includes(q)));
    ...
    tbody.innerHTML = rows.slice(0, 50).map((r, idx)=>`
      <tr>
        <td>${UI.esc(r.kind)} <div class="muted small">[${UI.esc(r.sourceTag)}]</div></td>
        <td>${UI.esc(r.desc || "")}</td>
        <td>${UI.esc(r.unit || "")}</td>
        <td style="text-align:right"><b>${UI.fmtMoney(r.pu || 0, "COP")}</b></td>
        <td><button class="btn primary" type="button" data-custom-insumo-pick="${idx}">Agregar</button></td>
      </tr>
    `).join("");

    window.__pproCreateApuSearchRows = rows.slice(0,50);
    tbody.querySelectorAll("[data-custom-insumo-pick]").forEach(btn=>{
      btn.onclick = ()=>{
        const idx = Number(btn.getAttribute("data-custom-insumo-pick"));
        const row = (window.__pproCreateApuSearchRows || [])[idx];
        if(!row) return;
        // dejar que el flujo existente haga el resto si ya existe algún handler externo
        btn.setAttribute("data-picked", "1");
      };
    });
  }

  function __pproInstallSearchBridges(){
    __pproInstallCustomCatalogFallbacks();
    if(page() !== "proyecto-detalle.html") return;

    const apuBtn = UI.qs("#btnApuSearch");
    const apuInput = UI.qs("#apuSearch");
    if(apuBtn && !apuBtn.__pproBridgeBound){
      apuBtn.__pproBridgeBound = true;
      apuBtn.addEventListener("click", function(){
        setTimeout(()=>{ __pproRunCustomApuSearch().catch(()=>{}); }, 0);
      }, true);
    }
    if(apuInput && !apuInput.__pproBridgeBound){
      apuInput.__pproBridgeBound = true;
      apuInput.addEventListener("keydown", function(e){
        if(e.key === "Enter"){
          e.preventDefault();
          setTimeout(()=>{ __pproRunCustomApuSearch().catch(()=>{}); }, 0);
        }
    ...
    const insBtn = UI.qs("#btnInsumoSearch");
    const insInput = UI.qs("#insumoSearch");
    if(insBtn && !insBtn.__pproBridgeBound){
      insBtn.__pproBridgeBound = true;
      insBtn.addEventListener("click", function(){
        setTimeout(()=>{ __pproRunCustomInsumoSearch().catch(()=>{}); }, 0);
      }, true);
    }
    if(insInput && !insInput.__pproBridgeBound){
      insInput.__pproBridgeBound = true;
      insInput.addEventListener("keydown", function(e){
        if(e.key === "Enter"){
          e.preventDefault();
          setTimeout(()=>{ __pproRunCustomInsumoSearch().catch(()=>{}); }, 0);
        }
  ...
  setTimeout(__pproInstallSearchBridges, 0);
  document.addEventListener("click", function(ev){
    const t = ev.target;
    if(t && (t.id === "btnApuSearch" || t.id === "btnInsumoSearch")){
      setTimeout(__pproInstallSearchBridges, 0);
    }
  }, true);

...
function renderIndirectRows(projectId){
  ...
  const rows = (baseRows || []).map((r, i)=>({ ...r, ...(calcRows[i] || {}) }));
  ...
  tbody.innerHTML = rows.map((row, idx)=>{
    const r = normalizeIndirectUiRow(row);
    const itemCode = UI.esc(r.item || "");
    const group = UI.esc(r.group || "");
    const desc = UI.esc(r.desc || "");
    const unit = UI.esc(r.unit || "");
    const qty = UI.esc(String(r.qty || 0));
    const unitValue = UI.fmtMoney(r.unitValue || 0, project.currency || "COP");
    const percent = Number(r.percent || 0).toFixed(2);
    const partial = UI.fmtMoney(calcIndirectRowPartial(r, directo), project.currency || "COP");
    const level = indirectItemLevel(r.item);
    const indent = Math.max(0, level * 18);

    if(r.rowType === "header"){
      return `
        <tr data-indirect-id="${UI.esc(r.id || ("ind_" + idx))}" data-row-type="header" data-item-code="${itemCode}" data-group-code="${group}" data-desc-text="${desc}">
          <td><b>${itemCode}</b></td>
          <td>${group}</td>
          <td colspan="5"><div style="margin-left:${indent}px"><b>${desc}</b></div></td>
    ...
    return `
      <tr data-indirect-id="${UI.esc(r.id || ("ind_" + idx))}" data-row-type="detail" data-source-ref="${UI.esc(r.sourceRef || "")}" data-source-label="${UI.esc(r.sourceLabel || "")}">
        <td>${itemCode}</td>
        <td>${group}</td>
        <td><div style="margin-left:${indent}px">${desc}</div></td>
        <td>${unit}</td>
        <td style="text-align:right">${qty}</td>
        <td style="text-align:right"><b>${unitValue}</b></td>
        <td style="text-align:right">${percent}</td>
        <td style="text-align:right"><b>${partial}</b></td>
        <td>
          <div class="row" style="gap:8px; flex-wrap:wrap">

...
function renderIndirectRows(projectId){
  ...
  let calcRows = [];
  try{
    if(window.Calc && typeof Calc.calcIndirectTable === "function"){
      const calcRes = Calc.calcIndirectTable(project, directo) || {};
      calcRows = Array.isArray(calcRes.rows) ? calcRes.rows : [];
    }
  }catch(_){}
  ...
  tbody.innerHTML = rows.map((row, idx)=>{
    ...
    return `
      ...
      >
        <td><input class="input" data-field="item" value="${UI.esc(r.item || String(idx + 1))}" placeholder="1.2.1.1"></td>
        <td>

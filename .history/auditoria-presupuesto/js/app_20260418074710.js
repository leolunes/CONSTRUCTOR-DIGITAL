...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  ...
  let meta = null;
  const project = StorageAPI.getProjectById(projectId);
  const baseId = getEffectiveBaseIdForProject(project || projectId);
  ...
  if(type === "insumo"){
    let rows = [];
    try{
      rows = await APUBase.listInsumos(term, 40, baseId);
    }catch(err){
  ...
  let list = [];
  try{
    list = await APUBase.listSubproductos(200, baseId);
  }catch(err){
  ...
  let subApu = null;
  try{
    subApu = await APUBase.getSubAPU(sel.subKey, baseId);
  }catch(err){
...
function bindBaseViewer(projectId){
  ...
  async function requireBase(){
    const project = (window.StorageAPI && typeof StorageAPI.getProjectById === "function")
      ? StorageAPI.getProjectById(projectId)
      : null;
    const baseId = getEffectiveBaseIdForProject(project || projectId);
  ...
  async function loadRows(q){
    ...
    try{
      if(mode === "formulario"){
        const project = (window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectId) : null;
        const baseId = getEffectiveBaseIdForProject(project || projectId);
        const rows = await APUBase.listFormularioItems(q || "", baseId);
      ...
      if(mode === "cd"){
        const project = (window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectId) : null;
        const baseId = getEffectiveBaseIdForProject(project || projectId);
        const rows = await APUBase.listCostosDirectosAll(q || "", baseId);
        if(!rows.length){ showEmpty(true); return; }
      ...
      if(mode === "sub"){
        const project = (window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectId) : null;
        const baseId = getEffectiveBaseIdForProject(project || projectId);
        const list = await APUBase.listSubproductos(500, baseId);
      ...
      if(mode === "insumos"){
        const project = (window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectId) : null;
        const baseId = getEffectiveBaseIdForProject(project || projectId);
        const rows = await APUBase.listInsumos(q || "", 200, baseId);
        if(!rows.length){ showEmpty(true); return; }
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      ...
      if(mode === "2" || mode === "3"){
        ...
        if(mode === "2"){
          ...
          let rows = [];
          try{
            rows = await APUBase.listInsumos("");
          }catch(err){
            alert("Error consultando insumos: " + (err?.message || err));
            return;
          }

...
function normalizeBudgetModeRuntime(v){
  ...
  return raw === "official" || raw === "oficial" ? "official"
    : (raw === "artificial" || raw === "artificialmente_bajo" || raw === "artificialmente-bajo" ? "artificial"
    : "base");
}

function storageBudgetKey(mode){
  const normalized = normalizeBudgetModeRuntime(mode);
  return normalized === "official" ? "oficial"
    : (normalized === "artificial" ? "artificial" : "base");
}
...
function ensureProjectBudgetsRuntime(project){
  ...
  if(p.budgets.artificial != null && (typeof p.budgets.artificial !== "object" || Array.isArray(p.budgets.artificial))){
    p.budgets.artificial = null;
  }
...
function getProjectBudgetRuntime(project, mode){
  ...
  if(key === "oficial") return buds.oficial || null;
  if(key === "artificial") return buds.artificial || null;
  return buds.base || null;
}
...
function setProjectActiveBudgetRuntime(projectId, mode){
  ...
  const targetBudget = targetKey === "oficial" ? nextBudgets.oficial : (targetKey === "artificial" ? nextBudgets.artificial : nextBudgets.base);
...
function getBudgetModeLabel(mode){
  const normalized = normalizeBudgetModeRuntime(mode);
  return normalized === "official" ? "OFICIAL"
    : (normalized === "artificial" ? "ARTIFICIALMENTE BAJO" : "BASE");
}
...
function getBudgetModeBadge(mode){
  ...
  return normalized === "official"
    ...
    : (normalized === "artificial"
      ? UI.chip("PRESUPUESTO ARTIFICIALMENTE BAJO", "warn")
      : UI.chip("PRESUPUESTO BASE", "ok"));
}

function getProjectBaseId(project){
  return String(project?.baseId || "default").trim() || "default";
}

function getProjectComparisonBaseStorageKey(projectId){
  return `ppro_compare_base__${String(projectId || "global").trim() || "global"}`;
}

function getGlobalComparisonBaseId(){
  try{
    const v = String(localStorage.getItem("activeComparisonBaseId") || localStorage.getItem("activeBaseId") || "").trim();
    if(v) return v;
  }catch(_){ }
  try{
    if(window.APUBase && typeof APUBase.getDefaultBaseId === "function"){
      const v = String(APUBase.getDefaultBaseId() || "").trim();
      if(v) return v;
    }
  }catch(_){ }
  try{
    if(window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function"){
      const v = String(StorageAPI.getDefaultBaseId() || "").trim();
      if(v) return v;
    }
  }catch(_){ }
  return "default";
...
function getProjectComparisonBaseId(projectOrId){
  const project = (typeof projectOrId === "string")
    ? ((window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectOrId) : null)
    : projectOrId;

  const projectId = String((typeof projectOrId === "string" ? projectOrId : project?.id) || "").trim();

  if(projectId){
    try{
      if(window.StorageAPI && typeof StorageAPI.getProjectComparisonBaseId === "function"){
        const stored = String(StorageAPI.getProjectComparisonBaseId(projectId) || "").trim();
        if(stored) return stored;
      }
    }catch(_){ }

    try{
      const saved = String(localStorage.getItem(getProjectComparisonBaseStorageKey(projectId)) || "").trim();
      if(saved) return saved;
    }catch(_){ }
  }

  if(project && project.comparisonBaseId){
    const fromProject = String(project.comparisonBaseId || "").trim();
    if(fromProject) return fromProject;
  }

  const globalBase = getGlobalComparisonBaseId();
  if(globalBase) return globalBase;

  const originBase = getProjectBaseId(project);
  if(originBase) return originBase;

  return "default";
}

function setGlobalComparisonBaseId(baseId){
  const next = String(baseId || "default").trim() || "default";
  try{
    localStorage.setItem("activeComparisonBaseId", next);
    localStorage.setItem("activeBaseId", next);
  }catch(_){ }
  try{
    if(window.APUBase && typeof APUBase.setDefaultBaseId === "function"){
      APUBase.setDefaultBaseId(next);
    }
  }catch(_){ }
  return next;
}

function setProjectComparisonBaseId(projectId, baseId){
  const next = String(baseId || "default").trim() || "default";
  const id = String(projectId || "").trim();
  if(id){
    try{
      if(window.StorageAPI && typeof StorageAPI.setProjectComparisonBaseId === "function"){
        StorageAPI.setProjectComparisonBaseId(id, next);
      }
    }catch(_){ }

    try{
      localStorage.setItem(getProjectComparisonBaseStorageKey(id), next);
    }catch(_){ }
  }
  setGlobalComparisonBaseId(next);
  return next;
}

function getEffectiveBaseIdForProject(projectOrId){
  return getProjectComparisonBaseId(projectOrId);
}


function getActiveReadBaseIdForProject(projectOrId){
  const baseId = String(getProjectComparisonBaseId(projectOrId) || getProjectBaseId(
    typeof projectOrId === "string"
      ? ((window.StorageAPI && typeof StorageAPI.getProjectById === "function") ? StorageAPI.getProjectById(projectOrId) : null)
      : projectOrId
  ) || getGlobalComparisonBaseId() || "default").trim();
  return baseId || "default";
}


function getInstalledBasesForUi(){
  try{
    if(window.StorageAPI && typeof StorageAPI.listInstalledBases === "function"){
      const raw = StorageAPI.listInstalledBases() || [];
      return (Array.isArray(raw) ? raw : []).map((b, idx)=>({
        id: String(b?.id || b?.baseId || "default").trim() || `base_${idx+1}`,
        name: String(b?.name || b?.nombre || b?.label || b?.periodo || b?.fileName || b?.filename || b?.id || b?.baseId || "Base APU").trim()
      }));
    }
  }catch(_){}
  return [{ id:"default", name:"Base actual" }];
}

function getBaseLabelById(baseId){
  const id = String(baseId || "default").trim() || "default";
  const list = getInstalledBasesForUi();
  const hit = list.find(b => String(b.id || "").trim() === id);
  return String(hit?.name || id || "default");
}

function getActiveBaseIdForUi(){
  return getGlobalComparisonBaseId();
}

function setActiveBaseIdForUi(baseId){
  return setGlobalComparisonBaseId(baseId);
}

function calcPUFromResolvedApuData(data, fallbackPU){
  if(data && Array.isArray(data.lines) && data.lines.length){
    return data.lines.reduce((sum, ln)=>{
      const partial = Number(ln?.parcial || 0) || (Number(ln?.qty || 0) * Number(ln?.pu || 0));
      return sum + Number(partial || 0);
    }, 0);
  }

  const directPU = Number(data?.pu);
  if(Number.isFinite(directPU)) return directPU;

  const unitValue = Number(data?.unitValue);
  if(Number.isFinite(unitValue)) return unitValue;

  return Number(fallbackPU || 0);
}

async function resolvePUForProjectItem(projectId, item){
  const lookupCode = String(item?.apuRefCode || item?.code || "").trim();
  if(!lookupCode) return Number(item?.pu || 0);

  const compareBaseId = getProjectComparisonBaseId(projectId);

  try{
    if(typeof tryResolveAPUFromProjectItem === "function"){
      const resolved = await tryResolveAPUFromProjectItem(projectId, lookupCode, compareBaseId);
      if(resolved?.data){
        return calcPUFromResolvedApuData(resolved.data, item?.pu);
      }
    }
  }catch(_){ }

  try{
    if(window.APUBase && typeof APUBase.getAPU === "function"){
      const apu = await APUBase.getAPU(lookupCode, compareBaseId);
      if(apu){
        return calcPUFromResolvedApuData(apu, item?.pu);
      }
    }
  }catch(_){ }
...
async function recalcItemListAgainstActiveBase(projectId, items){
  const arr = Array.isArray(items) ? deepCloneBudgetRuntime(items) : [];
  for(let i=0; i<arr.length; i++){
    arr[i].pu = await resolvePUForProjectItem(projectId, arr[i]);
  }
  return arr;
}

async function recalcProjectsForActiveBase(){
  if(!(window.StorageAPI && typeof StorageAPI.listProjects === "function" && typeof StorageAPI.getProjectById === "function" && typeof StorageAPI.updateProject === "function")){
    return;
  }

  const projects = StorageAPI.listProjects() || [];
  for(const p of projects){
    const fresh = StorageAPI.getProjectById(p.id);
    if(!fresh) continue;

    ensureProjectBudgetsRuntime(fresh);

    const currentKey = storageBudgetKey(fresh.activeBudget || "base");
    const nextBudgets = deepCloneBudgetRuntime(fresh.budgets || {});

    const baseSourceItems = Array.isArray(nextBudgets?.base?.items)
      ? nextBudgets.base.items
      : (Array.isArray(fresh.items) ? fresh.items : []);

    const recalculatedBaseItems = await recalcItemListAgainstActiveBase(fresh.id, baseSourceItems);

    if(!nextBudgets.base || typeof nextBudgets.base !== "object" || Array.isArray(nextBudgets.base)){
      nextBudgets.base = makeBudgetBranchFromProjectView(fresh);
    }
    nextBudgets.base.items = recalculatedBaseItems;

    const patch = {
      budgets: nextBudgets
    };

    if(currentKey === "base" || !Array.isArray(fresh.items) || !fresh.items.length){
      patch.items = deepCloneBudgetRuntime(recalculatedBaseItems);
    }

    StorageAPI.updateProject(fresh.id, patch);
  }
}

async function refreshUiAfterBaseChange(){
  try{ await renderKpis(); }catch(_){}
  try{ renderProjects(); }catch(_){}

  const projectId = (window.UI && typeof UI.getParam === "function")
    ? UI.getParam("projectId")
    : (new URL(location.href)).searchParams.get("projectId");

  if(projectId){
    try{
      refreshProjectBudgetAwareUI(projectId);
    }catch(_){
      try{
        const fresh = StorageAPI.getProjectById(projectId);
        if(fresh) renderProjectDetail(fresh);
      }catch(__){}
    }
  }
}
...
function renderBaseSelector(){
  const sel = UI.qs("#selectBase");
  if(!sel) return;

  const bases = getInstalledBasesForUi();
  const current = getActiveBaseIdForUi();

  sel.innerHTML = bases.map(b => `
    <option value="${UI.esc(b.id)}" ${String(b.id) === current ? "selected" : ""}>${UI.esc(b.name)}</option>
  `).join("");

  sel.onchange = async ()=>{
    const next = String(sel.value || "default").trim() || "default";
    setActiveBaseIdForUi(next);

    const projectId = (window.UI && typeof UI.getParam === "function")
      ? UI.getParam("projectId")
      : (new URL(location.href)).searchParams.get("projectId");
    if(projectId){
      setProjectComparisonBaseId(projectId, next);
    }

    try{
      await recalcProjectsForActiveBase();
    }catch(err){
      console.warn("[BaseComparacion] No se pudieron recalcular los proyectos con la nueva base:", err);
    }

    await refreshUiAfterBaseChange();
  };
}
...
function projectHasArtificialBudget(project){
  const fresh = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  const branch = getProjectBudgetRuntime(fresh, "artificial");
  return !!(branch && Array.isArray(branch.items) && branch.items.length);
}

function normalizeArtificialBudgetConfig(cfg){
  const raw = cfg || {};
  return {
    materialsPct: Number(raw.materialsPct ?? 0),
    laborPct: Number(raw.laborPct ?? 0),
    equipmentPct: Number(raw.equipmentPct ?? 0),
    transportPct: Number(raw.transportPct ?? 0),
    subcontractsPct: Number(raw.subcontractsPct ?? 0),
    othersPct: Number(raw.othersPct ?? 0),
    note: String(raw.note || "").trim(),
    updatedAt: String(raw.updatedAt || "")
  };
}

function getArtificialBudgetConfig(project){
  return normalizeArtificialBudgetConfig(project?.artificialBudgetConfig || {});
}

function getArtificialComponentPercent(cfg, component){
  const safe = normalizeArtificialBudgetConfig(cfg);
  const map = {
    "MATERIALES": Number(safe.materialsPct || 0),
    "MANO DE OBRA": Number(safe.laborPct || 0),
    "EQUIPO Y HERRAM": Number(safe.equipmentPct || 0),
    "TRANSPORTES": Number(safe.transportPct || 0),
    "SUBCONTRATOS": Number(safe.subcontractsPct || 0),
    "OTROS": Number(safe.othersPct || 0)
  };
  return Number(map[String(component || "").toUpperCase()] || 0);
}

function getArtificialAveragePercent(cfg){
  const safe = normalizeArtificialBudgetConfig(cfg);
  const vals = [
    Number(safe.materialsPct || 0),
    Number(safe.laborPct || 0),
    Number(safe.equipmentPct || 0),
    Number(safe.transportPct || 0),
    Number(safe.subcontractsPct || 0),
    Number(safe.othersPct || 0)
  ];
  return vals.reduce((a,b)=>a+b,0) / (vals.length || 1);
}

function ensureArtificialBudgetMount(){
  let host = UI.qs("#budgetArtificialMount");
  if(host) return host;

  const dashboard = UI.qs("#budgetAuditDashboardMount");
  const toolbar = UI.qs("#budgetToolbar");
  const anchor = dashboard || toolbar;
  if(!anchor || !anchor.parentElement) return null;

  host = document.createElement("section");
  host.id = "budgetArtificialMount";
  host.className = "card";
...
async function computeBudgetComponentTotals(projectView){
  const totals = {
    "MATERIALES": 0,
    "MANO DE OBRA": 0,
    "EQUIPO Y HERRAM": 0,
  ...
  const items = Array.isArray(projectView?.items) ? projectView.items : [];
  for(const item of items){
    const apu = await getAPUForBudgetItem(projectView, item);
    if(apu && Array.isArray(apu.lines) && apu.lines.length){
      apu.lines.forEach(ln=>{
        const comp = classifyAuditComponent(ln.group || ln.tipo || "");
        const partial = Number(ln.parcial || 0) || (Number(ln.qty || 0) * Number(ln.pu || 0));
        totals[comp] = Number(totals[comp] || 0) + Number(partial || 0);
      });
    }else{
      totals["OTROS"] = Number(totals["OTROS"] || 0) + (Number(item?.pu || 0) * Number(item?.qty || 0));
    }
  }
  return totals;
}

function cloneArtificialApuLines(lines, cfg){
  return (Array.isArray(lines) ? lines : []).map(ln=>{
    const copy = JSON.parse(JSON.stringify(ln || {}));
    const component = classifyAuditComponent(copy.group || copy.tipo || "");
    const pct = getArtificialComponentPercent(cfg, component);
    const factor = 1 + (Number(pct || 0) / 100);
    const qty = Number(copy.qty || 0);
    const pu = Number(copy.pu || 0) * factor;
    copy.pu = pu;
    copy.parcial = Number(copy.parcial || 0) ? Number(copy.parcial || 0) * factor : (qty * pu);
    return copy;
  });
}

async function buildArtificialBudgetBranch(project, cfg){
  const currentProject = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  if(!currentProject) throw new Error("Proyecto no encontrado.");

  ensureProjectBudgetsRuntime(currentProject);
  const baseView = getProjectViewForBudget(currentProject, "base");
  if(!baseView || !Array.isArray(baseView.items) || !baseView.items.length){
    throw new Error("El Presupuesto Base no tiene ítems para construir el presupuesto artificialmente bajo.");
  }

  const safeCfg = normalizeArtificialBudgetConfig(cfg);
  const branch = makeBudgetBranchFromProjectView(baseView);
  branch.apuOverrides = {};
  branch.items = [];
  const avgPct = getArtificialAveragePercent(safeCfg);

  for(const item of (baseView.items || [])){
    const clonedItem = JSON.parse(JSON.stringify(item || {}));
    const apuCode = String(clonedItem.apuRefCode || clonedItem.code || "").trim();
    const apu = await getAPUForBudgetItem(baseView, item);

    if(apu && Array.isArray(apu.lines) && apu.lines.length){
      const adjustedLines = cloneArtificialApuLines(apu.lines, safeCfg);
      const newPU = adjustedLines.reduce((sum, ln)=>{
        const partial = Number(ln.parcial || 0) || (Number(ln.qty || 0) * Number(ln.pu || 0));
        return sum + Number(partial || 0);
      }, 0);

      branch.apuOverrides[apuCode || clonedItem.code || makeId("apu")] = {
        code: apuCode || clonedItem.code || "",
        desc: String(apu.desc || clonedItem.desc || "").trim(),
        unit: String(apu.unit || clonedItem.unit || "").trim(),
        lines: adjustedLines
      };
      clonedItem.pu = Number(newPU || 0);
    }else{
      clonedItem.pu = Number(clonedItem.pu || 0) * (1 + (Number(avgPct || 0) / 100));
    }

    branch.items.push(clonedItem);
  }

  branch.generatedAt = new Date().toISOString();
  branch.generatedFrom = "base";
  branch.artificialConfig = safeCfg;
  return branch;
}

async function saveArtificialBudgetFromConfig(projectId, cfg){
  const current = window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? StorageAPI.getProjectById(projectId)
    : null;
  if(!current) throw new Error("Proyecto no encontrado.");

  const safeCfg = normalizeArtificialBudgetConfig({ ...cfg, updatedAt: new Date().toISOString() });
  const artificialBranch = await buildArtificialBudgetBranch(current, safeCfg);
  const patch = buildProjectPatchFromBudgetState({ ...current, artificialBudgetConfig: safeCfg }, "artificial", artificialBranch);
  patch.artificialBudgetConfig = safeCfg;
  patch.activeBudget = current.activeBudget || "base";
  StorageAPI.updateProject(projectId, patch);
  return StorageAPI.getProjectById(projectId);
}

function readArtificialConfigFromPanel(){
  const host = UI.qs("#budgetArtificialMount");
  if(!host) return normalizeArtificialBudgetConfig({});
  const getVal = (field)=> host.querySelector(`[data-artificial-field="${field}"]`)?.value || "";
  return normalizeArtificialBudgetConfig({
    materialsPct: getVal("materialsPct"),
    laborPct: getVal("laborPct"),
    equipmentPct: getVal("equipmentPct"),
    transportPct: getVal("transportPct"),
    subcontractsPct: getVal("subcontractsPct"),
    othersPct: getVal("othersPct"),
    note: getVal("note")
  });
}

async function exportArtificialBudgetPdf(project, opts={}){
  const current = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  if(!current) return;
  if(!projectHasArtificialBudget(current)){
    alert("Primero genera el Presupuesto Artificialmente Bajo.");
    return;
  }
  const artificialView = getProjectViewForBudget(current, "artificial");
  if(!artificialView){
    alert("No fue posible preparar el Presupuesto Artificialmente Bajo.");
    return;
  }
  artificialView.activeBudget = "artificial";
  artificialView.artificialBudgetConfig = getArtificialBudgetConfig(current);
  if(!window.PDF || typeof PDF.exportPresupuestoPDF !== "function"){
    alert("La exportación de PDF Presupuesto no está disponible.");
    return;
  }
  await PDF.exportPresupuestoPDF(artificialView, opts || {});
}

function getBudgetComparisonByModes(project, leftMode, rightMode){
  const leftView = getProjectViewForBudget(project, leftMode);
  const rightView = getProjectViewForBudget(project, rightMode);
  const tLeft = leftView ? totalsCompat(leftView) : { total:0, subtotal:0, directo:0 };
  const tRight = rightView ? totalsCompat(rightView) : { total:0, subtotal:0, directo:0 };
  const diff = Number(tRight.total || 0) - Number(tLeft.total || 0);
  const diffPct = Number(tLeft.total || 0) > 0 ? (diff / Number(tLeft.total || 0)) * 100 : 0;
  return {
    leftMode: normalizeBudgetModeRuntime(leftMode),
    rightMode: normalizeBudgetModeRuntime(rightMode),
    leftView,
    rightView,
    tLeft,
    tRight,
...
function getBudgetChapterComparisonRowsByModes(project, leftMode, rightMode){
  const cmp = getBudgetComparisonByModes(project, leftMode, rightMode);
  const leftGroups = (cmp.leftView && window.Calc && typeof Calc.groupByChapters === "function")
    ? ((Calc.groupByChapters(cmp.leftView) || {}).groups || [])
    : [];
  const rightGroups = (cmp.rightView && window.Calc && typeof Calc.groupByChapters === "function")
    ? ((Calc.groupByChapters(cmp.rightView) || {}).groups || [])
    : [];
  ...
  leftGroups.forEach(g=>{
    ...
    map.set(code, {
      chapterCode: code,
      chapterName: String(g.chapterName || "").trim(),
      leftSubtotal: Number(g.subtotal || 0),
      rightSubtotal: 0
    });
  });
  rightGroups.forEach(g=>{
    ...
    if(!map.has(code)){
      map.set(code, {
        ...
        chapterName: String(g.chapterName || "").trim(),
        leftSubtotal: 0,
        rightSubtotal: Number(g.subtotal || 0)
      });
      return;
    }
    const row = map.get(code);
    row.rightSubtotal = Number(g.subtotal || 0);
    if(!row.chapterName && g.chapterName) row.chapterName = String(g.chapterName || "").trim();
  });

  return Array.from(map.values()).map(r=>{
    const diff = Number(r.rightSubtotal || 0) - Number(r.leftSubtotal || 0);
    const pct = Number(r.leftSubtotal || 0) > 0 ? (diff / Number(r.leftSubtotal || 0)) * 100 : (Number(r.rightSubtotal || 0) > 0 ? 100 : 0);
    return { ...r, diff, pct };
  }).sort((a,b)=> Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)));
}

function getBudgetItemComparisonRowsByModes(project, leftMode, rightMode){
  const cmp = getBudgetComparisonByModes(project, leftMode, rightMode);
  const leftItems = Array.isArray(cmp.leftView?.items) ? cmp.leftView.items : [];
  const rightItems = Array.isArray(cmp.rightView?.items) ? cmp.rightView.items : [];
  ...
  function itemDesc(it){
    return String(it?.desc || it?.descripcion || "").trim();
  }

  function itemCompareKey(it){
    const code = normCode(it?.code);
    const desc = normText(itemDesc(it));
    const ref = normCode(it?.apuRefCode);

    if(code && desc) return `code:${code}__desc:${desc}`;
  ...
  function accumulate(side, row, it){
    const qty = Number(it?.qty || 0);
    const pu = Number(it?.pu || 0);
    const partial = qty * pu;

    if(side === "left"){
      row.leftQty += qty;
      row.leftPartial += partial;
      row.leftPU = row.leftQty ? (row.leftPartial / row.leftQty) : pu;
    }else{
      row.rightQty += qty;
      row.rightPartial += partial;
      row.rightPU = row.rightQty ? (row.rightPartial / row.rightQty) : pu;
    }

    if(!row.code) row.code = normCode(it?.code);
  ...
  leftItems.forEach(it=>{
    ...
    if(!key) return;
    const row = ensureRow(map, key, it);
    accumulate("left", row, it);
  });
  ...
  rightItems.forEach(it=>{
    ...
    if(!key) return;
    const row = ensureRow(map, key, it);
    accumulate("right", row, it);
  });

  return Array.from(map.values()).map(r=>{
    const leftPartial = Number(r.leftPartial || 0);
    const rightPartial = Number(r.rightPartial || 0);
    const diff = rightPartial - leftPartial;
    const pct = leftPartial > 0 ? (diff / leftPartial) * 100 : (rightPartial > 0 ? 100 : 0);
    return {
      ...r,
      leftQty: Number(r.leftQty || 0),
      leftPU: Number(r.leftPU || 0),
      rightQty: Number(r.rightQty || 0),
      rightPU: Number(r.rightPU || 0),
      leftPartial,
      rightPartial,
      diff,
      pct
...
function getBudgetIndirectComparisonByModes(project, leftMode, rightMode){
  const cmp = getBudgetComparisonByModes(project, leftMode, rightMode);
  const leftSum = cmp.leftView ? summarizeIndirectUi(cmp.leftView) : { totals:{} };
  const rightSum = cmp.rightView ? summarizeIndirectUi(cmp.rightView) : { totals:{} };
  function makeRow(label, leftValue, rightValue, leftPct, rightPct){
    const diff = Number(rightValue || 0) - Number(leftValue || 0);
    const diffPct = Number(leftValue || 0) > 0 ? (diff / Number(leftValue || 0)) * 100 : (Number(rightValue || 0) > 0 ? 100 : 0);
    return {
      label,
      leftValue: Number(leftValue || 0),
      rightValue: Number(rightValue || 0),
      leftPct: Number(leftPct || 0),
      rightPct: Number(rightPct || 0),
      diff,
      diffPct
    };
  }
  return {
    rows: [
      makeRow("Administración", leftSum.totals?.admin, rightSum.totals?.admin, leftSum.adminPct, rightSum.adminPct),
      makeRow("Imprevistos", leftSum.totals?.imprev, rightSum.totals?.imprev, leftSum.imprevPct, rightSum.imprevPct),
      makeRow("Utilidad", leftSum.totals?.util, rightSum.totals?.util, leftSum.utilPct, rightSum.utilPct),
      makeRow("Otros", leftSum.totals?.indirectOthers, rightSum.totals?.indirectOthers, leftSum.othersPct, rightSum.othersPct),
      makeRow("Subtotal", leftSum.totals?.subtotal, rightSum.totals?.subtotal, 0, 0),
      makeRow("IVA sobre utilidad", leftSum.totals?.ivaUtil, rightSum.totals?.ivaUtil, 0, 0),
      makeRow("Valor total", leftSum.totals?.total, rightSum.totals?.total, 0, 0)
    ],
    left: leftSum,
    right: rightSum
  };
...
async function exportArtificialJustificationPdf(project){
  const current = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  if(!current) return;
  if(!projectHasOfficialBudget(current)){
    alert("Primero debes generar el Presupuesto Oficial.");
    return;
  }
  if(!projectHasArtificialBudget(current)){
    alert("Primero debes generar el Presupuesto Artificialmente Bajo.");
    return;
  }
  ...
  const cmp = getBudgetComparisonByModes(current, "official", "artificial");
  const officialView = cmp.leftView;
  const artificialView = cmp.rightView;
  if(!officialView || !artificialView){
    alert("No fue posible preparar el comparativo Oficial vs Artificialmente Bajo.");
    return;
  }

  const currency = current.currency || "COP";
  const cfg = getArtificialBudgetConfig(current);
  const componentLeft = await computeBudgetComponentTotals(officialView);
  const componentRight = await computeBudgetComponentTotals(artificialView);
  const indirectCmp = getBudgetIndirectComparisonByModes(current, "official", "artificial");
  const chapterRows = getBudgetChapterComparisonRowsByModes(current, "official", "artificial");
  const itemRows = getBudgetItemComparisonRowsByModes(current, "official", "artificial");
  const topRows = itemRows.slice(0, 15);

  const doc = new JsPDFCtor({ unit:"pt", format:"letter", orientation:"portrait" });
  const W = doc.internal.pageSize.getWidth();
  const H = doc.internal.pageSize.getHeight();
  const M = { top:54, right:44, bottom:54, left:44 };
  const usableW = W - M.left - M.right;
  const contentTopY = 74;
  const usablePageHeight = H - M.bottom - contentTopY;
  ...
  function money(v){ return UI.fmtMoney(Number(v || 0), currency); }
  function fmtPct(v){ return `${Number(v || 0).toFixed(2)}%`; }
  function line(yPos, lw=0.35){
    doc.setLineWidth(lw);
    doc.line(M.left, yPos, W - M.right, yPos);
    doc.setLineWidth(0.2);
  }
  function sanitizeNameLocal(s){
    return String(s||"archivo").replace(/[^\w\d]+/g,"_").replace(/^_+|_+$/g,"").slice(0,60) || "archivo";
  }
  ...
  function tryAddImageLocal(dataUrl, x, y, w, h){
    const t = imageTypeFromDataUrl(dataUrl);
    if(!dataUrl || !t) return false;
    try{ doc.addImage(String(dataUrl), t, x, y, w, h); return true; }catch(_){ return false; }
  }
  async function makeSignatureTransparent(dataUrl){
        ...
        img.onload = ()=>{
          try{
            ...
            for(let i=0;i<data.length;i+=4){
              ...
              if(lum < 50){ data[i+3] = 0; }
              else { data[i] = 25; data[i+1] = 25; data[i+2] = 25; data[i+3] = 255; }
            }
  ...
  function getInstitutionHeaderLocal(projectObj){
    const country = up(projectObj?.instPais || "REPÚBLICA DE COLOMBIA");
    const deptRaw = up(projectObj?.instDepto || "");
    const deptLine = deptRaw ? (deptRaw.includes("DEPARTAMENTO") ? deptRaw : ("DEPARTAMENTO DE " + deptRaw)) : "DEPARTAMENTO DE —";
    const muniRaw = up(projectObj?.instMunicipio || "");
    const muniLine = muniRaw ? (muniRaw.includes("MUNICIPIO") ? muniRaw : ("MUNICIPIO DE " + muniRaw)) : "MUNICIPIO DE —";
    return { country, deptLine, muniLine };
  }
  function readProjectSupportText(obj, keys, fallback){
    for(const k of (keys || [])){
  ...
  const projectLabel = String(current?.instProyectoLabel || current?.name || "PROYECTO");
  const plazoTxt = readProjectSupportText(current, ["plazoEjecucion","plazo","executionTerm","duracion","duration"], "No se registró un plazo específico en el proyecto; en consecuencia, la propuesta se entiende ejecutable dentro del plazo oficial del proceso y con una secuencia constructiva optimizada.");
  const pagoTxt = readProjectSupportText(current, ["formaPago","paymentTerms","paymentMethod","metodoPago"], "La justificación económica asume una forma de pago acorde con el pliego del proceso, con flujo de caja suficiente para sostener la ejecución sin trasladar sobrecostos financieros innecesarios.");
  const programTxt = readProjectSupportText(current, ["programacionObra","cronograma","schedule"], "La programación de obra se soporta en una secuencia técnica ordenada por capítulos, control de frentes de trabajo, reducción de tiempos muertos y coordinación anticipada de suministros y cuadrillas.");
  const specsTxt = readProjectSupportText(current, ["especificacionesTecnicas","technicalSpecs","specs"], "La oferta artificialmente baja no implica reducción del alcance técnico ni modificación de especificaciones; conserva las actividades, unidades y estructura general del presupuesto del proyecto.");
  const noteTxt = String(cfg.note || "").trim() || "La reducción propuesta se soporta en estrategias de mercado, eficiencia operativa, planeación logística, control de recursos y mejor aprovechamiento de rendimientos.";
  const itemCountTxt = `La propuesta artificialmente baja conserva ${Array.isArray(artificialView?.items) ? artificialView.items.length : 0} ítems y ${getAllChaptersForProject(artificialView).length} capítulos, manteniendo la estructura presupuestal del proyecto bajo una optimización económica focalizada.`;

  const componentRows = [
    { label:"Materiales", leftValue: componentLeft["MATERIALES"], rightValue: componentRight["MATERIALES"], pctCfg: cfg.materialsPct, support:`La reducción en materiales se sustenta en negociación con proveedores, mejores condiciones de compra, control de desperdicios y programación de suministros conforme al frente de obra.` },
    { label:"Mano de obra", leftValue: componentLeft["MANO DE OBRA"], rightValue: componentRight["MANO DE OBRA"], pctCfg: cfg.laborPct, support:`La reducción en mano de obra se apoya en rendimientos optimizados, organización de cuadrillas, control de tiempos improductivos y una secuencia constructiva más eficiente.` },
    { label:"Equipo y herramienta", leftValue: componentLeft["EQUIPO Y HERRAM"], rightValue: componentRight["EQUIPO Y HERRAM"], pctCfg: cfg.equipmentPct, support:`La reducción en equipo y herramienta se sustenta en mejor utilización horaria, menor ociosidad, programación de equipos y uso eficiente de recursos disponibles.` },
    { label:"Transporte", leftValue: componentLeft["TRANSPORTES"], rightValue: componentRight["TRANSPORTES"], pctCfg: cfg.transportPct, support:`La reducción en transporte se justifica por una logística más precisa, consolidación de viajes, control de distancias efectivas y optimización de rutas de suministro.` },
    { label:"Subcontratos", leftValue: componentLeft["SUBCONTRATOS"], rightValue: componentRight["SUBCONTRATOS"], pctCfg: cfg.subcontractsPct, support:`La reducción en subcontratos obedece a mejores condiciones de contratación especializada, coordinación anticipada y alcance controlado de actividades tercerizadas.` },
    { label:"Otros", leftValue: componentLeft["OTROS"], rightValue: componentRight["OTROS"], pctCfg: cfg.othersPct, support:`La variación en otros componentes responde a eficiencias administrativas y técnicas que no comprometen la calidad ni el cumplimiento del objeto contractual.` }
  ].map(r=>{
    const diff = Number(r.rightValue || 0) - Number(r.leftValue || 0);
    const diffPct = Number(r.leftValue || 0) > 0 ? (diff / Number(r.leftValue || 0)) * 100 : (Number(r.rightValue || 0) > 0 ? 100 : 0);
  ...
  function drawFooter(){
    ...
    doc.setFontSize(9);
    doc.text(`Página ${pageNo}`, W/2, footerY, { align:"center" });
  }
  function stampHeader(){
    if(pageNo === 1) return;
    const logo = String(current.logoDataUrl || "").trim();
    if(logo){ tryAddImageLocal(logo, M.left, 14, 42, 24); }
    const textX = logo ? (M.left + 52) : M.left;
    doc.setFont("helvetica","bold");
    doc.setFontSize(9.5);
    doc.text("JUSTIFICACIÓN DEL PRESUPUESTO ARTIFICIALMENTE BAJO", textX, 28);
    doc.setFont("helvetica","normal");
    doc.setFontSize(9.2);
    doc.text(safeText(projectLabel), textX, 40);
    doc.text(nowTxt, W - M.right, 28, { align:"right" });
    line(50, 0.6);
  }
  let y = M.top;
  function addPage(){ drawFooter(); doc.addPage(); pageNo += 1; y = contentTopY; stampHeader(); }
  function ensureSpace(need=18){ if(y + need > H - M.bottom) addPage(); }
  function addTitle(text, opts={}){
    const beforeGap = Number(opts.beforeGap ?? 18);
    const minAfter = Number(opts.minAfter ?? 80);
    if(y > contentTopY + 8){ ensureSpace(beforeGap + 30 + minAfter); y += beforeGap; }
    else{ ensureSpace(30 + minAfter); }
    doc.setFont("helvetica","bold");
    doc.setFontSize(14);
  ...
  function addParagraph(text, fontSize=9.8, spacing=11, after=6){
    doc.setFont("helvetica","normal");
    doc.setFontSize(fontSize);
    const lines = doc.splitTextToSize(safeText(text), usableW);
    ensureSpace(lines.length * spacing + after + 2);
    doc.text(lines, M.left, y);
    y += lines.length * spacing + after;
  }
  function addBullets(items){ (items || []).forEach(txt=> addParagraph(`• ${txt}`, 10, 12, 4)); }
  function calcTableRowsHeight(rows, widths, fontSize=8.3, lineHeight=9){
    doc.setFont("helvetica","normal"); doc.setFontSize(fontSize);
    let total = 0;
    rows.forEach(row=>{
      const prepared = row.map((cell, i)=> doc.splitTextToSize(safeText(cell), Math.max(20, widths[i] - 6)));
      const maxLines = Math.max(...prepared.map(x=>x.length || 1), 1);
      total += maxLines * lineHeight + 11;
    });
    return total;
  }
  function addSimpleTable(headers, rows, widths, opts={}){
    const fontSize = Number(opts.fontSize || 8.3);
    const lineHeight = Number(opts.lineHeight || 9);
    ...
    function drawHeader(){
      ...
      let x = M.left;
      headers.forEach((h, i)=>{ doc.text(safeText(h), x, y); x += widths[i]; });
      y += 8; line(y, 0.55); y += 10;
      doc.setFont("helvetica","normal"); doc.setFontSize(fontSize);
    }
    const estimatedTotal = 18 + calcTableRowsHeight(rows, widths, fontSize, lineHeight) + 10;
    if(keepTogether && estimatedTotal <= usablePageHeight && y + estimatedTotal > H - M.bottom) addPage();
    else ensureSpace(38);
    drawHeader();
    rows.forEach(row=>{
      const prepared = row.map((cell, i)=> doc.splitTextToSize(safeText(cell), Math.max(20, widths[i] - 6)));
      const maxLines = Math.max(...prepared.map(v => v.length || 1), 1);
      const rowH = maxLines * lineHeight + 5;
      if(y + rowH + 10 > H - M.bottom){ addPage(); if(repeatHeader) drawHeader(); }
      let cx = M.left;
      prepared.forEach((lines, i)=>{ doc.text(lines, cx, y); cx += widths[i]; });
      y += rowH; line(y - 2, 0.35); y += 6;
    });
  }
  async function addSignatureBlock(){
    ...
    doc.setFont("helvetica","normal");
    doc.setFontSize(11);
    doc.text("Firma:", M.left, y);
    y += 14;
    const sigW = 340, sigH = 88, sigX = M.left, sigY = y;
    const cleanSignature = await makeSignatureTransparent(elaborador?.firmaDataUrl || "");
    if(cleanSignature){ try{ doc.addImage(cleanSignature, "PNG", sigX + 8, sigY + 8, sigW - 16, sigH - 16); }catch(_){} }
    y = sigY + sigH + 18; line(y, 0.8); y += 18;
    doc.setFont("helvetica","bold"); doc.setFontSize(11);
    if(elaborador?.nombre) doc.text(safeText(elaborador.nombre), M.left, y); y += 16;
    doc.setFont("helvetica","normal");
    if(elaborador?.profesion) doc.text(safeText(elaborador.profesion), M.left, y); y += 14;
    if(elaborador?.matricula) doc.text(`M.P. ${safeText(elaborador.matricula)}`, M.left, y); y += 12;
  }
  function drawCover(){
    const entidadContratante = String(current?.instEntidad || current?.entity || "—");
    const ubicacion = String(current?.location || "—");
    const fechaElab = String(current?.instFechaElab || "") || dateLongEsCO(now);
    const { country, deptLine, muniLine } = getInstitutionHeaderLocal(current);
    const logo = String(current?.logoDataUrl || "");
    if(logo){ tryAddImageLocal(logo, (W - 160)/2, 44, 160, 60); }
    let y0 = logo ? 160 : 130;
    doc.setFont("helvetica","bold"); doc.setFontSize(12);
    doc.text(country, W/2, y0, { align:"center" }); y0 += 18;
    doc.text(deptLine, W/2, y0, { align:"center" }); y0 += 18;
    doc.text(muniLine, W/2, y0, { align:"center" }); y0 += 30;
    doc.setFontSize(14); doc.text("INFORME DE JUSTIFICACIÓN", W/2, y0, { align:"center" }); y0 += 22;
    doc.setFontSize(11); doc.text("PRESUPUESTO ARTIFICIALMENTE BAJO", W/2, y0, { align:"center" }); y0 += 22;
    doc.setFontSize(10.5); doc.text("COMPARATIVO OFICIAL VS ARTIFICIALMENTE BAJO", W/2, y0, { align:"center" }); y0 += 24;
    doc.setFontSize(12); doc.text(`PROYECTO: ${up(projectLabel)}`, W/2, y0, { align:"center" }); y0 += 40;
    doc.setFont("helvetica","normal"); doc.setFontSize(11);
    doc.text(`Entidad Contratante: ${entidadContratante}`, M.left, y0); y0 += 16;
    doc.text(`Ubicación: ${ubicacion}`, M.left, y0); y0 += 16;
    doc.text(`Fecha de Elaboración: ${fechaElab}`, M.left, y0); y0 += 16;
    if(elaborador && (elaborador.nombre || elaborador.profesion || elaborador.matricula)){
      y0 += 6;
      if(elaborador.nombre){ doc.text(`Elaboró: ${elaborador.nombre}`, M.left, y0); y0 += 14; }
      if(elaborador.profesion){ doc.text(`Profesión: ${elaborador.profesion}`, M.left, y0); y0 += 14; }
      if(elaborador.matricula){ doc.text(`Matrícula Profesional: ${elaborador.matricula}`, M.left, y0); y0 += 14; }
    }
  }
  ...
  addTitle("1. Objeto y alcance del informe");
  addParagraph(
    `El presente documento tiene por objeto justificar, desde una perspectiva técnica, económica y operativa, la viabilidad del Presupuesto Artificialmente Bajo formulado para el proyecto "${safeText(current.name || "")}" frente al Presupuesto Oficial del proceso. ` +
    `La comparación visible del informe se limita exclusivamente a estas dos referencias: Presupuesto Oficial y Presupuesto Artificialmente Bajo, por ser el Presupuesto Oficial el referente conocido por la entidad.`
  );
  addParagraph(itemCountTxt);

  addTitle("2. Resumen ejecutivo");
  addBullets([
    `Presupuesto Oficial: ${money(cmp.tLeft.total || 0)}.`,
    `Presupuesto Artificialmente Bajo: ${money(cmp.tRight.total || 0)}.`,
    `Diferencia global: ${money(cmp.diff || 0)} (${fmtPct(cmp.diffPct || 0)}).`,
    `Costos directos Oficial: ${money(cmp.tLeft.directo || 0)}.`,
    `Costos directos Artificialmente Bajo: ${money(cmp.tRight.directo || 0)}.`,
    `Justificación estratégica registrada: ${noteTxt}`
  ]);

  addTitle("3. Sustentación comparativa por componentes directos");
  addSimpleTable(
    ["Componente", "Oficial", "Artificial", "Dif.", "% Dif.", "% aplicado"],
    componentRows.map(r=>[
      r.label,
      money(r.leftValue || 0),
      money(r.rightValue || 0),
      money(r.diff || 0),
      fmtPct(r.diffPct || 0),
      fmtPct(r.pctCfg || 0)
    ]),
  ...
  componentRows.forEach(r=>{
    addParagraph(`${r.label}: ${r.support} La variación observada frente al presupuesto oficial corresponde a ${money(r.diff || 0)} (${fmtPct(r.diffPct || 0)}).`, 9.6, 10.8, 5);
  });

  addTitle("4. Costos indirectos y estructura financiera");
  addSimpleTable(
    ["Concepto", "Oficial", "% Ofic.", "Artificial", "% Art.", "Dif.", "% Dif."],
    indirectCmp.rows.map(r=>[
      r.label || "",
      money(r.leftValue || 0),
      r.leftPct ? fmtPct(r.leftPct || 0) : "-",
      money(r.rightValue || 0),
      r.rightPct ? fmtPct(r.rightPct || 0) : "-",
      money(r.diff || 0),
      fmtPct(r.diffPct || 0)
    ]),
    [118, 84, 52, 84, 52, 84, 54],
    { keepTogether:true }
  );
  addParagraph(
    "La estructura de indirectos del presupuesto artificialmente bajo mantiene consistencia financiera, permitiendo sostener administración, imprevistos, utilidad e IVA sobre utilidad sin desnaturalizar la ejecución contractual. La propuesta optimiza costos sin suprimir obligaciones propias de la gestión del contrato.",
    9.7,
    10.8,
    6
  );

  addTitle("5. Comparación por capítulos");
  addSimpleTable(
    ["Capítulo", "Nombre", "Oficial", "Artificial", "Dif.", "% Dif."],
    chapterRows.slice(0, 18).map(r=>[
      r.chapterCode || "",
      r.chapterName || "",
      money(r.leftSubtotal || 0),
      money(r.rightSubtotal || 0),
      money(r.diff || 0),
      fmtPct(r.pct || 0)
    ]),
    [68, 190, 82, 82, 82, 48],
  ...
  const leadChapter = chapterRows[0];
  if(leadChapter){
    addParagraph(`El capítulo con mayor incidencia en la diferencia frente al presupuesto oficial corresponde a ${safeText(leadChapter.chapterCode)} ${safeText(leadChapter.chapterName)}, con una variación de ${money(leadChapter.diff || 0)} (${fmtPct(leadChapter.pct || 0)}).`, 9.6, 10.8, 6);
  }

  addTitle("6. Plazo de ejecución, forma de pago y programación de obra");
  addParagraph(`Plazo de ejecución: ${plazoTxt}`);
  addParagraph(`Forma de pago: ${pagoTxt}`);
  addParagraph(`Programación de obra: ${programTxt}`);
  addParagraph(
    "Desde el punto de vista operativo, la oferta artificialmente baja se apoya en una coordinación más precisa de suministros, equipos y cuadrillas, lo que reduce tiempos muertos y evita sobrecostos de permanencia, sin requerir alteraciones del alcance contractual.",
    9.7,
    10.8,
    6
  );

  addTitle("7. Especificaciones técnicas y mantenimiento del alcance");
  addParagraph(`Especificaciones técnicas: ${specsTxt}`);
  addParagraph(
    "La reducción económica propuesta no presupone disminución de calidad, cambio de unidades, eliminación de actividades ni modificación de especificaciones. La justificación se soporta en eficiencia de ejecución, estructura de costos y mejor gestión de recursos.",
    9.7,
    10.8,
    6
  );

  addTitle("8. Análisis de precios unitarios y actividades representativas");
  addParagraph(
    "Se presenta a continuación una selección de actividades con mayor impacto económico en la diferencia entre el presupuesto oficial y el presupuesto artificialmente bajo, con el fin de evidenciar que la optimización se concentra en APUs y actividades representativas del proyecto.",
    9.7,
    10.8,
    6
  );
  if(!topRows.length){
    addParagraph("No se identificaron actividades comparables para este análisis.");
  }else{
    addSimpleTable(
      ["Código", "Descripción", "Oficial", "Artificial", "Dif.", "% Dif."],
      topRows.map(r=>[
        r.code || "",
        r.desc || "",
        money(r.leftPartial || 0),
        money(r.rightPartial || 0),
        money(r.diff || 0),
        fmtPct(r.pct || 0)
      ]),
      [54, 216, 74, 74, 74, 48],
  ...
  addParagraph(
    "El análisis de precios unitarios evidencia que la propuesta artificialmente baja no surge de una compresión indiscriminada del presupuesto, sino de ajustes porcentuales focalizados por componente, aplicados desde la estructura del presupuesto base y contrastados frente al presupuesto oficial.",
    9.7,
    10.8,
    6
  );

  addTitle("9. Conclusión técnica y económica");
  addParagraph(
    `Con base en el comparativo Oficial vs Artificialmente Bajo, se concluye que la oferta propuesta por ${money(cmp.tRight.total || 0)} resulta inferior al presupuesto oficial en ${money(cmp.diff || 0)} (${fmtPct(cmp.diffPct || 0)}), sin que ello implique, por sí mismo, inviabilidad técnica o financiera. ` +
    `La sustentación se apoya en eficiencias verificables en materiales, mano de obra, equipo y herramienta, transporte, subcontratos, otros componentes directos y estructura indirecta, complementadas con una programación de obra más eficiente, una estrategia logística controlada y mantenimiento del alcance técnico del proyecto.`
  );
  addParagraph(
    "En consecuencia, el presupuesto artificialmente bajo aquí presentado puede entenderse como una propuesta económicamente competitiva y técnicamente defendible, siempre que su ejecución se mantenga dentro de la planeación operativa, financiera y contractual descrita en este informe.",
    9.7,
    10.8,
    6
  );
  ...
  const filename = `Justificacion_Presupuesto_Artificialmente_Bajo_${sanitizeNameLocal(current?.name || "proyecto")}_${Date.now()}.pdf`;
  doc.save(filename);
}

function renderArtificialBudgetPanel(project){
  const host = ensureArtificialBudgetMount();
  if(!host || !project) return;

  const cfg = getArtificialBudgetConfig(project);
  const hasArtificial = projectHasArtificialBudget(project);
  const artificialView = hasArtificial ? getProjectViewForBudget(project, "artificial") : null;
  const artificialTotals = artificialView ? totalsCompat(artificialView) : null;
  const info = getBudgetModeInfo(project);
  const officialHint = projectHasOfficialBudget(project)
    ? "Ajusta porcentajes positivos o negativos por componente del Presupuesto Base. La comparación con el Oficial te sirve como referencia estratégica."
    : "Puedes construir el Presupuesto Artificialmente Bajo desde el Presupuesto Base. Si creas el Oficial, tendrás una referencia adicional para la estrategia.";

  host.innerHTML = `
    <div class="cardhead">
      <h2>Presupuesto Artificialmente Bajo</h2>
      <p class="muted small">${UI.esc(officialHint)}</p>
    </div>

    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
      ${hasArtificial ? UI.chip("GENERADO", "warn") : UI.chip("NO GENERADO", "bad")}
      ${hasArtificial && artificialTotals ? UI.chip(`Total ${UI.fmtMoney(artificialTotals.total || 0, project.currency || "COP")}`, "warn") : ""}
      ${info.mode === "artificial" ? UI.chip("ACTIVO", "ok") : ""}
    </div>

    <div class="grid two" style="gap:12px">
      <label>
        <div class="muted small">Materiales (%)</div>
        <input class="input" data-artificial-field="materialsPct" type="number" step="any" value="${UI.esc(String(cfg.materialsPct || 0))}">
      </label>
      <label>
        <div class="muted small">Mano de obra (%)</div>
        <input class="input" data-artificial-field="laborPct" type="number" step="any" value="${UI.esc(String(cfg.laborPct || 0))}">
      </label>
      <label>
        <div class="muted small">Equipo y herramienta (%)</div>
        <input class="input" data-artificial-field="equipmentPct" type="number" step="any" value="${UI.esc(String(cfg.equipmentPct || 0))}">
      </label>
      <label>
        <div class="muted small">Transporte (%)</div>
        <input class="input" data-artificial-field="transportPct" type="number" step="any" value="${UI.esc(String(cfg.transportPct || 0))}">
      </label>
      <label>
        <div class="muted small">Subcontratos (%)</div>
        <input class="input" data-artificial-field="subcontractsPct" type="number" step="any" value="${UI.esc(String(cfg.subcontractsPct || 0))}">
      </label>
      <label>
        <div class="muted small">Otros (%)</div>
        <input class="input" data-artificial-field="othersPct" type="number" step="any" value="${UI.esc(String(cfg.othersPct || 0))}">
      </label>
    </div>

    <label style="display:block; margin-top:12px">
      <div class="muted small">Justificación / nota estratégica</div>
      <textarea class="input" data-artificial-field="note" rows="3" placeholder="Ej: reducción soportada en negociación de materiales, optimización de cuadrillas, mejor logística, descuentos comerciales...">${UI.esc(cfg.note || "")}</textarea>
    </label>

    <div class="row" style="gap:8px; flex-wrap:wrap; margin-top:12px">
      <button type="button" class="btn primary" data-budget-artificial-generate>${hasArtificial ? "Regenerar Presupuesto Artificialmente Bajo" : "Generar Presupuesto Artificialmente Bajo"}</button>
      <button type="button" class="btn ${info.mode === "artificial" ? "primary" : ""}" data-budget-artificial-switch ${hasArtificial ? "" : "disabled"}>Trabajar Presupuesto Artificialmente Bajo</button>
      <button type="button" class="btn" data-budget-artificial-pdf ${hasArtificial ? "" : "disabled"}>PDF Presupuesto Artificialmente Bajo</button>
      <button type="button" class="btn" data-budget-artificial-justify-pdf ${(hasArtificial && projectHasOfficialBudget(project)) ? "" : "disabled"}>PDF Justificación Artificialmente Bajo</button>
    </div>

    <div class="muted small" style="margin-top:10px">
      Usa porcentajes negativos para disminuir componentes y positivos para incrementarlos. El cálculo se hace APU por APU desde el Presupuesto Base y el resultado queda guardado como un tercer presupuesto dentro del proyecto.
    </div>
  `;

  host.querySelector("[data-budget-artificial-generate]")?.addEventListener("click", async ()=>{
    try{
      const config = readArtificialConfigFromPanel();
      const updated = await saveArtificialBudgetFromConfig(project.id, config);
      alert("Presupuesto Artificialmente Bajo generado y guardado dentro del proyecto.");
      refreshProjectBudgetAwareUI(updated.id);
    }catch(err){
      alert("No se pudo generar el Presupuesto Artificialmente Bajo: " + (err?.message || err));
    }
  });

  host.querySelector("[data-budget-artificial-switch]")?.addEventListener("click", ()=>{
    if(!hasArtificial) return;
    if(window.StorageAPI && typeof StorageAPI.setActiveBudget === "function"){
      StorageAPI.setActiveBudget(project.id, "artificial");
    }
    refreshProjectBudgetAwareUI(project.id);
  });

  host.querySelector("[data-budget-artificial-pdf]")?.addEventListener("click", async ()=>{
    try{
      const fresh = StorageAPI.getProjectById(project.id);
      if(!fresh) return;
      await exportArtificialBudgetPdf(fresh, { share: isIOS() });
    }catch(err){
      alert("No se pudo generar el PDF Presupuesto Artificialmente Bajo: " + (err?.message || err));
    }
  });

  host.querySelector("[data-budget-artificial-justify-pdf]")?.addEventListener("click", async ()=>{
    try{
      const fresh = StorageAPI.getProjectById(project.id);
      if(!fresh) return;
      await exportArtificialJustificationPdf(fresh);
    }catch(err){
      alert("No se pudo generar el PDF de Justificación Artificialmente Bajo: " + (err?.message || err));
    }
  });
}

function getProjectUiStateKey(projectId, sectionKey){
  return `ppro_ui_${String(projectId || "global")}__${String(sectionKey || "section")}`;
}

function readProjectUiState(projectId, sectionKey, fallbackOpen=true){
  try{
    const raw = localStorage.getItem(getProjectUiStateKey(projectId, sectionKey));
...
function writeProjectUiState(projectId, sectionKey, isOpen){
  try{
    localStorage.setItem(getProjectUiStateKey(projectId, sectionKey), isOpen ? "1" : "0");
  }catch(_){}
}

function ensureCollapsibleCardShell(host, opts){
  if(!host) return null;
  const options = opts || {};
  const sectionKey = String(options.sectionKey || host.id || "section");
  const projectId = String(options.projectId || "global");
  const fallbackOpen = options.defaultOpen !== false;

  let shell = host.querySelector(":scope > .collapsible-card-shell");
  let body = host.querySelector(":scope > .collapsible-card-body");

  if(!shell || !body){
    const children = Array.from(host.childNodes);
    const existingHeader = children.find(node => node.nodeType === 1 && node.classList?.contains("cardhead"));

    const titleText =
      String(options.title || existingHeader?.querySelector("h2")?.textContent || host.getAttribute("data-collapse-title") || "Sección").trim();
    const subtitleText =
      String(options.subtitle || existingHeader?.querySelector("p")?.textContent || host.getAttribute("data-collapse-subtitle") || "").trim();

    shell = document.createElement("div");
    shell.className = "collapsible-card-shell";
    ...
    head.innerHTML = `
      <div class="name collapsible-title" style="font-size:1.05rem; font-weight:800">${UI.esc(titleText)}</div>
      ${subtitleText ? `<div class="muted small collapsible-subtitle" style="margin-top:4px">${UI.esc(subtitleText)}</div>` : ``}
    `;
  ...
  const btn = host.querySelector(":scope > .collapsible-card-shell [data-collapse-toggle]");
  function apply(open){
    body.style.display = open ? "" : "none";
    if(btn) btn.textContent = open ? "Minimizar" : "Expandir";
    host.setAttribute("data-collapsed", open ? "0" : "1");
  }

  const initialOpen = readProjectUiState(projectId, sectionKey, fallbackOpen);
  apply(initialOpen);
  ...
  if(btn){
    btn.onclick = ()=>{
      ...
      apply(nextOpen);
      writeProjectUiState(projectId, sectionKey, nextOpen);
    };
  }

  return { host, body, button: btn };
}

function ensureNavGroupCollapsible(section, opts){
  if(!section) return null;
  const options = opts || {};
  const projectId = String(options.projectId || "global");
  const sectionKey = String(options.sectionKey || section.id || "navgroup");
  const fallbackOpen = options.defaultOpen !== false;
  const titleEl = section.querySelector(".navgroup-title, .navgroup-title-text");
  const buttonsEl = section.querySelector(".navgroup-buttons");
  if(!titleEl || !buttonsEl) return null;

  let shell = section.querySelector(":scope > .navgroup-header");
  let btn = shell?.querySelector?.("[data-collapse-toggle]") || null;

  if(!shell){
    shell = document.createElement("div");
    shell.className = "navgroup-header";
    shell.style.display = "flex";
    shell.style.alignItems = "center";
    shell.style.justifyContent = "space-between";
    ...
    titleEl.classList.remove("navgroup-title");
    titleEl.classList.add("navgroup-title-text");
    ...
    titleEl.replaceWith(shell);
    shell.appendChild(titleEl);
    shell.appendChild(btn);
    section.insertBefore(shell, buttonsEl);
  }

  function apply(open){
    buttonsEl.style.display = open ? "" : "none";
    if(btn) btn.textContent = open ? "Minimizar" : "Expandir";
    section.setAttribute("data-collapsed", open ? "0" : "1");
  }
  ...
  if(btn){
    btn.onclick = ()=>{
      const nowOpen = buttonsEl.style.display !== "none";
      const nextOpen = !nowOpen;
      apply(nextOpen);
      writeProjectUiState(projectId, sectionKey, nextOpen);
    };
  ...
  return { section, buttonsEl, button: btn };
}

function initProjectDetailCollapsibleSections(projectId){
  const toolbar = UI.qs("#budgetToolbar");
  if(toolbar){
    ensureCollapsibleCardShell(toolbar, {
      projectId,
      sectionKey: "budget_toolbar",
      title: "Doble presupuesto",
      subtitle: "Proyecto único con dos capas presupuestales: Base y Oficial.",
      defaultOpen: true
    });
  }
  ...
  const dashboard = UI.qs("#budgetAuditDashboardMount");
  if(dashboard){
    ensureCollapsibleCardShell(dashboard, {
      projectId,
      sectionKey: "dashboard_auditoria",
      title: "Dashboard Auditoría",
      subtitle: "Visualización en pantalla de la comparación Presupuesto Base vs Presupuesto Oficial.",
      defaultOpen: true
    });
  }

  const artificial = UI.qs("#budgetArtificialMount");
  if(artificial){
    ensureCollapsibleCardShell(artificial, {
      projectId,
      sectionKey: "budget_artificial",
      title: "Presupuesto Artificialmente Bajo",
      subtitle: "Generación del tercer presupuesto estratégico del proyecto.",
      defaultOpen: true
    });
  }

  document.querySelectorAll(".nav.nav-groups .navgroup").forEach(section=>{
    const rawTitle = String(section.querySelector(".navgroup-title, .navgroup-title-text")?.textContent || "").trim().toUpperCase();
    let key = rawTitle.toLowerCase().replace(/[^a-z0-9]+/g, "_");
    let defaultOpen = true;

    if(rawTitle.includes("EXPORTACIÓN") || rawTitle.includes("EXPORTACION")){
      key = "nav_exportacion";
      defaultOpen = false;
    }else if(rawTitle.includes("GENERACIÓN") || rawTitle.includes("GENERACION")){
      key = "nav_generacion";
    }else if(rawTitle.includes("ANÁLISIS") || rawTitle.includes("ANALISIS")){
      key = "nav_analisis";
    }

    ensureNavGroupCollapsible(section, {
      projectId,
      sectionKey: key,
      defaultOpen
    });
...
function wrapChartLabelText(text, maxLen=28){
  ...
  words.forEach(word=>{
    ...
    if(probe.length > maxLen && current){
      lines.push(current);
      current = word;
    }else{
      current = probe;
...
function shortenChartLabel(text, maxLen=44){
  const raw = String(text || "").trim();
  if(raw.length <= maxLen) return raw;
  return raw.slice(0, Math.max(10, maxLen - 1)).trimEnd() + "…";
}


function refreshPdfExportButtonsLabels(project){
  const modeInfo = getBudgetModeInfo(project || {});
  const modeLabel = modeInfo.label || "BASE";
  const modeLabelShort = modeInfo.mode === "artificial" ? "Artificialmente Bajo" : modeLabel;
  ...
  setText("btnPdfPresupuesto", `PDF Presupuesto ${modeLabelShort}`);
  setText("btnDlPdfPresupuesto", `Descargar PDF Presupuesto ${modeLabelShort}`);

  setText("btnPdfPresupuestoAPUs", `PDF Presupuesto ${modeLabelShort} + APUs`);
  setText("btnDlPdfPresupuestoAPUs", `Descargar PDF Presupuesto ${modeLabelShort} + APUs`);

  setText("btnPdfEspecificacionesTec", `PDF Especificaciones Técnicas ${modeLabelShort}`);
  setText("btnDlPdfEspecificacionesTec", `Descargar PDF Especificaciones Técnicas ${modeLabelShort}`);

  setText("btnPdfPresupuestoDesagregado", `PDF Presupuesto Desagregado ${modeLabelShort}`);
  setText("btnPdfResumenPresupuestoDesagregado", `PDF Resumen Desagregado ${modeLabelShort}`);
  setText("btnPdfDistribucionPctCD", `PDF Distribución % Costos Directos ${modeLabelShort}`);
  setText("btnPdfRendimientoEqMo", `PDF Rendimiento Eq./M.O. ${modeLabelShort}`);
  setText("btnPdfResumenMaterialesActividad", `PDF Materiales por Actividad ${modeLabelShort}`);
  setText("btnPdfCantRecursosInsumos", `PDF Recursos e Insumos ${modeLabelShort}`);
  setText("btnPdfCalculoIndirectos", `PDF Costos Indirectos ${modeLabelShort}`);
  setText("btnPdfCalcIndirectos", `PDF Costos Indirectos ${modeLabelShort}`);

  const auditBtn = document.querySelector("[data-budget-audit-pdf]");
  if(auditBtn) auditBtn.textContent = "PDF Auditoría Base vs Oficial";

  const artificialBtn = document.querySelector("[data-budget-artificial-pdf]");
  if(artificialBtn) artificialBtn.textContent = "PDF Presupuesto Artificialmente Bajo";

  const artificialJustifyBtn = document.querySelector("[data-budget-artificial-justify-pdf]");
  if(artificialJustifyBtn) artificialJustifyBtn.textContent = "PDF Justificación Artificialmente Bajo";
}
...
function getProjectViewForBudget(project, mode){
  ...
  clone.artificialBudgetConfig = normalizeArtificialBudgetConfig(clone.artificialBudgetConfig || b.artificialConfig || {});
...
function getBudgetAuditFindings(project){
  const cmp = getBudgetComparison(project);
  const methodAudit = getBudgetIndirectMethodAudit(project);
  const chapterRows = getBudgetChapterComparisonRows(project);
  const itemRows = getBudgetItemComparisonRows(project).map(r => ({ ...r, audit: detectBudgetItemAlerts(r) }));
  const indirectCmp = getBudgetIndirectComparison(project);

  const findings = [];

  if(!methodAudit.sameMethod){
    findings.push({
      id: "criterio_indirectos",
      severity: "bad",
      title: "Criterio distinto de indirectos",
      detail: `Base calcula indirectos por ${methodAudit.baseModeLabel} y Oficial por ${methodAudit.officialModeLabel}.`,
      action: "Ver indirectos",
      focusTarget: "indirects"
    });
  }

  if(Math.abs(Number(cmp.diffPct || 0)) >= 10){
    findings.push({
      id: "variacion_global",
      severity: Math.abs(Number(cmp.diffPct || 0)) >= 20 ? "bad" : "warn",
      title: "Variación global del presupuesto",
      detail: `La diferencia total entre Base y Oficial es de ${UI.fmtMoney(cmp.diff || 0, project?.currency || "COP")} (${Number(cmp.diffPct || 0).toFixed(2)}%).`,
      action: "Ver resumen",
      focusTarget: "summary"
    });
  }

  const repeatedBudget = Math.abs(Number(cmp.diff || 0)) < 1 && projectHasOfficialBudget(project);
  if(repeatedBudget){
    findings.push({
      id: "presupuesto_igual",
      severity: "warn",
      title: "Presupuesto Oficial igual al Base",
      detail: "El Presupuesto Oficial replica exactamente el Presupuesto Base en valor total.",
      action: "Ver resumen",
      focusTarget: "summary"
    });
  }

  const alertItems = itemRows.filter(r => Array.isArray(r.audit?.alerts) && r.audit.alerts.length);
  if(alertItems.length){
    const worst = alertItems
      .slice()
      .sort((a,b)=>{
        const ax = Math.max(Math.abs(Number(a.audit?.qtyPct || 0)), Math.abs(Number(a.audit?.puPct || 0)));
        const bx = Math.max(Math.abs(Number(b.audit?.qtyPct || 0)), Math.abs(Number(b.audit?.puPct || 0)));
        return bx - ax;
      })[0];

    findings.push({
      id: "items_alerta",
      severity: alertItems.length >= 3 ? "bad" : "warn",
      title: "Ítems con alerta automática",
      detail: `${alertItems.length} ítems presentan alertas. Mayor variación detectada en ${worst?.code || "ítem"} (${worst?.desc || "sin descripción"}).`,
      action: "Ver ítems",
      focusTarget: "items"
    });
  }

  const chapterAlerts = chapterRows
    .map(r=>{
    ...
    .filter(r => Math.abs(Number(r.pct || 0)) >= 10);

  if(chapterAlerts.length){
    const lead = chapterAlerts
      .slice()
      .sort((a,b)=>Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)))[0];

    findings.push({
      id: "capitulos_variacion",
      severity: Math.abs(Number(lead?.pct || 0)) >= 20 ? "bad" : "warn",
      title: "Capítulos con variación relevante",
      detail: `${chapterAlerts.length} capítulos muestran variaciones relevantes. El más sensible es ${lead?.chapterCode || ""} ${lead?.chapterName || ""} con ${Number(lead?.pct || 0).toFixed(2)}%.`,
      action: "Ver capítulos",
      focusTarget: "chapters"
    });
  }

  const indirectAlerts = indirectCmp.rows.filter(r => ["Administración","Imprevistos","Utilidad","Otros"].includes(String(r.label || "")) && Math.abs(Number(r.diffPct || 0)) >= 10);
  if(indirectAlerts.length){
    const lead = indirectAlerts
      .slice()
      .sort((a,b)=>Math.abs(Number(b.diffPct || 0)) - Math.abs(Number(a.diffPct || 0)))[0];

    findings.push({
      id: "indirectos_variacion",
      severity: Math.abs(Number(lead?.diffPct || 0)) >= 20 ? "bad" : "warn",
      title: "Diferencias en costos indirectos",
      detail: `${indirectAlerts.length} componentes de indirectos presentan variación relevante. El mayor cambio está en ${lead?.label || "indirectos"} (${Number(lead?.diffPct || 0).toFixed(2)}%).`,
      action: "Ver indirectos",
      focusTarget: "indirects"
    });
  }
  ...
  if(!findings.length){
    findings.push({
      id: "sin_hallazgos",
      severity: "ok",
      title: "Sin hallazgos automáticos relevantes",
      detail: "No se detectaron desviaciones relevantes en la auditoría automática actual.",
      action: "",
      focusTarget: ""
    });
  }
...
function scrollToBudgetAuditSection(target){
  const map = {
    summary: "#budgetComparisonBody",
    indirects: "#budgetComparisonBody",
    chapters: "#budgetComparisonChapters",
    items: "#budgetComparisonItems"
  };
  const sel = map[String(target || "").trim().toLowerCase()] || "#budgetComparisonMount";
  const el = document.querySelector(sel) || document.querySelector("#budgetComparisonMount");
  if(!el) return;
  try{
...
function renderBudgetToolbar(project){
  ...
  host.innerHTML = `
    <div class="cardhead">
      ...
      <p class="muted small">Proyecto único con tres capas presupuestales: Base, Oficial y Artificialmente Bajo.</p>
    </div>

    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
      ${getBudgetModeBadge(info.mode)}
      ${hasOfficial ? UI.chip(`Δ Oficial ${UI.fmtMoney(cmp.diff||0, currency)}`) : UI.chip("Oficial no creado")}
      ${hasArtificial && artificialTotals ? UI.chip(`Artificial ${UI.fmtMoney((artificialTotals.total||0) - (cmp.tBase.total||0), currency)}`, "warn") : UI.chip("Artificial no generado", hasOfficial ? "warn" : "bad")}
    ...
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:12px">
      ...
      <div class="card item">
        <div class="name">Artificialmente Bajo</div>
        <div class="muted small">
          Ítems: ${UI.esc(String(artificialView?.items?.length || 0))}<br>
          Total: <b>${hasArtificial && artificialTotals ? UI.fmtMoney(artificialTotals.total||0, currency) : "—"}</b>
        </div>
      </div>
    </div>
    ...
    <div class="row" style="gap:8px; flex-wrap:wrap; margin-top:12px">
      ...
      <button type="button" class="btn ${info.mode === "official" ? "primary" : ""}" data-budget-switch="official" ${hasOfficial ? "" : "disabled"}>Trabajar Presupuesto Oficial</button>
      <button type="button" class="btn ${info.mode === "artificial" ? "primary" : ""}" data-budget-switch="artificial" ${hasArtificial ? "" : "disabled"}>Trabajar Artificialmente Bajo</button>
      ...
      <button type="button" class="btn" data-budget-audit-pdf ${hasOfficial ? "" : "disabled"}>PDF Auditoría Base vs Oficial</button>
    </div>
  ...
  host.querySelector("[data-budget-audit-pdf]")?.addEventListener("click", ()=>{
    const fresh = StorageAPI.getProjectById(project.id);
    if(!fresh || !projectHasOfficialBudget(fresh)) return;
...
const __budgetDashboardCharts = {
  chapters: null,
  items: null
};

function destroyBudgetDashboardCharts(){
  try{ __budgetDashboardCharts.chapters?.destroy?.(); }catch(_){}
  try{ __budgetDashboardCharts.items?.destroy?.(); }catch(_){}
  __budgetDashboardCharts.chapters = null;
  __budgetDashboardCharts.items = null;
}

function ensureBudgetDashboardMount(){
  let host = UI.qs("#budgetAuditDashboardMount");
  if(host) return host;
  ...
  host = document.createElement("section");
  host.id = "budgetAuditDashboardMount";
  host.className = "card";
  host.style.marginTop = "14px";
...
function getBudgetDashboardRisk(project){
  const cmp = getBudgetComparison(project);
  const chapterRows = getBudgetChapterComparisonRows(project);
  ...
  const diffPct = Math.abs(Number(cmp.diffPct || 0));
  const chapterAlertCount = chapterRows
    .map(r=>{
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return Math.abs(pct) >= 10 ? 1 : 0;
    })
    .reduce((a,b)=>a+b,0);

  if(diffPct >= 20 || itemRows.length >= 5 || chapterAlertCount >= 2){
    return { label:"ALTO", type:"bad" };
  }
  if(diffPct >= 10 || itemRows.length >= 2 || chapterAlertCount >= 1){
    return { label:"MEDIO", type:"warn" };
  }
  return { label:"BAJO", type:"ok" };
}

function getBudgetDashboardChapterChartData(project){
  const rows = getBudgetChapterComparisonRows(project) || [];
  const sorted = rows
    .map(r => ({
      label: `${String(r.chapterCode || "").trim()} ${String(r.chapterName || "").trim()}`.trim(),
      base: Number(r.baseSubtotal || 0),
      official: Number(r.officialSubtotal || 0),
      impact: Math.abs(Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0))
    }))
    .sort((a,b)=> Number(b.impact || 0) - Number(a.impact || 0))
    .slice(0, 10);

  return {
    labels: sorted.map(r => wrapChartLabelText(r.label || "(Sin capítulo)", 18)),
    base: sorted.map(r => Number(r.base || 0)),
    official: sorted.map(r => Number(r.official || 0))
  };
}

function getBudgetDashboardItemChartData(project){
  ...
  const ranked = rows
    .map(r=>{
      ...
      const impact = Number(officialPartial || 0) - Number(basePartial || 0);
      const fullLabel = `${String(r.code || "").trim()} ${String(r.desc || "").trim()}`.trim();
      const shortLabel = shortenChartLabel(fullLabel, 38);
      return {
        label: wrapChartLabelText(shortLabel, 18),
        fullLabel,
        shortLabel,
        impact,
        impactAbs: Math.abs(impact)
      };
    })
    .sort((a,b)=> Number(b.impactAbs || 0) - Number(a.impactAbs || 0))
    .slice(0, 10);

  return {
    labels: ranked.map(r => r.label || ["(Sin ítem)"]),
    shortLabels: ranked.map(r => r.shortLabel || "(Sin ítem)"),
    fullLabels: ranked.map(r => r.fullLabel || "(Sin ítem)"),
    impact: ranked.map(r => Number(r.impact || 0))
  };
}

function formatBudgetDashboardNumber(v){
  return UI.fmtMoney(Number(v || 0), "COP");
}

function renderBudgetAuditDashboard(project){
  const host = ensureBudgetDashboardMount();
  if(!host || !project) return;

  const hasOfficial = projectHasOfficialBudget(project);
  if(!hasOfficial){
    destroyBudgetDashboardCharts();
  ...
  host.style.display = "";
  const cmp = getBudgetComparison(project);
  const risk = getBudgetDashboardRisk(project);
  const chapterData = getBudgetDashboardChapterChartData(project);
  const itemData = getBudgetDashboardItemChartData(project);
  const currency = project.currency || "COP";

  const itemLegendHtml = (itemData.fullLabels || []).map((label, idx)=>`
    <div class="muted small" style="margin:4px 0">
      <b>${idx + 1}.</b> ${UI.esc(label)}
    </div>
  `).join("");
  ...
  host.innerHTML = `
    <div class="cardhead">
      ...
      <p class="muted small">Visualización en pantalla de la comparación Presupuesto Base vs Presupuesto Oficial.</p>
    </div>

    <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:12px">
      ${UI.chip(`Variación ${Number(cmp.diffPct || 0).toFixed(2)}%`, Math.abs(Number(cmp.diffPct || 0)) >= 20 ? "bad" : (Math.abs(Number(cmp.diffPct || 0)) >= 10 ? "warn" : "ok"))}
      ${UI.chip(`Δ ${UI.fmtMoney(cmp.diff || 0, currency)}`, Number(cmp.diff || 0) >= 0 ? "bad" : "ok")}
      ${UI.chip(`Riesgo ${risk.label}`, risk.type)}
    </div>

    <div class="grid two" style="gap:14px">
      <div class="card item">
        <div class="name">Comparativo por capítulos</div>
        <div class="muted small" style="margin:4px 0 10px">Top capítulos con mayor diferencia entre Base y Oficial.</div>
        <div style="height:340px"><canvas id="budgetAuditChartChapters"></canvas></div>
      </div>

      <div class="card item">
        <div class="name">Top ítems con mayor impacto</div>
        <div class="muted small" style="margin:4px 0 10px">Variación económica de los ítems más sensibles.</div>
        <div style="height:340px"><canvas id="budgetAuditChartItems"></canvas></div>
        <div style="margin-top:10px">
          <div class="muted small" style="font-weight:700; margin-bottom:4px">Detalle completo de etiquetas</div>
          ${itemLegendHtml || `<div class="muted small">Sin datos.</div>`}
        </div>
      </div>
    </div>

    <div class="muted small" style="margin-top:10px">
      El Dashboard usa datos reales del proyecto activo y se actualiza al cambiar entre Presupuesto Base y Presupuesto Oficial.
    </div>
  `;
  ...
  if(!(window.Chart && typeof window.Chart === "function")){
    ...
    warn.textContent = "Para ver los gráficos debes cargar Chart.js en proyecto-detalle.html.";
  ...
  const ctxCh = document.getElementById("budgetAuditChartChapters");
  const ctxIt = document.getElementById("budgetAuditChartItems");

  if(ctxCh){
    __budgetDashboardCharts.chapters = new Chart(ctxCh, {
      type: "bar",
      data: {
        labels: chapterData.labels,
        datasets: [
          { label: "Base", data: chapterData.base, borderWidth: 1 },
          { label: "Oficial", data: chapterData.official, borderWidth: 1 }
        ]
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        layout: { padding: { left: 10, right: 8, top: 4, bottom: 0 } },
        plugins: {
          legend: { display: true },
          tooltip: {
            callbacks: {
              label: (ctx)=> `${ctx.dataset.label}: ${UI.fmtMoney(ctx.parsed.y || 0, currency)}`
            }
          }
        },
        scales: {
          x: {
            ticks: { autoSkip: false, maxRotation: 32, minRotation: 18 }
          },
          y: {
            beginAtZero: true,
            ticks: {
              callback: (value)=> UI.fmtMoney(Number(value || 0), currency)
            }
          }
        }
      }
  ...
  if(ctxIt){
    __budgetDashboardCharts.items = new Chart(ctxIt, {
      type: "bar",
      data: {
        labels: itemData.labels,
        datasets: [
          { label: "Impacto económico", data: itemData.impact, borderWidth: 1 }
        ]
      },
      options: {
        indexAxis: "y",
        responsive: true,
        maintainAspectRatio: false,
        layout: { padding: { left: 18, right: 10, top: 4, bottom: 0 } },
        plugins: {
          legend: { display: true },
          tooltip: {
            callbacks: {
              title: (items)=> {
                ...
                return itemData.fullLabels?.[idx] || itemData.shortLabels?.[idx] || "";
              },
              label: (ctx)=> `Impacto: ${UI.fmtMoney(ctx.parsed.x || 0, currency)}`
            }
          }
        },
        scales: {
...
function refreshProjectBudgetAwareUI(projectId){
  ...
  renderBudgetAuditDashboard(fresh);
  renderArtificialBudgetPanel(fresh);
  renderProjectDetail(fresh);
  refreshPdfExportButtonsLabels(fresh);
  ...
  initProjectDetailCollapsibleSections(projectId);
...
function renderBudgetComparisonCard(project){
  ...
  body.innerHTML = `
    ...
    <div class="card item" style="margin-bottom:14px">
      <div class="name">Hallazgos automáticos de auditoría</div>
      <div class="row" style="gap:8px; flex-wrap:wrap; margin:10px 0 6px">
        ${getBudgetAuditFindings(project).map(f => UI.chip(
          f.severity === "bad" ? "CRÍTICO" : (f.severity === "warn" ? "ALERTA" : "OK"),
          f.severity === "bad" ? "bad" : (f.severity === "warn" ? "warn" : "ok")
        )).join(" ")}
      </div>
      <div style="display:grid; gap:10px">
        ${getBudgetAuditFindings(project).map(f => `
          <div style="padding:10px 12px; border:1px solid var(--border); border-radius:14px; background:rgba(255,255,255,.03)">
            <div class="row" style="justify-content:space-between; gap:10px; flex-wrap:wrap; margin-bottom:4px">
              <div style="font-weight:800">${UI.esc(f.title || "")}</div>
              ${f.action ? `<button type="button" class="btn" data-finding-focus="${UI.esc(f.focusTarget || "")}">${UI.esc(f.action)}</button>` : ""}
            </div>
            <div class="muted small">${UI.esc(f.detail || "")}</div>
          </div>
        `).join("")}
      </div>
    </div>
  ...
  body.querySelectorAll("[data-finding-focus]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const target = btn.getAttribute("data-finding-focus") || "";
      scrollToBudgetAuditSection(target);
    });
  });
}
...
function getBudgetItemComparisonRows(project){
  ...
  function accumulate(side, row, it){
    ...
    if(side === "base"){
      row.baseQty += qty;
      row.basePartial += partial;
      row.basePU = row.baseQty ? (row.basePartial / row.baseQty) : pu;
    }else{
      row.officialQty += qty;
      row.officialPartial += partial;
      row.officialPU = row.officialQty ? (row.officialPartial / row.officialQty) : pu;
    }
  ...
  officialItems.forEach(it=>{
    ...
    if(!key) return;
    const row = ensureRow(map, key, it);
    accumulate("official", row, it);
  });

  return Array.from(map.values()).map(r=>({
    ...r,
    baseQty: Number(r.baseQty || 0),
    basePU: Number(r.basePU || 0),
    officialQty: Number(r.officialQty || 0),
    officialPU: Number(r.officialPU || 0)
  })).sort((a,b)=>
    String(a.code || "").localeCompare(String(b.code || ""), "es", { numeric:true, sensitivity:"base" })
  );
}
...
async function exportAuditPdf(project){
  if(!project) return;
  ...
  if(!projectHasOfficialBudget(project)){
    alert("Primero debe existir un Presupuesto Oficial para generar el PDF Auditoría Base vs Oficial.");
    return;
  }
  ...
  const indirectCmp = getBudgetIndirectComparison(project);
  const methodAudit = getBudgetIndirectMethodAudit(project);
  const findings = getBudgetAuditFindings(project);
  const chapterRows = getBudgetChapterComparisonRows(project);
  const itemRows = getBudgetItemComparisonRows(project);
  ...
  function drawAuditCover(){
    ...
    doc.setFont("helvetica","bold");
    doc.setFontSize(14);
    doc.text("INFORME DE AUDITORÍA PRESUPUESTAL", W/2, y0, { align:"center" }); y0 += 22;

    doc.setFont("helvetica","bold");
    doc.setFontSize(11);
    doc.text("COMPARATIVO PRESUPUESTO BASE VS PRESUPUESTO OFICIAL", W/2, y0, { align:"center" }); y0 += 22;

    doc.setFont("helvetica","bold");
    doc.setFontSize(12);
    doc.text(`PROYECTO: ${up(projectLabel)}`, W/2, y0, { align:"center" }); y0 += 40;
  ...
  function getAuditRiskInfo(){
    const diffPct = Math.abs(Number(cmp.diffPct || 0));
    const alertCount = itemAlerts.length;
    const chapterAlertCount = chapterRows.filter(r=>{
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return Math.abs(pct) >= 10;
    }).length;

    if(diffPct >= 20 || alertCount >= 5 || chapterAlertCount >= 2){
      return { code:"R3", level:"ALTO", criterio:"Revisión inmediata" };
    }
    if(diffPct >= 10 || alertCount >= 2 || chapterAlertCount >= 1){
      return { code:"R2", level:"MEDIO", criterio:"Revisión prioritaria" };
    }
    return { code:"R1", level:"BAJO", criterio:"Revisión de rutina" };
  }

  const riskInfo = getAuditRiskInfo();
  ...
  function classifyAuditComponent(grpRaw){
  ...
  async function getAPUForBudgetItem(projectView, item){
    const itemCode = String(item?.apuRefCode || item?.code || "").trim();
    const linesFromOverride = (projectView?.apuOverrides && itemCode && projectView.apuOverrides[itemCode] && Array.isArray(projectView.apuOverrides[itemCode].lines))
      ? projectView.apuOverrides[itemCode].lines
      : null;

    if(linesFromOverride && linesFromOverride.length){
      return {
        code: itemCode,
        lines: linesFromOverride
      };
    }

    try{
      if(window.APUBase && typeof APUBase.getAPU === "function" && itemCode){
        const compareBaseId = getProjectComparisonBaseId(projectView);
        const apu = await APUBase.getAPU(itemCode, compareBaseId);
        if(apu && Array.isArray(apu.lines)){
          return apu;
        }
      }
    }catch(_){}
  ...
  async function buildApuComponentComparisons(limit=8){
    const out = [];
    const baseItems = Array.isArray(cmp.baseView?.items) ? cmp.baseView.items : [];
    const officialItems = Array.isArray(cmp.officialView?.items) ? cmp.officialView.items : [];

    function keyForItem(it){
      return String(it?.apuRefCode || it?.code || it?.desc || "").trim();
    }

    function normalizeDetailKey(v){
      return String(v || "")
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
    ...
    function linePartial(ln){
      return Number(ln?.parcial || 0) || (Number(ln?.qty || 0) * Number(ln?.pu || 0));
    }

    function buildDetailedComponentRows(baseLines, officialLines){
      const map = new Map();

      function inject(side, ln){
        const desc = String(ln?.desc || ln?.descripcion || "").trim();
        const unit = String(ln?.unit || ln?.unidad || "").trim();
        const key = `${normalizeDetailKey(desc)}__${normalizeDetailKey(unit)}`;
        if(!map.has(key)){
          map.set(key, {
            desc,
            unit,
        ...
        if(side === "base"){
          row.baseQty = Number(ln?.qty || 0);
          row.basePU = Number(ln?.pu || 0);
          row.baseParcial = linePartial(ln);
        }else{
          row.officialQty = Number(ln?.qty || 0);
          row.officialPU = Number(ln?.pu || 0);
          row.officialParcial = linePartial(ln);
        }
      }

      (baseLines || []).forEach(ln => inject("base", ln));
      (officialLines || []).forEach(ln => inject("official", ln));

      return Array.from(map.values())
        .map(r=>{
          const diff = Number(r.officialParcial || 0) - Number(r.baseParcial || 0);
          const pct = Number(r.baseParcial || 0) > 0 ? (diff / Number(r.baseParcial || 0)) * 100 : (Number(r.officialParcial || 0) > 0 ? 100 : 0);
          return { ...r, diff, pct };
        })
        .sort((a,b)=>Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)));
    }
    ...
    [...(riskTopRows || []), ...(itemRows || [])].forEach(r=>{
      const key = String(r?.code || r?.desc || "").trim();
      if(!key || seenCandidateKeys.has(key)) return;
      seenCandidateKeys.add(key);
      rankedCandidates.push(r);
    });
    ...
    for(const row of rankedCandidates){
      ...
      const baseItem =
        baseItems.find(it => String(it?.code || "").trim() === String(row?.code || "").trim()) ||
        baseItems.find(it => keyForItem(it) === String(row?.code || row?.desc || "").trim()) ||
        baseItems.find(it => String(it?.desc || "").trim() === String(row?.desc || "").trim());

      const officialItem =
        officialItems.find(it => String(it?.code || "").trim() === String(row?.code || "").trim()) ||
        officialItems.find(it => keyForItem(it) === String(baseItem?.apuRefCode || baseItem?.code || row?.code || "").trim()) ||
        officialItems.find(it => String(it?.desc || "").trim() === String(row?.desc || "").trim());

      if(!baseItem || !officialItem) continue;

      const baseApu = await getAPUForBudgetItem(cmp.baseView, baseItem);
      const officialApu = await getAPUForBudgetItem(cmp.officialView, officialItem);
      if(!baseApu || !officialApu) continue;

      const bucketsBase = { "MATERIALES":0, "EQUIPO Y HERRAM":0, "MANO DE OBRA":0, "TRANSPORTES":0, "SUBCONTRATOS":0, "OTROS":0 };
      const bucketsOff = { "MATERIALES":0, "EQUIPO Y HERRAM":0, "MANO DE OBRA":0, "TRANSPORTES":0, "SUBCONTRATOS":0, "OTROS":0 };
      const groupedBase = { "MATERIALES":[], "EQUIPO Y HERRAM":[], "MANO DE OBRA":[], "TRANSPORTES":[], "SUBCONTRATOS":[], "OTROS":[] };
      const groupedOff = { "MATERIALES":[], "EQUIPO Y HERRAM":[], "MANO DE OBRA":[], "TRANSPORTES":[], "SUBCONTRATOS":[], "OTROS":[] };

      (baseApu.lines || []).forEach(ln=>{
        const k = classifyAuditComponent(ln.group || ln.tipo || "");
        const parcial = linePartial(ln);
        bucketsBase[k] = Number(bucketsBase[k] || 0) + parcial;
        groupedBase[k].push(ln);
      });

      (officialApu.lines || []).forEach(ln=>{
        const k = classifyAuditComponent(ln.group || ln.tipo || "");
        const parcial = linePartial(ln);
        bucketsOff[k] = Number(bucketsOff[k] || 0) + parcial;
        groupedOff[k].push(ln);
      });

      const componentRows = componentOrder.map(label=>{
        const baseVal = Number(bucketsBase[label] || 0);
        const offVal = Number(bucketsOff[label] || 0);
        const diff = offVal - baseVal;
        const pct = baseVal > 0 ? (diff / baseVal) * 100 : (offVal > 0 ? 100 : 0);
        return {
          label,
          baseVal,
          offVal,
          diff,
          pct,
          detailRows: buildDetailedComponentRows(groupedBase[label], groupedOff[label])
        };
      }).filter(r => r.baseVal > 0 || r.offVal > 0);

      if(!componentRows.length) continue;

      out.push({
        code: String(baseItem?.code || officialItem?.code || row?.code || "").trim(),
        desc: String(baseItem?.desc || officialItem?.desc || row?.desc || "").trim(),
        rows: componentRows
      });
    }
  ...
  function stampHeader(){
    ...
    if(logo){
      tryAddImageLocal(logo, M.left, 14, 42, 24);
    }
    const textX = logo ? (M.left + 52) : M.left;
    doc.setFont("helvetica","bold");
    doc.setFontSize(9.5);
    ...
    doc.setFont("helvetica","normal");
    doc.setFontSize(9.2);
    doc.text(safeText(project.instProyectoLabel || project.name || ""), textX, 40);
    doc.text(nowTxt, W - M.right, 28, { align:"right" });
    line(50, 0.6);
  }
  ...
  function addTitle(text, opts={}){
    const beforeGap = Number(opts.beforeGap ?? 18);
    const minAfter = Number(opts.minAfter ?? 90);

    if(y > contentTopY + 8){
      ensureSpace(beforeGap + 30 + minAfter);
      y += beforeGap;
    }else{
      ensureSpace(30 + minAfter);
    }
  ...
  function addBullets(items){
    items.forEach(txt=>{
      addParagraph(`• ${txt}`, 10, 12, 4);
    });
  }

  function calcParagraphHeight(text, fontSize=9.8, spacing=11, after=6){
    doc.setFont("helvetica","normal");
    doc.setFontSize(fontSize);
    const lines = doc.splitTextToSize(safeText(text), usableW);
    return (lines.length * spacing) + after + 2;
  }
  ...
  function calcTableBlockHeight(headers, rows, widths, opts={}){
    const fontSize = Number(opts.fontSize || 8.3);
    const lineHeight = Number(opts.lineHeight || 9);
    const headerGap = 18;
    const estimatedRowsHeight = calcTableRowsHeight(rows, widths, fontSize, lineHeight);
    return headerGap + estimatedRowsHeight + 10;
  }
  ...
  function addSimpleTable(headers, rows, widths, opts={}){
    ...
    const estimatedRowsHeight = calcTableRowsHeight(rows, widths, fontSize, lineHeight);
    const estimatedTotal = headerGap + estimatedRowsHeight + 10;

    if(keepTogether && estimatedTotal <= usablePageHeight && y + estimatedTotal > H - M.bottom){
      addPage();
    }else{
      ensureSpace(headerGap + 20);
    }
  ...
  async function addSignatureBlock(){
    ...
    if(cleanSignature){
      try{
        doc.addImage(cleanSignature, "PNG", sigX + 8, sigY + 8, sigW - 16, sigH - 16);
      }catch(_){}
    }
    ...
    y += 14;
    if(elaborador?.matricula) doc.text(`M.P. ${safeText(elaborador.matricula)}`, M.left, y);
    y += 12;
  }
  ...
  const chapterInsights = chapterRows
    .map(r=>{
      const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
      const pct = Number(r.baseSubtotal || 0) > 0 ? (diff / Number(r.baseSubtotal || 0)) * 100 : 0;
      return { ...r, diff, pct };
  ...
  const leadChapter = chapterInsights[0] || null;
  const leadRisk = riskTopRows[0] || null;

  addTitle("1. Resumen ejecutivo");
  addParagraph(
    `Se evaluó la consistencia del Presupuesto Oficial frente al Presupuesto Base del proyecto "${safeText(project.name || "")}". ` +
    `El análisis identifica una variación total de ${money(cmp.diff || 0)}, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, clasificada en nivel de riesgo ${riskInfo.code} ${riskInfo.level}. ` +
    `${methodAudit.sameMethod ? "Ambos presupuestos mantienen el mismo criterio de cálculo de indirectos." : `Se detectó una diferencia en el método de cálculo de indirectos: Base ${methodAudit.baseModeLabel} y Oficial ${methodAudit.officialModeLabel}.`}`
  );

  addBullets([
    `Presupuesto Base: ${money(cmp.tBase.total || 0)}.`,
    `Presupuesto Oficial: ${money(cmp.tOfficial.total || 0)}.`,
    `Diferencia en costos directos: ${money((cmp.tOfficial.directo || 0) - (cmp.tBase.directo || 0))}.`,
    leadChapter
      ? `Capítulo con mayor incidencia: ${leadChapter.chapterCode} ${leadChapter.chapterName} con variación de ${Number(leadChapter.pct || 0).toFixed(2)}%.`
      : "No se detectó capítulo dominante en la variación.",
    leadRisk
      ? `Ítem crítico con mayor impacto: ${leadRisk.code} – ${leadRisk.desc} con impacto de ${money(leadRisk.impact || 0)}.`
      : "No se detectaron ítems críticos con impacto económico relevante."
  ]);

  addTitle("2. Semáforo y lectura de riesgo");
  addKeyValueRows([
    ["Semáforo global", `${riskInfo.code}  ${riskInfo.level}`],
    ["Criterio de lectura", riskInfo.criterio],
    ["Método indirectos Base", methodAudit.baseModeLabel],
    ["Método indirectos Oficial", methodAudit.officialModeLabel]
  ]);
  addParagraph(
    "Interpretación técnica: el semáforo se asigna considerando la variación total entre presupuestos, la existencia de alertas automáticas por ítems, la concentración de diferencias por capítulos y la consistencia del criterio de cálculo de indirectos."
  );

  addTitle("3. Matriz de hallazgos");
  addSimpleTable(
    ["Estado", "Hallazgo", "Detalle", "Impacto / Recomendación"],
    findings.map(f=>[
      f.severity === "bad" ? "CRÍTICO" : (f.severity === "warn" ? "ALERTA" : "OK"),
      f.title || "",
      f.detail || "",
      f.severity === "bad"
        ? "Validar soportes técnicos y financieros antes de aprobación."
        : (f.severity === "warn" ? "Revisar trazabilidad y consistencia del dato." : "Sin acción correctiva prioritaria.")
    ]),
    [82, 130, 190, 122],
    { keepTogether:true }
  );

  addTitle("4. Comparación de costos indirectos");
  addSimpleTable(
  ...
  if(indirectCmp.rows.length){
    const indirectLead = indirectCmp.rows
      .filter(r => ["Administración","Imprevistos","Utilidad","Otros"].includes(String(r.label || "")))
      .sort((a,b)=> Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)))[0];
    if(indirectLead){
      addParagraph(
        `El componente indirecto de mayor variación es ${safeText(indirectLead.label)}, con diferencia de ${money(indirectLead.diff || 0)} (${Number(indirectLead.diffPct || 0).toFixed(2)}%).`
      );
    }
  }
  ...
  if(leadChapter){
    addParagraph(
      `El capítulo con mayor impacto económico corresponde a ${safeText(leadChapter.chapterCode)} ${safeText(leadChapter.chapterName)}, con una diferencia de ${money(leadChapter.diff || 0)} y variación relativa de ${Number(leadChapter.pct || 0).toFixed(2)}%.`
    );
  }

  addTitle("6. Ranking de ítems críticos por impacto económico");
  ...
  addTitle("7. Ítems con alerta automática");
  ...
  addTitle("8. Comparativo técnico por APU y componente");
  addParagraph(
    "La siguiente matriz compara la estructura interna de los APUs más sensibles, discriminando materiales, equipo y herramienta, mano de obra, transporte, subcontratos y otros. Los valores corresponden al costo directo unitario de cada APU."
  );

  const apuComparisons = await buildApuComponentComparisons(6);

  if(!apuComparisons.length){
    addParagraph(
      "No fue posible construir comparativos internos de APU para este proyecto con la información actualmente disponible en la base activa y en los overrides del proyecto."
    );
  }else{
    addParagraph(
      "Además del comparativo condensado por componente, a continuación se presenta el desglose interno de cada componente para identificar exactamente qué materiales, equipos, transportes, cuadrillas, subcontratos u otros recursos explican la variación entre Presupuesto Base y Presupuesto Oficial.",
      9.6,
      11,
      8
    );

    for(const apuCmp of apuComparisons){
      const apuTitleText = `${safeText(apuCmp.code)} — ${safeText(apuCmp.desc)}`;
      const apuTableRows = (apuCmp.rows || []).map(r=>[
        r.label || "",
        money(r.baseVal || 0),
        money(r.offVal || 0),
        money(r.diff || 0),
      ...
      const leadComponent = safeText(
        (apuCmp.rows || []).slice().sort((a,b)=>Math.abs(Number(b.diff || 0)) - Math.abs(Number(a.diff || 0)))[0]?.label || "componentes internos"
      );
      const lecturaText = `Lectura técnica: en el APU ${safeText(apuCmp.code)} la mayor diferencia se concentra en ${leadComponent}.`;

      const detailBlocksHeight = (apuCmp.rows || []).reduce((acc, compRow)=>{
        if(!compRow?.detailRows?.length) return acc;
        const titleH = calcParagraphHeight(`Detalle componente: ${safeText(compRow.label)}`, 9.4, 10.5, 3);
        const tableH = calcTableBlockHeight(
          ["Recurso / Insumo", "Und.", "Base Parcial", "Oficial Parcial", "Dif.", "% Dif."],
          compRow.detailRows.map(d=>[
            d.desc || "",
            d.unit || "",
            money(d.baseParcial || 0),
            money(d.officialParcial || 0),
            money(d.diff || 0),
            `${Number(d.pct || 0).toFixed(2)}%`
          ]),
          [210, 42, 78, 78, 72, 52],
          { keepTogether:false }
        );
        return acc + titleH + tableH + 4;
      }, 0);

      const blockHeight =
        calcParagraphHeight(apuTitleText, 10, 12, 4) +
        calcTableBlockHeight(["Componente", "Base", "Oficial", "Dif.", "% Dif."], apuTableRows, [150, 90, 90, 90, 70], { keepTogether:true }) +
        calcParagraphHeight(lecturaText, 9.4, 10.5, 8) +
        detailBlocksHeight + 6;

      if(blockHeight <= usablePageHeight && y + blockHeight > H - M.bottom){
        addPage();
      }

      addParagraph(apuTitleText, 10, 12, 4);
      addSimpleTable(
        ["Componente", "Base", "Oficial", "Dif.", "% Dif."],
        apuTableRows,
        [150, 90, 90, 90, 70],
        { keepTogether:true }
      );
      ...
      for(const compRow of (apuCmp.rows || [])){
        if(!compRow?.detailRows?.length) continue;

        const detailTitle = `Detalle componente: ${safeText(compRow.label)}`;
        const detailRows = compRow.detailRows.map(d=>[
          d.desc || "",
          d.unit || "",
          money(d.baseParcial || 0),
          money(d.officialParcial || 0),
        ...
        const detailBlockHeight =
          calcParagraphHeight(detailTitle, 9.4, 10.5, 3) +
          calcTableBlockHeight(
            ["Recurso / Insumo", "Und.", "Base Parcial", "Oficial Parcial", "Dif.", "% Dif."],
            detailRows,
            [210, 42, 78, 78, 72, 52],
            { keepTogether:false }
        ...
        if(detailBlockHeight <= usablePageHeight && y + detailBlockHeight > H - M.bottom){
          addPage();
        }

        addParagraph(detailTitle, 9.4, 10.5, 3);
        addSimpleTable(
          ["Recurso / Insumo", "Und.", "Base Parcial", "Oficial Parcial", "Dif.", "% Dif."],
          detailRows,
          [210, 42, 78, 78, 72, 52],
  ...
  addTitle("9. Conclusión técnica");
  const highAlerts = itemAlerts.length;
  const positiveChapters = chapterInsights.filter(r => Number(r.diff || 0) > 0).length;
  addParagraph(
    `Se concluye que el Presupuesto Oficial presenta una variación de ${money(cmp.diff || 0)} respecto del Presupuesto Base, equivalente a ${Number(cmp.diffPct || 0).toFixed(2)}%, con clasificación de riesgo ${riskInfo.code} ${riskInfo.level}. ` +
    `Se identificaron ${highAlerts} ítems priorizados por impacto económico y ${positiveChapters} capítulos con incremento frente al presupuesto base. ` +
    `${methodAudit.sameMethod ? "No se evidencia cambio en el criterio de cálculo de indirectos entre ambos presupuestos. " : `Se detectó diferencia en el criterio de indirectos: Base ${methodAudit.baseModeLabel} / Oficial ${methodAudit.officialModeLabel}. `}` +
    `Desde una perspectiva de auditoría forense, los resultados recomiendan validar soportes técnicos, trazabilidad de cantidades, análisis de precios unitarios y justificación contractual de los componentes que explican la desviación observada antes de dar por aprobado el presupuesto oficial.`
  );

  await addSignatureBlock();
  drawFooter();

  const filename = `Informe_Auditoria_Presupuestal_${sanitizeNameLocal(project?.name || "proyecto")}_${Date.now()}.pdf`;
  doc.save(filename);
}

async function exportPresupuestoArtificialmenteBajoPdf(project){
  const fresh = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  if(!fresh) return;
  await exportArtificialBudgetPdf(fresh, {});
}

async function exportJustificacionPresupuestoArtificialmenteBajoPdf(project){
  const fresh = project && project.id && window.StorageAPI && typeof StorageAPI.getProjectById === "function"
    ? (StorageAPI.getProjectById(project.id) || project)
    : project;
  if(!fresh) return;
...
async function getBaseAPUByAnyLookup(codeA, codeB, baseId){
  const tries = [normalizeLookupCode(codeA), normalizeLookupCode(codeB)].filter(Boolean);
  if(!tries.length || !window.APUBase || typeof APUBase.getAPU !== "function") return null;

  const compareBaseId = String(baseId || getGlobalComparisonBaseId() || "default").trim() || "default";

  for(const key of tries){
    try{
      const apu = await APUBase.getAPU(key, compareBaseId);
      if(apu) return { key, apu };
    }catch(_){ }
  }
  return null;
...
async function tryResolveAPUFromProjectItem(projectId, requestedCode, baseId){
  ...
  const compareBaseId = String(baseId || getProjectComparisonBaseId(projectId || project) || "default").trim() || "default";
  const baseHit = await getBaseAPUByAnyLookup(refCode, visibleCode, compareBaseId);
  if(baseHit){
    return {
      source: "base",
...
async function renderKpis(){
  ...
  try{
    const installed = getInstalledBasesForUi();
    const currentBaseId = getActiveBaseIdForUi();
    const activeLabel = getBaseLabelById(currentBaseId);
    const countBases = Array.isArray(installed) ? installed.length : 0;

    let meta = null;
    try{
      meta = await APUBase.getMeta(currentBaseId);
    }catch(_){
      try{ meta = await APUBase.getMeta(); }catch(__){}
    }

    if(meta){
      baseChip = UI.chip(`${activeLabel} · ${countBases} base${countBases === 1 ? "" : "s"} · ${meta.counts?.items||0} items`,"ok");
    }else{
      baseChip = UI.chip(`${activeLabel} · ${countBases} base${countBases === 1 ? "" : "s"}`,"ok");
    }
  }catch(_){}
...
function renderProjects(){
  ...
  tbody.innerHTML = ps.map(p=>{
    ...
    const baseId = getProjectBaseId(p);
    const baseLabel = getBaseLabelById(baseId);
    return `
      <tr>
        <td><b>${UI.esc(p.name)}</b></td>
        <td>${UI.esc(p.entity||"-")}</td>
        <td>${UI.esc(p.location||"-")}</td>
        <td>${UI.esc(baseLabel)}</td>
...
async function bindProjectsPage(){
  UI.qs("#btnNew")?.addEventListener("click", ()=>{
    ...
    const selectedBaseId = String(UI.qs("#selectBase")?.value || "").trim();
    const baseId = selectedBaseId || (
      (window.StorageAPI && typeof StorageAPI.getDefaultBaseId === "function")
        ? String(StorageAPI.getDefaultBaseId() || "default").trim() || "default"
        : "default"
    );
...
function renderProjectDetail(project){
  ...
  UI.qs("#projSub") && (UI.qs("#projSub").textContent = `${project.entity || "—"} · ${project.location || "—"} · ${project.currency || "COP"} · Base origen: ${getProjectBaseId(project)} · Base comparación: ${getProjectComparisonBaseId(project)} · Presupuesto activo: ${budgetInfo.label}`);
  ...
  if(info){
    info.innerHTML = `
      <div class="cardhead">
        ...
        <p class="muted small">Entidad: ${UI.esc(project.entity||"-")} · Ubicación: ${UI.esc(project.location||"-")} · Base origen: ${UI.esc(getProjectBaseId(project))} · Base comparación: ${UI.esc(getProjectComparisonBaseId(project))} · Presupuesto activo: ${UI.esc(budgetInfo.label)}</p>
      </div>
      <div class="row" style="gap:8px; flex-wrap:wrap; margin-bottom:10px">
...
async function pickIndirectResourceFromBase(projectId, rowId, sourceType){
  ...
  const activeBaseId = getActiveReadBaseIdForProject(projectId);
  let meta = null;
  try{ meta = await APUBase.getMeta(activeBaseId); }catch(_){}
  if(!meta){
  ...
  if(type === "insumo"){
    let rows = [];
    try{
      rows = await APUBase.listInsumos(term, 40, activeBaseId);
    }catch(err){
  ...
  let list = [];
  try{
    list = await APUBase.listSubproductos(200, activeBaseId);
  }catch(err){
  ...
  let subApu = null;
  try{
    subApu = await APUBase.getSubAPU(sel.subKey, activeBaseId);
  }catch(err){
...
function bindBaseViewer(projectId){
  ...
  async function loadRows(q){
    ...
    try{
      if(mode === "formulario"){
        const activeBaseId = getActiveReadBaseIdForProject(projectId);
        const rows = await APUBase.listFormularioItems(q || "", undefined, activeBaseId);
      ...
      if(mode === "cd"){
        const activeBaseId = getActiveReadBaseIdForProject(projectId);
        const rows = await APUBase.listCostosDirectosAll(q || "", undefined, activeBaseId);
        if(!rows.length){ showEmpty(true); return; }
      ...
      if(mode === "sub"){
        const activeBaseId = getActiveReadBaseIdForProject(projectId);
        const list = await APUBase.listSubproductos(undefined, activeBaseId);
      ...
      if(mode === "insumos"){
        const activeBaseId = getActiveReadBaseIdForProject(projectId);
        const rows = await APUBase.listInsumos(q || "", undefined, activeBaseId);
        if(!rows.length){ showEmpty(true); return; }
...
let __createApuLines = [];
let __createApuSearchResults = [];

function computeCreateApuLinesDirecto(lines){
  return (Array.isArray(lines) ? lines : []).reduce((sum, l)=>{
    const qty = Number(l?.qty || 0);
    const pu = Number(l?.pu || 0);
    const parcial = Number(l?.parcial || 0) || (qty * pu);
    return sum + Number(parcial || 0);
  }, 0);
}

function renderCreateApuBuildLines(project){
  const apuBuildLinesBody = UI.qs("#apuBuildLinesBody");
  const apuBuildTotalChip = UI.qs("#apuBuildTotalChip");
  if(!apuBuildLinesBody) return;

  if(!__createApuLines.length){
    apuBuildLinesBody.innerHTML = "";
    if(apuBuildTotalChip) apuBuildTotalChip.textContent = `PU APU: ${UI.fmtMoney(0, project?.currency || "COP")}`;
    return;
  }

  apuBuildLinesBody.innerHTML = __createApuLines.map((l, idx)=>{
    const qty = Number(l.qty || 0);
    const pu = Number(l.pu || 0);
    const parcial = Number(l.parcial || 0) || (qty * pu);
    return `
      <tr>
        ...
        <td>${UI.esc(l.desc || "")}</td>
        <td>${UI.esc(l.unit || "")}</td>
        <td style="text-align:right">${UI.esc(String(qty))}</td>
        <td style="text-align:right"><b>${UI.fmtMoney(pu, project?.currency || "COP")}</b></td>
        <td style="text-align:right"><b>${UI.fmtMoney(parcial, project?.currency || "COP")}</b></td>
        <td class="row" style="gap:8px">
          <button class="btn" type="button" data-apubuild-edit="${idx}">Editar</button>
          <button class="btn danger" type="button" data-apubuild-rm="${idx}">Quitar</button>
        </td>
      </tr>
    `;
  }).join("");

  if(apuBuildTotalChip){
    apuBuildTotalChip.textContent = `PU APU: ${UI.fmtMoney(computeCreateApuLinesDirecto(__createApuLines), project?.currency || "COP")}`;
  }

  apuBuildLinesBody.querySelectorAll("[data-apubuild-rm]").forEach(btn=>{
    btn.onclick = ()=>{
      const idx = Number(btn.getAttribute("data-apubuild-rm"));
      if(!(idx >= 0)) return;
      __createApuLines.splice(idx, 1);
      renderCreateApuBuildLines(project);
    };
  });

  apuBuildLinesBody.querySelectorAll("[data-apubuild-edit]").forEach(btn=>{
    btn.onclick = ()=>{
      const idx = Number(btn.getAttribute("data-apubuild-edit"));
      const row = __createApuLines[idx];
      if(!row) return;

      const nextQty = prompt("Cantidad:", String(row.qty || 1));
      if(nextQty == null) return;
      const qty = Number(String(nextQty).replaceAll(",", ""));
      if(!(qty >= 0)) return alert("Cantidad inválida.");

      const nextPu = prompt("PU:", String(row.pu || 0));
      if(nextPu == null) return;
      const pu = Number(String(nextPu).replaceAll(",", ""));
      if(!(pu >= 0)) return alert("PU inválido.");
...
function setCreateApuSearchResults(project, rows){
  __createApuSearchResults = Array.isArray(rows) ? rows.slice() : [];

  const insumoResultsBody = UI.qs("#insumoResultsBody");
  const insumoResultsEmpty = UI.qs("#insumoResultsEmpty");
  if(!insumoResultsBody) return;

  if(!__createApuSearchResults.length){
    insumoResultsBody.innerHTML = "";
    if(insumoResultsEmpty) insumoResultsEmpty.style.display = "";
    return;
  }

  if(insumoResultsEmpty) insumoResultsEmpty.style.display = "none";

  insumoResultsBody.innerHTML = __createApuSearchResults.map((r, idx)=>`
    <tr>
      <td>${UI.esc(r.kindLabel || r.group || "-")}</td>
      <td>${UI.esc(r.desc || "")}</td>
      <td>${UI.esc(r.unit || "")}</td>
      <td style="text-align:right"><b>${UI.fmtMoney(Number(r.pu || 0), project?.currency || "COP")}</b></td>
      <td class="row" style="gap:8px">
        <button class="btn primary" type="button" data-insumo-pick="${idx}">Agregar</button>
      </td>
    </tr>
  `).join("");

  insumoResultsBody.querySelectorAll("[data-insumo-pick]").forEach(btn=>{
    btn.onclick = ()=>{
      const idx = Number(btn.getAttribute("data-insumo-pick"));
      const row = __createApuSearchResults[idx];
      if(!row) return;

      __createApuLines.push({
        kind: String(row.kind || row.kindLabel || "INSUMO"),
        kindLabel: String(row.kindLabel || row.group || "INSUMO"),
        tipo: String(row.group || row.kindLabel || "INSUMO"),
        group: String(row.group || row.kindLabel || "INSUMO"),
        desc: String(row.desc || "").trim(),
        unit: String(row.unit || "").trim(),
        qty: 1,
        pu: Number(row.pu || 0),
        parcial: Number(row.pu || 0),
        subRef: String(row.subRef || "").trim(),
        baseCode: String(row.baseCode || "").trim(),
...
async function searchCreateApuInputs(project){
  const insumoSearchInput = UI.qs("#insumoSearch");
  const raw = String(insumoSearchInput?.value || "").trim();
  if(!raw){
    setCreateApuSearchResults(project, []);
    return;
  }
  ...
  const qNorm = normalizeTextForMatch(raw);
  const baseId = getProjectComparisonBaseId(project);

  let baseInsumos = [];
  try{
    if(typeof APUBase.listInsumos === "function"){
      baseInsumos = await APUBase.listInsumos(raw, 40, baseId);
    }
  }catch(_){
    try{
      if(typeof APUBase.listInsumos === "function"){
  ...
  let customInsumos = [];
  try{
    if(window.StorageAPI && typeof StorageAPI.listCustomInsumos === "function"){
      customInsumos = StorageAPI.listCustomInsumos() || [];
    }
  }catch(_){}

  let subproductos = [];
  try{
    if(typeof APUBase.listSubproductos === "function"){
      subproductos = await APUBase.listSubproductos(200, baseId);
    }
  }catch(_){
    try{
      if(typeof APUBase.listSubproductos === "function"){
  ...
  const insumoRows = [
    ...(Array.isArray(customInsumos) ? customInsumos : []).map(r=>({
      kind: "custom_insumo",
      kindLabel: "INSUMO",
      group: String(r?.tipo || r?.group || "MATERIALES").trim(),
      desc: String(r?.desc || r?.descripcion || "").trim(),
      unit: String(r?.unit || r?.unidad || "UND").trim(),
      pu: Number(r?.pu || 0),
      source: "custom_insumo",
      baseCode: String(r?.id || r?.code || r?.desc || "").trim()
    })),
    ...(Array.isArray(baseInsumos) ? baseInsumos : []).map(r=>({
      kind: "base_insumo",
      kindLabel: "INSUMO",
      group: String(r?.tipo || r?.group || "MATERIALES").trim(),
      desc: String(r?.desc || r?.descripcion || r?.name || "").trim(),
      unit: String(r?.unit || r?.unidad || "UND").trim(),
      pu: Number(r?.pu || 0),
      source: "base_insumo",
      baseCode: String(r?.code || r?.id || r?.desc || r?.descripcion || "").trim()
    }))
  ].filter(r=>{
    return [r.kindLabel, r.group, r.desc, r.unit, r.baseCode].some(v => normalizeTextForMatch(v).includes(qNorm));
  });

  const subRows = [];
  for(const r of (Array.isArray(subproductos) ? subproductos : [])){
    const desc = String(r?.subName || r?.name || r?.desc || r?.descripcion || r?.subKey || "").trim();
    const unit = String(r?.unit || r?.unidad || "GLB").trim();
    const match = [desc, r?.subKey, r?.name, r?.desc, r?.descripcion, unit].some(v => normalizeTextForMatch(v).includes(qNorm));
    if(!match) continue;

    let subApu = null;
    try{
      if(typeof APUBase.getSubAPU === "function"){
        subApu = await APUBase.getSubAPU(r.subKey, baseId);
      }
    }catch(_){
      try{
        if(typeof APUBase.getSubAPU === "function"){
    ...
    subRows.push({
      kind: "base_subproducto",
      kindLabel: "SUBPRODUCTO",
      group: "SUBPRODUCTO",
      desc,
      unit: String(subApu?.unit || unit || "GLB").trim(),
      pu: Number(subApu?.directo || 0),
      subRef: String(r?.subKey || "").trim(),
      source: "base_subproducto",
      baseCode: String(r?.subKey || "").trim()
    });
  }

  const merged = [...insumoRows, ...subRows]
    .sort((a,b)=>{
      if(String(a.kindLabel) !== String(b.kindLabel)) return String(a.kindLabel).localeCompare(String(b.kindLabel));
      return String(a.desc || "").localeCompare(String(b.desc || ""), "es", { sensitivity:"base" });
    })
    .slice(0, 40);

  setCreateApuSearchResults(project, merged);
}

function bindCreateApuFromInsumosUI(projectId){
  const project = StorageAPI.getProjectById(projectId);
  if(!project) return;

  const formCreateApu = UI.qs("#formCreateApuFromInsumos");
  const formCreateInsumo = UI.qs("#formCreateInsumo");
  const insumoSearchInput = UI.qs("#insumoSearch");
  const btnInsumoSearch = UI.qs("#btnInsumoSearch");

  if(btnInsumoSearch && !btnInsumoSearch.__boundCreateApuSearch){
    btnInsumoSearch.__boundCreateApuSearch = true;
    btnInsumoSearch.addEventListener("click", async (e)=>{
      e.preventDefault();
      try{
        const fresh = StorageAPI.getProjectById(projectId) || project;
        await searchCreateApuInputs(fresh);
      }catch(err){
        alert("No se pudo buscar insumos/subproductos: " + (err?.message || err));
      }
    });
  }

  if(insumoSearchInput && !insumoSearchInput.__boundCreateApuSearch){
    insumoSearchInput.__boundCreateApuSearch = true;
    insumoSearchInput.addEventListener("keydown", async (e)=>{
      if(e.key === "Enter"){
        e.preventDefault();
        btnInsumoSearch?.click();
      }
    });
  }

  if(formCreateInsumo && !formCreateInsumo.__boundCreateApuSaveInsumo){
    formCreateInsumo.__boundCreateApuSaveInsumo = true;
    formCreateInsumo.addEventListener("submit", (e)=>{
      e.preventDefault();
      try{
        const fd = new FormData(e.target);
        const tipo = String(fd.get("tipo") || "").trim() || "MATERIALES";
        const unit = String(fd.get("unit") || "").trim();
        const desc = String(fd.get("desc") || "").trim();
        const pu = Number(String(fd.get("pu") || "0").replaceAll(",", ""));

        if(!desc) throw new Error("Debes escribir la descripción del insumo.");
        if(!unit) throw new Error("Debes escribir la unidad.");
        if(!(pu >= 0)) throw new Error("El PU debe ser un número válido.");
        if(!window.StorageAPI || typeof StorageAPI.saveCustomInsumo !== "function"){
          throw new Error("No está disponible el guardado de insumos.");
        }

        StorageAPI.saveCustomInsumo({ tipo, unit, desc, pu });
        e.target.reset();
        alert("Insumo nuevo guardado correctamente.");
        const fresh = StorageAPI.getProjectById(projectId) || project;
        searchCreateApuInputs(fresh).catch(()=>{});
      }catch(err){
        alert("No se pudo guardar el insumo nuevo: " + (err?.message || err));
      }
    });
  }

  if(formCreateApu && !formCreateApu.__boundCreateApuSaveApu){
    formCreateApu.__boundCreateApuSaveApu = true;
    formCreateApu.addEventListener("submit", (e)=>{
      e.preventDefault();
      try{
        const fresh = StorageAPI.getProjectById(projectId) || project;
        const fd = new FormData(e.target);
        const chapterCode = String(fd.get("chapterCode") || UI.qs("#selChapterBase")?.value || "").trim();
        const chapterName = String(fd.get("chapterName") || UI.qs("#inpChapterName")?.value || lookupChapterName(fresh, chapterCode) || "").trim();
        const code = String(fd.get("code") || "").trim();
        const unit = String(fd.get("unit") || "").trim();
        const desc = String(fd.get("desc") || "").trim();

        if(!chapterCode) throw new Error("Selecciona el capítulo base.");
        if(!code || !unit || !desc) throw new Error("Completa código, unidad y descripción del APU.");
        if(!__createApuLines.length) throw new Error("Debes agregar al menos un insumo o subproducto al APU.");

        if(!window.StorageAPI || typeof StorageAPI.saveCustomAPU !== "function"){
          throw new Error("No está disponible el guardado de APUs creados.");
        }

        const cleaned = __createApuLines.map(l=>({
          tipo: String(l.group || l.tipo || "-").trim(),
        ...
        __createApuLines = [];
        __createApuSearchResults = [];
        e.target.reset();
        setCreateApuSearchResults(fresh, []);
        renderCreateApuBuildLines(fresh);
        refreshCreateApuChapterOptions(StorageAPI.getProjectById(projectId) || fresh);
        alert("APU nuevo guardado correctamente.");
      }catch(err){
        alert("No se pudo guardar el APU nuevo: " + (err?.message || err));
      }
    });
  }
...
async function bindProjectDetailPage(){
  ...
  refreshProjectBudgetAwareUI(project.id);
  refreshPdfExportButtonsLabels(StorageAPI.getProjectById(project.id) || project);
  renderDocs(projectId).catch(()=>{});
  ...
  UI.qs("#btnApuSearch")?.addEventListener("click", async ()=>{
    try{
      const activeBaseId = getActiveReadBaseIdForProject(projectId || null);
      const meta = await APUBase.getMeta(activeBaseId);
      if(!meta){
        alert("Primero instala la Base APU (XLSX) desde Proyectos.");
        return;
      }
      ...
      const results = await APUBase.search(q, 30, activeBaseId);
...
async function bindAPUPage(){
  ...
  if(apuCode){
    ...
    if(!apu){
      ...
      if(custom){
        ...
        apu = await APUBase.getAPU(apuCode, getProjectComparisonBaseId(projectId || null));
    ...
    if(!subApu){
      subApu = await APUBase.getSubAPU(subKey, getProjectComparisonBaseId(projectId || null));
    }
  ...
  if(projectId && apuCode){
    ...
    localLines = (apu.lines||[]).map(l=>({
      ...
      parcial: Number(l.parcial||0) || (Number(l.qty||0)*Number(l.pu||0)),
      subRef: String(l.subRef || "").trim(),
      source: String(l.source || "manual").trim(),
      baseCode: String(l.baseCode || "").trim()
    }));
    ...
    btnAddLine?.addEventListener("click", async ()=>{
      const mode = String(prompt(
        "¿Cómo deseas agregar la línea del APU?\n\n1 = Manual\n2 = Desde base de datos (insumo)\n3 = Desde base de datos (subproducto)\n\nEscribe 1, 2 o 3:",
        "1"
      ) || "1").trim();
      ...
      if(mode === "2" || mode === "3"){
        ...
        try{
          const activeBaseId = getActiveReadBaseIdForProject(projectId);
          meta = await APUBase.getMeta(activeBaseId);
        }catch(_){}
        ...
        if(mode === "2"){
          const term = String(prompt("Buscar insumo en la base:", "") || "").trim();

          let rows = [];
          try{
            const activeBaseId = getActiveReadBaseIdForProject(projectId);
            rows = await APUBase.listInsumos("", 500, activeBaseId);
          }catch(err){
            alert("Error consultando insumos: " + (err?.message || err));
            return;
          }

          const q = normalizeTextForMatch(term);
          rows = (rows || []).filter(r=>{
            if(!q) return true;
            return [
              r?.tipo,
              ...
              r?.code,
              r?.id
            ].some(v => normalizeTextForMatch(v).includes(q));
          });
          ...
          const menu = rows.map((r, i)=>
            `${i+1}) ${r.tipo || "-"} | ${r.desc || r.descripcion || ""} | ${r.unit || r.unidad || ""} | ${UI.fmtMoney(r.pu || 0, "COP")}`
          ).join("\n");
          ...
          localLines.push({
            group: String(sel.tipo || "MATERIALES").trim(),
            desc: String(sel.desc || sel.descripcion || "").trim(),
            unit: String(sel.unit || sel.unidad || "UND").trim(),
            qty,
            pu: Number(sel.pu || 0),
            parcial: qty * Number(sel.pu || 0),
            subRef: "",
            source: "base_insumo",
            baseCode: String(sel.code || sel.id || sel.desc || sel.descripcion || "").trim()
          });
        ...
        const term = String(prompt("Buscar subproducto en la base:", "") || "").trim();
        ...
        const menu = list.map((r, i)=>
          `${i+1}) ${r.subName || r.name || r.subKey || ""} | ${r.subKey || ""}`
        ).join("\n");
        ...
        const unitCost = Number(subApu?.directo || 0);

        localLines.push({
          group: "SUBPRODUCTO",
          desc: String(sel.subName || sel.name || sel.subKey || "").trim(),
          unit: String(subApu?.unit || sel?.unit || "GLB").trim(),
          qty,
          pu: unitCost,
          parcial: qty * unitCost,
          subRef: String(sel.subKey || "").trim(),
          source: "base_subproducto",
          baseCode: String(sel.subKey || "").trim()
        });

...
function exportAuditPdf(project){
  ...
  function textWrap(text, x, yPos, width, align){
    const lines = doc.splitTextToSize(String(text || ""), width);
    doc.text(lines, x, yPos, align ? { align } : undefined);
  ...
  function addRow(label, value){
    ...
    doc.setFont("helvetica","bold");
    doc.setFontSize(10);
    doc.text(String(label || ""), M.left, y);
    doc.setFont("helvetica","normal");
    doc.text(String(value || ""), W - M.right, y, { align:"right" });
    y += 14;
  }
  ...
  function addParagraph(text){
    ...
    y += (Array.isArray(lines) ? lines.length : 1) * 12 + 4;
  }
  ...
  function addSimpleTable(headers, rows, widths){
    ...
    rows.forEach(row => {
      const colLines = row.map((cell, i) => doc.splitTextToSize(String(cell || ""), Math.max(20, widths[i] - 6)));
      const maxLines = Math.max(...colLines.map(arr => Array.isArray(arr) ? arr.length : 1), 1);
      const rowH = (maxLines * 10) + rowPad;
  ...
  doc.setFont("helvetica","normal");
  doc.setFontSize(11);
  doc.text(String(project.name || ""), W / 2, y, { align:"center" });
  y += 18;
  doc.text(`Entidad: ${String(project.entity || "-")}`, W / 2, y, { align:"center" });
  y += 16;
  doc.text(`Ubicación: ${String(project.location || "-")}`, W / 2, y, { align:"center" });
  y += 16;
  doc.text(`Fecha: ${new Date().toLocaleString()}`, W / 2, y, { align:"center" });
  y += 26;
  line(y);
  y += 18;

  addTitle("1. Resumen general");
  addRow("Presupuesto Base", auditMoney(cmp.tBase.total || 0));
  addRow("Presupuesto Oficial", auditMoney(cmp.tOfficial.total || 0));
  addRow("Diferencia", auditMoney(cmp.diff || 0));
    ...
    addSimpleTable(
      ...
      chapterRows.map(r => {
        ...
        return [
          r.chapterCode || "",
          r.chapterName || "",
          auditMoney(r.baseSubtotal || 0),
          auditMoney(r.officialSubtotal || 0),
          auditMoney(diff || 0),
  ...
  addParagraph("Umbrales usados en la auditoría: Precio Unitario 10% y Cantidad 10%.");
  ...
  if(!historyRows.length){
    ...
    addSimpleTable(
      ...
      [88, 42, 72, 52, 95, 90, 90]
    );
  }

  addTitle("5. Conclusión automática");
  const highAlerts = itemAlerts.length;
  const positiveChapters = chapterRows.filter(r => {
    const diff = Number(r.officialSubtotal || 0) - Number(r.baseSubtotal || 0);
    return diff > 0;
  }).length;

  addParagraph(
    `El análisis comparativo entre el Presupuesto Base y el Presupuesto Oficial del proyecto "${String(project.name || "")}" `
    + `evidencia una diferencia total de ${auditMoney(cmp.diff || 0)} (${Number(cmp.diffPct || 0).toFixed(2)}%). `

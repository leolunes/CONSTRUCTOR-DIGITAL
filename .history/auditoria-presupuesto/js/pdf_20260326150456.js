(function(){
  /* =========================================================
     ...
     - El PDF “Presupuesto + APUs” ahora también muestra los SUBPRODUCTOS
       cuando existan dentro de la descomposición del APU
     - Se imprimen separados de las líneas normales del APU
     ========================================================= */
      ...
      if(!lines.length){
        ...
        if(subRefs.size){
          for(const subKey of subRefs){
            const subAPU = await APUBase.getSubAPU(subKey);
            if(!subAPU || !subAPU.lines || !subAPU.lines.length) continue;

            L.ensure(24);

            doc.setFont("helvetica","bold");
            doc.setFontSize(9.5);
            doc.text(`SUBPRODUCTO: ${subAPU.subtitle || subKey}`, L.margin, L.getY());
            L.setY(L.getY()+12);
        ...
        if(subproductos.length){
          L.ensure(24);

          doc.setFont("helvetica","bold");
          doc.setFontSize(9.5);
          doc.text("SUBPRODUCTOS", L.margin, L.getY());
          L.setY(L.getY()+12);

          L.tableHeaderAPU();

          for(const ln of subproductos){
            L.tableRowAPU({
              group: "SUBPRODUCTO",
              desc: ln.desc || "",
              unit: ln.unit || "",

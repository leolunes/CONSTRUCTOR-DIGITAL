...
/* ==========================================
   ✅ NUEVO: buscar APU base por descripción
   - se usa solo al abrir "Ver APU" desde un ítem exacto
   - evita que un apuRefCode viejo desvíe a otro APU
   ========================================== */
async function getBaseAPUByBestDesc(desc, unit){
  const q = String(desc || "").trim();
  const u = String(unit || "").trim().toLowerCase();
  if(!q || !window.APUBase || typeof APUBase.search !== "function") return null;

  try{
    const results = await APUBase.search(q, 40);
    const rows = (results || []).filter(r => r && !r.isChapter && r.code);

    if(!rows.length) return null;

    const qn = normalizeTextForMatch(q);

    // 1) exacto por descripción + unidad
    for(const r of rows){
      const rn = normalizeTextForMatch(r.desc || "");
      const ru = String(r.unit || "").trim().toLowerCase();
      if(rn === qn && (!u || !ru || ru === u)){
        const apu = await APUBase.getAPU(r.code);
        if(apu) return { key: String(r.code || "").trim(), apu };
      }
    }

    // 2) exacto solo por descripción
    for(const r of rows){
      const rn = normalizeTextForMatch(r.desc || "");
      if(rn === qn){
        const apu = await APUBase.getAPU(r.code);
    ...
    // 3) primera coincidencia fuerte por includes
    ...
    if(hit){
      const apu = await APUBase.getAPU(hit.code);
      if(apu) return { key: String(hit.code || "").trim(), apu };
    }
  }catch(_){}
...
function bindFirmaModal(){
 ...
 if(!Number.isFinite(idx)) return;
        const ln = currentLines[idx];
        if(!ln) return;

        const group = prompt("Grupo:", String(ln.group || ln.tipo || "")) ?? String(ln.group || ln.tipo || "");
        const desc = prompt("Descripción:", String(ln.desc || "")) ?? String(ln.desc || "");
        const unit = prompt("Unidad:", String(ln.unit || "")) ?? String(ln.unit || "");
        const qty = Number(prompt("Cant/Rend:", String(ln.qty ?? 0)) ?? String(ln.qty ?? 0));
        const pu = Number(prompt("PU:", String(ln.pu ?? 0)) ?? String(ln.pu ?? 0));
  ...
  function cleanedLines(){
    return (currentLines || []).map(ln=>{
      const qty = Number(ln?.qty || 0);
      const pu = Number(ln?.pu || 0);
      return {
        ...ln,
        group: String(ln?.group || ln?.tipo || "").trim(),
        desc: String(ln?.desc || "").trim(),
        unit: String(ln?.unit || "").trim(),
        qty,
        pu,
        parcial: Number(ln?.parcial || 0) || (qty * pu)
      };
    });
  }

  function loadSubproductMode(){
    isSubproduct = true;
    editMode = true;

    if(editor) editor.style.display = "";
    if(btnSaveOverride) btnSaveOverride.style.display = "";
    if(btnResetOverride) btnResetOverride.style.display = "";

    currentCode = subKey;
    currentRefCode = subKey;

    let subApu = null;
    const ov = (projectId && window.StorageAPI?.getSubApuOverride)
      ? StorageAPI.getSubApuOverride(projectId, subKey)
      : null;
    ...
    const finish = async ()=>{
      if(!subApu && window.APUBase?.getSubAPU){
        subApu = await APUBase.getSubAPU(subKey);
      }

      currentDesc = String(subApu?.subtitle || subApu?.header || subKey || "");
      currentUnit = String(subApu?.unit || "");
      currentLines = Array.isArray(subApu?.lines) ? JSON.parse(JSON.stringify(subApu.lines)) : [];
  ...
  async function loadApuMode(){
    isSubproduct = false;
    editMode = !!projectId;

    if(editor) editor.style.display = editMode ? "" : "none";
    if(btnSaveOverride) btnSaveOverride.style.display = editMode ? "" : "none";
    if(btnResetOverride) btnResetOverride.style.display = editMode ? "" : "none";

    if(itemId && project && Array.isArray(project.items)){
      item = project.items.find(x => String(x.id) === String(itemId)) || null;
    }

    // prioridad: item exacto
    if(item){
      currentCode = String(item.code || "").trim();
      currentRefCode = String(item.apuRefCode || item.code || "").trim();
      currentDesc = String(item.desc || "").trim();
      currentUnit = String(item.unit || "").trim();

      // 1) override exacto del item por code/ref
      const ovHit = getProjectOverrideByAnyLookup(projectId, currentCode, currentRefCode);
      if(ovHit && ovHit.ov){
        currentLines = JSON.parse(JSON.stringify(ovHit.ov.lines || []));
        currentDesc = String(ovHit.ov.desc || currentDesc || "").trim();
        currentUnit = String(ovHit.ov.unit || currentUnit || "").trim();
        refreshMeta();
        renderRows();
        return;
      }
      ...
      // 2) custom APU exacto
      const customHit = getCustomAPUByAnyLookup(currentCode, currentRefCode);
      if(customHit && customHit.apu){
        currentLines = JSON.parse(JSON.stringify(customHit.apu.lines || []));
        currentDesc = String(customHit.apu.desc || currentDesc || "").trim();
        currentUnit = String(customHit.apu.unit || currentUnit || "").trim();
        refreshMeta();
        renderRows();
        return;
      }

      // 3) buscar por descripción del item para evitar desvíos por apuRefCode viejo
      const descHit = await getBaseAPUByBestDesc(currentDesc, currentUnit);
      if(descHit && descHit.apu){
        currentRefCode = String(descHit.key || currentRefCode || currentCode).trim();
        currentLines = JSON.parse(JSON.stringify(descHit.apu.lines || []));
        currentDesc = String(descHit.apu.subtitle || currentDesc || "").trim();
        currentUnit = String(descHit.apu.unit || currentUnit || "").trim();
        refreshMeta();
        renderRows();
        return;
      }

      // 4) fallback por code/ref si no hubo match por descripción
      const baseHit = await getBaseAPUByAnyLookup(currentRefCode, currentCode);
      if(baseHit && baseHit.apu){
        currentRefCode = String(baseHit.key || currentRefCode || currentCode).trim();
        currentLines = JSON.parse(JSON.stringify(baseHit.apu.lines || []));
        currentDesc = String(baseHit.apu.subtitle || currentDesc || "").trim();
        currentUnit = String(baseHit.apu.unit || currentUnit || "").trim();
        refreshMeta();
        renderRows();
        return;
      }
    ...
    // fallback legacy si entran por code sin itemId
    const requestedCode = String(codeParam || "").trim();
    if(!requestedCode){
      alert("No se indicó APU.");
      goBack();
      return;
    }

    const resolved = projectId ? await tryResolveAPUFromProjectItem(projectId, requestedCode) : null;
    const data = resolved?.data || null;

    currentCode = String(resolved?.visibleCode || requestedCode).trim();
    currentRefCode = String(resolved?.resolvedCode || requestedCode).trim();
    currentDesc = String(data?.desc || data?.subtitle || "").trim();
    currentUnit = String(data?.unit || "").trim();
    currentLines = Array.isArray(data?.lines) ? JSON.parse(JSON.stringify(data.lines)) : [];

    if(!currentDesc){
      const baseHit = await getBaseAPUByAnyLookup(currentRefCode, currentCode);
      if(baseHit && baseHit.apu){
        currentRefCode = String(baseHit.key || currentRefCode || currentCode).trim();
        currentDesc = String(baseHit.apu.subtitle || "").trim();
        currentUnit = String(baseHit.apu.unit || "").trim();
        currentLines = Array.isArray(baseHit.apu.lines) ? JSON.parse(JSON.stringify(baseHit.apu.lines)) : [];
      }
    }
  ...
  btnAddLine?.addEventListener("click", ()=>{
    ...
    const group = String(prompt("Grupo:", "MATERIALES") || "").trim();
  ...
  btnSaveOverride?.addEventListener("click", ()=>{
    if(!projectId){
      alert("No hay proyecto para guardar override.");
      return;
    }
    ...
    if(isSubproduct){
      if(typeof StorageAPI.setSubApuOverride === "function"){
        StorageAPI.setSubApuOverride(projectId, currentRefCode || currentCode, payload);
        alert("Subproducto guardado solo para este proyecto.");
      }else{
        alert("Tu storage.js no soporta subApuOverride.");
      }
      return;
    }

    if(typeof StorageAPI.setApuOverride === "function"){
      StorageAPI.setApuOverride(projectId, currentRefCode || currentCode, payload);
      updateItemsPUByApuCompat(projectId, currentRefCode || currentCode, computeDirecto(currentLines));
  ...
  btnResetOverride?.addEventListener("click", ()=>{
    if(!projectId){
      alert("No hay proyecto para resetear override.");
      return;
    }
    if(!confirm("¿Quitar override del proyecto y volver a la base?")) return;

    if(isSubproduct){
      if(typeof StorageAPI.clearSubApuOverride === "function"){
        StorageAPI.clearSubApuOverride(projectId, currentRefCode || currentCode);
      }
      location.reload();
      return;
    }

    if(typeof StorageAPI.clearApuOverride === "function"){
      StorageAPI.clearApuOverride(projectId, currentRefCode || currentCode);
    }
    location.reload();
  });

  if(subKey){
    await loadSubproductMode();
  }else{
    await loadApuMode();
  }
}

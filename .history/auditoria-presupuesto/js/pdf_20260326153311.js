(function(){
  /* =========================================================
     ...
     ✅ AJUSTE NUEVO (2026-03 subproductos en PDF Presupuesto + APUs):
     - Cuando una línea del APU tenga subRef, el PDF consulta el subproducto real
       usando APUBase.getSubAPU(subKey)
     - El PDF imprime las líneas normales del APU y luego el/los SUBPRODUCTOS reales
       en su propia sección, sin alterar lo demás
     ========================================================= */
  ...
  async function exportPresupuestoConAPUsPDF(project, opts){
    ...
    for(const it of items){
      ...
      if(!lines.length){
        ...
        for(const ln of lines){
          if(ln && ln.subRef){
            subRefs.add(String(ln.subRef).trim());
          }else{
            normales.push(ln);
          }
        }
        ...
        if(subRefs.size){
          for(const subKey of subRefs){
            ...
            if(!subAPU || !Array.isArray(subAPU.lines) || !subAPU.lines.length){
              continue;
            }

...
async function bindProjectDetailPage(){
  ...
  (UI.qs("#btnPdfCalculoIndirectos") || UI.qs("#btnPdfCalcIndirectos"))?.addEventListener("click", async ()=>{
    try{
      ...
      await const activeMode = (StorageAPI.getProjectActiveBudgetMode)
  ? StorageAPI.getProjectActiveBudgetMode(fresh.id)
  : (fresh.activeBudget || "base");
...
PDF.exportCalculoCostosIndirectosPDF(pdfProject, { share: isIOS() });
    }catch(err){

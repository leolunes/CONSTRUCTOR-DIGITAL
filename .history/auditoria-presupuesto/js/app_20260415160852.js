...
function getBudgetItemComparisonRowsByModes(project, leftMode, rightMode){
  ...
  }).sort((a,b)=> String(a.code || "").localeCompare(String(b.code || ""), "es", { numeric:true, sensitivity:"base" }));
}
...
function refreshPdfExportButtonsLabels(project){
  ...
  setText("btnPdfIvaProyecto", `PDF IVA DEL PROYECTO ${modeLabelShort}`);
...
/* ==========================================
   ✅ Crear APU desde Insumos / Subproductos
   - Catálogo fallback en localStorage
   - Permite crear insumos manuales
   - Permite buscar y agregar insumos o subproductos al APU nuevo
   ========================================== */
function installCustomCatalogFallbacks(){
  if(!window.StorageAPI || window.__customCatalogFallbacksInstalled) return;
  window.__customCatalogFallbacksInstalled = true;

  const KEY_APUS = "ppro_custom_apus_catalog";
  const KEY_INSUMOS = "ppro_custom_insumos_catalog";
  ...
  if(typeof StorageAPI.listCustomAPUs !== "function"){
    StorageAPI.listCustomAPUs = function(){
      return safeArr(readJson(KEY_APUS, []));
    };
  }
  if(typeof StorageAPI.getCustomAPU !== "function"){
    StorageAPI.getCustomAPU = function(code){
      const key = normCode(code);
      return StorageAPI.listCustomAPUs().find(x => normCode(x?.code) === key) || null;
    };
  }
  if(typeof StorageAPI.saveCustomAPU !== "function"){
    StorageAPI.saveCustomAPU = function(payload){
      const code = normCode(payload?.code);
      if(!code) throw new Error("El APU nuevo debe tener código.");
      const all = StorageAPI.listCustomAPUs().filter(x => normCode(x?.code) !== code);
      all.push({
        code,
        desc: String(payload?.desc || "").trim(),
        unit: String(payload?.unit || "").trim(),
        chapterCode: String(payload?.chapterCode || "").trim(),
        chapterName: String(payload?.chapterName || "").trim(),
        createdAt: String(payload?.createdAt || new Date().toISOString()),
        updatedAt: new Date().toISOString(),
        lines: safeArr(payload?.lines).map(l => ({
          tipo: String(l?.tipo || l?.group || "-").trim(),
      ...
      writeJson(KEY_APUS, all);
      return StorageAPI.getCustomAPU(code);
    };
  }

  if(typeof StorageAPI.listCustomInsumos !== "function"){
    StorageAPI.listCustomInsumos = function(){
      return safeArr(readJson(KEY_INSUMOS, []));
    };
  }
  if(typeof StorageAPI.saveCustomInsumo !== "function"){
    StorageAPI.saveCustomInsumo = function(payload){
      const desc = String(payload?.desc || "").trim();
      const unit = String(payload?.unit || "").trim();
      if(!desc) throw new Error("El insumo nuevo debe tener descripción.");
      const id = String(payload?.id || `ins_${Date.now()}_${Math.random().toString(16).slice(2,8)}`);
      const all = StorageAPI.listCustomInsumos().filter(x => String(x?.id || "") != id);
      all.push({
        id,
        tipo: String(payload?.tipo || "MATERIALES").trim(),
        desc,
        unit,
        pu: Number(payload?.pu || 0),
        createdAt: String(payload?.createdAt || new Date().toISOString()),
        updatedAt: new Date().toISOString()
      });
      writeJson(KEY_INSUMOS, all);
      return all.find(x => String(x.id) === id) || null;
    };
  }
}

function computeApuLinesDirecto(lines){
...
function refreshCreateApuChapterOptions(project){
  const sel = UI.qs("#selChapterBase");
  const inpName = UI.qs("#inpChapterName");
  if(!sel) return;
  ...
  sel.innerHTML = chapters.length
    ? chapters.map(c=>{
        ...
        return `<option value="${UI.esc(code)}" data-name="${UI.esc(name)}">${UI.esc(label)}</option>`;
      }).join("")
    : `<option value="">(Sin capítulos definidos)</option>`;
  ...
  const pick = ()=>{
    const code = String(sel.value || "").trim();
    const name = code
      ? (sel.options[sel.selectedIndex]?.getAttribute("data-name") || lookupChapterName(project, code) || "")
      : "";
    if(inpName) inpName.value = name;
  };
...
async function getUnifiedApuSearchResults(term, limit=30){
  const q = String(term || "").trim();
  const maxRows = Math.max(1, Number(limit || 30));

  let baseRows = [];
  try{
    if(window.APUBase && typeof APUBase.search === "function"){
      baseRows = await APUBase.search(q, maxRows);
    }
  }catch(_){
    baseRows = [];
  }
  ...
  let customRows = [];
  try{
    const all = (window.StorageAPI && typeof StorageAPI.listCustomAPUs === "function")
      ? (StorageAPI.listCustomAPUs() || [])
      : [];

    const nq = normalizeTextForMatch(q);
    customRows = all
      .filter(r=>{
        if(!nq) return true;
        return [
          ...
          r?.chapterCode,
          r?.chapterName
        ].some(v => normalizeTextForMatch(v).includes(nq));
      })
      .map(r=>{
        const pu = Number(r?.pu || 0) || computeApuLinesDirecto(r?.lines || []);
        return {
          code: String(r?.code || "").trim(),
          desc: String(r?.desc || "").trim(),
          unit: String(r?.unit || "").trim(),
          pu,
          chapterCode: String(r?.chapterCode || "").trim(),
          chapterName: String(r?.chapterName || "").trim(),
  ...
  const merged = [
    ...(Array.isArray(baseRows) ? baseRows : []).map(r=>({ ...r, __source: "base" })),
    ...customRows
  ];
  ...
  for(const r of merged){
    const key = [
      ...
      normalizeTextForMatch(r?.desc || r?.descripcion || ""),
      String(r?.unit || r?.unidad || "").trim().toLowerCase()
    ].join("__");
...
function renderApuResults(projectId, results){
  ...
  tbody.innerHTML = list.map((r, idx)=>{
    const srcLabel = String(r.__source || "base") === "custom" ? "[PERSONALIZADO] " : "[BASE] ";
    const desc = `${srcLabel}${String(r.desc || "")}`;
    return `
      <tr>
        <td><b>${UI.esc(r.code||"")}</b></td>
        <td>${UI.esc(desc)}</td>
        ...
        <td>
          <button class="btn primary" type="button" data-addapu-idx="${idx}">Agregar</button>
        </td>
      </tr>
    `;
  }).join("");

  tbody.querySelectorAll("[data-addapu-idx]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const idx = Number(btn.getAttribute("data-addapu-idx"));
      const sel = list[idx];
      if(!sel) return;
      ...
      const payload = {
        ...
        desc: String(sel.desc || "").trim(),
        unit: String(sel.unit || "").trim(),
        pu: Number(sel.pu || 0),
        chapterCode: String(sel.chapterCode || "").trim(),
        chapterName: String(sel.chapterName || "").trim(),
        lines: Array.isArray(sel.lines) ? sel.lines : []
      };

      openAddApuModal(project, payload);
    });
  });
}
...
async function bindProjectDetailPage(){
  ...
  UI.qs("#btnPdfIvaProyecto")?.addEventListener("click", async ()=>{
    try{
      const fresh = StorageAPI.getProjectById(projectId);
      if(!fresh) return;

      const exportFn =
        window.PDF?.exportIvaProyectoPDF ||
        window.PDF?.exportIvaProyectoPdf ||
        window.PDF?.exportIVAProyectoPDF ||
        null;

      if(typeof exportFn !== "function"){
        alert("Este PDF aún no está implementado. Falta actualizar pdf.js (IVA DEL PROYECTO).");
        return;
      }

      await exportFn(fresh, { share: isIOS() });
    }catch(err){
      alert("Error generando PDF IVA DEL PROYECTO: " + (err?.message || err));
    }
  });
  ...
  UI.qs("#btnApuSearch")?.addEventListener("click", async ()=>{
    try{
      ...
      const results = await getUnifiedApuSearchResults(q, 40);
      renderApuResults(projectId, results);
    }catch(err){
      alert("Error buscando APUs: " + (err?.message || err));
    }
  });
...
/* ===== Crear APU nuevo desde insumos / subproductos ===== */
  ...
  function renderCreateApuBuildLines(){
    ...
    if(apuBuildTotalChip) apuBuildTotalChip.textContent = `PU APU: ${UI.fmtMoney(computeApuLinesDirecto(createApuLines), project.currency || "COP")}`;

    apuBuildLinesBody.querySelectorAll("[data-apubuild-rm]").forEach(btn=>{
      btn.onclick = ()=>{
        const idx = Number(btn.getAttribute("data-apubuild-rm"));
        if(!(idx >= 0)) return;
        createApuLines.splice(idx, 1);
        renderCreateApuBuildLines();
      };
    });
    ...
    apuBuildLinesBody.querySelectorAll("[data-apubuild-edit]").forEach(btn=>{
      btn.onclick = ()=>{
        ...
        const cur = createApuLines[idx];
        if(!cur) return;
        const qty = Number(prompt("Cantidad / rendimiento:", String(cur.qty || 1)) || "0");
        if(!(qty > 0)) return;
        const pu = Number(prompt("Precio unitario:", String(cur.pu || 0)) || "0");
        if(!(pu >= 0)) return;
        createApuLines[idx] = { ...cur, qty, pu, parcial: qty * pu };
        renderCreateApuBuildLines();
      };
    });
  }

  function setCreateApuSearchResults(rows){
    ...
    insumoResultsBody.innerHTML = list.map((r, idx)=>`
      <tr>
        <td>${UI.esc(r.kindLabel || r.tipo || "-")}</td>
        <td>${UI.esc(r.desc || r.descripcion || r.subName || r.name || r.subKey || "")}</td>
        <td>${UI.esc(r.unit || r.unidad || "GLB")}</td>
        <td style="text-align:right"><b>${UI.fmtMoney(r.pu || 0, project.currency || "COP")}</b></td>
        <td><button class="btn primary" type="button" data-pick-insumo="${idx}">Agregar</button></td>
      </tr>
    `).join("");

    insumoResultsBody.querySelectorAll("[data-pick-insumo]").forEach(btn=>{
      btn.onclick = ()=>{
        const idx = Number(btn.getAttribute("data-pick-insumo"));
        const row = list[idx];
        if(!row) return;

        const qty = Number(prompt("Cantidad / rendimiento:", "1") || "0");
        if(!(qty > 0)) return;

        createApuLines.push({
          group: String(row.group || row.tipo || row.kindLabel || "MATERIALES").trim(),
          desc: String(row.desc || row.descripcion || row.subName || row.name || row.subKey || "").trim(),
          unit: String(row.unit || row.unidad || "UND").trim(),
          qty,
          pu: Number(row.pu || 0),
          parcial: qty * Number(row.pu || 0),
          subRef: String(row.subRef || "").trim(),
          source: String(row.source || "manual").trim(),
          baseCode: String(row.baseCode || row.code || row.id || row.subKey || "").trim()
        });
        renderCreateApuBuildLines();
      };
    });
  ...
  async function searchCreateApuInputs(){
    if(!window.APUBase || typeof APUBase.getMeta !== "function"){
      alert("La base APU no está disponible.");
      return;
    }
    ...
    const term = String(insumoSearchInput?.value || "").trim();
    const q = normalizeTextForMatch(term);

    let baseInsumos = [];
    let subproductos = [];
    try{ baseInsumos = await APUBase.listInsumos(""); }catch(_){}
    try{ subproductos = await APUBase.listSubproductos(); }catch(_){}

    const customInsumos = (window.StorageAPI && typeof StorageAPI.listCustomInsumos === "function")
      ? (StorageAPI.listCustomInsumos() || [])
      : [];
    ...
    const insumoRows = [
      ...customInsumos.map(r=>({
        kind: "custom_insumo",
        kindLabel: "INSUMO",
        group: String(r.tipo || "MATERIALES").trim(),
        desc: String(r.desc || "").trim(),
        unit: String(r.unit || r.unidad || "UND").trim(),
        pu: Number(r.pu || 0),
        source: "custom_insumo",
        baseCode: String(r.id || r.desc || "").trim()
      })),
      ...baseInsumos.map(r=>({
        kind: "base_insumo",
        kindLabel: "INSUMO",
      ...
      if(!q) return true;
      return [r.kindLabel, r.group, r.desc, r.unit, r.baseCode].some(v => normalizeTextForMatch(v).includes(q));
    });

    const subRows = [];
    for(const r of (subproductos || [])){
      const desc = String(r.subName || r.name || r.subKey || "").trim();
      const unit = String(r.unit || "GLB").trim();
      const match = !q || [desc, r.subKey, r.name, r.desc, r.descripcion, unit].some(v => normalizeTextForMatch(v).includes(q));
      if(!match) continue;

      let subApu = null;
      try{ subApu = await APUBase.getSubAPU(r.subKey); }catch(_){}
    ...
    setCreateApuSearchResults(merged);
  }

  btnInsumoSearch?.addEventListener("click", async ()=>{
    try{
      await searchCreateApuInputs();
    }catch(err){
  ...
  formCreateApu?.addEventListener("submit", (e)=>{
    ...
    try{
      ...
      if(!createApuLines.length) throw new Error("Debes agregar al menos un insumo o subproducto al APU.");
      ...
      const cleaned = createApuLines.map(l=>({
      ...
      createApuLines = [];
      renderCreateApuBuildLines();
      if(insumoResultsBody) insumoResultsBody.innerHTML = "";
      if(insumoResultsEmpty) insumoResultsEmpty.style.display = "";
      formCreateApu.reset();
      refreshCreateApuChapterOptions(StorageAPI.getProjectById(projectId) || project);
      alert("APU nuevo guardado correctamente. Ya puedes usar ese código dentro de la app.");
    }catch(err){

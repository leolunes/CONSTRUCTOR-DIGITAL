...
async function bindBaseImportPage(){
  ...
  const searchInput = UI.qs("#apuSearch");
  const searchBtn = UI.qs("#btnSearchApu");
  const results = UI.qs("#apuSearchResults");

  async function doSearch(){
    if(!results) return;
    const q = String(searchInput?.value || "").trim();
    if(!q){
      results.innerHTML = `<div class="muted">Digite un criterio de búsqueda.</div>`;
      return;
    }

    try{
      if(!(window.APUBase && typeof APUBase.search === "function")){
        throw new Error("APUBase.search no está disponible.");
      }

      const rows = await APUBase.search(q, 100);
      if(!Array.isArray(rows) || !rows.length){
        results.innerHTML = `<div class="muted">No se encontraron resultados.</div>`;
        return;
      }
      ...
      results.innerHTML = rows.map(r=> `
        <article class="card">
          <div class="row" style="justify-content:space-between;gap:12px;align-items:flex-start">
            <div>
              <h4 style="margin:0 0 6px 0">${UI.esc(r.code||"")} · ${UI.esc(r.desc||"")}</h4>
              <div class="muted">${UI.esc(r.unit||"")} · ${UI.fmtMoney(r.pu||0, "COP")}</div>
            </div>
            ${projectId
              ? `<button class="btn" type="button" data-add-apu='${UI.esc(JSON.stringify(r))}'>Agregar al proyecto</button>`
              : ``}
          </div>
        </article>
      `).join("");

      results.querySelectorAll("[data-add-apu]").forEach(btn=>{
        btn.addEventListener("click", ()=>{
          const raw = btn.getAttribute("data-add-apu");
          const apu = JSON.parse(raw || "{}");
          const project = StorageAPI.getProjectById(projectId);
          openAddApuModal(project, apu);
        });
      });
    }catch(err){
      results.innerHTML = `<div class="muted">Error: ${UI.esc(err?.message || String(err))}</div>`;
    }
  }
...
function bindOfficialImport(projectId){
  ...
  async function doImport(file){
    ...
    try{
      ...
      const result = await OfficialImport.importOfficialBudgetFromFile(file, projectId);
      ...
      renderItemsTable(fresh);
      renderResumenItems(fresh);
      renderOfficialImportInfo(fresh);

      alert(`Presupuesto Oficial importado.\n\nCapítulos: ${result?.chapters||0}\nÍtems: ${result?.items||0}`);
    }catch(err){
  ...
  btnClear?.addEventListener("click", ()=>{
    if(!confirm("¿Eliminar todo el Presupuesto Oficial importado?")) return;
    ...
    renderResumenItems(fresh);
    renderOfficialImportInfo(fresh);
    if(info) info.innerHTML = `<div class="muted">No hay presupuesto oficial importado.</div>`;
  });
}
...
async function bindProjectDetailPage(){
  ...
  UI.qs("#formAddItem")?.addEventListener("submit", (e)=>{
    ...
    const chapterCode = String(form.chapterCode?.value || "").trim();
    let chapterName = String(form.chapterName?.value || "").trim();
    const code = String(form.code?.value || "").trim();
    const desc = String(form.desc?.value || "").trim();
    const unit = String(form.unit?.value || "").trim();
    const pu = Number(form.pu?.value || 0);
    const qty = Number(form.qty?.value || 0);

    if(!code || !desc || !unit){
      alert("Complete código, descripción y unidad.");
      return;
    }
    if(!(pu > 0) || !(qty > 0)){
...
function normalizeApuLine(line){
  return {
    ...
    parcial: Number(line?.parcial ?? (Number(line?.qty ?? line?.cant ?? line?.cantidad ?? 0) * Number(line?.pu ?? line?.price ?? line?.precio ?? 0)))
  };
}
...
function renderApuHeaderGeneric({ project, apuCode, apuName, apuUnit, apuPU, mode }){
  ...
  const backEl  = qsAny(["#btnBackProjects", "#btnBack", "a[href='proyectos.html']"]);
  ...
  if(backEl && project?.id){
    backEl.href = `proyecto-detalle.html?projectId=${encodeURIComponent(project.id)}`;
  }
}
...
/* =========================
   Páginas sueltas
   ========================= */
async function bindComparativoPage(){
  const projectId = String(UI.getParam("projectId") || "").trim();
  if(!projectId){
    alert("No se encontró projectId.");
    location.href = "proyectos.html";
  ...
  UI.qs("#compareProjectName") && (UI.qs("#compareProjectName").textContent = project.name || "Comparativo");
  renderComparison(project);
}

async function bindAuditoriaPage(){
  const projectId = String(UI.getParam("projectId") || "").trim();
  if(!projectId){
    alert("No se encontró projectId.");
    location.href = "proyectos.html";
  ...
  UI.qs("#auditProjectName") && (UI.qs("#auditProjectName").textContent = project.name || "Auditoría");
  renderAudit(project);
}
...
(function main(){
  ...
  if(p === "comparativo.html"){
    bindComparativoPage().catch(err=>{
      console.error(err);
      alert("Error inicializando comparativo: " + (err?.message || err));
    });
    return;
  }

  if(p === "auditoria.html"){
    bindAuditoriaPage().catch(err=>{
      console.error(err);
      alert("Error inicializando auditoría: " + (err?.message || err));
    });
    return;
  }
})();

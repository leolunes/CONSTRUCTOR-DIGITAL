  ...
  if(("admin" in t) || ("imprev" in t) || ("util" in t) || ("ivaUtil" in t) || ("subtotal" in t) || ("iva" in t)){
    ...
    const subtotal =
      Number.isFinite(Number(t.subtotal))
        ? Number(t.subtotal||0)
        : (directo + admin + imprev + util + Number(t.indirectOthers||0));
    ...
    return {
      directo, admin, imprev, util, subtotal, ivaUtil, total,
      adminPct: Number(t.adminPct||0),
      imprevPct: Number(t.imprevPct||0),
      utilPct: Number(t.utilPct||0),
      indirectOthers: Number(t.indirectOthers||0),
      indirectOthersPct: Number(t.indirectOthersPct||0),
      indirectRows: Array.isArray(t.indirectRows) ? t.indirectRows : []
    };
  }
...
function cloneProjectSafe(project){
  try{
...
function projectHasDualBudget(project){
  return !!(project && project.budgets && typeof project.budgets === "object" && project.budgets.base);
}

function getProjectBudgetView(project, mode){
  const p = cloneProjectSafe(project);
  if(!projectHasDualBudget(p)) return p;

  const useMode = String(mode || "base").trim().toLowerCase() === "oficial" ? "oficial" : "base";
  const active = useMode === "oficial" ? p?.budgets?.oficial : p?.budgets?.base;
  if(!active || typeof active !== "object") return p;

  p.activeBudget = useMode;
  if(Array.isArray(active.items)) p.items = active.items;
  if(Array.isArray(active.chapters)) p.chapters = active.chapters;
  if(active.apuOverrides && typeof active.apuOverrides === "object") p.apuOverrides = active.apuOverrides;
  if(active.subApuOverrides && typeof active.subApuOverrides === "object") p.subApuOverrides = active.subApuOverrides;
  if("indirectMode" in active) p.indirectMode = active.indirectMode;
  if(Array.isArray(active.indirectTable)) p.indirectTable = active.indirectTable;
  return p;
}

function ensureBudgetCompareCard(){
  let el = UI.qs("#budgetCompareCard");
  if(el) return el;

  const totalsRows = UI.qs("#totalsRows");
  if(!totalsRows || !totalsRows.parentElement) return null;

  el = document.createElement("section");
  el.className = "card";
  el.id = "budgetCompareCard";
  el.style.marginTop = "14px";
  el.innerHTML = `
    <div class="cardhead">
      <h2>Comparación Presupuesto Base vs Oficial</h2>
      <p class="muted small">Resumen comparativo del mismo proyecto.</p>
    </div>
    <div id="budgetCompareBody"></div>
  `;
  totalsRows.parentElement.insertAdjacentElement("afterend", el);
  return el;
}

function renderBudgetComparison(project){
  const card = ensureBudgetCompareCard();
  if(!card) return;

  const body = UI.qs("#budgetCompareBody");
  if(!body) return;

  if(!projectHasDualBudget(project)){
  ...
  const baseProj = getProjectBudgetView(project, "base");
  const ofProj = getProjectBudgetView(project, "oficial");

  const tb = totalsCompat(baseProj);
  const to = totalsCompat(ofProj);

  const baseItems = Array.isArray(baseProj.items) ? baseProj.items.length : 0;
  const ofItems = Array.isArray(ofProj.items) ? ofProj.items.length : 0;

  const diffDirecto = Number(to.directo || 0) - Number(tb.directo || 0);
  const diffTotal = Number(to.total || 0) - Number(tb.total || 0);
  const pctTotal = Number(tb.total || 0) > 0 ? (diffTotal / Number(tb.total || 0)) * 100 : 0;
  ...
  body.innerHTML = `
    <div class="grid two">
      <div class="card item">
        ...
        <div class="muted small">Costos directos: <b>${UI.fmtMoney(tb.directo || 0, project.currency || "COP")}</b></div>
        <div class="muted small">Valor total: <b>${UI.fmtMoney(tb.total || 0, project.currency || "COP")}</b></div>
      </div>

      <div class="card item">
        <div class="name">Presupuesto Oficial</div>
        <div class="muted small" style="margin-top:6px">Ítems: ${UI.esc(String(ofItems))}</div>
        <div class="muted small">Costos directos: <b>${UI.fmtMoney(to.directo || 0, project.currency || "COP")}</b></div>
        <div class="muted small">Valor total: <b>${UI.fmtMoney(to.total || 0, project.currency || "COP")}</b></div>
      </div>
    </div>
    ...
    <div class="grid two">
      <div class="card item">
        ...
        <div style="margin-top:6px"><b>${UI.fmtMoney(diffDirecto, project.currency || "COP")}</b></div>
      </div>

      <div class="card item">
        <div class="name">Diferencia en valor total</div>
        <div style="margin-top:6px"><b>${UI.fmtMoney(diffTotal, project.currency || "COP")}</b></div>
        <div class="muted small" style="margin-top:4px">${pctTotal.toFixed(2)}% frente al Base</div>
      </div>
    </div>
  `;
}
...
function renderProjectDetail(project){
  ...
  renderBudgetComparison(project);
}
...
function renderIndirectRows(projectId){
  ...
  const totals = totalsCompat(project);
  const directo = Number(totals.directo || 0);
  const baseRows = getIndirectRows(project);
  const calcRows = Array.isArray(totals.indirectRows) ? totals.indirectRows : [];

  let rows = [];
  if(baseRows.length){
    rows = baseRows.map((r, i)=>({ ...r, ...(calcRows[i] || {}) }));
  }else if(calcRows.length){
    rows = calcRows.map(r => normalizeIndirectUiRow(r));
  }
  ...
  function hiddenInput(field, value, type="text"){
    return `<input class="input" data-field="${field}" type="${type}" value="${UI.esc(String(value ?? ""))}" style="display:none">`;
  }
  function hiddenSelect(field, value, options){
    return `<select class="input" data-field="${field}" style="display:none">${options.map(o=>`<option value="${o}" ${String(value)===String(o)?"selected":""}>${o}</option>`).join("")}</select>`;
  }

  tbody.innerHTML = rows.map((row, idx)=>{
    const r = normalizeIndirectUiRow(row);
    const partial = Number(row?.partial || calcIndirectRowPartial(r, directo));
    ...
    if(r.rowType === "header"){
      return `
        <tr
          data-indirect-id="${UI.esc(r.id || ("ind_" + idx))}"
        ...
        >
          <td><b>${UI.esc(r.item || "—")}</b></td>
          <td>${UI.esc(r.group || "OTRO")}</td>
          <td colspan="5">
            <div style="margin-left:${indent}px"><b>${UI.esc(r.desc || "SUBTÍTULO")}</b></div>
            <div class="muted small" style="margin-left:${indent}px">Subtítulo (no calcula)</div>
          </td>
          <td class="muted small">Jerarquía</td>
          <td style="text-align:right"><b>${UI.fmtMoney(0, project.currency||"COP")}</b></td>
          <td>
            <div class="row" style="gap:8px; flex-wrap:wrap">
              <button type="button" class="btn" data-indirect-edit="${UI.esc(r.id || ("ind_" + idx))}">Editar</button>
              <button type="button" class="btn danger" data-indirect-del="${UI.esc(r.id || ("ind_" + idx))}">Quitar</button>
            </div>
          </td>
        </tr>
      `;
    ...
    return `
      ...
      >
        <td>
          <div>${UI.esc(r.item)}</div>
          ${hiddenInput("item", r.item)}
        </td>
        <td>
          <div>${UI.esc(r.group)}</div>
          ${hiddenSelect("group", r.group, ["ADMIN","IMPREV","UTIL","OTRO"])}
        </td>
        <td>
          <div style="margin-left:${indent}px">${UI.esc(r.desc)}</div>
          ${hiddenInput("desc", r.desc)}
        </td>
        <td>
          <div>${UI.esc(isPercentCd ? "%CD" : r.unit)}</div>
          ${hiddenInput("unit", isPercentCd ? "%CD" : r.unit)}
        </td>
        <td style="text-align:right">
          <div>${UI.esc(String(isPercentCd ? 1 : (r.qty||0)))}</div>
          ${hiddenInput("qty", isPercentCd ? 1 : (r.qty||0), "number")}
        </td>
        <td style="text-align:right">
          <div><b>${UI.fmtMoney(unitVal, project.currency||"COP")}</b></div>
          ${hiddenInput("unitValue", unitVal, "number")}
        </td>
        <td style="text-align:right">
          <div>${Number(r.percent||0).toFixed(2)}</div>
          ${hiddenSelect("calcType", r.calcType, ["fixed","percent_direct"])}
          ${hiddenInput("percent", r.percent||0, "number")}
        </td>
        <td>
          <div class="muted small">${UI.esc(sourceLabel)}</div>
          ${hiddenSelect("sourceType", r.sourceType, ["manual","insumo","subproducto","percent_cd"])}
        </td>
        <td style="text-align:right">
          <div><b>${UI.fmtMoney(partial, project.currency||"COP")}</b></div>
        </td>
        <td>
          <div class="row" style="gap:8px; flex-wrap:wrap">
            ${(r.sourceType === "insumo" || r.sourceType === "subproducto") ? `<button type="button" class="btn" data-indirect-pick="${UI.esc(r.id || ("ind_" + idx))}">Base</button>` : ""}
            <button type="button" class="btn" data-indirect-edit-detail="${UI.esc(r.id || ("ind_" + idx))}">Editar</button>
            <button type="button" class="btn primary" data-indirect-save="${UI.esc(r.id || ("ind_" + idx))}">Guardar</button>
            <button type="button" class="btn danger" data-indirect-del="${UI.esc(r.id || ("ind_" + idx))}">Quitar</button>
          </div>
        </td>
      </tr>

...
async function getSubproductoAPU(project, subKey){
  ...
  async function findLaborInsumoMatch(role){
    try{
      ...
      if(window.APUBase?.getInsumoByDesc){
        ...
        if(rate > 0){
          return {
            desc: String(exact?.desc || q),
            unit: String(exact?.unit || exact?.unidad || ""),
            pu: rate
          };
        }
      }
      ...
      if(window.APUBase?.searchInsumos){
        ...
        if(contains){
          ...
          if(rate > 0){
            return {
              desc: String(contains?.desc || q),
              unit: String(contains?.unit || contains?.unidad || ""),
              pu: rate
            };
          }
        }
  ...
  async function findLaborRateByRole(role){
    const m = await findLaborInsumoMatch(role);
    return Number(m?.pu || 0);
  }
  ...
  async function buildLaborBreakdownForLine(ln){
    ...
    for(const part of crew){
      const match = await findLaborInsumoMatch(part.role);
      const sourceRate = Number(match?.pu || 0);

      if(sourceRate > 0){
        knownTotal += part.qty * sourceRate;
      }else{
        unknownQty += part.qty;
      }
      ...
      rows.push({
        desc: part.role,
        unit: String(match?.unit || "JOR"),
        qty: part.qty,
        sourceDesc: String(match?.desc || part.role),
        sourceRate,
        factor: 1,
        pu: sourceRate,
        parcial: part.qty * sourceRate,
        explain: sourceRate > 0 ? "valor_base_insumo" : "sin_fuente_directa"
      });
    }
    ...
    if(lineUnitValue > 0){
      if(knownTotal > 0 && Math.abs(knownTotal - lineUnitValue) / lineUnitValue > 0.03){
        ...
        for(const r of rows){
          if(Number(r.sourceRate || 0) > 0){
            r.factor = factor;
            r.pu = Number(r.sourceRate || 0) * factor;
            r.parcial = Number(r.qty || 0) * Number(r.pu || 0);
            r.explain = "valor_base_mas_factor";
          }
        }
        ...
        for(const r of rows){
          ...
          r.explain = "prorrateo_directo_cuadrilla";
        }
  ...
  async function drawLaborBreakdownUnderRow(doc, L, ln){
    const rows = await buildLaborBreakdownForLine(ln);
    if(!rows.length) return;

    const estimatedHeight = 18 + rows.length * 38;
    L.ensure(estimatedHeight);
    ...
    for(const r of rows){
      doc.setFont("helvetica","bold");
      doc.setFontSize(8.3);
      const head = `${safe(r.desc)} · Cant: ${fmt2(r.qty)} · Vr Unit final: ${moneyCOP0(r.pu)} · Parcial: ${moneyCOP0(r.parcial)}`;
      doc.text(head, L.margin + 28, L.getY());
      L.setY(L.getY() + 10);

      doc.setFont("helvetica","normal");
      doc.setFontSize(8.1);

      const fuente = `Fuente base: ${safe(r.sourceDesc || "No encontrada")} · Unidad: ${safe(r.unit || "-")}`;
      doc.text(fuente, L.margin + 40, L.getY());
      L.setY(L.getY() + 9);

      if(Number(r.sourceRate || 0) > 0){
        doc.text(`Vr base encontrado: ${moneyCOP0(r.sourceRate)}`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
      }

      if(Number(r.factor || 1) !== 1){
        doc.text(`Factor de ajuste aplicado: x ${Number(r.factor || 1).toLocaleString("es-CO", { minimumFractionDigits: 4, maximumFractionDigits: 4 })}`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
        doc.text(`Cálculo valor final: ${moneyCOP0(r.sourceRate)} × ${Number(r.factor || 1).toLocaleString("es-CO", { minimumFractionDigits: 4, maximumFractionDigits: 4 })} = ${moneyCOP0(r.pu)}`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
      }else if(String(r.explain || "").includes("prorrateo")){
        doc.text(`Cálculo valor final: prorrateo directo de cuadrilla = ${moneyCOP0(r.pu)}`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
      }else if(String(r.explain || "") === "saldo_repartido"){
        doc.text(`Cálculo valor final: saldo repartido en roles sin fuente directa = ${moneyCOP0(r.pu)}`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
      }else{
        doc.text(`Cálculo valor final: coincide con valor base encontrado`, L.margin + 40, L.getY());
        L.setY(L.getY() + 9);
      }

      doc.text(`Cálculo parcial: ${fmt2(r.qty)} × ${moneyCOP0(r.pu)} = ${moneyCOP0(r.parcial)}`, L.margin + 40, L.getY());
      L.setY(L.getY() + 12);
    }
 ...
 if(chap !== currentChap){
  ...
  if(subproductosUsados.size){
    ...
    for(const info of subproductosUsados.values()){
      ...
      doc.setFont("helvetica","bold");
      doc.setFontSize(9.8);
      doc.text(`SUBPRODUCTO: ${safe(subAPU.subtitle || subAPU.header || subKey)}`, L.margin, L.getY());
      L.setY(L.getY()+14);

      doc.setFont("helvetica","normal");
      doc.setFontSize(10);
      L.p(`Ítem origen: ${safe(info.sourceItemCode || "")} — ${safe(info.sourceItemDesc || "")}`);
  ...
  if(manoObraPorItem.length){
    ...
    for(const mo of manoObraPorItem){
      ...
      for(const ln of lines){
        ...
        await drawLaborBreakdownUnderRow(doc, L, ln);
      }

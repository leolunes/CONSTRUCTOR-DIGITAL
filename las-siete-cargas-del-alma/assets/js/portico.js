...
function updateLoadBlocks(result) {
  ...
  const ids = ["miedo", "culpa", "ansiedad", "responsabilidad", "rechazo", "escasez", "soledad"];
  ...
  ids.forEach(id => {
    ...
    if (!section) {
      ...
      block.dataset.tooltip = `${capitalize(id)}: sin lectura`;
      return;
    }
...
function updateMainStructure(result) {
  ...
  if (!result) {
    ...
    viga.dataset.tooltip = "Alma / Viga: sin lectura";
    columna.dataset.tooltip = "Espíritu / Columna: sin lectura";
    cimiento.dataset.tooltip = "Cimiento / Cristo: sin lectura";
    return;
  }
...
function updateAxisNotesAndHalo(result) {
  ...
  const nodeComunion = document.getElementById("node_comunion");
  const nodeInterseccion = document.getElementById("node_interseccion");
  const nodeCongregacion = document.getElementById("node_congregacion");

  nodeComunion?.setAttribute("data-context-id", "alma");
  nodeInterseccion?.setAttribute("data-context-id", "espiritu");
  nodeCongregacion?.setAttribute("data-context-id", "cimiento");

  if (!result) {
    nodeComunion?.setAttribute("data-tooltip", "Diagnóstico breve: sin lectura del alma.");
    nodeInterseccion?.setAttribute("data-tooltip", "Diagnóstico breve: sin lectura del espíritu.");
    nodeCongregacion?.setAttribute("data-tooltip", "Diagnóstico breve: sin lectura del cimiento.");
    return;
  }
  ...
  nodeComunion?.setAttribute("data-tooltip", `Diagnóstico breve: el alma está ${alma >= 60 ? "sobrecargada" : "operando con menor retención"}.`);
  nodeInterseccion?.setAttribute("data-tooltip", `Diagnóstico breve: la columna espiritual muestra ${espiritu >= 60 ? "transmisión firme" : "debilidad de transmisión"}.`);
  nodeCongregacion?.setAttribute("data-tooltip", `Diagnóstico breve: el descanso en el Cimiento es ${cimiento >= 60 ? "estable" : "limitado"}.`);
}

function updateLabelHighlights(result) {
  const almaLabel = document.querySelector(".label-alma");
  const espirituLabel = document.querySelector(".label-espiritu");
  const cimientoLabel = document.querySelector(".label-cimiento");

  if (!almaLabel || !espirituLabel || !cimientoLabel) return;

  if (!result) {
    almaLabel.setAttribute("fill", "#60a5fa");
    almaLabel.style.fill = "#60a5fa";

    espirituLabel.setAttribute("fill", "#34d399");
    espirituLabel.style.fill = "#34d399";

    cimientoLabel.setAttribute("fill", "#facc15");
    cimientoLabel.style.fill = "#facc15";

    almaLabel.style.fontWeight = "700";
    espirituLabel.style.fontWeight = "700";
    cimientoLabel.style.fontWeight = "700";
    return;
  }

  const almaBand = bandClass(getBandLabel(result.axes.alma, getState().data?.rubrics?.globalBands || []));
  const espirituBand = bandClass(getBandLabel(100 - result.axes.espiritu, getState().data?.rubrics?.globalBands || []));
  const cimientoBand = bandClass(getBandLabel(100 - result.axes.cimiento, getState().data?.rubrics?.globalBands || []));

  applyLabelColor(almaLabel, almaBand, "#60a5fa");
  applyLabelColor(espirituLabel, espirituBand, "#34d399");
  applyLabelColor(cimientoLabel, cimientoBand, "#facc15");
}

function applyLabelColor(el, band, defaultColor) {
  let color = defaultColor;

  if (band === "ok") color = defaultColor;
  else if (band === "warn") color = "#facc15";
  else if (band === "risk") color = "#fb923c";
  else if (band === "critical") color = "#f87171";

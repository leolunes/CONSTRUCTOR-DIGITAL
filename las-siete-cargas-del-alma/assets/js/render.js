...
function renderCurrentSection() {
  ...
  if (verseEl) {
    const fullVerse = section.verse && section.verseText
      ? `${section.verse} — “${section.verseText}”`
      : safeText(section.verse);
    verseEl.textContent = fullVerse;
  }

  if (introEl) introEl.textContent = safeText(section.intro);
  if (introTopEl) {
    introTopEl.textContent = `Estás evaluando la carga: ${section.title}. Responde desde la realidad actual, no desde lo ideal.`;
  }
...
function renderEmptyResult() {
  ...
  if (mainDiagnosticText) {
    mainDiagnosticText.textContent = "Completa la evaluación para ver el diagnóstico pastoral estructural.";
  }

  if (structuralObservationText) {
    structuralObservationText.textContent = "La lectura del pórtico mostrará si la viga del alma está reteniendo peso, si la columna espiritual está transmitiendo adecuadamente y cuál es el nivel de descanso en el Cimiento.";
  }
...
function renderPlanner() {
  ...
  days.forEach(day => {
    ...
    $(".planner-day-verse", node).textContent = `Verso: ${verseFull}`;
...
function renderAxisCards(result) {
  ...
  if (almaNote) {
    almaNote.textContent = result.axes.alma >= 70
      ? "Sobrecargada"
      : result.axes.alma >= 40
      ? "Con retención moderada"
      : "Con retención leve";
  }

  if (espirituValue) espirituValue.textContent = `${result.axes.espiritu}%`;
  if (espirituNote) {
    espirituNote.textContent = result.transferLevel === "Alta"
      ? "Columna firme"
      : result.transferLevel === "Media"
  ...
  if (cimientoNote) {
    cimientoNote.textContent = result.axes.cimiento >= 70
      ? "Descanso alto"
      : result.axes.cimiento >= 40
      ? "Descanso parcial"
      : "Descanso bajo";
  }
}

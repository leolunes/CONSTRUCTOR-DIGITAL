(function(){
  // =========================================================
// FIX GLOBAL SUBPRODUCTOS Y MANO DE OBRA
// =========================================================

function getUsedSubRefsFromAPU(apu){
  const used = [];
  const seen = new Set();

  (apu?.lines || []).forEach(ln=>{
    const ref = String(ln?.subRef || "").trim();
    if(!ref) return;
...
function getManoObraLines(apu){
  return (apu?.lines || []).filter(ln=>{
    const g = String(ln?.group || ln?.tipo || "").toUpperCase();
    return g.includes("MANO") && g.includes("OBRA");
  });
}

async function getSubproductoAPU(project, subKey){
  const key = String(subKey || "").trim();
  if(!key) return null;

  try{
    if(window.APUBase && APUBase.getSubAPU){
      return await APUBase.getSubAPU(key);
    }
  }catch(_){}
...
function parseLaborCrewFromDesc(desc){
  const s = String(desc || "").toUpperCase().replace(/\s+/g, " ").trim();
  const out = [];
  function add(role, qty){
    const q = Number(qty || 0);
    if(q > 0) out.push({ role, qty: q });
  }

  let m;
  m = s.match(/(\d+)\s*OF\b/);
  if(m) add("OFICIAL", m[1]);
  else if(/\bOFICIAL\b/.test(s)) add("OFICIAL", 1);

  m = s.match(/(\d+)\s*AY\b/);
  if(m) add("AYUDANTE", m[1]);
  else if(/\bAYUDANTE\b/.test(s)) add("AYUDANTE", 1);

  m = s.match(/(\d+)\s*OP\b/);
  if(m) add("OPERADOR", m[1]);
  else if(/\bOPERADOR\b/.test(s)) add("OPERADOR", 1);

  m = s.match(/(\d+)\s*CAD\b/);
  if(m) add("CADENERO", m[1]);

  m = s.match(/(\d+)\s*MAEST\b/);
  if(m) add("MAESTRO", m[1]);

  return out;
}

async function resolveLaborRoleSubproduct(role){
  const roleName = String(role || "").trim();
  if(!roleName) return null;
  const roleKey = laborRoleNorm(roleName);

  try{
    if(window.APUBase?.getSubAPU){
      const exact = await APUBase.getSubAPU(roleKey);
      if(exact && Array.isArray(exact.lines) && exact.lines.length){
        return exact;
      }
    }
  }catch(_){}
  ...
  try{
    if(window.APUBase?.listSubproductos){
      const subs = await APUBase.listSubproductos(500);
      const arr = Array.isArray(subs) ? subs : [];
      const exact = arr.find(x => laborRoleNorm(x?.subName || x?.subtitle || x?.header || x?.subKey) === roleKey);
      const contains = exact || arr.find(x => laborRoleNorm(x?.subName || x?.subtitle || x?.header || x?.subKey).includes(roleKey));
      const hit = contains || null;
      if(hit && window.APUBase?.getSubAPU){
        const key = String(hit.subKey || hit.subName || hit.subtitle || hit.header || "").trim();
        const sub = await APUBase.getSubAPU(key);
        if(sub && Array.isArray(sub.lines) && sub.lines.length){
          return sub;
        }
      }
    }
...
async function buildLaborBreakdownForLine(ln){
  const crew = parseLaborCrewFromDesc(ln?.desc || "");
  const lineUnitValue = Number(ln?.pu || 0);
  if(!crew.length || !(lineUnitValue >= 0)) return [];
  ...
  for(const c of crew){
    const sub = await resolveLaborRoleSubproduct(c.role);
    const rate = Number(sub?.directo || 0);
    totalQty += Number(c.qty || 0);
    if(rate > 0) totalKnown += Number(c.qty || 0) * rate;
    rows.push({
      role: c.role,
      qty: Number(c.qty || 0),
      subAPU: sub || null,
  ...
  if(totalKnown > 0 && Math.abs(totalKnown - lineUnitValue) / Math.max(1, lineUnitValue) <= 0.03){
    for(const r of rows){
      r.parcial = Number(r.qty || 0) * Number(r.pu || 0);
    }
  }else if(totalKnown > 0){
    const factor = lineUnitValue / totalKnown;
    for(const r of rows){
      r.pu = Number(r.pu || 0) * factor;
      r.parcial = Number(r.qty || 0) * Number(r.pu || 0);
    }
  }else{
    const unitPerRole = totalQty ? (lineUnitValue / totalQty) : 0;
    for(const r of rows){
      r.pu = unitPerRole;
      r.parcial = Number(r.qty || 0) * unitPerRole;
    }
  }
...
async function drawFormalSubproductBlock(doc, L, subAPU, options){
  const o = options || {};
  const title = safe(o.title || subAPU?.subtitle || subAPU?.header || "SUBPRODUCTO");
  const origin = safe(o.origin || "");

  doc.setFont("helvetica","bold");
  doc.setFontSize(9.8);
  doc.text(`SUBPRODUCTO: ${title}`, L.margin, L.getY());
  L.setY(L.getY() + 14);

  doc.setFont("helvetica","normal");
  doc.setFontSize(10);
  if(origin) L.p(origin);
  L.p(`Costo directo subproducto: ${moneyCOP(subAPU?.directo||0)}`);

  const subLines = Array.isArray(subAPU?.lines) ? subAPU.lines : [];
  if(!subLines.length){
    L.p("Sin líneas en el subproducto.");
    L.p(" ");
    return;
  }
  ...
  for(const sl of subLines){
    L.tableRowAPU({
      group: sl.group || sl.tipo || "-",
      desc: sl.desc || "",
      unit: sl.unit || "",
      qty: sl.qty || 0,
      pu: sl.pu || 0,
      parcial: sl.parcial || 0
    });
  }
  L.p(" ");
}
  ...
  /* =========================================================
     ...
     - Solución robusta: getAPUForProjectItem ahora:
         1) si existe apuRefCode, confía en esa referencia y busca por ahí
         2) si no existe apuRefCode, intenta code visible
         3) si falla: busca en la BASE por DESCRIPCIÓN (match fuerte) y usa ese code real
       Esto evita falsos mapeos tipo 1.9 -> 1.09 y también evita perder APUs
       reales cuando la descripción visible del ítem fue modificada.
     ...
     ✅ AJUSTE NUEVO (2026-03 PDF Presupuesto + APUs):
     - El PDF “Presupuesto + APUs” separa los subproductos del desglose del APU
       y los imprime al final del documento en una sección independiente
     - SOLO se imprimen los subproductos que realmente fueron usados
       en las líneas del APU (line.subRef)
     ========================================================= */
  ...
  function buildFilenameCalculoIndirectos(project){
    const name = sanitizeName(project?.name || "proyecto");
    return `Calculo_Costos_Indirectos_${name}_${Date.now()}.pdf`;
  }
  ...
  // =========================
  // Helpers de subproductos
  // =========================
  function collectUsedSubRefsFromLines(lines){
    const used = [];
    const seen = new Set();

    for(const ln of (lines || [])){
      const subRef = String(ln?.subRef || "").trim();
      if(!subRef) continue;
      if(seen.has(subRef)) continue;
      seen.add(subRef);
      used.push(subRef);
    }
  ...
  async function resolveSubproductoForPDF(project, subKey){
    const key = String(subKey || "").trim();
    if(!key) return null;
    ...
    try{
      const ov = window.StorageAPI?.getSubApuOverride
        ? StorageAPI.getSubApuOverride(project.id, key)
        : null;
    ...
    if(!subAPU){
      try{
        subAPU = await (window.APUBase?.getSubAPU ? APUBase.getSubAPU(key) : null);
      }catch(_){
        subAPU = null;
      }
    }
  ...
  function mkLayout(doc){
    ...
    function shortGroup(g){
      ...
      if(up.includes("SUBPRODUCT")) return "SUBPRODUCTO";
...
doc.setFontSize(9.2);
  ...
  async function getAPUForProjectItem(project, it){
    ...
    async function buildFromOverride(apuCode, matchType, validateStrict){
      try{
        if(!window.StorageAPI?.getApuOverride) return null;
    ...
    async function buildFromCustom(apuCode, matchType, validateStrict){
      try{
        ...
        const lines = Array.isArray(custom.lines) ? custom.lines : [];
        const directo = lines.reduce((s,l)=>{
          const qty = Number(l.qty||0);
          const pu = Number(l.pu||0);
          const parcial = Number(l.parcial||0) || (qty*pu);
    ...
    async function buildFromBase(apuCode, matchType, validateStrict){
      try{
    ...
    async function tryByCode(apuCode, matchType, validateStrict){
      if(!apuCode) return null;

      const ov = await buildFromOverride(apuCode, matchType, validateStrict);
      if(ov) return ov;

      const custom = await buildFromCustom(apuCode, matchType, validateStrict);
      if(custom) return custom;

      const base = await buildFromBase(apuCode, matchType, validateStrict);
      if(base) return base;
    ...
    if(hasRealRef){
      const gotRef = await tryByCode(c1, "apuRef", false);
      if(gotRef) return gotRef;
      return null;
    }

    if(c2){
      const gotCode = await tryByCode(c2, "code", true);
    ...
    if(desc){
      ...
      if(codeByDesc){
        const gotDesc = await tryByCode(codeByDesc, "desc", true);
        if(gotDesc){
          gotDesc._match = "desc";
          gotDesc._descCode = codeByDesc;
          return gotDesc;
        }
      }
    }
...
const rows = Array.from(map.values()).sort((a,b)=> a.desc.localeCompare(b.desc, "es"));
  ...
  async function exportPresupuestoPDF(project, opts){
    ...
    L.row(`ADMINISTRACIÓN (${Number(adminPct||0).toFixed(2)}%):`, moneyCOP(totals.admin||0));
    L.row(`IMPREVISTOS (${Number(imprevPct||0).toFixed(2)}%):`, moneyCOP(totals.imprev||0));
    L.row(`UTILIDAD (${Number(utilPct||0).toFixed(2)}%):`, moneyCOP(totals.util||0));
    L.row("SUBTOTAL:", moneyCOP(totals.subtotal||0));
    L.row(`IVA sobre Utilidad (${Number(ivaUtilPct||0).toFixed(2)}%):`, moneyCOP(totals.ivaUtil||0));
  ...
  const subproductosUsados = new Map();
  const manoObraPorItem = [];
  const moBaseUnicos = new Map();
  ...
  for(const it of items){
    ...
    const lines = Array.isArray(apuObj.lines) ? apuObj.lines : [];
    if(!lines.length){
      L.p("Sin líneas en la descomposición.");
      continue;
    }

    const subRefsDelApu = getUsedSubRefsFromAPU(apuObj);
    for(const subRef of subRefsDelApu){
      if(!subproductosUsados.has(subRef)){
        subproductosUsados.set(subRef, {
          subKey: subRef,
          sourceItemCode: codeVisible,
          sourceItemDesc: safe(it.desc || "")
        });
      }
    }

    const moLines = getManoObraLines(apuObj);
    if(moLines.length){
      manoObraPorItem.push({
        itemCode: codeVisible,
        itemDesc: safe(it.desc || ""),
        unit: safe(apuObj.unit || it.unit || ""),
        lines: moLines
      });

      for(const ln of moLines){
        const rows = await buildLaborBreakdownForLine(ln);
        for(const r of rows){
          const key = laborRoleNorm(r.role || "");
          if(key && r.subAPU && !moBaseUnicos.has(key)){
            moBaseUnicos.set(key, {
              role: r.role,
              subAPU: r.subAPU
            });
          }
    ...
    for(const ln of lines){
      let desc = ln.desc || "";
      const subRef = String(ln?.subRef || "").trim();
      if(subRef) desc = `${desc} [Subproducto]`;
  ...
  // Subproductos del presupuesto: cada uno en nueva página
  if(subproductosUsados.size){
    let headerDrawn = false;
    for(const info of subproductosUsados.values()){
      const subKey = String(info?.subKey || "").trim();
      if(!subKey) continue;

      const subAPU = await getSubproductoAPU(project, subKey);

      doc.addPage();
      L.setY(PDF_THEME.safe.top);

      if(!headerDrawn){
        drawBand(doc, L.margin, L.getY(), L.contentW, 18, "SUBPRODUCTOS DEL PRESUPUESTO", { fontSize:10, bold:true });
        L.setY(L.getY()+24);
        headerDrawn = true;
      }
      ...
      if(!subAPU){
        doc.setFont("helvetica","bold");
        doc.setFontSize(9.8);
        doc.text(`SUBPRODUCTO: ${subKey}`, L.margin, L.getY());
        L.setY(L.getY()+14);
        doc.setFont("helvetica","normal");
        doc.setFontSize(10);
        L.p("No se encontró información del subproducto.");
        continue;
      }

      await drawFormalSubproductBlock(doc, L, subAPU, {
        title: safe(subAPU.subtitle || subAPU.header || subKey),
        origin: `Ítem origen: ${safe(info.sourceItemCode || "")} — ${safe(info.sourceItemDesc || "")}`
      });
    }
  }

  // Desglose independiente de mano de obra por ítem
  if(manoObraPorItem.length){
    let first = true;
    for(const mo of manoObraPorItem){
      doc.addPage();
      L.setY(PDF_THEME.safe.top);

      if(first){
        drawBand(doc, L.margin, L.getY(), L.contentW, 18, "DESGLOSE INDEPENDIENTE DE MANO DE OBRA", { fontSize:10, bold:true });
        L.setY(L.getY()+24);
        first = false;
      }
      ...
      doc.setFont("helvetica","bold");
      doc.setFontSize(9.8);
      doc.text(`ÍTEM ${safe(mo.itemCode || "")}: ${safe(mo.itemDesc || "")}`, L.margin, L.getY());
      L.setY(L.getY()+14);

      doc.setFont("helvetica","normal");
      doc.setFontSize(10);
      L.p(`Unidad: ${safe(mo.unit || "-")}`);

      const lines = Array.isArray(mo.lines) ? mo.lines : [];
      if(!lines.length){
        L.p("Sin líneas de mano de obra.");
        continue;
      }
      ...
      for(const ln of lines){
        const rows = await buildLaborBreakdownForLine(ln);
        const displayPu = rows.length ? rows.reduce((s, r) => s + Number(r.parcial || 0), 0) : Number(ln.pu || 0);
        ...
        L.tableRowAPU({
          ...
          qty: ln.qty || 0,
          pu: displayPu,
          parcial: Number(ln.qty || 0) * displayPu
        });
      }
    }
  }

  // Subproductos base de mano de obra únicos, una sola vez al final
  if(moBaseUnicos.size){
    let first = true;
    for(const info of moBaseUnicos.values()){
      doc.addPage();
      L.setY(PDF_THEME.safe.top);

      if(first){
        drawBand(doc, L.margin, L.getY(), L.contentW, 18, "SUBPRODUCTOS BASE DE MANO DE OBRA", { fontSize:10, bold:true });
        L.setY(L.getY()+24);
        first = false;
      }

      await drawFormalSubproductBlock(doc, L, info.subAPU, {
        title: safe(info.subAPU?.subtitle || info.subAPU?.header || info.role || "SUBPRODUCTO"),
        origin: "Base de mano de obra única del presupuesto"
      });
    }
  }
  ...
  async function exportCalculoCostosIndirectosPDF(project, opts){
    const doc = newDoc();
    const L = mkLayout(doc);

    drawInstitutionalCover(doc, L, project, "CÁLCULO DE COSTOS INDIRECTOS");
    ...
    const calcTotalsPdf = (window.Calc && typeof Calc.calcTotals === "function") ? (Calc.calcTotals(project) || {}) : {};
    const adminPct = Number(calcTotalsPdf.adminPct ?? pct(project, "adminPct", "aiuPct") ?? 0);
    const imprevPct = Number(calcTotalsPdf.imprevPct ?? pct(project, "imprevPct", null) ?? 0);
    const utilPct = Number(calcTotalsPdf.utilPct ?? pct(project, "utilPct", null) ?? 0);
    const ivaUtilPct = Number(calcTotalsPdf.ivaUtilPct ?? pct(project, "ivaUtilPct", "ivaPct") ?? 0);

    const indirectRows = Array.isArray(project?.indirectTable) ? project.indirectTable.slice() : [];
    const indirectMode = String(project?.indirectMode || "manual").toLowerCase() === "table" ? "table" : "manual";

    function normItem(v){
      return String(v || "").trim().replace(/\s+/g, "");
    }
    function itemLevel(item){
      const code = normItem(item);
      if(!code) return 0;
      return Math.max(0, code.split(".").filter(Boolean).length - 1);
    }
    function segs(item){
      return normItem(item).split(".").filter(Boolean).map(x => /^\d+$/.test(x) ? Number(x) : x);
    }
    function cmpItem(a,b){
      const aa = segs(a), bb = segs(b);
    ...
    function normalizeIndirectRow(row){
      ...
      if(!["manual","insumo","subproducto","percent_cd"].includes(sourceType)) sourceType = "manual";
      let calcType = String(r.calcType || "").trim().toLowerCase() === "percent_direct" ? "percent_direct" : "fixed";
      if(sourceType === "percent_cd") calcType = "percent_direct";

      const out = {
        rowType,
        item: normItem(r.item),
        group: String(r.group || (rowType === "header" ? "OTRO" : "ADMIN")).trim().toUpperCase() || "OTRO",
      ...
      if(out.sourceType === "percent_cd"){
        ...
        out.unitValue = directo;
      }
      return out;
    }
    function partialIndirect(row){
      const r = normalizeIndirectRow(row);
      if(r.rowType === "header") return 0;
      if(r.calcType === "percent_direct"){
        return directo * (Number(r.percent || 0) / 100);
      }
      return Number(r.qty || 0) * Number(r.unitValue || 0);
    }

    const rows = indirectRows.map(normalizeIndirectRow).sort((a,b)=>{
      const c = cmpItem(a.item, b.item);
      if(c !== 0) return c;
    ...
    L.row("Proyecto:", project?.name || "—");
    L.row("Entidad:", project?.entity || "—");
    L.row("Ubicación:", project?.location || "—");
    L.row("Modo de cálculo de indirectos:", indirectMode === "table" ? "Por tabla de indirectos" : "Manual");
    L.row("Fecha generación:", new Date().toLocaleString());
    L.line();

    L.row("TOTAL COSTOS DIRECTOS:", moneyCOP(directo));
    L.row(`ADMINISTRACIÓN (${Number(adminPct||0).toFixed(2)}%):`, moneyCOP(totals.admin||0));
    L.row(`IMPREVISTOS (${Number(imprevPct||0).toFixed(2)}%):`, moneyCOP(totals.imprev||0));
    L.row(`UTILIDAD (${Number(utilPct||0).toFixed(2)}%):`, moneyCOP(totals.util||0));
    L.row("SUBTOTAL:", moneyCOP(totals.subtotal||0));
    L.row(`IVA sobre Utilidad (${Number(ivaUtilPct||0).toFixed(2)}%):`, moneyCOP(totals.ivaUtil||0));
    L.row("VALOR TOTAL:", moneyCOP(totals.total||0));
    L.p(" ");

    if(indirectMode === "manual"){
      drawBand(doc, L.margin, L.getY(), L.contentW, 18, "INDIRECTOS EN MODO MANUAL", { fontSize:10, bold:true });
      L.setY(L.getY()+24);
      L.p("El proyecto está configurado en modo manual. Los costos indirectos se calculan con los porcentajes de Administración, Imprevistos y Utilidad definidos en Ajustes.");
      await appendElaboradorFirma(doc, L);

      stampHeaderFooter(doc, {
        docType:"CÁLCULO DE COSTOS INDIRECTOS",
        projectName: project.name || "",
        logoDataUrl: project.logoDataUrl || ""
      });

      const filename = buildFilenameCalculoIndirectos(project);
      return await finalizePDF(doc, filename, opts);
    }

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, "CUADRO DE COSTOS INDIRECTOS", { fontSize:10, bold:true });
    L.setY(L.getY()+24);

    const margin = L.margin;
    const contentW = L.contentW;
    let cols = [
      { key:"item", label:"ÍTEM", w:60, align:"left" },
      { key:"group", label:"GRUPO", w:72, align:"left" },
      { key:"desc", label:"DESCRIPCIÓN", w:270, align:"left" },
      { key:"unit", label:"UNID", w:46, align:"center" },
      { key:"qty", label:"CANT", w:56, align:"right" },
      { key:"unitValue", label:"VR UNIT", w:86, align:"right" },
      { key:"partial", label:"VR PARCIAL", w:102, align:"right" }
    ];
    ...
    function dataRow(r){
      const yTop = L.getY();
      const level = itemLevel(r.item);
      const indent = Math.max(0, level * 12);
      const descText = r.desc || (r.rowType === "header" ? "SUBTÍTULO" : "");
      const descLines = splitToLines(doc, descText, cols[2].w - 8 - indent, r.rowType === "header" ? 2 : 3);
      const rowH = Math.max(18, 8 + descLines.length * 9.5);

      const jumped = L.ensure(rowH + 10);
      if(jumped) tableHeader();

      let x = margin;
      doc.setFont("helvetica", r.rowType === "header" ? "bold" : "normal");
      doc.setFontSize(7.4);

      drawCellText(doc, x, yTop, cols[0].w, rowH, r.item || "—", "left", { bold:r.rowType==="header", fontSize:7.4 }); x += cols[0].w;
      drawCellText(doc, x, yTop, cols[1].w, rowH, r.group || "OTRO", "left", { bold:r.rowType==="header", fontSize:7.4 }); x += cols[1].w;

      const descX = x + indent;
      const descW = cols[2].w - indent;
      drawCellText(doc, descX, yTop, descW, rowH, descLines, "left", { bold:r.rowType==="header", fontSize:7.4 }); x += cols[2].w;

      if(r.rowType === "header"){
        drawCellText(doc, x, yTop, cols[3].w, rowH, "", "center", { fontSize:7.4 }); x += cols[3].w;
        drawCellText(doc, x, yTop, cols[4].w, rowH, "", "right", { fontSize:7.4 }); x += cols[4].w;
        drawCellText(doc, x, yTop, cols[5].w, rowH, "", "right", { fontSize:7.4 }); x += cols[5].w;
        drawCellText(doc, x, yTop, cols[6].w, rowH, "", "right", { fontSize:7.4 });
      }else{
        const part = partialIndirect(r);
        drawCellText(doc, x, yTop, cols[3].w, rowH, r.unit || "", "center", { fontSize:7.4 }); x += cols[3].w;
        drawCellText(doc, x, yTop, cols[4].w, rowH, fmtQty(r.qty||0), "right", { fontSize:7.4 }); x += cols[4].w;
        drawCellText(doc, x, yTop, cols[5].w, rowH, moneyCOP0(r.unitValue||0), "right", { fontSize:7.4 }); x += cols[5].w;
        drawCellText(doc, x, yTop, cols[6].w, rowH, moneyCOP0(part), "right", { bold:true, fontSize:7.4 });
      }
    ...
    tableHeader();
    if(!rows.length){
      L.p("No hay líneas en la tabla de costos indirectos.");
    }else{
      for(const r of rows){
        dataRow(r);
      }
    ...
    const totalIndirectos = Number(totals.admin||0) + Number(totals.imprev||0) + Number(totals.util||0) + Number(totals.indirectOthers||0);
    const pctIndirectos = directo > 0 ? (totalIndirectos / directo) * 100 : 0;

    L.row("Total costos indirectos:", moneyCOP(totalIndirectos));
    L.row("Porcentaje total sobre costos directos:", pctIndirectos.toFixed(2) + "%");
    L.row("Administración:", moneyCOP(totals.admin||0));
    L.row("Imprevistos:", moneyCOP(totals.imprev||0));
    L.row("Utilidad:", moneyCOP(totals.util||0));
    L.row("Otros:", moneyCOP(totals.indirectOthers||0));
    L.row("Subtotal:", moneyCOP(totals.subtotal||0));
    L.row("IVA sobre utilidad:", moneyCOP(totals.ivaUtil||0));
    L.row("Valor total:", moneyCOP(totals.total||0));
  ...
  function buildFilenameIvaProyecto(project, budgetLabel){
    const name = sanitizeName(project?.name || "proyecto");
    const lbl = sanitizeName(budgetLabel || "presupuesto");
    return `IVA_Proyecto_${name}_${lbl}_${Date.now()}.pdf`;
  }

  function getBudgetModeLabel(mode){
    const m = String(mode || "").trim().toLowerCase();
    if(m === "official") return "PRESUPUESTO OFICIAL";
    if(m === "artificial") return "PRESUPUESTO PRECIO ARTIFICIALMENTE BAJO";
    return "PRESUPUESTO BASE";
  }

  function getIvaBudgetContext(project){
    ...
    let mode = String(current?.activeBudgetMode || "base").trim().toLowerCase();
    let label = getBudgetModeLabel(mode);
    let view = current;

    try{
      if(window.StorageAPI?.getActiveBudgetInfo){
        const info = StorageAPI.getActiveBudgetInfo(current?.id);
        if(info){
          mode = String(info.mode || mode || "base").trim().toLowerCase();
          label = String(info.label || getBudgetModeLabel(mode));
        }
      }
    }catch(_){}

    try{
      if(window.StorageAPI?.getActiveBudgetView && current?.id){
        view = StorageAPI.getActiveBudgetView(current.id) || view;
      }
    }catch(_){}

    return { current, view, mode, label };
  }

  async function exportIvaProyectoPDF(project, opts){
    const ctx = getIvaBudgetContext(project);
    const current = ctx.current;
    const budgetView = ctx.view;
    const budgetMode = ctx.mode;
    const budgetLabel = ctx.label;

    if(!current || !budgetView){
      alert("No fue posible preparar el presupuesto para el PDF IVA del proyecto.");
      return;
    }
    ...
    const grouped = (window.Calc && typeof Calc.groupByChapters === "function")
      ? (Calc.groupByChapters(budgetView) || {})
      : {};
    const items = Array.isArray(grouped.items) ? grouped.items : [];

    const agg = await calcDesagregadoByItemFromAPU(current, items);
    const totals = agg?.totals || {};

    const materiales = Number(totals.MATERIALES || 0);
    const equipos = Number(totals["EQUIPO Y HERRAM"] || 0);
    const manoObra = Number(totals["MANO DE OBRA"] || 0);
    const subcontratos = Number(totals.SUBCONTRATOS || 0);
    const transportes = Number(totals.TRANSPORTES || 0);
    const otros = Number(totals.OTROS || 0);

    const directos = Number(agg?.totalDirecto || 0);

    const totalsProject = totalsCompat(budgetView);
    const calcRaw = (window.Calc && typeof Calc.calcTotals === "function")
      ? (Calc.calcTotals(budgetView) || {})
      : {};
    const ivaUtilPct = Number(calcRaw.ivaUtilPct ?? pct(budgetView, "ivaUtilPct", "ivaPct") ?? 0);
    const ivaUtil = Number(totalsProject.ivaUtil || 0);

    const baseGravadaDirectos = materiales + equipos + subcontratos + otros;
    const baseNoGravadaDirectos = manoObra + transportes;
    const ivaDirectos = baseGravadaDirectos * 0.19;
    const ivaTotalProyecto = ivaDirectos + ivaUtil;

    drawInstitutionalCover(doc, L, current, `IVA DEL PROYECTO — ${budgetLabel}`);

    doc.addPage();
    L.setY(PDF_THEME.safe.top);

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, "DATOS GENERALES", { fontSize:10, bold:true });
    L.setY(L.getY()+24);

    L.row("Proyecto:", current?.name || "—");
    L.row("Presupuesto analizado:", budgetLabel);
    L.row("Entidad:", current?.entity || "—");
    L.row("Ubicación:", current?.location || "—");
    L.row("Fecha generación:", new Date().toLocaleString());
    L.p(" ");

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, "BASE DE ANÁLISIS DEL IVA", { fontSize:10, bold:true });
    L.setY(L.getY()+24);

    const margin = L.margin;
    const contentW = L.contentW;
    let cols = [
      { key:"comp", label:"COMPONENTE", w:210, align:"left" },
      { key:"valor", label:"VALOR", w:120, align:"right" },
      { key:"trat", label:"TRATAMIENTO IVA", w:110, align:"center" },
      { key:"base", label:"BASE IVA", w:120, align:"right" }
    ];
    cols = fitColsToWidth(cols, contentW, 28);

    function ivaTableHeader(){
      const yTop = L.getY();
      const h = 18;
      L.ensure(h + 8);
      ...
      doc.setFontSize(8.2);
      for(const c of cols){
        drawCellText(doc, x, yTop, c.w, h, c.label, "center", { bold:true, fontSize:8.2 });
        x += c.w;
      }
      hLine(doc, margin, margin+contentW, yTop + h, PDF_THEME.lines.header.w, PDF_THEME.lines.header.c);
      L.setY(yTop + h + 6);
    ...
    function ivaRow(label, value, treatment, baseVal, bold=false){
      const rowH = 18;
      const jumped = L.ensure(rowH + 10);
      if(jumped) ivaTableHeader();

      const yTop = L.getY();
      let x = margin;

      drawCellText(doc, x, yTop, cols[0].w, rowH, label, "left", { bold, fontSize:8.2 }); x += cols[0].w;
      drawCellText(doc, x, yTop, cols[1].w, rowH, moneyCOP0(value), "right", { bold, fontSize:8.2 }); x += cols[1].w;
      drawCellText(doc, x, yTop, cols[2].w, rowH, treatment, "center", { bold, fontSize:8.2 }); x += cols[2].w;
      drawCellText(doc, x, yTop, cols[3].w, rowH, moneyCOP0(baseVal), "right", { bold, fontSize:8.2 });

      hLine(doc, margin, margin+contentW, yTop + rowH, bold ? PDF_THEME.lines.band.w : PDF_THEME.lines.row.w, bold ? PDF_THEME.lines.band.c : PDF_THEME.lines.row.c);
      L.setY(yTop + rowH + 2);
    }

    ivaTableHeader();
    ivaRow("Materiales", materiales, "GRAVA 19%", materiales);
    ivaRow("Equipo y herramienta", equipos, "GRAVA 19%", equipos);
    ivaRow("Mano de obra", manoObra, "IVA 0%", 0);
    ivaRow("Subcontratos", subcontratos, "GRAVA 19%", subcontratos);
    ivaRow("Transportes", transportes, "IVA 0%", 0);
    ivaRow("Otros", otros, "GRAVA 19%", otros);
    ivaRow("TOTAL COSTOS DIRECTOS", directos, "—", baseGravadaDirectos, true);

    L.p(" ");

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, "LIQUIDACIÓN DEL IVA", { fontSize:10, bold:true });
    L.setY(L.getY()+24);

    L.row("Base gravada costos directos:", moneyCOP(baseGravadaDirectos));
    L.row("Base no gravada costos directos:", moneyCOP(baseNoGravadaDirectos));
    L.row("IVA costos directos (19%):", moneyCOP(ivaDirectos));
    L.row(`IVA sobre utilidad (${Number(ivaUtilPct || 0).toFixed(2)}%):`, moneyCOP(ivaUtil));
    L.row("IVA TOTAL DEL PROYECTO:", moneyCOP(ivaTotalProyecto));
    L.p(" ");

    drawBand(doc, L.margin, L.getY(), L.contentW, 18, "CONCLUSIÓN TÉCNICA", { fontSize:10, bold:true });
    L.setY(L.getY()+24);

    L.p(
      "El presente informe liquida el IVA del proyecto discriminando los componentes directos del presupuesto analizado. " +
      "Para este cálculo se consideran gravados al 19% los componentes de materiales, equipo y herramienta, subcontratos y otros; " +
      "mientras que mano de obra y transportes se presentan como componentes no gravados dentro del análisis del proyecto."
    );
    L.p(
      "De forma complementaria se incorpora el IVA sobre utilidad calculado según la configuración activa del presupuesto analizado, " +
      "con el fin de dejar plenamente identificado el IVA total asociado al proyecto."
    );

    await appendElaboradorFirma(doc, L);

    stampHeaderFooter(doc, {
      docType:`IVA DEL PROYECTO — ${budgetLabel}`,
      projectName: current.name || "",
      logoDataUrl: current.logoDataUrl || ""
    });

    const suffix = budgetMode === "official" ? "OFICIAL" : (budgetMode === "artificial" ? "ARTIFICIALMENTE_BAJO" : "BASE");
    const filename = buildFilenameIvaProyecto(current, suffix);
    return await finalizePDF(doc, filename, opts);
  }
  ...
  window.PDF = {
    ...
    exportCalculoCostosIndirectosPDF,

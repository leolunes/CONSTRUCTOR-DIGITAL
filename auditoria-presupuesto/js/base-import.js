(function(){
  ...
  function getProjectScopedComparisonBaseId(){
    const projectId = getProjectIdFromLocation();
    if(!projectId) return "";
    try{
      const key = `ppro_compare_base__${projectId}`;
      const v = safeBaseId(localStorage.getItem(key) || "");
      if(v && v !== "default") return v;
    }catch(_){}
    try{
      if(window.StorageAPI && typeof window.StorageAPI.getProjectComparisonBaseId === "function"){
        const v = safeBaseId(window.StorageAPI.getProjectComparisonBaseId(projectId) || "");
        if(v && v !== "default") return v;
      }
    }catch(_){}
    return "";
  ...
  function getActiveBaseId(){
    try{
      const fromProject = getProjectScopedComparisonBaseId();
      if(fromProject && fromProject !== "default") return fromProject;
    }catch(_){}

    try{
      const fromCompareUi = safeBaseId(localStorage.getItem("activeComparisonBaseId") || "");
      if(fromCompareUi && fromCompareUi !== "default") return fromCompareUi;
    }catch(_){}

    try{
      const fromUi = safeBaseId(localStorage.getItem("activeBaseId") || "");
      if(fromUi && fromUi !== "default") return fromUi;
    }catch(_){}

    return getDefaultBaseId();
  }

  function resolveReadBaseId(baseId){
    const raw = String(baseId || "").trim();
    if(raw) return safeBaseId(raw);
    return safeBaseId(getActiveBaseId() || getDefaultBaseId() || "default");
  }
  ...
  function makeUniqueBaseId(baseId){
    const wanted = safeBaseId(baseId || "default");
    const installed = listInstalledBases().map(x => safeBaseId(x.id));
    if(!installed.includes(wanted)) return wanted;

    let n = 2;
    while(installed.includes(`${wanted}_${n}`)){
      n += 1;
    }
    return `${wanted}_${n}`;
  }

  function buildSuggestedBaseId(fileName){
    const cleanName = String(fileName || "").trim().replace(/\.[^.]+$/,"");
    const proposed = safeBaseId(cleanName || "base");
    return makeUniqueBaseId(proposed);
  }
  ...
  async function getMeta(baseId="default"){
    baseId = resolveReadBaseId(baseId);
  ...
  async function installFromFile(file, opts={}){
    ...
    const explicitBaseIdRaw = String(opts?.baseId || opts?.id || "").trim();
    const requestedBaseId = explicitBaseIdRaw ? safeBaseId(explicitBaseIdRaw) : "";
    const replaceExisting = !!opts?.replaceExisting;
    const baseId = requestedBaseId
      ? (replaceExisting ? requestedBaseId : makeUniqueBaseId(requestedBaseId))
      : buildSuggestedBaseId(String(file.name || "base.xlsx"));
    ...
    const currentDefault = getDefaultBaseId();
    const mustSetDefault =
      opts?.setAsDefault === true ||
      !catalog.length ||
      !catalog.some(x => safeBaseId(x.id) === safeBaseId(currentDefault));
